...
public class CipherByteBufferOverwriteTest {
    ...
    private static final AlgorithmParameterSpec PARAMS = new IvParameterSpec(new byte[16]);
    ...
    public static void main(String[] args) throws Exception {
        ...
        prepareBuffers(BufferType.ALLOCATE, useRO, buf.length, buf, 0, PLAINTEXT_SIZE, offset);
        runTest(offset, expectedPT, expectedCT);
        System.out.println("\tALLOCATE: passed");
        prepareBuffers(BufferType.DIRECT, useRO, buf.length, buf, 0, PLAINTEXT_SIZE, offset);
        System.out.println("\tDIRECT: passed");
        runTest(offset, expectedPT, expectedCT);
        prepareBuffers(BufferType.WRAP, useRO, buf.length, buf, 0, PLAINTEXT_SIZE, offset);
        runTest(offset, expectedPT, expectedCT);
        System.out.println("\tWRAP: passed");
        System.out.println("All Tests Passed");
    }

    private static void prepareBuffers(BufferType type, boolean useRO, int bufSz, byte[] in, int inOfs, int inLen, int outOfs) {
        switch(type) {
            case ALLOCATE:
        ...
        if (DEBUG) {
            System.out.println("inBuf, pos = " + inBuf.position() + ", capacity = " + inBuf.capacity() + ", limit = " + inBuf.limit() + ", remaining = " + inBuf.remaining());
            System.out.println("outBuf, pos = " + outBuf.position() + ", capacity = " + outBuf.capacity() + ", limit = " + outBuf.limit() + ", remaining = " + outBuf.remaining());
        }
    }

    private static void runTest(int ofs, byte[] expectedPT, byte[] expectedCT) throws Exception {
        Cipher c = Cipher.getInstance(TRANSFORMATION);
        c.init(Cipher.ENCRYPT_MODE, KEY, PARAMS);
        ...
        if (DEBUG) {
            ...
            System.out.println("runTest, ofs = " + ofs + ", remaining = " + finalCT.length + ", limit = " + outBuf.limit());
        }
        outBuf.get(finalCT);
        ...
        ByteBuffer finalPTBuf = ByteBuffer.allocate(c.getOutputSize(outBuf.remaining()));
