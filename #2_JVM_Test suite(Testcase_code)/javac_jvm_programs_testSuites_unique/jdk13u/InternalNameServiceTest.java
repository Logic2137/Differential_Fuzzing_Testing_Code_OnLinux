...
public class InternalNameServiceTest {
    ...
    private static void testHostToIPAddressMappings(String hostsFileName) throws Exception, UnknownHostException {
        ...
        if (!Arrays.equals(retrievedIpAddr, expectedIpAddrIpv6_1)) {
            ...
            throw new RuntimeException("retrieved IPV6 Addr not equal to expected IPV6 Addr");
        }
    }
    ...
    private static void testIpAddressToHostNameMappings(String hostsFileName) throws Exception {
        ...
        if (!expectedHost.equals(retrievedHost)) {
            throw new RuntimeException("retrieved host name " + retrievedHost + " not equal to expected host name" + expectedHost);
        }
        testAddress = InetAddress.getByAddress(testHostIpAddr4);
        System.out.println("*******   testAddress == " + testAddress);
        if ("10.8.9.11".equalsIgnoreCase(testAddress.getCanonicalHostName())) {
            System.out.println("addr = " + addrToString(testHostIpAddr4) + "  resolve to a host address as expected");
        } else {
            System.out.println("addr = " + addrToString(testHostIpAddr4) + " does not resolve as expected, testAddress == " + testAddress.getCanonicalHostName());
