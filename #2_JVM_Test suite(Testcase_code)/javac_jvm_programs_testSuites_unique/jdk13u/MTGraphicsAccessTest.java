...
public class MTGraphicsAccessTest {
    ...
    BufferedImage sharedBI = new BufferedImage(50, 50, BufferedImage.TYPE_INT_RGB);

    static final Paint[] colors = { Color.red, new Color(0x7f, 0xff, 0x00, 0x7f), new GradientPaint(0, 0, Color.red, 50, 50, new Color(0x7f, 0xff, 0x00, 0x7f)) };

    static final Font[] fonts = { new Font("Dialog", Font.PLAIN, 12), new Font("Dialog", Font.BOLD, 16), new Font("Dialog", Font.ITALIC, 18) };

    static final AlphaComposite[] comps = { AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0f), AlphaComposite.Src, AlphaComposite.Xor, AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f), null };

    static final Stroke[] strokes = { new BasicStroke(), new BasicStroke(0.0f), new BasicStroke(2.0f), new BasicStroke(2.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_BEVEL), new BasicStroke(5.0f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_ROUND), new BasicStroke(0.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0, new float[] { 0, 6, 0, 6 }, 0) };

    static final AffineTransform[] transforms = { new AffineTransform(), AffineTransform.getRotateInstance(10.0), AffineTransform.getShearInstance(10.0, 4.0), AffineTransform.getScaleInstance(1.1, 1.2), AffineTransform.getScaleInstance(3.0, 2.0) };
    ...
    public MTGraphicsAccessTest() {
        ...
        if (numexceptions == 0) {
            ...
            throw new RuntimeException("Test failed with " + numexceptions + " exceptions");
        } else {
            System.err.println("Test finished with " + numexceptions + " exceptions");
        }
    }
    ...
    public static void usage(String message) {
        ...
        System.err.println("Usage: MTGraphicsAccessTest [-full] " + "[-time N/forever] [-help]");
        System.err.println(" -full: run full suite of tests " + "(default: limited number of tests is run)");
        System.err.println(" -time N: test duration in seconds/forever" + " (default: " + JTREG_RUN_TIME + "s for the short suite, " + STANDALONE_RUN_TIME + "s for the full suite)");
    ...
    public static void main(String[] args) {
        ...
        if (!testRunSet) {
            testRunTime = 1000 * (standaloneMode ? STANDALONE_RUN_TIME : JTREG_RUN_TIME);
        }
        System.err.println("Approximate test run time: " + testRunTime / 1000 + " seconds");
    ...
    final Runnable[] stateChangers = { new Runnable() {
        ...
        public void run() {
            if (AA) {
                sharedGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            } else {
    ...
    final Runnable[] renderTests = { new Runnable() {
