...
public class CompareToEqualsTests {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            String[] names1 = new String[] { "ou=Sales+cn=Bob", "ou=Sales+cn=Bob", "ou=Sales+cn=Bob", "ou=Sales+cn=Scott+c=US", "cn=config" };
            String[] names2 = new String[] { "ou=Sales+cn=Bob", "cn=Bob+ou=Sales", "ou=Sales+cn=Scott", "ou=Sales+cn=Scott", "Cn=COnFIG" };
            int[] expectedResults = { 0, 0, -1, -1, 0 };
            for (Locale locale : Locale.getAvailableLocales()) {
                Locale.setDefault(locale);
                for (int i = 0; i < names1.length; i++) {
                    checkResults(new LdapName(names1[i]), new LdapName(names2[i]), expectedResults[i]);
                }
    ...
    static void checkResults(LdapName name1, LdapName name2, int expectedResult) throws Exception {
