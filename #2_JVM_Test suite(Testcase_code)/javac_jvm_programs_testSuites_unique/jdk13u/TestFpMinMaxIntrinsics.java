...
public class TestFpMinMaxIntrinsics {
    ...
    private static final float[][] f_cases = { { fPos, fPos, fPos, fPos }, { fNeg, fNeg, fNeg, fNeg }, { fPos, fNeg, fNeg, fPos }, { fNeg, fPos, fNeg, fPos }, { fPosZero, fNegZero, fNegZero, fPosZero }, { fNegZero, fPosZero, fNegZero, fPosZero }, { fNegZero, fNegZero, fNegZero, fNegZero }, { fPos, fPosInf, fPos, fPosInf }, { fNeg, fNegInf, fNegInf, fNeg }, { fPos, fNaN, fNaN, fNaN }, { fNaN, fPos, fNaN, fNaN }, { fNeg, fNaN, fNaN, fNaN }, { fNaN, fNeg, fNaN, fNaN }, { fPosInf, fNaN, fNaN, fNaN }, { fNaN, fPosInf, fNaN, fNaN }, { fNegInf, fNaN, fNaN, fNaN }, { fNaN, fNegInf, fNaN, fNaN } };

    private static final double[][] d_cases = { { dPos, dPos, dPos, dPos }, { dNeg, dNeg, dNeg, dNeg }, { dPos, dNeg, dNeg, dPos }, { dNeg, dPos, dNeg, dPos }, { dPosZero, dNegZero, dNegZero, dPosZero }, { dNegZero, dPosZero, dNegZero, dPosZero }, { dNegZero, dNegZero, dNegZero, dNegZero }, { dPos, dPosInf, dPos, dPosInf }, { dNeg, dNegInf, dNegInf, dNeg }, { dPos, dNaN, dNaN, dNaN }, { dNaN, dPos, dNaN, dNaN }, { dNeg, dNaN, dNaN, dNaN }, { dNaN, dNeg, dNaN, dNaN }, { dPosInf, dNaN, dNaN, dNaN }, { dNaN, dPosInf, dNaN, dNaN }, { dNegInf, dNaN, dNaN, dNaN }, { dNaN, dNegInf, dNaN, dNaN } };
    ...
    private static void fCheck(float a, float b, float fmin, float fmax, float efmin, float efmax) {
        ...
        if (min != emin || max != emax) {
            throw new AssertionError("Unexpected result of float min/max: " + "a = " + a + ", b = " + b + ", " + "result = (" + fmin + ", " + fmax + "), " + "expected = (" + efmin + ", " + efmax + ")");
        }
    }
    ...
    private static void dCheck(double a, double b, double dmin, double dmax, double edmin, double edmax) {
        ...
        if (min != emin || max != emax) {
            throw new AssertionError("Unexpected result of double min/max: " + "a = " + a + ", b = " + b + ", " + "result = (" + dmin + ", " + dmax + "), " + "expected = (" + edmin + ", " + edmax + ")");
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < Integer.parseInt(args[1]); i++) m.invoke(null);
    }
    ...
    public static void init() {
        for (int i = 0; i < COUNT; i++) pool[i] = new Node(Double.NaN);
    }
    ...
    public static void randomSearchTree() {
        ...
        for (int l = 0; l < LOOPS; l++) {
            ...
            for (int i = 1; i < COUNT; i++) insert(root, pool[i].reset(r.nextDouble()));
        }
        finish();
    }
    ...
    public static void sortedSearchTree() {
        ...
        for (int l = 0; l < LOOPS; l++) {
            ...
            for (int i = 1; i < COUNT; i++) insert(root, pool[i].reset(i - 1));
        }
        finish();
    }
    ...
    private static class Node {
        ...
        public String toString() {
            return (min != null ? min + ", " : "") + value + (max != null ? ", " + max : "");
        }
    }
