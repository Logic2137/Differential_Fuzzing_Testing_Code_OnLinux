...
public class TestCipherMode {
    ...
    private TestCipherMode() throws NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException {
        c = Cipher.getInstance(ALGO + "/ECB/PKCS5Padding", "SunJCE");
        String output = c.toString();
        if (!output.equals("Cipher.DES/ECB/PKCS5Padding, mode: not initialized, algorithm from: SunJCE")) {
            throw new RuntimeException("Unexpected Cipher.toString() output:" + output);
        }
