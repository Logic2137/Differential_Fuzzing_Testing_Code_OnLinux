...
public class AltTabCrashTest extends Frame {
    ...
    private Ball createRandomBall(final int y, final int x) {
        ...
        switch(type) {
            case VIMAGES:
                b = new VISpriteBall(x, y);
                break;
            case AAOVALS:
                b = new AAOvalBall(x, y);
                break;
            case BIMAGES:
                b = new BISpriteBall(x, y);
                break;
            case TEXT:
                b = new TextBall(x, y, "Text Sprite!");
                break;
            default:
                b = new Ball(x, y);
                break;
    ...
    class VISpriteBall extends SpriteBall {
        ...
        public Image createSprite() {
            return gd.getDefaultConfiguration().createCompatibleVolatileImage(20, 20);
        }

        public void validateSprite() {
            int result = ((VolatileImage) image).validate(getGraphicsConfiguration());
    ...
    public void render(Graphics g) {
        do {
            ...
            if (returnCode == VolatileImage.IMAGE_RESTORED) {
                ...
                vimg = getGraphicsConfiguration().createCompatibleVolatileImage(width, height);
                renderOffscreen();
    ...
    public static void main(String[] args) {
        for (String arg : args) {
            if (arg.equalsIgnoreCase("-auto")) {
                ...
                System.err.println("Usage: AltTabCrashTest [-usebs][-auto]" + "[-changedm][-vi|-bi|-ov|-aaov|-tx]");
                System.err.println(" -usebs: use BufferStrategy instead of VI");
                System.err.println(" -auto: automatically alt+tab in and out" + " of the application ");
                System.err.println(" -changedm: change display mode");
                System.err.println(" -(vi|bi|ov|tx|aaov) : use only VI, BI, " + "text or [AA] [draw]Oval sprites");
                System.exit(0);
            }
        }
        if (spriteType != null) {
