...
public class RuleParsingTest {
    ...
    void testParseComplexExpressions(String sourceVersion) throws Exception {
        String[] expressions = { "(a)", "a", "a + a", "~a + a", "a = a", "a += a", "a + (a)", "a + (a) b", "true ? a : b", "m(() -> {})", "m(() -> 1)", "m(a -> 1)", "m((t a) -> 1)" };
        ...
        JavacTask ct = (JavacTask) tool.getTask(out, null, noErrors, List.of("--enable-preview", "-source", sourceVersion), null, Arrays.asList(new MyFileObject(code.toString())));
        ...
        new TreePathScanner<Void, Void>() {
            ...
            public Void visitCase(CaseTree node, Void p) {
                ...
                if (!spans.remove(new SimpleEntry<>(start, end))) {
                    throw new AssertionError("Did not find an expression span in expected spans: " + start + "-" + end + " '" + node.getExpression().toString() + "'");
                }
