...
public class SetOutgoingIf {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (NetIf netIf : netIfs) {
            ...
            netIf.groups(groups);
            Thread sender = new Thread(new Sender(netIf, groups, PORT));
            ...
            for (InetAddress group : netIf.groups()) {
                ...
                if (from != null) {
                    if (!from.equals(shouldbe)) {
                        System.out.println("Packets on group " + group + " should come from " + shouldbe.getName() + ", but came from " + from.getName());
                    }
                }
...
class Sender implements Runnable {
    ...
    public Sender(NetIf netIf, List<InetAddress> groups, int port) {
        this.netIf = netIf;
        this.groups = groups;
        this.port = port;
    }
    ...
    public void run() {
        try {
            ...
            for (; ; ) {
                for (DatagramPacket packet : packets) mcastsock.send(packet);
                Thread.sleep(1000);
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
