...
public class numeric010 {
    ...
    public static int run(String[] args, PrintStream out) {
        ...
        for (; argsShift < args.length; argsShift++) {
            ...
            if (argument.startsWith("-tolerance:")) {
                String percents = argument.substring("-tolerance:".length(), argument.length());
            ...
            if (argument.startsWith("-CPU:")) {
                String value = argument.substring("-CPU:".length(), argument.length());
        ...
        if ((args.length < argsShift + 1) || (args.length > argsShift + 2)) {
            ...
            complain("    java numeric010 [-verbose] [-performance] " + "[-tolerance:percents] [-CPU:number] matrixSize [threads]");
            return 2;
        }
        ...
        double[] elapsed = { 0, 0 };
        for (int i = 0; i < 2; i++) {
            double seconds = elapsedTime((i == 0 ? 1 : threads), intA, intAA, longA, longAA, floatA, floatAA, doubleA, doubleAA);
            elapsed[i] = seconds;
    ...
    private static double elapsedTime(int threads, IntegerMatrix intA, IntegerMatrix intAA, LongMatrix longA, LongMatrix longAA, FloatMatrix floatA, FloatMatrix floatAA, DoubleMatrix doubleA, DoubleMatrix doubleAA) {
        println("");
    ...
    private static class IntegerMatrix {
        ...
        public IntegerMatrix(int size) {
            ...
            for (int line = 0; line < size; line++) for (int column = 0; column < size; column++) value[line][column] = Math.round((float) ((1 - 2 * Math.random()) * size));
        }

        public void setSquareOf(IntegerMatrix A, int threads) {
            if (this.size() != A.size())
                throw new IllegalArgumentException("this.size() != A.size()");
            if ((size() % threads) != 0)
            ...
            for (int t = 0; t < threads; t++) {
                int line0 = bunch * t;
                MatrixComputer computer = new MatrixComputer(value, A.value, line0, bunch);
                task[t] = new Thread(computer);
            }
            for (int t = 0; t < threads; t++) task[t].start();
            for (int t = 0; t < threads; t++) if (task[t].isAlive())
                try {
                    task[t].join();
        ...
        private static class MatrixComputer implements Runnable {
            ...
            public MatrixComputer(int[][] result, int[][] source, int line0, int bunch) {
                this.result = result;
                this.source = source;
                this.line0 = line0;
                this.bunch = bunch;
    ...
    private static class LongMatrix {
        ...
        public LongMatrix(IntegerMatrix A) {
            ...
            for (int line = 0; line < size; line++) for (int column = 0; column < size; column++) value[line][column] = A.value[line][column];
        }
        ...
        private static class MatrixComputer implements Runnable {
            ...
            public MatrixComputer(long[][] result, long[][] source, int line0, int bunch) {
                this.result = result;
                this.source = source;
                this.line0 = line0;
                this.bunch = bunch;
    ...
    private static class FloatMatrix {
        ...
        private static class MatrixComputer implements Runnable {
            ...
            public MatrixComputer(float[][] result, float[][] source, int line0, int bunch) {
                this.result = result;
                this.source = source;
                this.line0 = line0;
                this.bunch = bunch;
    ...
    private static class DoubleMatrix {
        ...
        private static class MatrixComputer implements Runnable {
            ...
            public MatrixComputer(double[][] result, double[][] source, int line0, int bunch) {
                this.result = result;
                this.source = source;
                this.line0 = line0;
                this.bunch = bunch;
