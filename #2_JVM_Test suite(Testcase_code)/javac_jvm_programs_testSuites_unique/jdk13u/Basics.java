...
public class Basics {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if ((ssle.getEnabledCipherSuites().length != 1) || !(suites[0].equals(secondSuite))) {
        ...
        if ((ssle.getEnabledProtocols().length != 1) || !(protocols[0].equals(secondProtocol))) {
        ...
        if (ssle.wrap(appBB, netBB).getStatus() != Status.BUFFER_OVERFLOW) {
        ...
        if ((sslER = ssle.unwrap(ByteBuffer.wrap(smallSSLHeader), appBB)).getStatus() != Status.BUFFER_UNDERFLOW) {
            System.out.println(sslER);
            throw new Exception("unwrap should underflow");
        }
        if ((sslER = ssle.unwrap(ByteBuffer.wrap(incompleteSSLHeader), appBB)).getStatus() != Status.BUFFER_UNDERFLOW) {
            System.out.println(sslER);
            throw new Exception("unwrap should underflow");
        }
        if ((sslER = ssle.unwrap(ByteBuffer.wrap(smallv2Header), appBB)).getStatus() != Status.BUFFER_UNDERFLOW) {
            System.out.println(sslER);
            throw new Exception("unwrap should underflow");
        }
        try {
    ...
    static byte[] incompleteSSLHeader = new byte[] { (byte) 0x16, (byte) 0x03, (byte) 0x01, 
    (byte) 0x00, (byte) 0x5, (byte) 0x16, (byte) 0x03, (byte) 0x01, (byte) 0x00 };

    static byte[] smallv2Header = new byte[] { (byte) 0x80, (byte) 0x03, (byte) 0x01, (byte) 0x00 };

    static byte[] gobblydegook = new byte[] { (byte) 0x48, (byte) 0x45, (byte) 0x4C, (byte) 0x4C, (byte) 0x20, (byte) 0x48, (byte) 0x45, (byte) 0x4C, (byte) 0x4C };
