...
public class DrawImageBilinear extends Canvas {
    ...
    private static void testRegion(BufferedImage bi, Rectangle affectedRegion) {
        ...
        for (int y = y1; y < y2; y++) {
            for (int x = x1; x < x2; x++) {
                ...
                if ((actual != 0xfffe0000) && (actual != 0xffff0000)) {
                    throw new RuntimeException("Test failed at x=" + x + " y=" + y + " (expected=0xffff0000" + " actual=0x" + Integer.toHexString(actual) + ")");
                }
            }
        }
    }
