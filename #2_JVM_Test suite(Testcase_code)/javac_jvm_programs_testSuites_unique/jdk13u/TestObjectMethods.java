...
public class TestObjectMethods {
    ...
    static void testToString(Class<?> clazz) {
        ...
        for (Method m : methods) {
            ...
            String typeString = (type instanceof Class) ? type.getTypeName() : type.toString();
            ...
            switch(relation) {
                case EQUAL:
                    ...
                    break;
                case POSTFIX:
                    valid = annotTypeString.endsWith(typeString) && !annotTypeString.startsWith(typeString);
                    break;
                case STRIPPED:
    ...
    static void testEqualsReflexivity(Class<?> clazz) {
        ...
        Method[] methods = clazz.getDeclaredMethods();
        for (Method m : methods) {
            checkTypesForEquality(m.getAnnotatedReturnType(), m.getAnnotatedReturnType(), true);
        }
    }

    private static void checkTypesForEquality(AnnotatedType annotType1, AnnotatedType annotType2, boolean expected) {
        ...
        for (int i = 0; i < methods.length; i++) {
            for (int j = 0; j < methods.length; j++) {
                if (i == j)
                    ...
                    checkTypesForEquality(methods[i].getAnnotatedReturnType(), methods[j].getAnnotatedReturnType(), false);
                }
            }
        }
    }
    ...
    static void testAnnotationsMatterForEquals(Class<?> clazz1, Class<?> clazz2) {
        ...
        for (Method method : clazz1.getDeclaredMethods()) {
            ...
            try {
                checkTypesForEquality(method.getAnnotatedReturnType(), clazz2.getDeclaredMethod(methodName).getAnnotatedReturnType(), false);
            } catch (Exception e) {
                errors++;
    ...
    static void testWildcards() {
        ...
        if (!awt1.equals(extractWildcard("fooNumberSet")) || !awt2.equals(extractWildcard("fooNumberSet2"))) {
            errors++;
    ...
    private static AnnotatedWildcardType extractWildcard(String methodName) {
        try {
            return (AnnotatedWildcardType) (((AnnotatedParameterizedType) (AnnotatedTypeHost.class.getMethod(methodName).getAnnotatedReturnType())).getAnnotatedActualTypeArguments()[0]);
        } catch (Exception e) {
    ...
    static class TypeHost<E, F extends Number> {
        ...
        @AnnotTypeInfo
        public int[] fooIntArray() {
            return null;
        }

        @AnnotTypeInfo
        public String[] fooStringArray() {
            return null;
        }

        @AnnotTypeInfo
        public String[][] fooStringArrayArray() {
            return null;
        }

        @AnnotTypeInfo
        public Set<String> fooSetString() {
            return null;
        }

        @AnnotTypeInfo
        public Set<Number> fooSetNumber() {
            return null;
        }
        ...
        @AnnotTypeInfo
        public Set<? extends Number> fooNumberSet() {
            return null;
        }

        @AnnotTypeInfo
        public Set<? extends Integer> fooNumberSet2() {
            return null;
        }

        @AnnotTypeInfo
        public Set<? extends Long> fooNumberSet3() {
            return null;
        }

        @AnnotTypeInfo
        public Set<?> fooObjectSet() {
            return null;
        }

        @AnnotTypeInfo
        public List<? extends Object> fooObjectList() {
            return null;
        }
    }
    ...
    static private enum Relation {
        ...
        EQUAL, POSTFIX, STRIPPED, ARRAY, OTHER
    }
    ...
    static class AnnotatedTypeHost<E, F extends Number> {
        ...
        public int @AnnotType(3) [] fooIntArray() {
            return null;
        }

        @AnnotTypeInfo(count = 1, relation = Relation.ARRAY)
        public String @AnnotType(4) [] fooStringArray() {
            return null;
        }

        @AnnotTypeInfo(count = 3, relation = Relation.ARRAY)
        @AnnotType(5)
        public String @AnnotType(0) [] @AnnotType(1) [] fooStringArrayArray() {
            return null;
        }
        ...
        @AnnotType(7)
        public Set<@AnnotType(8) Number> fooSetNumber() {
            return null;
        }
        ...
        @AnnotTypeInfo(count = 2, relation = Relation.STRIPPED)
        @AnnotType(13)
        public Set<@AnnotType(14) ? extends Number> fooNumberSet2() {
            return null;
        }

        @AnnotTypeInfo(count = 2, relation = Relation.STRIPPED)
        @AnnotType(15)
        public Set<? extends @AnnotType(16) Long> fooNumberSet3() {
            return null;
        }

        @AnnotTypeInfo(count = 2, relation = Relation.STRIPPED)
        @AnnotType(16)
        public Set<@AnnotType(17) ?> fooObjectSet() {
            return null;
        }

        @AnnotTypeInfo(count = 2, relation = Relation.OTHER)
        @AnnotType(18)
        public List<? extends @AnnotType(19) Object> fooObjectList() {
            return null;
        }
    }
}
