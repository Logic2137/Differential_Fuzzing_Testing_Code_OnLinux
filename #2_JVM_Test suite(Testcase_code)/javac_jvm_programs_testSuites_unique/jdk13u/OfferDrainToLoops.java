                ...
                for (long i = 0; !quittingTime(i); i++) {
                    if (q.offer(c)) {
                        if ((++c % 1024) == 0) {
                            ...
                            while (approximateCount.get() > 10000) Thread.yield();
                        }
                    } else {
                        Thread.yield();
                    }
        ...
        Thread drainer = new CheckedThread("drainer", rnd.split()) {
            ...
            protected void realRun() {
                while (!quittingTime()) {
                    ...
                    int n = rnd.nextBoolean() ? q.drainTo(list) : q.drainTo(list, 100);
                    approximateCount.getAndAdd(-n);
                    equal(list.size(), n);
                    for (int j = 0; j < n - 1; j++) equal((Long) list.get(j) + 1L, list.get(j + 1));
                    Thread.yield();
                }
                q.clear();
