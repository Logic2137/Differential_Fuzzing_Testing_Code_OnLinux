...
public class ResumeAtExit extends Thread {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N_THREADS; i++) {
            ...
            System.out.println("INFO: thread #" + i + ": made " + late_count + " late calls to java.lang.Thread.resume()");
            System.out.println("INFO: thread #" + i + ": N_LATE_CALLS==" + N_LATE_CALLS + " value is " + ((late_count >= N_LATE_CALLS) ? "NOT " : "") + "large enough to cause a Thread.resume() " + "call after thread exit.");
            try {
                threads[i].join();
            } catch (InterruptedException e) {
                throw new Error("Unexpected: " + e);
