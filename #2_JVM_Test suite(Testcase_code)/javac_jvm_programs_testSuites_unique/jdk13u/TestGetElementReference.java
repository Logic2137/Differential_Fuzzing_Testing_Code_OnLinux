...
public class TestGetElementReference {
    ...
    private static TreePath pathFor(final Trees trees, final CompilationUnitTree cut, final int pos) {
        ...
        new TreePathScanner<Void, Void>() {
            ...
            public Void scan(Tree node, Void p) {
                if (node != null && trees.getSourcePositions().getStartPosition(cut, node) <= pos && pos <= trees.getSourcePositions().getEndPosition(cut, node)) {
    ...
    private static String symbolToString(Element el) {
        switch(el.getKind()) {
            case METHOD:
                return symbolToString(el.getEnclosingElement()) + "." + el.toString();
            case CONSTRUCTOR:
                return symbolToString(el.getEnclosingElement().getEnclosingElement()) + "." + el.toString();
            default:
                return el.toString();
        }
    }
