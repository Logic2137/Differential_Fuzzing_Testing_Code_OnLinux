...
public class TestSessionLocalPrincipal {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String tlsProtocol : new String[] { "TLSv1.3", "TLSv1.2", "TLSv1.1", "TLSv1" }) {
            for (boolean clientAuth : new boolean[] { true, false }) {
                System.out.printf("Protocol %s: Client side auth enabled: %s%n", tlsProtocol, clientAuth);
                CountDownLatch serverReady = new CountDownLatch(1);
                Server server = new Server(tlsProtocol, clientAuth, serverReady);
                server.start();
                serverReady.await();
    ...
    public static class Server implements Runnable {
        ...
        public Server(String tlsProtocol, boolean clientAuth, CountDownLatch latch) {
