...
public class ProxyServer extends Thread implements Closeable {
    ...
    public ProxyServer(Integer port, Boolean debug, String username, String password) throws IOException {
    ...
    public void close() throws IOException {
        if (debug)
            System.out.println("Proxy: closing server");
        done = true;
        listener.close();
    ...
    class Connection {
        ...
        private boolean authorized(Credentials credentials, List<String> requestHeaders) {
            List<String> authorization = requestHeaders.stream().filter(n -> n.toLowerCase(Locale.US).startsWith("proxy-authorization")).collect(toList());
        ...
        public void init() {
            try {
                ...
                while (true) {
                    ...
                    if (findCRLF(buf) == -1) {
                        if (debug)
                            System.out.println("Proxy: no CRLF closing, buf contains:[" + new String(buf, UTF_8) + "]");
                        close();
                        return;
                    }
        ...
        void doProxy(String dest, byte[] buf, int p, String cmdLine) throws IOException {
            try {
