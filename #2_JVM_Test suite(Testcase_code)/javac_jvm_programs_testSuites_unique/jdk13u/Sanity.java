...
public class Sanity {
    ...
    static void test(InetAddress addr) throws Exception {
        ...
        try {
            ...
            try {
                ...
                try {
                    testConnection(Channels.newOutputStream(client), Channels.newInputStream(peer));
                } finally {
                    peer.close();
                }
            } finally {
            ...
            try {
                client.bind(new InetSocketAddress(addr, 0)).connect(new InetSocketAddress(addr, port));
                ssc.accept().close();
            } finally {
                client.close();
            }
        ...
        try {
            ...
            try {
                client.bind(new InetSocketAddress(addr, 0)).connect(new InetSocketAddress(addr, port)).get();
    ...
    static void testConnection(OutputStream out, InputStream in) throws IOException {
