...
public class numeric004 {
    ...
    public static int run(String[] args, PrintStream out) {
        ...
        if ((args.length < argsShift + 1) || (args.length > argsShift + 2)) {
            ...
            complain("    java numeric004 [-verbose] [-performance] [-CPU:number] " + "matrixSize [threads]");
            return 2;
        }
        ...
        for (int line = 0; line < size; line++) for (int column = 0; column < size; column++) if (A1.value[line][column] != Am.value[line][column]) {
            println("");
    ...
    private static double elapsedTime(PrintStream out, SquareMatrix A, SquareMatrix AA, int size, int threads) {
    ...
    private static class SquareMatrix {
        ...
        public SquareMatrix(int size) {
            ...
            for (int line = 0; line < size; line++) for (int column = 0; column < size; column++) value[line][column] = Math.round((float) (Math.random() * size));
        }

        public void setSquareOf(SquareMatrix A, int threads) {
            if (this.value.length != A.value.length)
                throw new IllegalArgumentException("this.value.length != A.value.length");
