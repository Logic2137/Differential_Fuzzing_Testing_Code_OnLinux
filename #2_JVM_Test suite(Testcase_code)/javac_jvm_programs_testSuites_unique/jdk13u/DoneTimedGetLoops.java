...
public class DoneTimedGetLoops {
    ...
    DoneTimedGetLoops(String[] args) {
        testDurationMillis = (args.length > 0) ? Long.valueOf(args[0]) : testDurationMillisDefault;
    }
    ...
    void test(String[] args) throws Throwable {
        ...
        final AtomicReference<PublicFutureTask> normalRef = new AtomicReference<>();
        final AtomicReference<PublicFutureTask> abnormalRef = new AtomicReference<>();
        ...
        for (Thread thread : new Thread[] { setter, setterException, doneTimedGetNormal, doneTimedGetAbnormal }) {
            thread.join(timeoutMillis + testDurationMillis);
            if (thread.isAlive()) {
                System.err.printf("Hung thread: %s%n", thread.getName());
                failed++;
                for (StackTraceElement e : thread.getStackTrace()) System.err.println(e);
