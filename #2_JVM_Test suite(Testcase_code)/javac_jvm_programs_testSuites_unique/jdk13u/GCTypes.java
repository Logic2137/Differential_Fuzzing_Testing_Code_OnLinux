...
public final class GCTypes {
    ...
    private static <T extends GCType> T getCurrentGCType(Class<T> type) {
        return ManagementFactory.getGarbageCollectorMXBeans().stream().map(bean -> getGCTypeByName(type, bean.getName())).filter(Objects::nonNull).findFirst().orElse(null);
    }

    private static <T extends GCType> T getGCTypeByName(Class<T> type, String name) {
        return Arrays.stream(type.getEnumConstants()).filter(e -> e.getGCName().equals(name)).findFirst().orElse(null);
    }

    private static <T extends GCType> GarbageCollectorMXBean getGCBeanByType(Class<T> type) {
        return ManagementFactory.getGarbageCollectorMXBeans().stream().filter(bean -> Arrays.stream(type.getEnumConstants()).filter(enumName -> enumName.getGCName().equals(bean.getName())).findFirst().isPresent()).findFirst().orElse(null);
    }
    ...
    public static enum YoungGCType implements GCType {
        ...
        DefNew("Copy"), ParNew("ParNew"), PSNew("PS Scavenge"), G1("G1 Young Generation");
    ...
    public static enum OldGCType implements GCType {
        ...
        Serial("MarkSweepCompact"), CMS("ConcurrentMarkSweep"), PSOld("PS MarkSweep"), G1("G1 Old Generation");
