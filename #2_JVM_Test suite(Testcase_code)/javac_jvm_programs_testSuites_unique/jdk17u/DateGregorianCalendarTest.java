...
public class DateGregorianCalendarTest {
        ...
        try {
                ...
                public void run() {
                    ...
                    try {
                        for (long t = Long.MIN_VALUE; runrun && t < Long.MAX_VALUE - delta; t += delta) {
                            ...
                            int y;
                            if (!((y = gc.get(YEAR)) == (date.getYear() + 1900) && gc.get(MONTH) == date.getMonth() && gc.get(DAY_OF_MONTH) == date.getDate() && gc.get(HOUR_OF_DAY) == date.getHours() && gc.get(MINUTE) == date.getMinutes() && gc.get(SECOND) == date.getSeconds())) {
                                throw new RuntimeException("GregorinCalendar and Date returned different dates." + " (millis=" + t + ")\n" + "GC=" + gc + "\nDate=" + date);
                            }
                            ++count;
                            if (y >= 1) {
            ...
            t[index++] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    int delta;
                    try {
                        for (long year = Integer.MIN_VALUE + 1900; runrun && year <= Integer.MAX_VALUE; year += delta) {
                            checkTimes(gc, year, JANUARY, 1, 0, 0, 0);
                            ++count;
                            delta = getDelta((int) year);
                        }
                        for (long month = Integer.MIN_VALUE; runrun && month <= Integer.MAX_VALUE; month += delta) {
                            checkTimes(gc, 1900, month, 1, 0, 0, 0);
                            ++count;
                            delta = getDelta(gc.get(YEAR));
                        }
                        for (long dayOfMonth = Integer.MIN_VALUE; runrun && dayOfMonth <= Integer.MAX_VALUE; dayOfMonth += delta) {
            ...
            t[index++] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    int delta;
                    try {
                        for (long hourOfDay = Integer.MIN_VALUE; runrun && hourOfDay <= Integer.MAX_VALUE; hourOfDay += delta) {
                            checkTimes(gc, 1970, JANUARY, 1, hourOfDay, 0, 0);
                            ++count;
                            delta = getDelta(gc.get(YEAR));
                        }
                        for (long minutes = Integer.MIN_VALUE; runrun && minutes <= Integer.MAX_VALUE; minutes += delta) {
                            checkTimes(gc, 1970, JANUARY, 1, 0, minutes, 0);
                            ++count;
                            delta = getDelta(gc.get(YEAR)) * 60;
                        }
                        for (long seconds = Integer.MIN_VALUE; runrun && seconds <= Integer.MAX_VALUE; seconds += delta) {
    ...
    static void checkTimes(GregorianCalendar gc, long year, long month, long dayOfMonth, long hourOfDay, long minutes, long seconds) {
        gc.clear();
        gc.set((int) year, (int) month, (int) dayOfMonth, (int) hourOfDay, (int) minutes, (int) seconds);
        long time = gc.getTimeInMillis();
        Date date = new Date((int) year - 1900, (int) month, (int) dayOfMonth, (int) hourOfDay, (int) minutes, (int) seconds);
