...
public class WindowResizingOnSetLocationTest {
    ...
    private static final String INSTRUCTIONS = "INSTRUCTIONS:\n" + "Verify that a window is properly resized after setting the location" + " to a display with different DPI.\n" + "\n" + "The test is applicable for a multi-monitor system where displays" + " are configured to have different DPI\n" + "\n" + "1. Press Show Frame button\n" + "  (note that the button is disabled in case there are no two monitors" + " with different DPI)\n" + "The frame should appear in the center of the display (either" + " on the first or on the second).\n" + "2. Check that the string \"scales [ScaleX, ScaleY]\" is painted on the window" + " where ScaleX and ScaleY are the scales for current display.\n" + "The scales are calculated as DPI / 96 and are 1 for the DPI value 96" + " and 2 for the DPI value 192.\n" + "3. Press 'Move to another display' button.\n" + "4. Check that the frame appears on the another display.\n" + "5. Check that the string \"scales [ScaleX, ScaleY]\" is updated" + " to show the right display scales.\n" + "6. Check that the window is properly resized.\n" + "7. Check that the window is properly repainted and does not contain drawing artifacts\n" + "If all tests are passed, press PASS, else press FAIL.\n";
    ...
    private static void createUI() {
        ...
        frameButton.addActionListener((e) -> {
            GraphicsConfiguration gc = GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices()[screen1].getDefaultConfiguration();
    ...
    static JComponent getDisplaysComponent() {
        ...
        for (int i = 0; i < screenBounds.length; i++) {
            ...
            g.drawString(String.format("Scale: [%2.1f, %2.1f]", scaleX, scaleY), r.x + r.width / 8, r.y + r.height / 2);
        }
        g.dispose();
    ...
    static class TestFrame extends JFrame {
        ...
        public TestFrame(GraphicsConfiguration gc, Rectangle rect) throws HeadlessException {
            ...
            button.addActionListener((e) -> {
                ...
                GraphicsDevice[] devices = GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices();
                int index = devices[screen1].getDefaultConfiguration().equals(config) ? screen2 : screen1;
    ...
    static class TestMultiResolutionImage extends AbstractMultiResolutionImage {
        ...
        public Image getResolutionVariant(double destImageWidth, double destImageHeight) {
            ...
            g.drawString(String.format("scales: [%1.2fx, %1.2fx]", scaleX, scaleY), width / 6, height / 2);
            g.dispose();
            return img;
        }
