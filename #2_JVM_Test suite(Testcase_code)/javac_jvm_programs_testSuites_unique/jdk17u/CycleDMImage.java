...
public class CycleDMImage extends Component implements Runnable, KeyListener {
    ...
    public synchronized void paint(Graphics g) {
        ...
        if (rImage == null) {
            ...
            wImage = gc.createCompatibleImage(imgSize, imgSize, Transparency.BITMASK);
            bImage = gc.createCompatibleImage(imgSize, imgSize, Transparency.TRANSLUCENT);
    ...
    public boolean checkResult(DisplayMode dm) {
        ...
        BufferedImage clientPixels = robot.createScreenCapture(new Rectangle(bounds.x, bounds.y, imgSize * 4, 1));
        ...
        for (int color = 0; color < 4; ++color) {
            for (int i = 0; i < imgSize; ++i) {
                ...
                if (pixels[pixelIndex] != colors[color]) {
                    errorMessage = "\n    DisplayMode(" + dm.getWidth() + " x " + dm.getHeight() + " x " + dm.getBitDepth() + "bpp x " + dm.getRefreshRate() + ")\n    Pixel " + i + ": Expected " + Integer.toHexString(colors[color]) + ", got " + Integer.toHexString(pixels[pixelIndex]) + " at " + i;
                    return false;
                }
            }
        }
    ...
    boolean displayModesEqual(DisplayMode dm1, DisplayMode dm2) {
        ...
        if (dm1.getWidth() != dm2.getWidth() || dm1.getHeight() != dm2.getHeight() || dm1.getBitDepth() != dm2.getBitDepth()) {
            return false;
        }
    ...
    public void run() {
        ...
        if (!gd.isDisplayChangeSupported()) {
            System.err.println("Display change is not supported," + " the test is considered passed.");
            finished();
            return;
        }
        ...
        for (final DisplayMode dm : gd.getDisplayModes()) {
            ...
            for (final DisplayMode dmUnique : dmSubset) {
                ...
                if (bitDepth == 24 || width <= 800 || height <= 600 || (dmUnique.getWidth() == width && dmUnique.getHeight() == height && dmUnique.getBitDepth() == bitDepth)) {
                    skip = true;
                    break;
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (final GraphicsDevice gd : ge.getScreenDevices()) {
            if (!gd.isFullScreenSupported()) {
                System.err.println("FullScreen mode is not supported," + " the test is considered passed.");
                continue;
            }
            done = false;
