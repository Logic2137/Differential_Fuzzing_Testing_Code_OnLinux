...
public class RepaintOnFrameIconifiedStateChangeTest {
    ...
    private static final String[][] strsForComps = new String[][] { { "JLabel AAA", "JLabel BBB" }, { "JButton AAA", "JButton BBB" } };
    ...
    public static void main(String[] args) {
        ...
        if (!toolkit.isFrameStateSupported(JFrame.ICONIFIED) || !toolkit.isFrameStateSupported(JFrame.NORMAL)) {
            System.out.println("ICONIFIED or NORMAL frame states are not" + "supported by a toolkit.");
            return;
        }
        try {
            ...
            for (int i = 0; i < compRedrawn.length; i++) {
                if (!compRedrawn[i]) {
                    ...
                    System.out.println(String.format("Not redrawn component #%d: '%s'", i, comps[i]));
                }
            }
            if (notRedrawnCompsCount > 0) {
                throw new RuntimeException(String.format("'%d' components were not redrawn.", notRedrawnCompsCount));
            }
            System.out.println("Test passed.");
        } catch (InterruptedException | InvocationTargetException | AWTException e) {
