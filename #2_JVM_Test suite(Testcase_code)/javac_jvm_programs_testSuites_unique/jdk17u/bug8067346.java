...
public class bug8067346 {
    ...
    private String[] menuClasses = { "MenuItem", "Menu", "CheckBoxMenuItem", "RadioButtonMenuItem" };
    ...
    public void performTest() {
        try {
            ...
            if (retVal != 0) {
                errorMessageBuilder.append(menuClasses[retVal]).append(MARGIN).append("\n");
                error = true;
            }
            retVal = testCheckIconOffset();
            if (retVal != 0) {
                errorMessageBuilder.append(errorMessage).append(menuClasses[retVal]).append(CHECKICONOFFSET);
            }
            if (error || retVal != 0) {
    ...
    private int testMargin() {
        for (int inx = 0; inx < menuClasses.length; inx++) {
            ...
            if (margin != null && margin.bottom == 0 && margin.left == 0 && margin.right == 0 && margin.top == 0) {
                return inx + 1;
            }
        }
        return 0;
    ...
    private int testCheckIconOffset() {
        for (int inx = 0; inx < menuClasses.length; inx++) {
            Object checkIconOffset = UIManager.get(menuClasses[inx] + CHECKICONOFFSET);
