...
public class StackStreamTest {
    ...
    static class G {
        ...
        private static final List<String> GOLDEN_CLASS_NAMES = Arrays.asList("StackStreamTest$G", "StackStreamTest$F", "StackStreamTest$E", "StackStreamTest$D", "StackStreamTest$C", "StackStreamTest$B", "StackStreamTest$A", "StackStreamTest", "StackStreamTest");
        ...
        public static void g() {
            ...
            List<String> sfs = DEFAULT_WALKER.walk(s -> {
                return s.filter(StackStreamTest::isTestClass).map(StackFrame::getClassName).collect(Collectors.toList());
            });
            equalsOrThrow("class names", sfs, GOLDEN_CLASS_NAMES);
            System.out.println("methodNames()");
            sfs = DEFAULT_WALKER.walk(s -> {
                return s.filter(StackStreamTest::isTestClass).map(StackFrame::getMethodName).collect(Collectors.toList());
            });
            ...
            StackWalker.getInstance().walk(s -> {
                s.map(StackFrame::toStackTraceElement).forEach(ste -> System.out.println("STE: " + ste));
                return null;
            });
            System.out.println("Collect StackTraceElement");
            List<StackTraceElement> stacktrace = STE_WALKER.walk(s -> {
                return s.filter(StackStreamTest::isTestClass).collect(Collectors.mapping(StackFrame::toStackTraceElement, Collectors.toList()));
            });
            int i = 0;
        ...
        static void checkStackTraceElements(List<String> classNames, List<String> methodNames, List<StackTraceElement> stes) {
            ...
            for (int i = 0; i < classNames.size(); i++) {
                if (!classNames.get(i).equals(stes.get(i).getClassName()) || !methodNames.get(i).equals(stes.get(i).getMethodName())) {
                    dumpSTEInfo(classNames, methodNames, stes);
        ...
        static void dumpSTEInfo(List<String> classNames, List<String> methodNames, List<StackTraceElement> stes) {
        ...
        static void firstFrame() {
            ...
            Optional<StackFrame> frame = sw.walk(s -> {
                return s.filter(e -> {
                    System.err.println(e.getClassName() + " == " + e.getClassName().equals("StackStreamTest"));
