...
public class TransformedPaintTest {
    ...
    private static enum PaintType {
        ...
        COLOR, GRADIENT, LINEAR_GRADIENT, RADIAL_GRADIENT, TEXTURE
    }
    ...
    private void renderLine(PaintType type, Graphics2D g, int startx, int starty, int w, int h) {
    ...
    private void render(Graphics2D g, int w, int h) {
        ...
        for (PaintType type : PaintType.values()) {
            renderLine(type, (Graphics2D) g.create(), 0, y, h / paintTypes, h / paintTypes);
            y += ystep;
        }
    }
    ...
    private static void showFrame(final TransformedPaintTest t) {
        ...
        final BufferedImage bi = new BufferedImage(R_WIDTH, R_HEIGHT, BufferedImage.TYPE_INT_RGB);
        JPanel p = new JPanel() {
