...
public class TestButtonGroupFocusTraversal {
    ...
    private static void pressKey(int... keys) {
        int num = keys.length;
        for (int i = 0; i < num; i++) robot.keyPress(keys[i]);
        for (int i = num; i > 0; i--) robot.keyRelease(keys[i - 1]);
        robot.waitForIdle();
        robot.delay(200);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (UIManager.LookAndFeelInfo info : infos) {
            ...
            try {
                ...
                if (!textFieldFirst.equals(KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner())) {
                    try {
                        Thread.sleep(100);
