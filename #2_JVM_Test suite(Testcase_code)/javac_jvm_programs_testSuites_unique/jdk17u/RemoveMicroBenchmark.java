...
public class RemoveMicroBenchmark {
    ...
    final ArrayList<Integer> elements;
    ...
    RemoveMicroBenchmark(String[] args) {
        ...
        elements = ThreadLocalRandom.current().ints(size).mapToObj(x -> x).collect(toCollection(ArrayList::new));
    }
    ...
    static void forceFullGc() {
        ...
        try {
            for (int tries = 3; tries-- > 0; ) {
                System.gc();
                if (finalized.await(timeoutMillis, MILLISECONDS) && queue.remove(timeoutMillis) != null && ref.get() == null) {
    ...
    void time(List<Job> jobs) throws Throwable {
        ...
        for (int i = 0; i < size; i++) System.out.printf(format, jobs.get(i).name(), milliss[i], ratios[i]);
    }
    ...
    String goodClassName(Class<?> klazz) {
        return goodClassName.computeIfAbsent(klazz, k -> {
            String simple = k.getSimpleName();
            return (simple.equals("SubList")) ? k.getName().replaceFirst(".*\\.", "") : simple;
        });
    }
    ...
    Stream<Job> jobs(Collection<Integer> x) {
        return concatStreams(collectionJobs(x), (CopyOnWriteArrayList.class.isAssignableFrom(topLevelClass(x))) ? Stream.empty() : iteratorRemoveJobs(x), (x instanceof Queue) ? queueJobs((Queue<Integer>) x) : Stream.empty(), (x instanceof Deque) ? dequeJobs((Deque<Integer>) x) : Stream.empty(), (x instanceof BlockingQueue) ? blockingQueueJobs((BlockingQueue<Integer>) x) : Stream.empty(), (x instanceof BlockingDeque) ? blockingDequeJobs((BlockingDeque<Integer>) x) : Stream.empty());
    }
    ...
    Stream<Job> collectionJobs(Collection<Integer> x) {
        ...
        return Stream.of(new Job(klazz + " removeIf") {
    ...
    Stream<Job> iteratorRemoveJobs(Collection<Integer> x) {
        final String klazz = goodClassName(x);
        return Stream.of(new Job(klazz + " Iterator.remove") {
    ...
    Stream<Job> queueJobs(Queue<Integer> x) {
        final String klazz = goodClassName(x);
        return Stream.of(new Job(klazz + " poll()") {
    ...
    Stream<Job> dequeJobs(Deque<Integer> x) {
        final String klazz = goodClassName(x);
        return Stream.of(new Job(klazz + " descendingIterator().remove") {
    ...
    Stream<Job> blockingQueueJobs(BlockingQueue<Integer> x) {
        final String klazz = goodClassName(x);
        return Stream.of(new Job(klazz + " timed poll()") {
    ...
    Stream<Job> blockingDequeJobs(BlockingDeque<Integer> x) {
        final String klazz = goodClassName(x);
        return Stream.of(new Job(klazz + " timed pollFirst()") {
