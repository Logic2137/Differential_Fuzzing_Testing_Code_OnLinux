...
public class EmbeddedFormatTest {
    ...
    static int[] bi_types = { BufferedImage.TYPE_INT_RGB, BufferedImage.TYPE_3BYTE_BGR, BufferedImage.TYPE_USHORT_555_RGB, BufferedImage.TYPE_BYTE_GRAY, BufferedImage.TYPE_BYTE_BINARY };
    ...
    protected void checkResult(BufferedImage img) {
        ...
        if (imgBlack != 0xff000000) {
            throw new RuntimeException("Wrong black color: " + Integer.toHexString(imgBlack));
        }
        int imgWhite = img.getRGB(100, 100);
        if (imgWhite != 0xffffffff) {
            throw new RuntimeException("Wrong white color: " + Integer.toHexString(imgWhite));
        }
    }
