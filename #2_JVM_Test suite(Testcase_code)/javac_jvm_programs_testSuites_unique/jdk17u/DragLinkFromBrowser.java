...
public class DragLinkFromBrowser implements ActionListener {
    ...
    public static void dragLinkFromWebBrowser() {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                urlFrame = new JFrame();
                urlPanel = new JPanel();
                failMessage = "Dragging link from web browser Failed. " + "getTransferData returns null";
                        ...
                        for (final DataFlavor flavor : flavors) {
                            try {
                                ...
                                if (transferData == null) {
                                    JOptionPane.showMessageDialog(urlPanel, failMessage);
                                    break;
                                } else {
                                    String flavorMessage = flavor.toString();
                                    String transferDataMessage = transferData.toString();
                                    if (flavorMessage.contains("error") || transferDataMessage.contains("null")) {
                                        JOptionPane.showMessageDialog(urlPanel, failMessage);
                                        break;
                                    }
                                }
                            } catch (UnsupportedFlavorException e) {
                                testFailed("UnsupportedFlavorException - " + "test Failed");
                            } catch (IOException e) {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        if (!isInterrupted) {
            throw new RuntimeException("Test Timed out after " + testTimeOut / 1000 + " seconds");
        }
    }
}
