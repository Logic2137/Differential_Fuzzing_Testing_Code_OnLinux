...
public class TestGreyReclaimedHumongousObjects {
    ...
    static class Runner extends Thread {
        ...
        public void run() {
            try {
                ...
                System.out.println("Aborted after " + (new Date().getTime() - startDate.getTime()) + " ms");
                interrupt();
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        int region_size = Integer.parseInt(diagnostic.getVMOption("G1HeapRegionSize").getValue());
        if (obj_size < (region_size / 2)) {
            throw new RuntimeException("Object size " + obj_size + " is not humongous with region size " + region_size);
        }
        ExecutorService executor = Executors.newFixedThreadPool(THREAD_COUNT, new NamedThreadFactory());
