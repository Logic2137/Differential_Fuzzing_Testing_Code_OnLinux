...
public class StringMonitorDeadlockTest {
    ...
    private static abstract class TestCase {
        ...
        void run() throws Exception {
            ...
            final StringMonitorMBean monitorProxy = JMX.newMBeanProxy(mbs, monitorName, StringMonitorMBean.class);
            ...
            if (when == When.IN_NOTIFY) {
                ...
                for (int i = 0; i < 500 && notifCount.get() == 0; i++) Thread.sleep(10);
                if (notifCount.get() == 0)
                    throw new Exception("Test failed: presumable deadlock");
            }
        }
    ...
    private static final TestCase[] testCases = { new TestCase("Remove monitored MBean within monitored getAttribute", When.IN_GET_ATTRIBUTE) {
        ...
        @Override
        void doSensitiveThing(StringMonitorMBean monitorProxy, ObjectName observedName) {
