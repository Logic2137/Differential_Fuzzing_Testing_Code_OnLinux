public class Unsigned {
    ...
    private static int testParseUnsignedInt() {
        int errors = 0;
        long[] inRange = { 0L, 1L, 10L, 2147483646L, 2147483647L, 2147483648L, MAX_UNSIGNED_INT - 1L, MAX_UNSIGNED_INT };
        for (long value : inRange) {
            for (int radix = Character.MIN_RADIX; radix <= Character.MAX_RADIX; radix++) {
                ...
                if (Integer.toUnsignedLong(intResult) != value) {
                    errors++;
                    System.err.printf("Bad roundtrip conversion of %d in base %d" + "\tconverting back ''%s'' resulted in %d%n", value, radix, longString, intResult);
                }
        ...
        String[] outOfRange = { null, "", "-1", Long.toString(MAX_UNSIGNED_INT + 1L), Long.toString(Long.MAX_VALUE) };
        for (String s : outOfRange) {
            try {
                int result = Integer.parseUnsignedInt(s);
                errors++;
                System.err.printf("Unexpected got %d from an unsigned conversion of %s", result, s);
