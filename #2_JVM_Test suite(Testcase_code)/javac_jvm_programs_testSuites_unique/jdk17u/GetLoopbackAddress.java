...
public class GetLoopbackAddress {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!(addr.equals(IPv4Loopback) || addr.equals(IPv6Loopback))) {
            throw new RuntimeException("Failed: getLoopbackAddress" + " not returning a valid loopback address");
        }
        InetAddress addr2 = InetAddress.getLoopbackAddress();
        if (addr != addr2) {
            throw new RuntimeException("Failed: getLoopbackAddress" + " should return a reference to the same InetAddress loopback instance.");
        }
        InetAddress addrFromNullHost = InetAddress.getByName(null);
        if (!addrFromNullHost.isLoopbackAddress()) {
            throw new RuntimeException("getByName(null) did not return a" + " loopback address, but " + addrFromNullHost);
        }
        InetAddress addrFromEmptyHost = InetAddress.getByName("");
        if (!addrFromEmptyHost.isLoopbackAddress()) {
            throw new RuntimeException("getByName with a host of length == 0," + " did not return a loopback address, but " + addrFromEmptyHost);
        }
        InetAddress[] addrsByNull = InetAddress.getAllByName(null);
        if (!addrsByNull[0].isLoopbackAddress()) {
            throw new RuntimeException("getAllByName(null) did not return" + " a loopback address, but " + addrsByNull[0]);
        }
        InetAddress[] addrsByEmpty = InetAddress.getAllByName("");
        if (!addrsByEmpty[0].isLoopbackAddress()) {
            throw new RuntimeException("getAllByName with a host of length" + " == 0, did not return a loopback address, but " + addrsByEmpty[0]);
        }
    }
}
