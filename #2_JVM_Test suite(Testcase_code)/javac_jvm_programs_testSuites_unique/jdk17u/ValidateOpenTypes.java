...
public class ValidateOpenTypes {
    ...
    private static String getProperty(TabularData td, String propName) {
        ...
        if (cd != null) {
            ...
            if (!propName.equals(key)) {
                throw new RuntimeException("TEST FAILED: " + key + " property found" + " but expected to be " + propName);
            }
    ...
    private static void checkMemoryUsage() throws Exception {
        ...
        if (getCommitted(u1) < 0 || getCommitted(u2) < 0 || getCommitted(u3) < 0 || getCommitted(u4) < 0) {
    ...
    private static void checkThreadInfo() throws Exception {
        ...
        result = server.invoke(thread, "getThreadInfo", new Object[] { ids, new Integer(2) }, new String[] { "[J", "int" });
        for (CompositeData cd : (CompositeData[]) result) {
            printThreadInfo(cd);
        }
        long id = Thread.currentThread().getId();
        result = server.invoke(thread, "getThreadInfo", new Object[] { new Long(id) }, new String[] { "long" });
        printThreadInfo((CompositeData) result);
        result = server.invoke(thread, "getThreadInfo", new Object[] { new Long(id), new Integer(2) }, new String[] { "long", "int" });
        printThreadInfo((CompositeData) result);
    }
