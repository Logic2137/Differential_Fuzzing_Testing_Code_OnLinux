...
public class Dynamic {
    ...
    static final String[] MODE = { "ECb", "CbC", "CTR", "PCBC", "OFB", "OFB150", "cFB", "CFB7", "cFB8", "cFB16", "cFB24", "cFB32", "Cfb40", "cfB48", "cfB56", "cfB64", "cfB72", "cfB80", "cfB88", "cfB96", "cfb104", "cfB112", "cfB120", "cfB128", "OFB8", "OFB16", "OFB24", "OFB32", "OFB40", "OFB48", "OFB56", "OFB64", "OFB72", "OFB80", "OFB88", "OFB96", "OFB104", "OFB112", "OFB120", "OFB128", "GCM" };

    static final String[] PADDING = { "NoPadding", "PKCS5Padding", "ISO10126Padding" };
        ...
        try {
            for (int i = 0; i < 3; i++) {
                ...
                for (int j = 0; j < MODE.length; j++) {
                    if (MODE[j].equalsIgnoreCase("ECB") || MODE[j].equalsIgnoreCase("PCBC") || MODE[j].equalsIgnoreCase("CBC")) {
                    ...
                    for (int k = 0; k < padKinds; k++) {
                        ...
                        try {
                            if (!runTest(ALGORITHM, MODE[j], PADDING[k])) {
                                result = false;
                                failedCnt++;
                                failedList.append(ALGORITHM + "/" + MODE[j] + "/" + PADDING[k] + " ");
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                            result = false;
            ...
            if (result) {
                out.println("STATUS:Passed. Test " + testCount + " cases, All Passed");
                return true;
            }
            out.println("STATUS:Failed. " + failedCnt + " Failed: " + failedList);
            return false;
        } catch (Exception ex) {
            ex.printStackTrace();
