...
public class ButtonGradientTest {
    ...
    private static void testGradient() throws Exception {
        ...
        int y = rect.y + mid / 2;
        Color buttonColor = new Color(((BufferedImage) (images.get(1))).getRGB((int) (x), (int) (y)));
    ...
    private static void getImageFromRobot() {
        try {
            ...
            MultiResolutionImage multiResolutionImage = robot.createMultiResolutionScreenCapture(new Rectangle((int) butLoc.getX(), (int) butLoc.getY(), (int) butSize.getWidth(), (int) butSize.getHeight()));
            images = multiResolutionImage.getResolutionVariants();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        if (images.size() < 2) {
            throw new RuntimeException("HiDpi Image not captured - " + "Check is this HiDpi display system?");
        }
    }
