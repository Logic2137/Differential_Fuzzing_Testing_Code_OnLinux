...
public class VerifyStackTrace {
    ...
    static final class TestCase2 implements TestCase {
        ...
        private final StackWalker walker = StackWalker.getInstance(EnumSet.of(RETAIN_CLASS_REFERENCE, SHOW_REFLECT_FRAMES));
    ...
    static class TestCase3 implements TestCase {
        ...
        private final StackWalker walker = StackWalker.getInstance(EnumSet.of(RETAIN_CLASS_REFERENCE, SHOW_HIDDEN_FRAMES));
    ...
    public static class Handle implements Runnable {
        ...
        public void run() {
            ...
            try {
                handle = lookup.findVirtual(Handle.class, "execute", MethodType.methodType(void.class, Runnable.class));
    ...
    static String prepare(String produced, boolean eraseSensitiveInfo) {
        if (eraseSensitiveInfo) {
            return produced.replaceAll(":[1-9][0-9]*\\)", ":00)").replaceAll("/0x[0-9a-f]+\\.run", "/xxxxxxxx.run").replaceAll("/0x[0-9a-f]+\\.invoke", "/xxxxxxxx.invoke").replaceAll("DirectMethodHandle\\$Holder", "LambdaForm\\$DMH").replaceAll("Invokers\\$Holder", "LambdaForm\\$MH").replaceAll("MH\\.invoke", "MH/xxxxxxxx.invoke").replaceAll("xx\\.invoke([A-Za-z]*)_[A-Z_]+", "xx.invoke$1").replaceAll("\\$[0-9]+", "\\$??");
        } else {
            return produced;
        }
    }
    ...
    static final class Recorder {
        ...
        public void recordSTE(long counter, StringBuilder s, StackFrame f) {
            if (found)
                return;
            found = VerifyStackTrace.class.equals(f.getDeclaringClass()) && "main".equals(f.getMethodName());
    ...
    static void test(TestCase test) {
        ...
        final Runnable run = () -> test.walker().forEach(f -> recorder.recordSTE(counter.incrementAndGet(), builder, f));
        ...
        PrivilegedAction<Object> pa = new PrivilegedAction<Object>() {
            ...
            public Object run() {
                try {
                    return VerifyStackTrace.class.getMethod("invoke", Runnable.class).invoke(null, handle);
