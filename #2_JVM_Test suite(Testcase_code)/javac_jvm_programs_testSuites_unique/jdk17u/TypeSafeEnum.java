...
public class TypeSafeEnum implements Serializable, ObjectInputValidation {
    ...
    private Object writeReplace() {
        numWriteReplace++;
        if (verbose) {
            System.out.println("TypeSafeEnum.writeReplace() " + this.toString());
        }
        return this;
    }
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        if (verbose) {
            System.out.println("TypeSafeEnum.writeObject() " + this.toString());
        }
    }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        for (int i = 0; i < writeArray.length - 1; i++) {
            if (writeArray[i] != readArray[i]) {
                throw new Error("Serializa/deserialize did not preserve " + "singleton for " + readArray[i].toString() + " and " + writeArray[i].toString());
            }
        }
    }
}
