...
public class MacVolumesTest {
    ...
    private static final void checkDataVolume() throws IOException {
        ...
        try (SeekableByteChannel sbc = Files.newByteChannel(tempFile, StandardOpenOption.WRITE)) {
    ...
    static void checkFirmlinks() throws IOException {
        ...
        if (!Files.exists(firmlinks)) {
            System.err.format("%s does not exist: skipping firmlinks test%n", firmlinks);
            return;
        } else if (!Files.isReadable(firmlinks)) {
            throw new RuntimeException(String.format("%s is not readable", firmlinks));
        }
        try (BufferedReader br = Files.newBufferedReader(firmlinks)) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (!Files.exists(path)) {
                    System.err.format("Firmlink %s does not exist: skipping%n", file);
                    continue;
                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (major < 10 || (major == 10 && minor < 15)) {
            System.out.format("macOS version %d.%d too old: skipping test%n", major, minor);
            return;
        }
        checkSystemVolume();
        checkDataVolume();
