...
public class InitialContextTest {
    ...
    private static void unknownInitialContextFactory() throws Throwable {
        String factoryClassFqn = "net.java.openjdk.test.UnknownInitialContextFactory";
        ...
        javac(build, dst);
        Map<String, String> props = singletonMap(Context.INITIAL_CONTEXT_FACTORY, factoryClassFqn);
        Result r = java(props, singleton(build), "Test");
        if (r.exitValue == 0 || !r.output.startsWith(stackTraceStringForClassNotFound(factoryClassFqn))) {
    ...
    private static void initialContextFactoryInAJar() throws Throwable {
        String factoryClassFqn = "net.java.openjdk.test.DummyInitialContextFactory";
        Path tmp = Files.createDirectory(Paths.get("InitialContextTest-2"));
        Path src = templatesHome().resolve("test.template");
        Path dst = tmp.resolve("Test.java");
        Files.copy(src, dst);
        Path dst1 = createFactoryFrom(templatesHome().resolve("factory.template"), factoryClassFqn, tmp);
        Path build = Files.createDirectory(tmp.resolve("build"));
        javac(build, dst);
    ...
    private static Path createFactoryFrom(Path srcTemplate, String factoryFqn, Path dstFolder) throws IOException {
        ...
        try (BufferedReader r = new BufferedReader(new FileReader(src));
            ...
            List<String> lines = processTemplate(packageName, factorySimpleName, r.lines()).collect(Collectors.toList());
    ...
    private static Stream<String> processTemplate(String packageName, String factorySimpleName, Stream<String> lines) {
        ...
        Function<String, String> factory = s -> s.replaceAll("\\$factoryName", factorySimpleName);
        return lines.map(pckg).map(factory);
    }

    private static void initialContextFactoryAsService() throws Throwable {
        String factoryClassFqn = "net.java.openjdk.test.BrokenInitialContextFactory";
        Path tmp = Files.createDirectory(Paths.get("InitialContextTest-3"));
        Path src = templatesHome().resolve("test.template");
        Path dst = tmp.resolve("Test.java");
        Files.copy(src, dst);
        Path dst1 = createFactoryFrom(templatesHome().resolve("broken_factory.template"), factoryClassFqn, tmp);
        Path build = Files.createDirectory(tmp.resolve("build"));
        javac(build, dst);
        Path explodedJar = Files.createDirectory(tmp.resolve("exploded-jar"));
        Path services = Files.createDirectories(explodedJar.resolve("META-INF").resolve("services"));
    ...
    private static void jar(Path jarName, Path jarRoot) {
        ...
        ProcessBuilder p = new ProcessBuilder(jar, "cf", jarName.toString(), "-C", jarRoot.toString(), ".");
        quickFail(run(p));
    }
    ...
    private static void javac(Path compilationOutput, Path... sourceFiles) {
        ...
        commands.addAll(paths.stream().map(Path::toString).collect(Collectors.toList()));
    ...
    private static Result java(Map<String, String> properties, Collection<Path> classpath, String classname) {
        String java = getJDKTool("java");
        List<String> commands = new ArrayList<>();
        commands.add(java);
        commands.addAll(properties.entrySet().stream().map(e -> "-D" + e.getKey() + "=" + e.getValue()).collect(Collectors.toList()));
    ...
    private static Result run(ProcessBuilder b) {
        ...
        try {
            ...
            throw new RuntimeException(format("Couldn't start process '%s'", b.command()), e);
        }
        String output;
        try {
            output = toString(p.getInputStream(), p.getErrorStream());
        } catch (IOException e) {
            throw new RuntimeException(format("Couldn't read process output '%s'", b.command()), e);
        }
        try {
            p.waitFor();
        } catch (InterruptedException e) {
            throw new RuntimeException(format("Process hasn't finished '%s'", b.command()), e);
        }
    ...
    private static String toString(InputStream... src) throws IOException {
        ...
        Reader concatenated = new InputStreamReader(new SequenceInputStream(Collections.enumeration(asList(src))));
