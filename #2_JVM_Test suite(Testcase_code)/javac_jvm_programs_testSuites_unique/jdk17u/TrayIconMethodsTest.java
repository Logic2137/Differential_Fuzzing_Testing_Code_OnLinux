...
public class TrayIconMethodsTest {
    ...
    void doTest() throws Exception {
        ...
        if (actionListeners == null || actionListeners.length != 2)
            throw new RuntimeException("FAIL: getActionListeners did not return the correct value " + "when there were two listeners present " + actionListeners);
        if (!isPresent(actionListeners, al1) || !isPresent(actionListeners, al2))
            throw new RuntimeException("FAIL: All the action listeners added are not returned " + "by the method");
        MouseListener[] mouseListeners = icon.getMouseListeners();
        if (mouseListeners == null || mouseListeners.length != 2)
            throw new RuntimeException("FAIL: getMouseListeners did not return the correct value " + "when there were two listeners present " + mouseListeners);
        if (!isPresent(mouseListeners, ml1) || !isPresent(mouseListeners, ml2))
            throw new RuntimeException("FAIL: All the mouse listeners added are not returned " + "by the method");
        ...
        if (mouseMotionListeners == null || mouseMotionListeners.length != 2)
            throw new RuntimeException("FAIL: getMouseMotionListeners did not return the correct value " + "when there were two listeners present " + mouseMotionListeners);
        if (!isPresent(mouseMotionListeners, mml1) || !isPresent(mouseMotionListeners, mml2))
            throw new RuntimeException("FAIL: All the mouse motion listeners added are not returned " + "by the method");
        Image im = icon.getImage();
        if (!image.equals(im))
            throw new RuntimeException("FAIL: Images are not the same getImage()=" + im + " Image=" + image);
        if (!pm.equals(icon.getPopupMenu()))
            throw new RuntimeException("FAIL: PopupMenus are not the same getPopupMenu()=" + icon.getPopupMenu() + " PopupMenu=" + pm);
        if (!toolTip.equals(icon.getToolTip()))
            throw new RuntimeException("FAIL: ToolTips are not the same getToolTip()=" + icon.getToolTip() + " ToolTip=" + toolTip);
        if (icon.isImageAutoSize())
            throw new RuntimeException("FAIL: Auto size property is true by default");
        icon.setImageAutoSize(true);
        if (!icon.isImageAutoSize())
            throw new RuntimeException("FAIL: Auto size property is not set to " + "true by call to setImageAutoSize(true)");
        ...
        if (actionListeners == null || actionListeners.length != 0)
            throw new RuntimeException("FAIL: removeActionListener did not " + "remove the ActionListeners added " + actionListeners);
        ...
        if (mouseListeners == null || mouseListeners.length != 0)
            throw new RuntimeException("FAIL: removeMouseListener did not " + "remove the MouseListeners added " + mouseListeners);
        ...
        if (mouseMotionListeners == null || mouseMotionListeners.length != 0)
            throw new RuntimeException("FAIL: removeMouseMotionListener did not " + "remove the MouseMotionListeners added " + mouseMotionListeners);
        try {
            icon.setImage(null);
