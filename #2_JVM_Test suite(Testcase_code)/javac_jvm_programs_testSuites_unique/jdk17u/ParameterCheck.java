...
public class ParameterCheck {
    ...
    static boolean[] results = { false, false, false, false, false, false, true, true, false, true, true, true, false, false, false };
        ...
        try {
            ...
            for (int i = 0; i < numCases; i++) {
                try {
                    ...
                    if (results[i]) {
                        printErr(method, numGood, i, "java.lang.IndexOutOfBoundsException");
                    } else {
                        numGood++;
                    }
                    continue;
                } catch (OutOfMemoryError ome) {
                    printErr(method, numGood, i, "java.lang.OutOfMemoryError");
                    continue;
                }
                if (results[i]) {
                    numGood++;
                } else {
                    printErr(method, numGood, i, "No java.lang.IndexOutOfBoundsException");
                }
            }
            raf.seek(0);
            ...
            if (!thrown) {
                printErr(method, numGood, -1, "no NullPointerException for null b");
            }
    ...
    private static void printErr(String method, int numGood, int i, String expStr) {
        numBad++;
