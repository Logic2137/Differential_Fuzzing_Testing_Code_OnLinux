...
class Traffic {
    ...
    public void initiate(int n) throws IOException {
        if (n == 0)
            initiateConst();
        else if (n < 0)
    ...
    public void respond(int n) throws IOException {
        if (n == 0)
            respondConst();
        else if (n < 0)
            while (true) respondRandom();
        else
            while (n-- > 0) respondRandom();
    }

    private static final int MAX_BLOCKSIZE = 8 * 2;

    private void writeConstData(int n) throws IOException {
        if (n <= 0)
            return;
        byte[] buf = new byte[n];
    ...
    private void readConstData(int n) throws IOException {
        if (n <= 0)
            return;
        byte[] buf = new byte[n];
        in.read(buf);
    ...
    private void initiateConst() throws IOException {
    ...
    private void respondConst() throws IOException {
        for (int i = 1; i <= MAX_BLOCKSIZE; i++) {
            readConstData(i);
            writeConstData(i);
        }
    ...
    private void readRandomData() throws IOException {
        int n = readInt();
    ...
    private void initiateRandom() throws IOException {
        writeRandomData();
        readRandomData();
    }

    private void respondRandom() throws IOException {
        readRandomData();
        writeRandomData();
    }
