...
public class GetPolicyQualifiers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        X509Certificate mostTrustedCaCert = (X509Certificate) cf.generateCertificate(new FileInputStream(f));
        Set trustAnchors = Collections.singleton(new TrustAnchor(mostTrustedCaCert, null));
        f = new File(System.getProperty("test.src", "."), "speech2eve");
        X509Certificate eeCert = (X509Certificate) cf.generateCertificate(new FileInputStream(f));
        CertPathValidator cpv = CertPathValidator.getInstance("PKIX", "SUN");
        PKIXParameters params = new PKIXParameters(trustAnchors);
        params.setPolicyQualifiersRejected(false);
        params.setRevocationEnabled(false);
        params.setDate(DateFormat.getDateInstance(DateFormat.MEDIUM, Locale.US).parse("July 01, 2020"));
        ...
        try {
            ...
            throw new Exception("Validation of CertPath containing critical " + "qualifiers should have failed when policyQualifiersRejected " + "flag is true");
