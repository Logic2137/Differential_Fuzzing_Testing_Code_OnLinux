...
public class BaseClassesNotReRead extends AbstractProcessor {
    ...
    void run() throws IOException {
        ...
        try (StandardJavaFileManager fm = compiler.getStandardFileManager(null, null, null)) {
            Iterable<? extends JavaFileObject> files = fm.getJavaFileObjects(new File(sources, "BaseClassesNotReReadSource.java"));
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        if (round++ == 1) {
            for (int c = 1; c <= 6; c++) {
                generateSource("GenClass" + c, "public class GenClass" + c + " { public void test() { } }");
            }
            for (int c = 1; c <= 3; c++) {
                generateSource("GenIntf" + c, "public interface GenIntf" + c + " { public void test(); }");
            }
            generateSource("GenAnnotation", "public @interface GenAnnotation { }");
            generateSource("GenException", "public class GenException extends Exception { }");
        }
        return false;
    }
    ...
    final class OnlyOneReadFileManager extends ForwardingJavaFileManager<JavaFileManager> {
        ...
        public String inferBinaryName(Location location, JavaFileObject file) {
            return super.inferBinaryName(location, ((OnlyOneReadJavaFileObject) file).getFileObject());
        }
    }
