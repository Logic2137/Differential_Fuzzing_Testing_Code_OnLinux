...
class A implements Serializable {
    ...
    A() {
        ...
        for (int i = 0; i < arrayOfString.length; i++) arrayOfString[i] = new String("array element " + i);
    }
...
class SubstituteObjectOutputStream extends ObjectOutputStream {
    ...
    protected Object replaceObject(Object obj) throws IOException {
        ...
        if (obj.getClass().isArray()) {
            ...
            Object[] newarray = (Object[]) Array.newInstance(arrayComponentType, array.length * 2);
            for (int i = 0; i < array.length; i++) newarray[i] = array[i];
            for (int ni = array.length; ni < 2 * array.length; ni++) newarray[ni] = array[ni - array.length];
            numArraysCounted++;
            obj = newarray;
        }
        return obj;
...
public class ReplaceStringArray {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        if (!expectedResult)
            throw new Error("Expected " + 4 + " strings to be replaced during serialization;" + " only " + out.numStringsReplaced + " strings were replaced.");
        if (out.numArraysCounted != 1)
            throw new Error("Expected 1 array during serialization; only " + out.numArraysCounted + " arrays");
        if (verbose) {
            System.out.println("DeSerialize A from SubstituteObjectInputStream");
            System.out.println("");
        }
        SubstituteObjectInputStream in = new SubstituteObjectInputStream(new ByteArrayInputStream(baos.toByteArray()));
        a = (A) in.readObject();
        in.close();
        if (in.numStringsReplaced != 4)
            throw new Error("Expected 4 strings to be resolved during deserialization;" + " only " + in.numStringsReplaced + " strings were resolved.");
        if (in.numArraysCounted != 1)
            throw new Error("Expected 1 array during deserialization; only " + out.numArraysCounted + " arrays");
        if (a.arrayOfString.length != 8)
            throw new Error("Expected a.arrayOfString.length to be 8, observed " + a.arrayOfString.length);
        if (verbose) {
