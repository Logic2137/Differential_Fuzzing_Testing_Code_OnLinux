...
public class RenderPerfTest {
    ...
    static class WhiteTextParticleRenderer implements ParticleRenderer {
        ...
        public void render(Graphics2D g2d, int id, float[] x, float[] y, float[] vx, float[] vy) {
            setPaint(g2d, id);
            g2d.drawString("The quick brown fox jumps over the lazy dog", (int) (x[id] - r), (int) (y[id] - r));
            g2d.drawString("The quick brown fox jumps over the lazy dog", (int) (x[id] - r), (int) y[id]);
            g2d.drawString("The quick brown fox jumps over the lazy dog", (int) (x[id] - r), (int) (y[id] + r));
        }
    }
    ...
    static class FlatOvalRotParticleRenderer extends FlatParticleRenderer {
        ...
        public void render(Graphics2D g2d, int id, float[] x, float[] y, float[] vx, float[] vy) {
            ...
            if (Math.abs(vx[id] + vy[id]) > 0.001) {
                ...
                g2d.fillOval((int) (x[id] - r), (int) (y[id] - 0.5 * r), (int) (2 * r), (int) r);
            }
        }
    }
    ...
    static class LinGrad3OvalRotParticleRenderer extends FlatOvalRotParticleRenderer {
        ...
        void setPaint(Graphics2D g2d, int id) {
            ...
            Color[] cls = { colors[id % colors.length], colors[(colors.length - id) % colors.length], colors[(id * 5) % colors.length] };
            LinearGradientPaint p = new LinearGradientPaint(start, end, dist, cls);
            g2d.setPaint(p);
        }
    }
    ...
    static class ImgParticleRenderer extends FlatParticleRenderer {
        ...
        ImgParticleRenderer(int n, float r) {
            super(n, r);
            try {
                dukeImg = ImageIO.read(Objects.requireNonNull(RenderPerfTest.class.getClassLoader().getResourceAsStream("renderperf/images/duke.png")));
            } catch (IOException e) {
    ...
    static class SegParticleRenderer extends FlatParticleRenderer {
        ...
        public void render(Graphics2D g2d, int id, float[] x, float[] y, float[] vx, float[] vy) {
            ...
            g2d.setColor(colors[id % colors.length]);
            g2d.drawLine((int) (x[id] - r * nvx), (int) (y[id] - r * nvy), (int) (x[id] + 2 * r * nvx), (int) (y[id] + 2 * r * nvy));
        }
    }
    ...
    static class PerfMeter {
        ...
        PerfMeter exec(final Renderable renderable) throws Exception {
            ...
            Timer timer = new Timer(DELAY, e -> {
                if (waiting.compareAndSet(false, true)) {
                    Color c = robot.getPixelColor(panel.getTopLevelAncestor().getX() + panel.getTopLevelAncestor().getInsets().left + BW / 2, panel.getTopLevelAncestor().getY() + panel.getTopLevelAncestor().getInsets().top + BW / 2);
                    ...
                    while (!isAlmostEqual(robot.getPixelColor(panel.getTopLevelAncestor().getX() + panel.getTopLevelAncestor().getInsets().left + BW / 2, panel.getTopLevelAncestor().getY() + panel.getTopLevelAncestor().getInsets().top + BH / 2), expColor)) {
                        try {
        ...
        private boolean isAlmostEqual(Color c1, Color c2) {
            return Math.abs(c1.getRed() - c2.getRed()) < COLOR_TOLERANCE || Math.abs(c1.getGreen() - c2.getGreen()) < COLOR_TOLERANCE || Math.abs(c1.getBlue() - c2.getBlue()) < COLOR_TOLERANCE;
        }
    }
    ...
    private static final Configurable AA = (Graphics2D g2d) -> g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);

    private static final Configurable TextLCD = (Graphics2D g2d) -> g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_LCD_HRGB);

    private static final Configurable TextAA = (Graphics2D g2d) -> g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
