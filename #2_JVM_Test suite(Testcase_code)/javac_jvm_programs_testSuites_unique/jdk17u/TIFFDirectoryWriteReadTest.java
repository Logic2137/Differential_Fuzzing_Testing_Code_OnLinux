...
public class TIFFDirectoryWriteReadTest {
    ...
    private static final byte[] ICC_PROFILE = ICC_ProfileRGB.getInstance(ColorSpace.CS_sRGB).getData();
    ...
    private void writeImage() throws Exception {
        ...
        try (ImageOutputStream ios = ImageIO.createImageOutputStream(s)) {
            ...
            addASCIIField(dir, "Copyright", COPYRIGHT, BaselineTIFFTagSet.TAG_COPYRIGHT);
            addASCIIField(dir, "ImageDescription", DESCRIPTION, BaselineTIFFTagSet.TAG_IMAGE_DESCRIPTION);
            addASCIIField(dir, "Software", SOFTWARE, BaselineTIFFTagSet.TAG_SOFTWARE);
            dir.addTIFFField(new TIFFField(new TIFFTag("XResolution", BaselineTIFFTagSet.TAG_X_RESOLUTION, 1 << TIFFTag.TIFF_RATIONAL), TIFFTag.TIFF_RATIONAL, 1, RES_X));
            dir.addTIFFField(new TIFFField(new TIFFTag("YResolution", BaselineTIFFTagSet.TAG_Y_RESOLUTION, 1 << TIFFTag.TIFF_RATIONAL), TIFFTag.TIFF_RATIONAL, 1, RES_Y));
            dir.addTIFFField(new TIFFField(new TIFFTag("ICC Profile", BaselineTIFFTagSet.TAG_ICC_PROFILE, 1 << TIFFTag.TIFF_UNDEFINED), TIFFTag.TIFF_UNDEFINED, ICC_PROFILE.length, ICC_PROFILE));
    ...
    private void readAndCheckImage() throws Exception {
        ...
        check(img.getWidth() == SZ && img.getHeight() == SZ, "invalid image size");
        ...
        reader.dispose();
        s.close();
        checkASCIIField(dir, "copyright", COPYRIGHT, BaselineTIFFTagSet.TAG_COPYRIGHT);
        checkASCIIField(dir, "description", DESCRIPTION, BaselineTIFFTagSet.TAG_IMAGE_DESCRIPTION);
        checkASCIIField(dir, "software", SOFTWARE, BaselineTIFFTagSet.TAG_SOFTWARE);
        ...
        for (int b : bps) {
            check(b == 8, "invalid bits per sample");
        }
        f = dir.getTIFFField(BaselineTIFFTagSet.TAG_PHOTOMETRIC_INTERPRETATION);
        check(f.getCount() == 1, "invalid count");
        check(f.getAsInt(0) == BaselineTIFFTagSet.PHOTOMETRIC_INTERPRETATION_RGB, "invalid photometric interpretation value");
        ...
        check(f.getCount() == 1 && f.getAsInt(0) == (int) (RES_X[0][0] / RES_X[0][1]), "invalid x resolution");
        f = dir.getTIFFField(BaselineTIFFTagSet.TAG_Y_RESOLUTION);
        check(f.getType() == TIFFTag.TIFF_RATIONAL, "y" + rat);
        check(f.getCount() == 1 && f.getAsInt(0) == (int) (RES_Y[0][0] / RES_Y[0][1]), "invalid y resolution");
        ...
        check((cnt == ICC_PROFILE.length) && (cnt == icc.length), "invalid ICC profile");
        for (int i = 0; i < cnt; i++) {
