...
public class stack008 {
    ...
    public static int run(String[] args, PrintStream out) {
        ...
        for (depth = 100; ; depth += 100) try {
            ...
            if ((target instanceof StackOverflowError) || (target instanceof OutOfMemoryError))
                break;
    ...
    private static Throwable getTargetException(Throwable exception) {
        Throwable target;
        for (target = exception; target instanceof InvocationTargetException; target = ((InvocationTargetException) target).getTargetException()) ;
        return target;
    }
