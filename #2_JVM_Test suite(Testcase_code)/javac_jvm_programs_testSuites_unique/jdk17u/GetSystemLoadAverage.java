...
public class GetSystemLoadAverage {
    ...
    private static OperatingSystemMXBean mbean = ManagementFactory.getOperatingSystemMXBean();
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length == 0) {
            ...
            if (!args[0].equals("-1.0")) {
                ...
                if (loadavg != -1.0) {
                    throw new RuntimeException("Expected load average : -1.0" + " but getSystemLoadAverage returned: " + loadavg);
                }
            }
        }
    ...
    private static String LOAD_AVERAGE_TEXT = System.getProperty("os.name").contains("OS X") ? "load averages:" : "load average:";
    ...
    private static void checkLoadAvg() throws Exception {
        ...
        double loadavg = mbean.getSystemLoadAverage();
        output = output.substring(output.lastIndexOf(LOAD_AVERAGE_TEXT) + LOAD_AVERAGE_TEXT.length() + 1);
        System.out.println("Load average returned from uptime = " + output);
        System.out.println("getSystemLoadAverage() returned " + loadavg);
        String[] lavg = System.getProperty("os.name").contains("OS X") ? output.split(" ") : output.split(",");
        ...
        if (loadavg < lowRange || loadavg > highRange) {
            throw new RuntimeException("Expected load average : " + expected + " but getSystemLoadAverage returned: " + loadavg);
        }
    }
