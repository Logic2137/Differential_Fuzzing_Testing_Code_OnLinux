...
public class ImportExport {
    ...
    void equal(byte[] bytes, BitSet s) {
        ...
        equal(s, BitSet.valueOf(ByteBuffer.wrap(Arrays.copyOf(bytes, bytes.length + 8 + rnd.nextInt(8))).order(ByteOrder.LITTLE_ENDIAN).asLongBuffer()));
    }
    ...
    void test(String[] args) throws Throwable {
        for (int i = 0; i < 17; i++) {
            ...
            if (i > 0) {
                ...
                for (int j = 0; j < 8; j++) {
                    ...
                    s.set(8 * k + j);
                    equal(bytes, s);
                    byte[] expected = new byte[k + 1];
                    expected[k] = bytes[k];
                    equal(BitSet.valueOf(bytes).toByteArray(), expected);
                    ByteBuffer bb = ByteBuffer.wrap(bytes);
                    bb.position(k);
                    equal(BitSet.valueOf(bb).toByteArray(), new byte[] { bytes[k] });
                }
            }
        }
        for (int i = 0; i < 100; i++) {
            byte[] bytes = new byte[rnd.nextInt(17)];
            for (int j = 0; j < bytes.length; j++) bytes[j] = (byte) rnd.nextInt(0x100);
        ...
        }
        {
            ...
            for (int i = 0; i < longs.length; i++) longs[i] = rnd.nextLong();
            LongBuffer b1 = LongBuffer.wrap(longs);
            LongBuffer b2 = LongBuffer.allocate(longs.length + 10);
            for (int i = 0; i < b2.limit(); i++) b2.put(i, rnd.nextLong());
