...
public class EagerInterfaceCompletionTest {
    ...
    EagerInterfaceCompletionTest(JavaCompiler javacTool, File testDir, VersionKind versionKind, HierarchyKind hierarchyKind, TestKind testKind, ActionKind actionKind) {
    ...
    void compile(DiagnosticChecker dc, JavaSource... sources) {
        try {
            CompilationTask ct = javacTool.getTask(null, null, dc, Arrays.asList("-d", testDir.getAbsolutePath(), "-cp", testDir.getAbsolutePath(), versionKind.optsArr[0], versionKind.optsArr[1]), null, Arrays.asList(sources));
            ct.call();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    enum VersionKind {
        ...
        PRE_LAMBDA("-source", "7"), LAMBDA("-source", "8");
    ...
    enum HierarchyKind {
        ...
        INTERFACE("interface A { boolean f = false; void m(); }\n" + "class B implements A { public void m() {} }"), CLASS("class A { boolean f; void m() {} }\n" + "class B extends A { void m() {} }"), ABSTRACT_CLASS("abstract class A { boolean f; abstract void m(); }\n" + "class B extends A { void m() {} }");
    ...
    enum ActionKind {
        ...
        REMOVE_A("A.class"), REMOVE_B("B.class");
    ...
    enum TestKind {
        ...
        ACCESS_ONLY("class C { B b; }"), SUPER("class C extends B {}"), METHOD("class C { void test(B b) { b.m(); } }"), FIELD("class C { void test(B b) { boolean b2 = b.f; } }"), CONSTR("class C { void test() { new B(); } }");
        ...
        boolean completionFailure(VersionKind vk, ActionKind ak, HierarchyKind hk) {
            switch(this) {
                case ACCESS_ONLY:
                case CONSTR:
                    return ak == ActionKind.REMOVE_B;
                case FIELD:
                case SUPER:
                    return true;
                case METHOD:
                    return hk != HierarchyKind.INTERFACE || ak == ActionKind.REMOVE_B || (hk == HierarchyKind.INTERFACE && ak == ActionKind.REMOVE_A);
                default:
                    throw new AssertionError("Unexpected test kind " + this);
            }
        }
    }
