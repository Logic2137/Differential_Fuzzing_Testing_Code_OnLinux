...
public class VisibleAdvance {
    ...
    private static AttributedString getString(Boolean direction, boolean multipleStyles) {
    ...
    private static FontRenderContext fontRenderContext = new FontRenderContext(new AffineTransform(), true, true);

    private static float[] showAndCalculateAdvance(String what, AttributedString as, float[] advances) {
        TextLayout layout = new TextLayout(as.getIterator(), fontRenderContext);
        System.out.println(what + "Advance = " + layout.getAdvance() + ", Visible advance = " + layout.getVisibleAdvance());
        ...
        if (advances == null) {
            ...
            throw new RuntimeException("MISMATCH in advance.. " + what + "Advance = " + layout.getAdvance() + ", Visible advance = " + layout.getVisibleAdvance() + ", previous values were: [" + advances[0] + "," + advances[1] + "]");
        }
        advances[0] = advance;
