...
public final class BuffersTestingKit {
    ...
    public static ByteBuffer relocate(ByteBuffer buffer, int newPosition, int newCapacity) {
    ...
    public static Iterable<? extends ByteBuffer> relocateBuffers(Iterable<? extends ByteBuffer> source) {
    ...
    public static Iterable<? extends ByteBuffer> injectEmptyBuffers(Iterable<? extends ByteBuffer> source) {
        return injectEmptyBuffers(source, () -> allocate(0));
    }

    public static Iterable<? extends ByteBuffer> injectEmptyBuffers(Iterable<? extends ByteBuffer> source, Supplier<? extends ByteBuffer> emptyBufferFactory) {
        return () -> new Iterator<ByteBuffer>() {
    ...
    public static ByteBuffer concat(Iterable<? extends ByteBuffer> split, Function<? super Integer, ? extends ByteBuffer> concatBufferFactory) {
        int size = 0;
    ...
    public static void forEachSplit(ByteBuffer bb, Consumer<? super Iterable<? extends ByteBuffer>> action) {
        forEachSplit(bb.remaining(), (lengths) -> {
            int end = bb.position();
    ...
    private static void forEachSplit(int n, Stack<Integer> path, Consumer<? super Iterable<? extends Integer>> action) {
        if (n == 0) {
            action.accept(path);
        } else {
            for (int i = 1; i <= n; i++) {
