...
public class CheckParity {
    ...
    static byte[] testKey = { (byte) 0x00, (byte) 0x01, (byte) 0x02, (byte) 0x03, (byte) 0xfc, (byte) 0xfd, (byte) 0xfe, (byte) 0xff };

    static byte[] expectedKey = { (byte) 0x01, (byte) 0x01, (byte) 0x02, (byte) 0x02, (byte) 0xfd, (byte) 0xfd, (byte) 0xfe, (byte) 0xfe };

    static private void check(String alg, byte[] key, byte[] expected, KeySpec ks) throws Exception {
    ...
    static private void checkDESedeKey() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            System.arraycopy(testKey, 0, key3, i * testKey.length, testKey.length);
            System.arraycopy(expectedKey, 0, expectedKey3, i * testKey.length, testKey.length);
        }
