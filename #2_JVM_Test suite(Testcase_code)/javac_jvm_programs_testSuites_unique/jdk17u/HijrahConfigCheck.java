...
public class HijrahConfigCheck {
    ...
    public static void main(String... args) {
        if (Chronology.getAvailableChronologies().stream().filter(c -> c.getCalendarType().equals(CALTYPE)).count() != 1) {
            throw new RuntimeException(CALTYPE + " chronology was not found, or " + "appeared more than once in Chronology.getAvailableChronologies()");
        }
        ...
        if (!iso.toInstant(ZoneOffset.UTC).equals(hijrah.toInstant(ZoneOffset.UTC))) {
            throw new RuntimeException("test Hijrah date is incorrect. LocalDate: " + iso + ", test date: " + hijrah);
        }
    }
}
