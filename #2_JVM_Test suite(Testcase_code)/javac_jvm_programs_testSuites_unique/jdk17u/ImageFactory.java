...
public class ImageFactory {
    ...
    public static void fill(BufferedImage image) {
        for (int i = 0; i < WIDTH; i++) {
            for (int j = 0; j < HEIGHT; j++) {
                image.setRGB(i, j, 0xFF000000 | (i << 16) | (j << 8) | ((i + j) >> 1));
            }
        }
    }
    ...
    public static void fillCCM(DataBuffer data, SampleModel sm, ColorSpace cs) {
        switch(cs.getType()) {
            ...
            case ColorSpace.TYPE_3CLR:
                if (data.getDataType() == DataBuffer.TYPE_BYTE || data.getDataType() == DataBuffer.TYPE_INT) {
                    int[] pixel = new int[3];
                    for (int i = 0; i < WIDTH; i++) {
                        for (int j = 0; j < HEIGHT; j++) {
                            pixel[0] = i;
                    ...
                    for (int i = 0; i < WIDTH; i++) {
                        for (int j = 0; j < HEIGHT; j++) {
                            pixel[0] = (i / 255.0) * (cs.getMaxValue(0) - cs.getMinValue(0)) + cs.getMinValue(0);
                            pixel[1] = (j / 255.0) * (cs.getMaxValue(1) - cs.getMinValue(1)) + cs.getMinValue(1);
                            pixel[2] = (((i + j) >> 1) / 255.0) * (cs.getMaxValue(2) - cs.getMinValue(2)) + cs.getMinValue(2);
                            sm.setPixel(i, j, pixel, data);
                        }
                    }
                    ...
                    for (int i = 0; i < WIDTH; i++) {
                        for (int j = 0; j < HEIGHT; j++) {
                            pixel[0] = (i / 255.0f) * (cs.getMaxValue(0) - cs.getMinValue(0)) + cs.getMinValue(0);
                            pixel[1] = (j / 255.0f) * (cs.getMaxValue(1) - cs.getMinValue(1)) + cs.getMinValue(1);
                            pixel[2] = (((i + j) >> 1) / 255.0f) * (cs.getMaxValue(2) - cs.getMinValue(2)) + cs.getMinValue(2);
                            sm.setPixel(i, j, pixel, data);
                        }
                    }
                } else {
    ...
    public static void fillDCM(DataBuffer data, SampleModel sm, int csType, int c1Bits, int c2Bits, int c3Bits) {
        int[] pixel;
        pixel = new int[4];
        for (int i = 0; i < WIDTH; i++) {
            for (int j = 0; j < HEIGHT; j++) {
