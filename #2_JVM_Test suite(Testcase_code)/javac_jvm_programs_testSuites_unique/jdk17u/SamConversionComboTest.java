...
public class SamConversionComboTest {
    ...
    enum FInterface {
        ...
        A("A", "interface A { Integer m(int i); }"), B("B", "interface B { int m(Integer i); }"), C("C", "interface C { int m(Integer i) throws Exception; }");
    ...
    enum LambdaKind {
        ...
        EXPRESSION("#VAL"), STATEMENT("{return #VAL;}"), EXCEPTION_STMT("{throw new Exception();}");
    ...
    enum ReturnValue {
        ...
        INT("i + 1"), INTEGER("new Integer(i+1)"), INT2("i.intValue() + 1"), STRING("i + \"\""), DOUBLE("i * 1.0");
    ...
    enum LambdaBody {
        ...
        IMPLICIT("i -> #RET"), EXPLICIT("(#Type i) -> #RET");
    ...
    SourceFile samSourceFile = new SourceFile("FInterface.java", "#C") {
        ...
        public String toString() {
            String interfaces = "";
            for (FInterface fi : FInterface.values()) interfaces += fi.interfaceDef + "\n";
