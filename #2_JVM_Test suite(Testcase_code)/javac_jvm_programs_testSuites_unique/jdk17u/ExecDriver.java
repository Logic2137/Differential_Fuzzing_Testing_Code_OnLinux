...
public class ExecDriver {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        if (launcher) {
            ...
            if (System.getProperty("os.name").toLowerCase().startsWith("win")) {
                value = dir.resolve("bin").resolve(variant()).toAbsolutePath().toString();
                value += File.pathSeparator;
                value += dir.resolve("bin").toAbsolutePath().toString();
            } else {
                value = dir.resolve("lib").resolve(variant()).toAbsolutePath().toString();
            }
            System.out.println("  with " + name + " = " + pb.environment().merge(name, value, (x, y) -> y + File.pathSeparator + x));
            System.out.println("  with CLASSPATH = " + pb.environment().put("CLASSPATH", TEST_CLASS_PATH));
        }
        Process p = pb.start();
    ...
    private static String javaBin() {
        return Paths.get(TEST_JDK).resolve("bin").resolve("java").toAbsolutePath().toString();
    }
}
