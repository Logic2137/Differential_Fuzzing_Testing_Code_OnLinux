...
public enum EnumImplicitPrivateConstructor {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        for (Enum e : YetAnotherEnum.values()) classes.add(e.getClass());
        for (Class clazz : classes) {
            ...
            if (em != null) {
                ...
                if (expected != (classModifiers & (Modifier.ABSTRACT | Modifier.FINAL | Modifier.STATIC))) {
                    passed = false;
                    System.out.println("\tFAILED: Expected 0x" + Integer.toHexString(expected) + " got 0x" + Integer.toHexString(classModifiers));
                }
            }
            for (Constructor ctor : clazz.getDeclaredConstructors()) {
                ...
                if (clazz.isEnum()) {
                    if ((modifiers & Modifier.PRIVATE) == 0 && !ctor.isSynthetic()) {
                        passed = false;
                        System.out.println("\tFAILED: Constructor not marked private: modifiers 0x" + Integer.toHexString(modifiers));
                    }
                }
                try {
                    Object o = null;
