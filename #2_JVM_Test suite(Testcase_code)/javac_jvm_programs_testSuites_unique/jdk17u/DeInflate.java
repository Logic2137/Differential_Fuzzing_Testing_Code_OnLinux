...
public class DeInflate {
    ...
    static void checkStream(Deflater def, byte[] in, int len, byte[] out1, byte[] out2, boolean nowrap) throws Throwable {
    ...
    static void checkByteBuffer(Deflater def, Inflater inf, ByteBuffer in, ByteBuffer out1, ByteBuffer out2, byte[] expected, int len, byte[] result, boolean out1ReadOnlyWhenInflate) throws Throwable {
        def.reset();
        inf.reset();
        def.setInput(in);
        def.finish();
        ...
        out2.flip();
        out2.get(result, 0, n);
        if (n != len || out2.position() != len || !Arrays.equals(Arrays.copyOf(expected, len), Arrays.copyOf(result, len)) || inf.inflate(result) != 0) {
            throw new RuntimeException("De/inflater(buffer) failed:" + def);
        }
    }

    static void checkByteBufferReadonly(Deflater def, Inflater inf, ByteBuffer in, ByteBuffer out1, ByteBuffer out2) throws Throwable {
        def.reset();
        inf.reset();
        def.setInput(in);
        def.finish();
    ...
    static void check(Deflater def, byte[] in, int len, byte[] out1, byte[] out2, boolean nowrap) throws Throwable {
        Arrays.fill(out1, (byte) 0);
        Arrays.fill(out2, (byte) 0);
    ...
    static void checkDict(Deflater def, Inflater inf, byte[] src, byte[] dstDef, byte[] dstInf, ByteBuffer dictDef, ByteBuffer dictInf) throws Throwable {
        def.reset();
        inf.reset();
    ...
    public static void main(String[] args) throws Throwable {
        ...
        for (int level = Deflater.DEFAULT_COMPRESSION; level <= Deflater.BEST_COMPRESSION; level++) {
            for (int strategy = Deflater.DEFAULT_STRATEGY; strategy <= Deflater.HUFFMAN_ONLY; strategy++) {
                for (boolean dowrap : new boolean[] { false, true }) {
                    System.out.println("level:" + level + ", strategy: " + strategy + ", dowrap: " + dowrap);
                    for (int i = 0; i < 5; i++) {
                        int len = (i == 0) ? dataIn.length : new Random().nextInt(dataIn.length);
