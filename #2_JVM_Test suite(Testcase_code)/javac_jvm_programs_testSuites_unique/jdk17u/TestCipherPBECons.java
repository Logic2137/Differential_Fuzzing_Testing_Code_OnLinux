...
public class TestCipherPBECons {
    ...
    private static final String[] PBEAlgorithms = { "pbeWithMD5ANDdes", "PBEWithMD5AndTripleDES" };

    private static final String[] cipherModes = { "ECb", "cbC", "cFB", "Cfb32", "OfB", "oFb64", "pCbC" };
        ...
        for (String algorithm : PBEAlgorithms) {
            for (String mode : cipherModes) {
                for (String padding : cipherPaddings) {
                    out.println("Running test with " + algorithm + "/" + mode + "/" + padding);
                    try {
    ...
    public boolean runTest(Provider p, String algo, String mo, String pad, PrintStream out) throws Exception {
        try {
            Cipher ci = Cipher.getInstance(algo + "/" + mo + "/" + pad, p);
            return (mo.equalsIgnoreCase("CBC")) && (pad.equalsIgnoreCase("PKCS5Padding"));
        } catch (NoSuchAlgorithmException ex) {
            if (p.getName().compareTo("SunJCE") == 0) {
                if (!(mo.equalsIgnoreCase("CBC") && pad.equalsIgnoreCase("PKCS5Padding"))) {
            ...
            if (mo.equalsIgnoreCase("CBC") && pad.equalsIgnoreCase("NoPadding")) {
