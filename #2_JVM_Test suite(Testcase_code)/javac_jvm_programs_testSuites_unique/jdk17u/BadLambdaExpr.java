...
public class BadLambdaExpr {
    ...
    enum ParameterListKind {
        ...
        ZERO_ARY("()"), UNARY("(#P)"), TWO_ARY("(#P, #P)"), THREE_ARY("(#P, #P, #P)");
    ...
    enum ExprKind {
        ...
        NONE("#P#A"), METHOD_CALL("m(#P#A)"), CONSTR_CALL("new Foo(#P#A)");
        ...
        String expressionString(ParameterListKind plk, ParameterKind pk, ArrowKind ak) {
            return expressionTemplate.replaceAll("#P", plk.getParameterString(pk)).replaceAll("#A", ak.arrowStr);
        }
    }
    ...
    void check() {
        boolean errorExpected = ak != ArrowKind.NONE || plk != ParameterListKind.UNARY || pk != ParameterKind.IMPLICIT;
