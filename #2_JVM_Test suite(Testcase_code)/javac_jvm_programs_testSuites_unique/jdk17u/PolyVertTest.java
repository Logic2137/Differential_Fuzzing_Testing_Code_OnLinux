...
public class PolyVertTest {
    ...
    public static void main(String[] argv) {
        ...
        if (totalfuzzypixels > 0) {
            System.err.println(totalfuzzypixels + " fuzzy pixels found in " + numfuzzytests + " tests");
        }
        if (totalbadpixels > 0) {
            throw new RuntimeException(totalbadpixels + " bad pixels found in " + numbadtests + " tests");
        }
    ...
    public static void verify(BufferedImage bimg, String name) {
            ...
            for (int x = 0; x < TESTWIDTH; x++) {
                if (refImg.getRGB(x, y) != bimg.getRGB(x, y)) {
                    ...
                    if (showerrors || showresults) {
                        ...
                        throw new RuntimeException("Error at " + x + ", " + y + " while testing: " + name);
                    }
                    if (isfuzzy) {
                        numfuzzypixels++;
                    } else {
        ...
        if (numbadpixels > 0 || numfuzzypixels > 0) {
            ...
            System.out.println(numbadpixels + " bad pixels and " + numfuzzypixels + " questionable pixels " + "found while testing " + name);
            if (showerrors || showresults) {
                    ...
                    if (errorImg.getRGB(x, y) == RED) {
                        for (int iy = y - FRINGE; iy <= y + FRINGE; iy++) {
                            for (int ix = x - FRINGE; ix <= x + FRINGE; ix++) {
                                if (ix >= 0 && ix < TESTWIDTH && iy >= 0 && iy < TESTHEIGHT && errorImg.getRGB(ix, iy) == GREEN) {
    ...
    public static void testCompatibleImages(GraphicsConfiguration gconfig) {
        test(gconfig.createCompatibleImage(TESTWIDTH, TESTHEIGHT), gconfig + ".createCompat()");
        test(gconfig.createCompatibleImage(TESTWIDTH, TESTHEIGHT, Transparency.OPAQUE), gconfig + ".createCompat(OPAQUE)");
        test(gconfig.createCompatibleImage(TESTWIDTH, TESTHEIGHT, Transparency.BITMASK), gconfig + ".createCompat(BITMASK)");
        test(gconfig.createCompatibleImage(TESTWIDTH, TESTHEIGHT, Transparency.TRANSLUCENT), gconfig + ".createCompat(TRANSLUCENT)");
        test(gconfig.createCompatibleVolatileImage(TESTWIDTH, TESTHEIGHT), gconfig + ".createCompatVolatile()");
    }
    ...
    public static class TestCanvas extends Canvas {
        ...
        public void paint(Graphics g) {
            if (bimg != null || getWidth() < TESTWIDTH || getHeight() < TESTHEIGHT) {
                return;
            }
            render((Graphics2D) g);
