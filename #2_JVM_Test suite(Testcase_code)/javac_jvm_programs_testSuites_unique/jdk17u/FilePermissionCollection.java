...
public class FilePermissionCollection {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("test 8: implies returns false for non-match on wildcard");
        if (perms.implies(new FilePermission("/usr/tmp/bar/foo", "read"))) {
            System.out.println("Expected false, returned true");
            testFail++;
        }
        System.out.println("test 9: implies returns true for deep wildcard match");
        perms.add(new FilePermission("/usr/tmp/-", "read"));
        if (!perms.implies(new FilePermission("/usr/tmp/bar/foo", "read"))) {
            System.out.println("Expected true, returned false");
            testFail++;
        ...
        System.out.println("test 11: implies returns true for all " + "wildcard and match on action");
        perms.add(new FilePermission("<<ALL FILES>>", "read"));
        if (!perms.implies(new FilePermission("/tmp/foobar", "read"))) {
            System.out.println("Expected true, returned false");
            testFail++;
        }
        System.out.println("test 12: implies returns false for wildcard " + "and non-match on action");
