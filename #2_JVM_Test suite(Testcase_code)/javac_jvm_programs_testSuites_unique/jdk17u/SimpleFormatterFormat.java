...
public class SimpleFormatterFormat {
    ...
    private static String[] loggers = new String[] { "test.foo", "test.foo", "test.bar", "test.bar", "test.bar", "test.bar" };

    private static String[] messages = new String[] { "severe hello world", "warning lost connection", "info welcome", "warning beware of traps", "warning { {ok7} }", "warning exception thrown" };
    ...
    private static void checkLogRecords(File log) throws Exception {
        ...
        try (FileInputStream in = new FileInputStream(log);
            ...
            String line;
            int i = 0;
            while (i < messages.length && (line = reader.readLine()) != null) {
                ...
                if (m.groupCount() != 3) {
                    throw new RuntimeException("Unexpected group count = " + m.groupCount());
                }
                String[] ss = m.group(1).split(":");
                int len = ss.length;
                if (len != 2) {
                    throw new RuntimeException("Unexpected logger name and level" + m.group(1));
                }
    ...
    private static void verify(String expectedLogger, String expectedMsg, String logger, String level, String msg, String source) {
