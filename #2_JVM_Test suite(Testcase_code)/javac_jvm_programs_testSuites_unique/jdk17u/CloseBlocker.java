...
public class CloseBlocker {
    ...
    private static void init() {
        String[] instructions = { " the test will be run 6 times, to start next test just close all ", " windows of previous; the instructions are the same for all tests: ", " 1) there are two frames (one the frames has 'show modal' button), ", " 2) press the button to show a dialog, ", " 3) close the dialog (an alternative scenario - activate another", "    native window before closing the dialog), ", " 4) the frame with button should become next active window, ", "    if it's true, then the test passed, otherwise, it failed. ", " Press 'pass' button only after all of the 6 tests are completed, ", " the number of the currently executed test is displayed on the ", " output window. " };
        Sysout.createDialog();
