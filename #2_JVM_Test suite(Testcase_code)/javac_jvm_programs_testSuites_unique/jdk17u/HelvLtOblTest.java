...
public class HelvLtOblTest extends JComponent {
    ...
    static int[] codes = { 0x23, 0x4a, 0x48, 0x3, 0x4a, 0x55, 0x42, 0x4d, 0x4a, 0x44, 0x3, 0x53, 0x46, 0x45, 0x3, 0x55, 0x46, 0x59, 0x55 };
        ...
        for (int x = 0; x < wid; x++) {
            for (int y = 0; y < hgt; y++) {
                ...
                if (rdiff > 1 || gdiff > 1 || bdiff > 1) {
                    throw new RuntimeException("Images differ at x=" + x + " y=" + y + " " + Integer.toHexString(rgb0) + " vs " + Integer.toHexString(rgb1));
                }
            }
        }
    }
