...
public class TestConfigurationLock {
    ...
    public static void test() throws Exception {
        ...
        System.out.println("\t LogManager.reset() called " + (resetCount.get() - sUpdateCount) + " times by " + RESETERS + " Thread(s).");
            ...
            while (goOn) {
                try {
                    ...
                    if (ids.length == 1) {
                        ...
                        deadlock = true;
                        ThreadInfo[] infos = ManagementFactory.getThreadMXBean().getThreadInfo(ids, true, true);
    ...
    static String asString(ThreadInfo inf) {
        ...
        sb.append("\"").append(inf.getThreadName()).append("\"").append(inf.isDaemon() ? " daemon" : "").append(" prio=").append(inf.getPriority()).append(" Id=").append(inf.getThreadId()).append(" ").append(inf.getThreadState());
        if (inf.getLockName() != null) {
            sb.append(" on ").append(inf.getLockName());
        }
        if (inf.getLockOwnerName() != null) {
            sb.append(" owned by \"").append(inf.getLockOwnerName()).append("\" Id=").append(inf.getLockOwnerId());
        }
        if (inf.isSuspended()) {
            sb.append(" (suspended)");
        }
