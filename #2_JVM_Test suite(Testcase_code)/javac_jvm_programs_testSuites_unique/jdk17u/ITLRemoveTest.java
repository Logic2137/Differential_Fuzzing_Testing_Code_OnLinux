public class ITLRemoveTest {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            for (int removeId : removeNode) if (removeId == i) {
                checkValue = INITIAL_VALUE;
                break;
            }
            for (int removeId : removeAndSet) if (removeId == i) {
    ...
    private static class MyThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                for (int removeId : removeNode) if ((threadId - INITIAL_VALUE) == removeId) {
                    n.remove();
                    break;
                }
                for (int removeId : removeAndSet) if ((threadId - INITIAL_VALUE) == removeId) {
                    n.remove();
