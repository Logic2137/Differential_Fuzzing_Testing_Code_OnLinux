...
public class OutputSizeTest {
    ...
    private static void testCC20GetOutSize() throws GeneralSecurityException {
        boolean result = true;
        KeyGenerator kg = KeyGenerator.getInstance("ChaCha20", "SunJCE");
        kg.init(256);
        Cipher cc20 = Cipher.getInstance("ChaCha20", "SunJCE");
        cc20.init(Cipher.ENCRYPT_MODE, kg.generateKey(), new ChaCha20ParameterSpec(getRandBuf(12), 10));
        testOutLen(cc20, 0, 0);
        testOutLen(cc20, 5, 5);
        ...
        cc20.init(Cipher.DECRYPT_MODE, kg.generateKey(), new ChaCha20ParameterSpec(getRandBuf(12), 10));
        testOutLen(cc20, 0, 0);
        testOutLen(cc20, 5, 5);
        testOutLen(cc20, 5120, 5120);
        cc20.update(input);
    ...
    private static void testCC20P1305GetOutSize() throws GeneralSecurityException {
        KeyGenerator kg = KeyGenerator.getInstance("ChaCha20", "SunJCE");
        kg.init(256);
        Cipher cc20 = Cipher.getInstance("ChaCha20-Poly1305", "SunJCE");
        cc20.init(Cipher.ENCRYPT_MODE, kg.generateKey(), new IvParameterSpec(getRandBuf(12)));
        ...
        cc20.init(Cipher.DECRYPT_MODE, kg.generateKey(), new IvParameterSpec(getRandBuf(12)));
        testOutLen(cc20, 0, 0);
        testOutLen(cc20, 5, 0);
    ...
    private static void testMultiPartAEADDec() throws GeneralSecurityException {
        ...
        ByteBuffer ctBuf = ByteBuffer.allocateDirect(enc.getOutputSize(pText.length));
        ...
        if (pTextBase.compareTo(ptBuf) != 0) {
            ...
            sb.append("Plaintext mismatch: Original: ").append(pTextBase.toString()).append("\nActual :").append(ptBuf);
    ...
    private static void testOutLen(Cipher c, int inLen, int expOut) {
        ...
        if (actualOut != expOut) {
            throw new RuntimeException("Cipher " + c + ", in: " + inLen + ", expOut: " + expOut + ", actual: " + actualOut);
        }
    }
