...
public class TestIBMDB {
    ...
    static char[] decode(byte[] bb, Charset cs, boolean testDirect, Time t) throws Exception {
        ...
        if (cr != CoderResult.UNDERFLOW) {
            ...
            System.out.printf("  cr=%s, bbf.pos=%d, bb[pos]=%x,%x,%x,%x%n", cr.toString(), pos, bb[pos++] & 0xff, bb[pos++] & 0xff, bb[pos++] & 0xff, bb[pos++] & 0xff);
    ...
    static void compare(Charset cs1, Charset cs2, char[] cc) throws Exception {
        ...
        System.out.printf("    Encoding TimeRatio %s/%s: %d,%d :%f%n", csn2, csn1, t2.t, t1.t, (double) (t2.t) / (t1.t));
        if (!Arrays.equals(bb1, bb2)) {
            System.out.printf("        encoding failed%n");
        }
        char[] cc2 = decode(bb1, cs2, false, t2);
        char[] cc1 = decode(bb1, cs1, false, t1);
        System.out.printf("    Decoding TimeRatio %s/%s: %d,%d :%f%n", csn2, csn1, t2.t, t1.t, (double) (t2.t) / (t1.t));
        if (!Arrays.equals(cc1, cc2)) {
            System.out.printf("        decoding failed%n");
        }
        bb1 = encode(cc, cs1, true, t1);
        bb2 = encode(cc, cs2, true, t2);
        System.out.printf("    Encoding(dir) TimeRatio %s/%s: %d,%d :%f%n", csn2, csn1, t2.t, t1.t, (double) (t2.t) / (t1.t));
        if (!Arrays.equals(bb1, bb2))
            System.out.printf("        encoding (direct) failed%n");
        cc1 = decode(bb1, cs1, true, t1);
        cc2 = decode(bb1, cs2, true, t2);
        System.out.printf("    Decoding(dir) TimeRatio %s/%s: %d,%d :%f%n", csn2, csn1, t2.t, t1.t, (double) (t2.t) / (t1.t));
        if (!Arrays.equals(cc1, cc2)) {
    ...
    static String[] csnames = new String[] { "IBM930", "IBM933", "IBM935", "IBM937", "IBM939", "IBM942", "IBM943", "IBM948", "IBM949", "IBM950", "IBM970", "IBM942C", "IBM943C", "IBM949C", "IBM1381", "IBM1383", "EUC_CN", "EUC_KR", "GBK", "Johab", "MS932", "MS936", "MS949", "MS950", "EUC_JP", "EUC_JP_LINUX", "EUC_JP_Open", "SJIS", "PCK" };
    ...
    public static void main(String[] args) throws Exception {
        for (String csname : csnames) {
            ...
            if (csname.startsWith("x-IBM93")) {
                checkMalformed(csNew, new byte[][] { { 1, 0x26, 0x0f, 0x27 }, { 1, 0x0e, 0x41, 0x41, 0xe }, { 2, 0x0e, 0x40, 0x41, 0xe } });
            } else if (csname.equals("x-IBM970") || csname.equals("x-IBM1383")) {
                checkMalformed(csNew, new byte[][] { { 1, 0x26, (byte) 0x8f, 0x27 }, { 1, (byte) 0xa1, (byte) 0xa1, (byte) 0x8e, 0x51 } });
            }
        }
    }
}
