...
public class MBeanInfoFailTest {
    ...
    public static class ThornyDevil extends StandardMBean implements ThornyDevilMBean {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("Test queryNames() and queryMBeans() with a " + "buggy DynamicMBean");
        try {
            final MBeanServer server = MBeanServerFactory.createMBeanServer();
            final ObjectName troubleKeeper = new ObjectName("ThornyDevil:name=TroubleKeeper");
            final ObjectName troubleMaker = new ObjectName("ThornyDevil:name=TroubleMaker");
            final ObjectName thornyPattern = new ObjectName("ThornyDevil:*");
            server.createMBean(ThornyDevil.class.getName(), troubleKeeper);
            server.createMBean(ThornyDevil.class.getName(), troubleMaker);
            final MBeanInfo info1 = server.getMBeanInfo(troubleKeeper);
            System.out.println(String.valueOf(troubleKeeper) + " is a " + info1.getClassName());
            final MBeanInfo info2 = server.getMBeanInfo(troubleMaker);
            System.out.println(String.valueOf(troubleMaker) + " is a " + info2.getClassName());
            final Set thorny1 = server.queryNames(thornyPattern, null);
            if (thorny1.size() != 2) {
                System.err.println("queryNames(): " + "Expected to find 2 ThornyDevils before" + " trouble started! ");
                System.err.println("Found " + thorny1.size() + " instead: " + thorny1);
                System.exit(1);
            }
            final Set thornyM1 = server.queryMBeans(thornyPattern, null);
            if (thornyM1.size() != 2) {
                System.err.println("queryMBeans(): " + "Expected to find 2 ThornyDevils before" + " trouble started! ");
            ...
            for (Iterator it1 = thornyM1.iterator(); it1.hasNext(); ) {
                ...
                final ObjectName on = oi.getObjectName();
                final String cn = oi.getClassName();
                if (cn == null || !ThornyDevil.class.getName().equals(cn)) {
            ...
            ThornyDevilMBean troubleMakerproxy = (ThornyDevilMBean) MBeanServerInvocationHandler.newProxyInstance(server, troubleMaker, ThornyDevilMBean.class, false);
            ...
            try {
                ...
                if (x.getCause() instanceof UnspeakableException)
                    System.out.println("Trouble started as expected: " + x.getCause());
                else {
            ...
            if (thorny2.size() != 2) {
                System.err.println("Expected to find 2 ThornyDevils after" + " trouble started! ");
                System.err.println("Found " + thorny2.size() + " instead: " + thorny2);
                System.exit(4);
            }
            final Set thornyM2 = server.queryMBeans(thornyPattern, null);
            if (thornyM2.size() != 2) {
                System.err.println("queryMBeans(): " + "Expected to find 2 ThornyDevils after" + " trouble started! ");
            ...
            System.out.println("Ahahah! " + troubleMaker + " successfully thwarted!");
        } catch (Exception x) {
