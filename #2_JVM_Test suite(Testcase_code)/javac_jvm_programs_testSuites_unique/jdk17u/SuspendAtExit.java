            ...
            try {
                ...
                while (true) {
                    ...
                    if (retCode == JVMTI_ERROR_THREAD_NOT_ALIVE) {
                        ...
                        throw new RuntimeException("thread " + thread.getName() + ": suspendThread() " + "retCode=" + retCode + ": unexpected value.");
                    }
                    if (!thread.isAlive()) {
                        throw new RuntimeException("thread " + thread.getName() + ": is not alive " + "after successful " + "suspendThread().");
                    }
                    retCode = resumeThread(thread);
                    if (retCode != 0) {
                        throw new RuntimeException("thread " + thread.getName() + ": resumeThread() " + "retCode=" + retCode + ": unexpected value.");
                    }
                }
            } catch (InterruptedException e) {
                throw new RuntimeException("Unexpected: " + e);
