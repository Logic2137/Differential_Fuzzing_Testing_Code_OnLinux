...
public class WildcardPortSupport {
    ...
    public void runAllTests() throws Exception {
        ListeningConnector connector = Bootstrap.virtualMachineManager().listeningConnectors().stream().filter(c -> c.name().equals("com.sun.jdi.SocketListen")).findFirst().get();
        if (connector == null) {
    ...
    private void testWithDefaultArgs1(ListeningConnector connector) throws IOException, IllegalConnectorArgumentsException {
    ...
    private void testWithDefaultArgs2(ListeningConnector connector) throws IOException, IllegalConnectorArgumentsException {
    ...
    private void testWithWildcardPort1(ListeningConnector connector) throws IOException, IllegalConnectorArgumentsException {
    ...
    private void testWithWildcardPort2(ListeningConnector connector) throws IOException, IllegalConnectorArgumentsException {
    ...
    private void testWithDefaultArgsNegative(ListeningConnector connector) throws IOException, IllegalConnectorArgumentsException {
        Map<String, Connector.Argument> args = connector.defaultArguments();
        connector.startListening(args);
        String port = args.get(PORT_ARG).value();
        if (port.isEmpty() || "0".equals(port)) {
            throw new RuntimeException("Test testWithDefaultArgsNegative failed." + " The argument map was not updated with the bound port number.");
        }
        try {
            connector.startListening(args);
        ...
        throw new RuntimeException("Test testWithDefaultArgsNegative failed. No expected " + "com.sun.jdi.IllegalConnectorArgumentsException exception was thrown.");
    }

    private int startListening(ListeningConnector connector, Map<String, Connector.Argument> args) throws IOException, IllegalConnectorArgumentsException {
