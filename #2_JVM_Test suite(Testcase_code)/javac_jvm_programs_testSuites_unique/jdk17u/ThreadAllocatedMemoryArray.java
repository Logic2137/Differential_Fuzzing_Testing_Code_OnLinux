...
public class ThreadAllocatedMemoryArray {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            long s = sizes[i];
            if (s != -1) {
                throw new RuntimeException("Invalid ThreadAllocatedBytes returned for thread " + threads[i].getName() + " = " + s + " expected = -1");
            }
        }
        if (!mbean.isThreadAllocatedMemoryEnabled()) {
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            long s = sizes[i];
            if (s < 0) {
                throw new RuntimeException("Invalid allocated bytes returned for thread " + threads[i].getName() + " = " + s);
            }
        }
        synchronized (obj) {
            done = true;
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
            ...
            if (sizes[i] > newSize) {
                throw new RuntimeException("TEST FAILED: " + threads[i].getName() + " previous allocated bytes = " + sizes[i] + " > current allocated bytes = " + newSize);
            }
            System.out.println(threads[i].getName() + " Previous allocated bytes = " + sizes[i] + " Current allocated bytes = " + newSize);
        }
        try {
