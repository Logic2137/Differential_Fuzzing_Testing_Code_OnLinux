...
public class DialogOwnerTest extends JPanel {
    ...
    static PrintService[] services = PrintServiceLookup.lookupPrintServices(null, null);
    ...
    boolean isSupported(PrintRequestAttributeSet aset, int ownerFlags, Window owner, int dlgFlags) {
        ...
        if (is2D(dlgFlags)) {
            ...
            if (dst != null && !service.isAttributeValueSupported(dst, null, aset)) {
                supported = false;
            }
            if (dst != null) {
                aset.add(dst);
            }
            if (ownerAttr != null && !service.isAttributeValueSupported(ownerAttr, null, aset)) {
                supported = false;
            }
        }
        return supported;
