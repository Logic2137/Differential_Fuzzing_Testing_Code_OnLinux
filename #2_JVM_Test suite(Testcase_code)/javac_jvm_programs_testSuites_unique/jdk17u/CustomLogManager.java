...
public class CustomLogManager extends LogManager {
    ...
    public synchronized boolean addLogger(Logger logger) {
        ...
        for (; ; ) {
            ...
            if (props.get(pname + ".level") != null || props.get(pname + ".handlers") != null) {
    ...
    public static void checkLogger(String name, String resourceBundleName) {
        ...
        System.out.format("Logger \"%s\" level=%s handlers=%s resourcebundle=%s useParentHandlers=%s%n", name, logger.getLevel(), Arrays.toString(logger.getHandlers()), logger.getResourceBundleName(), logger.getUseParentHandlers());
        String rb = logger.getResourceBundleName();
        if (rb != resourceBundleName && (rb == null || rb.equals(resourceBundleName))) {
            throw new RuntimeException("Logger \"" + name + "\" unexpected resource bundle: " + rb);
        }
        ...
        if ((hdl == null && handlers.length != 0) || (hdl != null && handlers.length != 1)) {
            throw new RuntimeException("Logger \"" + name + "\" unexpected handler: " + Arrays.toString(handlers));
        }
        ...
        if (logger.getUseParentHandlers() != uph) {
            throw new RuntimeException("Logger \"" + name + "\" unexpected useParentHandlers: " + logger.getUseParentHandlers());
        }
        checkParents(name);
    }
    ...
    private static void checkParents(String name) {
        ...
        for (; ; ) {
            ...
            String pname = name.substring(0, ix2);
            if (INSTANCE.getProperty(pname + ".level") != null || INSTANCE.getProperty(pname + ".handlers") != null) {
                checkLogger(pname);
            }
            ix = ix2 + 1;
        }
