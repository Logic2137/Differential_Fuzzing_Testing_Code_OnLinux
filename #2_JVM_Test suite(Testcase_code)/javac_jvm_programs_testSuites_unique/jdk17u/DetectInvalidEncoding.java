...
public class DetectInvalidEncoding {
    ...
    public static void main(String[] args) throws Exception {
        ...
        validTests.add(new GenMultiCertTest("Single, valid certificate", SINGLE_ROOT_CERT.getBytes(), null, new X500Principal("CN=Root, O=SomeCompany")));
        validTests.add(new GenMultiCertTest("PEM-encoded PKCS#7 chain", PKCS7_INTERMED_ROOT_CERTS.getBytes(), null, new X500Principal("CN=Intermed, O=SomeCompany"), new X500Principal("CN=Root, O=SomeCompany")));
        validTests.add(new GenMultiCertTest("Two PEM-encoded X509 certs", (INTERMED_CA_CERT + "\n" + SINGLE_ROOT_CERT).getBytes(), null, new X500Principal("CN=Intermed, O=SomeCompany"), new X500Principal("CN=Root, O=SomeCompany")));
        ...
        validTests.add(new GenMultiCertTest("Two PEM-encoded certs with leading/trailing " + "text data around each.", sb.toString().getBytes(), null, new X500Principal("CN=Root, O=SomeCompany"), new X500Principal("CN=Intermed, O=SomeCompany")));
        validTests.add(new GenMultiCertTest("BER-encoded PKCS#7 with empty certificates segment", PKCS7_BER_EMPTY, null));
        validTests.add(new GenMultiCRLTest("CRL with leading and trailing text data", ("This is a CRL\n" + JTEST_ROOT_CRL + "\nSee? Told you so\n\n").getBytes(), null, new X500Principal("CN=JTest Root CA,OU=IT,O=JTest")));
        validTests.add(new GenMultiCRLTest("Two CRLs, one after the other with leading/trailing text", ("This is a CRL\n" + JTEST_ROOT_CRL + "\nAnd this is another CRL\n" + JTEST_INTERMED_CRL + "\nAnd this is trailing text\n").getBytes(), null, new X500Principal("CN=JTest Root CA,OU=IT,O=JTest"), new X500Principal("CN=JTest Intermediate CA 1,OU=IT,O=JTest")));
        validTests.add(new GenMultiCRLTest("Two CRLs in a PKCS#7 CRL set", PKCS7_CRL_SET.getBytes(), null, new X500Principal("CN=JTest Root CA,OU=IT,O=JTest"), new X500Principal("CN=JTest Intermediate CA 1,OU=IT,O=JTest")));
        invalidTests.add(new GenSingleCertTest("Invalid PEM encoding", INVALID_CERT.getBytes(), new CertificateParsingException()));
        invalidTests.add(new GenMultiCertTest("Invalid PEM encoding", INVALID_CERT.getBytes(), new CertificateParsingException()));
        invalidTests.add(new GenMultiCertTest("Two cert sequence, one valid and one invalid", (INTERMED_CA_CERT + "\n" + INVALID_CERT).getBytes(), new CertificateParsingException()));
        invalidTests.add(new GenMultiCertTest("Non-certificate text", "This is not a certificate".getBytes(), new CertificateException()));
        invalidTests.add(new GenMultiCertTest("Non-certificate text with partial PEM header (4 hyphens)", "----This is not a valid x509 certificate".getBytes(), new CertificateException()));
        invalidTests.add(new GenMultiCertTest("Leading non-certificate text plus valid PEM header, " + "but not on new line", "This is not valid -----BEGIN CERTIFICATE-----".getBytes(), new CertificateException()));
        byte[] emptyCString = { 0 };
        invalidTests.add(new GenMultiCertTest("Empty C-style string", emptyCString, new CertificateException()));
        invalidTests.add(new GenMultiCRLTest("Non-CRL text", "This is not a CRL".getBytes(), new CRLException()));
    ...
    public static abstract class DecodeTest {
        ...
        DecodeTest(String name, byte[] input, Throwable failType, X500Principal... princs) throws CertificateException {
            testName = name;
    ...
    public static class GenMultiCertTest extends DecodeTest {
        ...
        public GenMultiCertTest(String name, byte[] input, Throwable failType, X500Principal... princs) throws CertificateException {
        ...
        public void passTest() throws GeneralSecurityException {
            ...
            if (certs.size() == principals.length) {
                for (Certificate crt : certs) {
                    ...
                    if (!xc.getSubjectX500Principal().equals(principals[i])) {
                        throw new RuntimeException("Name mismatch: " + "cert: " + xc.getSubjectX500Principal() + ", expected: " + principals[i]);
                    }
                    i++;
                }
            } else {
                throw new RuntimeException("Size mismatch: certs = " + certs.size() + ", expected = " + principals.length);
            }
        }
        ...
        public void failTest() throws GeneralSecurityException {
            ...
            if (expectedException == null) {
                throw new RuntimeException("failTest requires non-null " + "expectedException");
            }
            try {
                certs = cf.generateCertificates(new ByteArrayInputStream(testData));
            ...
            if (caughtException != null) {
                if (!caughtException.getClass().equals(expectedException.getClass())) {
                    System.err.println("Unexpected exception thrown. " + "Received: " + caughtException + ", Expected: " + expectedException.getClass());
                    throw new RuntimeException(caughtException);
                }
            } else {
                throw new RuntimeException("Failed to catch expected " + "exception " + expectedException.getClass());
            }
        }
    }
    ...
    public static class GenSingleCertTest extends DecodeTest {
        ...
        public GenSingleCertTest(String name, byte[] input, Throwable failType, X500Principal... princs) throws CertificateException {
            super(name, input, failType, princs);
        }
        ...
        public void passTest() throws GeneralSecurityException {
            ...
            cert = (X509Certificate) cf.generateCertificate(new ByteArrayInputStream(testData));
            if (!cert.getSubjectX500Principal().equals(principals[0])) {
                throw new RuntimeException("Name mismatch: " + "cert: " + cert.getSubjectX500Principal() + ", expected: " + principals[0]);
            }
        }
    ...
    public static class GenMultiCRLTest extends DecodeTest {
        ...
        public GenMultiCRLTest(String name, byte[] input, Throwable failType, X500Principal... princs) throws CertificateException {
            super(name, input, failType, princs);
        }
            ...
            if (crls.size() == principals.length) {
                for (CRL revlist : crls) {
                    ...
                    if (!xc.getIssuerX500Principal().equals(principals[i])) {
                        throw new RuntimeException("Name mismatch: " + "CRL: " + xc.getIssuerX500Principal() + ", expected: " + principals[i]);
                    }
                    i++;
                }
            } else {
                throw new RuntimeException("Size mismatch: crls = " + crls.size() + ", expected = " + principals.length);
            }
        }
