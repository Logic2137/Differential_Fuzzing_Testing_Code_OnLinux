...
public class CheckZombieLockTest {
    ...
    private static void runTests(File writableDir, String... args) throws SecurityException, RuntimeException, IOException {
        for (String arg : args) {
            switch(TestCase.valueOf(arg)) {
                case WRITABLE:
                    checkWritable(writableDir);
                    break;
                case CLOSE:
                    testFileHandlerClose(writableDir);
                    break;
                case CREATE_FIRST:
                    testFileHandlerCreate(writableDir, true);
                    break;
                case CREATE_NEXT:
                    testFileHandlerCreate(writableDir, false);
                    break;
                case REUSE:
                    testFileHandlerReuse(writableDir);
                    break;
                case CLEANUP:
                    delete(writableDir);
                    break;
    ...
    private static void checkWritable(File writableDir) throws SecurityException, RuntimeException, IOException {
    ...
    private static List<File> listLocks(File writableDir, boolean print) throws IOException {
    ...
    private static File setup() throws RuntimeException {
        ...
        try {
            FileChannel fc = FileChannel.open(Paths.get(writableDir.getAbsolutePath(), uniqueFileName), StandardOpenOption.CREATE_NEW, StandardOpenOption.APPEND, StandardOpenOption.DELETE_ON_CLOSE);
            try {
                fc.tryLock();
            } catch (IOException x) {
            ...
            System.err.println("Failed to create new file " + uniqueFileName + " in " + writableDir.getAbsolutePath());
