...
public class Test6890943 {
    ...
    private int solve() {
        ...
        for (int digDown = 0; digDown < r; digDown++) {
            ...
            for (Map.Entry<Integer, Integer> m : tries.entrySet()) {
                ...
                if (fallLeft > 0) {
                    fallLeftDelta = 1;
                    if (fallLeft <= f)
                        addToM(calcWalkingRange(digDown + fallLeft, start(q)), m.getValue());
                }
                int fallRight = fall(digDown, end(q));
                if (fallRight > 0) {
                    fallRightDelta = 1;
                    if (fallRight <= f)
                        addToM(calcWalkingRange(digDown + fallRight, end(q)), m.getValue());
                }
        ...
        for (Map.Entry<Integer, Integer> m : M.entrySet()) {
            if (depth(m.getKey()) == r - 1)
                result = Math.min(m.getValue(), result);
        }
