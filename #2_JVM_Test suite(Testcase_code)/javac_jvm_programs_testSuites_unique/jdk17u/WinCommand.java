...
class CommandRunner {
    ...
    CommandRunner(String... args) throws Exception {
        Process p = (generator.nextInt(2) == 0) ? new ProcessBuilder(args).start() : Runtime.getRuntime().exec(args);
...
public class WinCommand {
    ...
    public static void main(String[] args) throws Exception {
        File systemRoot = getenv("SystemRoot") != null ? new File(getenv("SystemRoot")) : getenv("WINDIR") != null ? new File(getenv("WINDIR")) : null;
        ...
        if (new File(systemDirW, "cmd.exe").exists()) {
            try {
                ...
                checkCD("cmd", "cmd.exe", systemDirW + "\\cmd.exe", systemDirW + "\\cmd", systemDirM + "/cmd.exe", systemDirM + "/cmd", "/" + systemDirM + "/cmd", "cdcmd.cmd", "./cdcmd.cmd", ".\\cdcmd.cmd", "cdbat.bat", "./cdbat.bat", ".\\cdbat.bat");
            } finally {
