...
public class InterJVMGetDropSuccessTest {
    ...
    public void start() {
        ...
        try {
            ...
            String command = javaPath + File.separator + "bin" + File.separator + "java -cp " + System.getProperty("test.classes", ".") + " Child " + p.x + " " + p.y + " " + d.width + " " + d.height;
            ...
            default:
                if (dropCount == 2) {
                    ...
                    if (expectedRetCode != returnCode) {
                        throw new RuntimeException("The test failed. Expected:" + expectedRetCode + ". Returned:" + returnCode);
                    }
                }
                break;
        }
...
final class Util implements AWTEventListener {
    ...
    public static boolean pointInComponent(Robot robot, Point p, Component comp) throws InterruptedException {
        return theInstance.pointInComponentImpl(robot, p, comp);
    }

    private boolean pointInComponentImpl(Robot robot, Point p, Component comp) throws InterruptedException {
        robot.waitForIdle();
        reset();
...
class Child {
    ...
    final DragGestureRecognizer dragGestureRecognizer = dragSource.createDefaultDragGestureRecognizer(frame, DnDConstants.ACTION_COPY, dragGestureListener);
    ...
    public void run(String[] args) {
        try {
            ...
            robot.mousePress(InputEvent.BUTTON1_MASK);
            for (Point p = new Point(sourcePoint); !p.equals(targetPoint); p.translate(Util.sign(targetPoint.x - p.x), Util.sign(targetPoint.y - p.y))) {
                robot.mouseMove(p.x, p.y);
                Thread.sleep(50);
            }
            synchronized (Util.SYNC_LOCK) {
