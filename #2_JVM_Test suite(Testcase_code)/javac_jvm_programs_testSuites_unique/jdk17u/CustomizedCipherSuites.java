...
public class CustomizedCipherSuites {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (isClientMode) {
            ...
            try (SSLSocket socket = (SSLSocket) factory.createSocket()) {
                System.out.println("Checking SSLSocket default cipher suites ...");
                checkEnabledCiphers(socket.getEnabledCipherSuites());
                System.out.println("Checking SSLSocket supported cipher suites ...");
                checkSupportedCiphers(socket.getSupportedCipherSuites());
            }
        } else {
            SSLServerSocketFactory factory = context.getServerSocketFactory();
            try (SSLServerSocket socket = (SSLServerSocket) factory.createServerSocket()) {
                System.out.println("Checking SSLServerSocket default cipher suites ...");
                checkEnabledCiphers(socket.getEnabledCipherSuites());
                System.out.println("Checking SSLServerSocket supported cipher suites ...");
                checkSupportedCiphers(socket.getSupportedCipherSuites());
            }
        }
    ...
    private static void checkEnabledCiphers(String[] ciphers) throws Exception {
        if (ciphers.length == 0) {
    ...
    private static void checkSupportedCiphers(String[] ciphers) throws Exception {
        if (ciphers.length == 0) {
