...
public class bug4473075 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (headerH != USER_HEADER_HEIGHT) {
            throw new RuntimeException("TableHeader height was not set: " + headerH + " !=" + USER_HEADER_HEIGHT);
        }
        double tableX = table.getX();
        int headerX = table.getTableHeader().getX();
        if (tableX != headerX) {
            throw new RuntimeException("TableHeader X position is wrong: " + tableX + " !=" + headerX);
        }
        double tableW = table.getWidth();
        int headerW = table.getTableHeader().getWidth();
        if (tableW != headerW) {
            throw new RuntimeException("TableHeader width is wrong: " + tableW + " !=" + headerW);
        }
