...
public class stringvalueof002 {
    ...
    public static int run(String[] argv, PrintStream out) {
            ...
            try {
                ...
                for (; ; ) {
                    try {
                        ...
                        try {
                            argument = (Connector.BooleanArgument) defaultArguments.get(argName);
                            break;
                ...
                out.println(sErr1 + "no Connector with BooleanArgument found\n");
                return exitCode0;
            }
        }
        ...
        if (argument.booleanValue() != true) {
            exitCode = 2;
            out.println(sErr2 + "case: stringValueOf(true) doesn't change value true\n" + "error: argument's value != true\n");
        }
        argument.setValue(true);
        argumentValue = argument.stringValueOf(false);
        if (argument.booleanValue() != true) {
            exitCode = 2;
            out.println(sErr2 + "case: stringValueOf(false) doesn't change value true\n" + "error: argument's value != true\n");
        }
        argument.setValue(false);
        argumentValue = argument.stringValueOf(true);
        if (argument.booleanValue() != false) {
            exitCode = 2;
            out.println(sErr2 + "case: stringValueOf(true) doesn't change value false\n" + "error: a returned value != false\n");
        }
        argument.setValue(false);
        argumentValue = argument.stringValueOf(false);
        if (argument.booleanValue() != false) {
            exitCode = 2;
            out.println(sErr2 + "case: stringValueOf(false) doesn't change value false\n" + "error: a returned value != false\n");
        }
