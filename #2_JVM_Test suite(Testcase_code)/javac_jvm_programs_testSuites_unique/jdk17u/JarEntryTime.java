...
public class JarEntryTime {
    ...
    static void extractJar(File jarFile, boolean useExtractionTime) throws Throwable {
        ...
        if (useExtractionTime) {
            args = new String[] { jarcmd, "-J-Dsun.tools.jar.useExtractionTime=true", "xf", jarFile.getName() };
        } else {
            args = new String[] { jarcmd, "xf", jarFile.getName() };
        }
    ...
    static void checkFileTime(long now, long original) {
        ...
        if (Math.abs(now - original) > PRECISION) {
            System.out.format("Extracted to %s, expected to be close to %s%n", FileTime.fromMillis(now), FileTime.fromMillis(original));
            fail();
        }
    }
    ...
    static void checkFileTime(long start, long now, long end) {
        ...
        if (now < start || now > end) {
            System.out.format("Extracted to %s, " + "expected to be after %s and before %s%n", FileTime.fromMillis(now), FileTime.fromMillis(start), FileTime.fromMillis(end));
            fail();
        }
    }
