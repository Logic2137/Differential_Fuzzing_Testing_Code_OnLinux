...
public class TIFFImageReadParamTest {
    ...
    private void addASCIIField(TIFFDirectory d, String name, String data, int num) {
        String[] a = { data };
        d.addTIFFField(new TIFFField(new TIFFTag(name, num, 1 << TIFFTag.TIFF_ASCII), TIFFTag.TIFF_ASCII, 1, a));
    }

    private void checkASCIIValue(TIFFDirectory d, String what, String data, int num) {
        ...
        check(f.getValueAsString(0).equals(data), "invalid " + what + " data");
    }
    ...
    private void writeImage() throws Exception {
        ...
        try (ImageOutputStream ios = ImageIO.createImageOutputStream(s)) {
            ...
            dir.addTIFFField(new TIFFField(new TIFFTag("CleanFaxData", FAX_N, 1 << TIFFTag.TIFF_SHORT), FAX_DATA));
    ...
    private TIFFDirectory getDir(TIFFTagSet[] add, TIFFTagSet[] remove) throws Exception {
        ...
        for (TIFFTagSet ts : add) {
            param.addAllowedTagSet(ts);
        }
        for (TIFFTagSet ts : remove) {
    ...
    private void simpleChecks() {
        ...
        check(allowed.contains(BaselineTIFFTagSet.getInstance()), "must contain BaselineTIFFTagSet");
        check(allowed.contains(FaxTIFFTagSet.getInstance()), "must contain FaxTIFFTagSet");
        check(allowed.contains(ExifParentTIFFTagSet.getInstance()), "must contain ExifParentTIFFTagSet");
        check(allowed.contains(GeoTIFFTagSet.getInstance()), "must contain GeoTIFFTagSet");
        TIFFTagSet gps = ExifGPSTagSet.getInstance();
        param.addAllowedTagSet(gps);
        check(param.getAllowedTagSets().contains(gps), "must contain ExifGPSTagSet");
        param.removeAllowedTagSet(gps);
        check(!param.getAllowedTagSets().contains(gps), "must not contain ExifGPSTagSet");
        param.removeAllowedTagSet(gps);
        boolean ok = false;
        try {
            param.addAllowedTagSet(null);
        ...
        ok = false;
        try {
            param.removeAllowedTagSet(null);
        } catch (IllegalArgumentException e) {
            ok = true;
        }
    ...
    private void run() {
        ...
        try {
            writeImage();
            TIFFTagSet[] empty = {}, geo = { GeoTIFFTagSet.getInstance() }, exif = { ExifTIFFTagSet.getInstance() }, gps = { ExifGPSTagSet.getInstance() }, fax = { FaxTIFFTagSet.getInstance() };
            ...
            check((dir.getTIFFField(FAX_N).getCount() == 1) && (dir.getTIFFField(FAX_N).getAsInt(0) == FAX_DATA), "invalid Fax field value");
