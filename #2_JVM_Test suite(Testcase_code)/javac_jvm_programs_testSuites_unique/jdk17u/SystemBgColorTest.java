...
public class SystemBgColorTest {
    ...
    static int[] cmap = { 0x00000000, 0xffffffff };

    public static void test(int dsttype) {
        BufferedImage src = new BufferedImage(TESTW, TESTH, BufferedImage.TYPE_INT_ARGB);
        test(src, dsttype);
        IndexColorModel icm = new IndexColorModel(8, 2, cmap, 0, true, 0, DataBuffer.TYPE_BYTE);
        src = new BufferedImage(TESTW, TESTH, BufferedImage.TYPE_BYTE_INDEXED, icm);
        test(src, dsttype);
    }

    public static void test(Image src, int dsttype) {
        BufferedImage dst = new BufferedImage(TESTW, TESTH, dsttype);
