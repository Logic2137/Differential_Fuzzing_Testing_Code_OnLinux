...
public class SourceClippingBlitTest extends Canvas {
    ...
    public void runTests() {
        ...
        for (Image srcIm : new Image[] { getBufferedImage(gc, IMAGEW, IMAGEH, BufferedImage.TYPE_INT_RGB, true), getBufferedImage(gc, IMAGEW, IMAGEH, BufferedImage.TYPE_INT_RGB, false), getVImage(gc, IMAGEW, IMAGEH) }) {
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                if (grabbedBI.getRGB(x, y) != rgb) {
                    ...
                    System.out.println(" expected: " + Integer.toHexString(rgb) + " got:" + Integer.toHexString(grabbedBI.getRGB(x, y)));
    ...
    static void initImage(GraphicsConfiguration gc, Image image) {
        ...
        g.dispose();
        if (dstImage == null) {
            dstImage = gc.createCompatibleVolatileImage(TESTW, TESTH, Transparency.OPAQUE);
        }
    ...
    static Image getBMImage(GraphicsConfiguration gc, int w, int h) {
        Image image = gc.createCompatibleImage(w, h, Transparency.BITMASK);
        initImage(gc, image);
        return image;
    }

    static Image getBufferedImage(GraphicsConfiguration gc, int w, int h, int type, boolean acceleratable) {
