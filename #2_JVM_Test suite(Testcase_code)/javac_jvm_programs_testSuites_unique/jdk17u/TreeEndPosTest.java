...
public class TreeEndPosTest {
    ...
    private static JavaFileManager getJavaFileManager(JavaCompiler compiler, DiagnosticCollector dc) {
    ...
    static class JavaSource extends SimpleJavaFileObject {
        ...
        static JavaSource createJavaSource(String preamble, String body, String postamble, String expected) {
        ...
        private static JavaSource createJavaSource(String preamble, String body, String postamble, int start, int end) {
    ...
    static void testUninitializedVariable() throws IOException {
        compile(JavaSource.createJavaSource("Object o = new A().new BT(); class A { }", "BT"));
    }

    static void testMissingAnnotationValue() throws IOException {
        compile(JavaSource.createJavaSource("@Foo(\"vvvv\")", null, "@interface Foo { }", "\"vvvv\""));
    }

    static void testUnresolvableAnnotationAttribute() throws IOException {
        compile(JavaSource.createJavaSource("@Foo(value=\"vvvv\")", null, "@interface Foo { }", "value"));
    }

    static void testFinalVariableWithDefaultConstructor() throws IOException {
        compile(JavaSource.createJavaSource("private static final String Foo; public void bar() { }", "private static final String Foo;"));
    }
    ...
    static void compile(JavaSource src) throws IOException {
        ...
        try (JavaFileManager javaFileManager = getJavaFileManager(compiler, dc)) {
            ...
            sources.add(src);
            JavaCompiler.CompilationTask task = compiler.getTask(writer, javaFileManager, dc, options, null, sources);
            task.call();
    ...
    static void treeSpan(JavaSource src) throws IOException {
        ...
        try (JavaFileManager javaFileManager = getJavaFileManager(compiler, dc)) {
            ...
            List<JavaFileObject> sources = new ArrayList<>();
            sources.add(src);
            JavacTask task = (JavacTask) compiler.getTask(writer, javaFileManager, dc, options, null, sources);
            ...
            new TreeScanner<Void, Void>() {
                ...
                public Void scan(Tree tree, Void p) {
                    ...
                    found[0] |= (sp.getStartPosition(cut, tree) == src.startPos) && (sp.getEndPosition(cut, tree) == src.endPos);
