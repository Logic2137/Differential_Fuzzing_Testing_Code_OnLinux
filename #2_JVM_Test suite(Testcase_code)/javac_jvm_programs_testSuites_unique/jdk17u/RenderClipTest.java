...
public class RenderClipTest {
    ...
    public static abstract class AnnotatedRenderOp {
        ...
        public static AnnotatedRenderOp parse(String str) {
            ...
            if (((ar = Cubic.tryparse(str)) != null) || ((ar = Quad.tryparse(str)) != null) || ((ar = Poly.tryparse(str)) != null) || ((ar = Path.tryparse(str)) != null) || ((ar = Rect.tryparse(str)) != null) || ((ar = Line.tryparse(str)) != null) || ((ar = RectMethod.tryparse(str)) != null) || ((ar = LineMethod.tryparse(str)) != null)) {
                return ar;
            }
    ...
    public static void usage(String err) {
        ...
        System.err.println("usage: java RenderClipTest " + "[-read[file F]] [-rectsuite] [-fill] [-draw]");
        System.err.println("                           " + "[-aa] [-pure] [-lw N] [-rot N]");
        System.err.println("                           " + "[-rectmethod] [-linemethod] [-rect] [-line]");
        System.err.println("                           " + "[-cubic] [-quad] [-poly] [-path]");
        System.err.println("                           " + "[-silent] [-verbose] [-showerr] [-count N]");
        System.err.println("                           " + "[-strict] [-usage]");
        ...
        System.err.println("    -lw N         Test line widths of N " + "(default 1.0)");
        System.err.println("    -rot N        Test rotation by N degrees " + "(default 0.0)");
        ...
        System.err.println("    -count N      N tests per shape, then exit " + "(default 1000)");
    ...
    public static void main(String[] argv) {
        ...
        if (readTests) {
            if (rectsuite || testDraw || testFill || useAA || strokePure || lw != 1.0f || rot != 0.0 || testOps.size() > 0) {
                usage("Should not specify test types with -read options");
            }
        } else if (rectsuite) {
            if (testDraw || testFill || useAA || strokePure || lw != 1.0f || rot != 0.0 || testOps.size() > 0) {
                usage("Should not specify test types with -rectsuite option");
            }
        } else {
            if (!testDraw && !testFill) {
                usage("No work: Must specify one or both of " + "-fill or -draw");
            }
            if (testOps.size() == 0) {
                usage("No work: Must specify one or more of " + "-rect[method], -line[method], " + "-cubic, -quad, -poly, or -path");
            }
        }
        initImages();
        if (readTests) {
            try {
                ...
                if (readFile == null) {
                    ...
                    File f = new File(System.getProperty("test.src", "."), readFile);
    ...
    public static void runRectSuite(int count) {
        AnnotatedRenderOp[] ops = { new Rect(), new RectMethod(), new Line(), new LineMethod() };
        float[] filllinewidths = { 0.0f, 2.0f };
        float[] drawlinewidths = { 0.0f, 0.5f, 1.0f, 2.0f, 2.5f, 5.0f, 5.3f };
        double[] rotations = { 0.0, 15.0, 90.0, 135.0, 180.0, 200.0, 270.0, 300.0 };
    ...
    public static void initGCs() {
        ...
        if (strokePure) {
            grefrender.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE);
            gtstrender.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE);
        }
        if (useAA) {
            grefrender.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            gtstrender.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            maxerr = 1;
        }
    }
    ...
    public static void parseAndRun(InputStream in) throws IOException {
        ...
        while ((str = br.readLine()) != null) {
            ...
            if (str.indexOf(" out of ") > 0 && str.indexOf(" tests failed...") > 0) {
                continue;
            }
    ...
    public static int getOffset(BufferedImage img) {
        Raster r = img.getRaster();
        SinglePixelPackedSampleModel sppsm = (SinglePixelPackedSampleModel) r.getSampleModel();
        return sppsm.getOffset(-r.getSampleModelTranslateX(), -r.getSampleModelTranslateY());
    }
    ...
    public static void check(BufferedImage imgref, BufferedImage imgtst, AnnotatedRenderOp ar, boolean wasfill) {
        numtests++;
        ...
        if (check(ar, wasfill, null, 0, 0, datatst, scantst, offtst, 0, 0, 40, 10)) {
            return;
        }
        if (check(ar, wasfill, null, 0, 0, datatst, scantst, offtst, 0, 30, 40, 40)) {
            return;
        }
        if (check(ar, wasfill, null, 0, 0, datatst, scantst, offtst, 0, 10, 10, 30)) {
            return;
        }
        if (check(ar, wasfill, null, 0, 0, datatst, scantst, offtst, 30, 10, 40, 30)) {
            return;
        }
        check(ar, wasfill, dataref, scanref, offref, datatst, scantst, offtst, 10, 10, 30, 30);
    }

    public static boolean check(AnnotatedRenderOp ar, boolean wasfill, int[] dataref, int scanref, int offref, int[] datatst, int scantst, int offtst, int x0, int y0, int x1, int y1) {
        offref += scanref * y0;
        offtst += scantst * y0;
        for (int y = y0; y < y1; y++) {
            for (int x = x0; x < x1; x++) {
                ...
                if (dataref == null) {
                    ...
                    failed = (rgbref != rgbtst && maxdiff(rgbref, rgbtst) > maxerr);
                ...
                if (failed) {
                    ...
                    if (!silent) {
                        System.out.println("Failed: " + reason + " at " + x + ", " + y + " [" + Integer.toHexString(rgbref) + " != " + Integer.toHexString(rgbtst) + "]");
    ...
    public static class Cubic extends AnnotatedShapeOp {
        ...
        public static Cubic tryparse(String str) {
            ...
            Cubic c = new Cubic();
            c.cubic.setCurve(coords[0], coords[1], coords[2], coords[3], coords[4], coords[5], coords[6], coords[7]);
            return c;
        }
        ...
        public String toString() {
            return ("Cubic(" + cubic.getX1() + ", " + cubic.getY1() + ", " + cubic.getCtrlX1() + ", " + cubic.getCtrlY1() + ", " + cubic.getCtrlX2() + ", " + cubic.getCtrlY2() + ", " + cubic.getX2() + ", " + cubic.getY2() + ")");
        }
    }
    ...
    public static class Quad extends AnnotatedShapeOp {
        ...
        public static Quad tryparse(String str) {
            ...
            Quad c = new Quad();
            c.quad.setCurve(coords[0], coords[1], coords[2], coords[3], coords[4], coords[5]);
            return c;
        }
        ...
        public String toString() {
            return ("Quad(" + quad.getX1() + ", " + quad.getY1() + ", " + quad.getCtrlX() + ", " + quad.getCtrlY() + ", " + quad.getX2() + ", " + quad.getY2() + ")");
        }
    }
            ...
            for (int i = randInt(5) + 3; i > 0; --i) {
                switch(randInt(5)) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        path.quadTo(randFltCoord(), randFltCoord(), randFltCoord(), randFltCoord());
                        break;
                    case 3:
                        path.curveTo(randFltCoord(), randFltCoord(), randFltCoord(), randFltCoord(), randFltCoord(), randFltCoord());
                        break;
                    case 4:
                        path.closePath();
                        break;
    ...
    public static class Rect extends AnnotatedShapeOp {
        ...
        public static Rect tryparse(String str) {
            ...
            Rect r = new Rect();
            r.rect.setRect(coords[0], coords[1], coords[2], coords[3]);
            return r;
        }
        ...
        public String toString() {
            return ("Rect(" + rect.getX() + ", " + rect.getY() + ", " + rect.getWidth() + ", " + rect.getHeight() + ")");
        }
    }
    ...
    public static class Line extends AnnotatedShapeOp {
        ...
        public static Line tryparse(String str) {
            ...
            Line l = new Line();
            l.line.setLine(coords[0], coords[1], coords[2], coords[3]);
            return l;
        }
        ...
        public String toString() {
            return ("Line(" + line.getX1() + ", " + line.getY1() + ", " + line.getX2() + ", " + line.getY2() + ")");
        }
    }
    ...
    public static class RectMethod extends AnnotatedRenderOp {
        ...
        public static RectMethod tryparse(String str) {
            ...
            rm.rect.setBounds(coords[0], coords[1], coords[2], coords[3]);
            return rm;
        }
        ...
        public void randomize() {
            rect.setBounds(randIntCoord(), randIntCoord(), randIntCoord(), randIntCoord());
        }
        ...
        public String toString() {
            return ("RectMethod(" + rect.x + ", " + rect.y + ", " + rect.width + ", " + rect.height + ")");
        }
    }
    ...
    public static class LineMethod extends AnnotatedRenderOp {
        ...
        public String toString() {
            return ("LineMethod(" + line[0] + ", " + line[1] + ", " + line[2] + ", " + line[3] + ")");
        }
    }
