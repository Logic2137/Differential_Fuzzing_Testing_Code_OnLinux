    ...
    private static void test(MidiDevice.Info info) throws Exception {
        try (MidiDevice device = MidiSystem.getMidiDevice(info)) {
            ...
            try (Receiver r = device.getReceiver()) {
                ...
                r.send(new SysexMessage(new byte[] { (byte) SYSTEM_EXCLUSIVE, 0x0, 0x03, 0x04, (byte) SPECIAL_SYSTEM_EXCLUSIVE }, 5), -1);
                ...
                r.send(new RawMidiMessage(new byte[] { (byte) SYSTEM_EXCLUSIVE, 0x02, 0x03, 0x04 }), -1);
                System.err.println("raw 4");
                r.send(new RawMidiMessage(new byte[] { (byte) 0x02, 0x02, 0x03, 0x04 }), -1);
                System.err.println("raw 5");
                r.send(new RawMidiMessage(new byte[] { (byte) 0x02, 0x02, 0x03, (byte) SPECIAL_SYSTEM_EXCLUSIVE }), -1);
                ...
                r.send(new RawMidiMessage(new byte[] { (byte) SPECIAL_SYSTEM_EXCLUSIVE }), -1);
