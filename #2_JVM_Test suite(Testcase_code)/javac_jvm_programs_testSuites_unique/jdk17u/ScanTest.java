...
public class ScanTest {
    ...
    public static void hasNextLineTest(int sourceType) throws Exception {
        ...
        try (Scanner sc2 = new Scanner(inputFile)) {
            ...
            while (sc2.hasNextLine()) lastLine = sc2.nextLine();
    ...
    public static void floatPatternTest(int sourceType) throws Exception {
        String input = "090.090 1 22.0 -3 -44.05 +.123 -.1234 -3400000 56,566.6 " + "Infinity +Infinity -Infinity NaN -NaN +NaN 5.4.0 5-.00 ++6.07";
        Scanner sc = scannerFor(input, sourceType);
        floatPatternBody(sc);
        CharBuffer cb = CharBuffer.wrap(input);
        sc = new Scanner(cb);
    ...
    public static void resetTest() throws Exception {
        ...
        if (sc.radix() != 16 || !sc.locale().equals(dummy) || !sc.delimiter().pattern().equals(a.pattern())) {
            ...
            sc.reset();
            if (sc.radix() != radix || !sc.locale().equals(locale) || !sc.delimiter().pattern().equals(delimiter.pattern())) {
                failCount++;
            }
        }
        sc.close();
    ...
    static List<BiConsumer<Scanner, Integer>> methodWRList = Arrays.asList((s, r) -> s.hasNextByte(r), (s, r) -> s.nextByte(r), (s, r) -> s.hasNextShort(r), (s, r) -> s.nextShort(r), (s, r) -> s.hasNextInt(r), (s, r) -> s.nextInt(r), (s, r) -> s.hasNextLong(r), (s, r) -> s.nextLong(r), (s, r) -> s.hasNextBigInteger(r), (s, r) -> s.nextBigInteger(r));

    public static void outOfRangeRadixTest() throws Exception {
        int[] bad = new int[] { -1, 0, 1, 37, 38 };
        int[] good = IntStream.rangeClosed(Character.MIN_RADIX, Character.MAX_RADIX).toArray();
    ...
    public static void streamCloseTest() throws Exception {
        ...
        try {
            sc2.tokens().peek(s -> sc2.close()).count();
        } catch (IllegalStateException ise) {
        }
        ...
        try (Scanner sc4 = new Scanner(inputFile)) {
            sc4.findAll("[0-9]+").peek(s -> sc4.close()).count();
            failCount++;
        } catch (IllegalStateException ise) {
        }
    ...
    public static void streamComodTest() {
        try {
            ...
            sc.tokens().peek(s -> sc.hasNext()).count();
            failCount++;
        ...
        try {
            String input = IntStream.range(0, 100).mapToObj(String::valueOf).collect(Collectors.joining(" "));
            Scanner sc = new Scanner(input);
            sc.findAll("[0-9]+").peek(s -> sc.hasNext()).count();
            failCount++;
        } catch (ConcurrentModificationException cme) {
        }
        try {
