...
public class MetaInf {
    ...
    static final ToolProvider JAR_TOOL = ToolProvider.findFirst("jar").orElseThrow(() -> new RuntimeException("jar tool not found"));
    ...
    static String contents = System.getProperty("test.src") + File.separatorChar + "jarcontents";
    ...
    static boolean contains(File jarFile, String entryName) throws IOException {
    ...
    static void checkContains(File jarFile, String entryName) throws IOException {
        if (!contains(jarFile, entryName))
            throw new Error(String.format("expected jar %s to contain %s", jarFile, entryName));
    }
    ...
    static void testIndex(String jarName) throws IOException {
        ...
        BufferedReader index = new BufferedReader(new InputStreamReader(f.getInputStream(f.getJarEntry(INDEX))));
        String line;
