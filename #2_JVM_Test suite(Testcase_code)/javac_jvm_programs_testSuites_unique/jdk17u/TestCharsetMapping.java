...
public class TestCharsetMapping {
    ...
    private TestCharsetMapping(CharsetInfo csinfo) throws Exception {
        this.csinfo = csinfo;
        this.encoder = csinfo.cs.newEncoder().onUnmappableCharacter(CodingErrorAction.REPLACE).onMalformedInput(CodingErrorAction.REPLACE);
        this.decoder = csinfo.cs.newDecoder().onUnmappableCharacter(CodingErrorAction.REPLACE).onMalformedInput(CodingErrorAction.REPLACE);
    }
    ...
    private class Test {
        ...
        private boolean decode(ByteBuffer refBytes, CharBuffer refChars) throws Exception {
            ...
            while (refChars.hasRemaining()) {
                ...
                if (rcp != ocp) {
                    log.println("      Error: " + printBytes(bs) + " --> " + printCodePoint(ocp) + ", expected " + printCodePoint(rcp));
                    if (++e >= MAXERRS) {
        ...
        private boolean encode(ByteBuffer refBytes, CharBuffer refChars) throws Exception {
            ...
            while (refChars.hasRemaining()) {
                ...
                if (!eq) {
                    log.println("      Error: " + printCodePoint(cp) + " --> " + printBytes(obs) + ", expected " + printBytes(rbs));
                    if (++e >= MAXERRS) {
                        log.println("      Too many errors, giving up");
                        break;
                    }
        ...
        private boolean run(int mode) throws Exception {
            log.println("  " + bytesPerChar + " byte" + plural(bytesPerChar) + "/char");
            ...
            if (dRefChars.capacity() < refChars.capacity()) {
                dRefChars = ByteBuffer.allocateDirect(refChars.capacity() * 2).asCharBuffer();
            }
            refBytes.flip();
            refChars.flip();
            dRefBytes.clear();
    ...
    private boolean testStringConv() throws Exception {
        ...
        for (Entry e : csinfo.mappings) {
            ...
            if (e.cp2 != 0)
                sbDec.append(e.cp2);
            if (csinfo.nr != null && csinfo.nr.containsKey(e.bb) || csinfo.c2b != null && !csinfo.c2b.containsKey(e.cp))
                continue;
            baosEnc.write(e.bs);
    ...
    private static class CharsetInfo {
        ...
        boolean loadMappings(Path dir) throws IOException {
            ...
            mappings = Files.lines(path).filter(ln -> !ln.startsWith("#") && m.reset(ln).lookingAt()).map(ln -> parse(m)).filter(e -> e.cp != UNMAPPABLE).collect(Collectors.toList());
            path = dir.resolve(clzName + ".nr");
            if (Files.exists(path)) {
                nr = Files.lines(path).filter(ln -> !ln.startsWith("#") && m.reset(ln).lookingAt()).map(ln -> parse(m)).collect(Collectors.toMap(e -> e.bb, Function.identity()));
            }
            path = dir.resolve(clzName + ".c2b");
            if (Files.exists(path)) {
                c2b = Files.lines(path).filter(ln -> !ln.startsWith("#") && m.reset(ln).lookingAt()).map(ln -> parse(m)).collect(Collectors.toMap(e -> e.cp, Function.identity()));
            }
            return true;
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        Path dir = Paths.get(System.getProperty("test.src", ".") + "/../../../../make/data/charsetmapping");
        if (!Files.exists(dir)) {
        ...
        for (CharsetInfo csinfo : charsets(dir.resolve("charsets"))) {
            ...
            if (!new TestCharsetMapping(csinfo).run()) {
                if (csinfo.csName.equals("x-IBM948") || csinfo.csName.equals("x-IBM950") || csinfo.csName.equals("x-IBM937") || csinfo.csName.equals("x-IBM1383")) {
        ...
        log.println();
        log.println(tested + " charset" + plural(tested) + " tested, " + skipped + " skipped, " + known + " known issue(s)");
        log.println();
        if (errors > 0)
