...
public class TestGetElement extends AbstractProcessor {
    ...
    public void run() throws Exception {
        ...
        for (int i = 1; i <= NUM_TESTS; i++) {
            ...
            classesDir.mkdirs();
            String[] args = { "-d", classesDir.getPath(), "-processorpath", testClasses, "-processor", myClassName, "-proc:only", "-Atest=" + i, "-Alast=" + (i == NUM_TESTS), mySrc };
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnvironment) {
        ...
        int EXPECT = 1;
        if (nelems != EXPECT) {
            m.printMessage(Diagnostic.Kind.ERROR, "Unexpected number of elements found: " + nelems + " expected: " + EXPECT);
        }
        return true;
    }
    ...
    class Scanner extends TreePathScanner<Integer, Integer> {
        ...
        int check(int test) {
            ...
            System.err.println(String.format("%3d: %-" + MAXLEN + "s -- %s", count, text, (e == null ? "null" : e.getKind() + " " + e)));
            Messager m = processingEnv.getMessager();
            if (e == null) {
            ...
            if (last && !e.getSimpleName().contentEquals("last")) {
                m.printMessage(Diagnostic.Kind.ERROR, "Unexpected name in last test: " + e.getSimpleName() + ", expected: last");
            }
            return 1;
        }
    }
