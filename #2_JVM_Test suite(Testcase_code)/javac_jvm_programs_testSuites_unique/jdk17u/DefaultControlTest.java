...
public class DefaultControlTest {
    ...
    static final ResourceBundle.Control CONTROL = ResourceBundle.Control.getControl(FORMAT_DEFAULT);
    ...
    static final Object[] FULLARGS = { "any", Locale.US, FORMAT_PROPERTIES.get(0), LOADER, BUNDLE };
    ...
    private static void testGetCandidateLocales() {
        ...
        candidateData.put(new Locale("ja", "JP", "YOK"), new Locale[] { new Locale("ja", "JP", "YOK"), new Locale("ja", "JP"), new Locale("ja"), Locale.ROOT });
        candidateData.put(new Locale("ja", "JP"), new Locale[] { new Locale("ja", "JP"), new Locale("ja"), Locale.ROOT });
        candidateData.put(new Locale("ja"), new Locale[] { new Locale("ja"), Locale.ROOT });
        candidateData.put(new Locale("ja", "", "YOK"), new Locale[] { new Locale("ja", "", "YOK"), new Locale("ja"), Locale.ROOT });
        candidateData.put(new Locale("", "JP", "YOK"), new Locale[] { new Locale("", "JP", "YOK"), new Locale("", "JP"), Locale.ROOT });
        candidateData.put(new Locale("", "", "YOK"), new Locale[] { new Locale("", "", "YOK"), Locale.ROOT });
        candidateData.put(new Locale("", "JP"), new Locale[] { new Locale("", "JP"), Locale.ROOT });
        candidateData.put(Locale.ROOT, new Locale[] { Locale.ROOT });
        candidateData.put(Locale.forLanguageTag("nb-NO-POSIX"), new Locale[] { Locale.forLanguageTag("nb-NO-POSIX"), Locale.forLanguageTag("no-NO-POSIX"), Locale.forLanguageTag("nb-NO"), Locale.forLanguageTag("no-NO"), Locale.forLanguageTag("nb"), Locale.forLanguageTag("no"), Locale.ROOT });
        candidateData.put(Locale.forLanguageTag("no-NO-POSIX"), new Locale[] { Locale.forLanguageTag("no-NO-POSIX"), Locale.forLanguageTag("nb-NO-POSIX"), Locale.forLanguageTag("no-NO"), Locale.forLanguageTag("nb-NO"), Locale.forLanguageTag("no"), Locale.forLanguageTag("nb"), Locale.ROOT });
        for (Locale locale : candidateData.keySet()) {
            ...
            if (!candidates.equals(expected)) {
                error("Wrong candidates for %s: got %s, expected %s%n", toString(locale), candidates, expected);
            }
        }
        final int NARGS = 2;
        for (int mask = 0; mask < (1 << NARGS) - 1; mask++) {
            ...
            try {
                ...
                error("getCandidateLocales(%s, %s) doesn't throw NPE.%n", data[0], toString((Locale) data[1]));
            } catch (NullPointerException e) {
            }
        }
    }
    ...
    private static void testNewBundle() {
        ...
        for (int mask = 0; mask < (1 << NARGS) - 1; mask++) {
            ...
            try {
                ...
                error("newBundle(%s, %s, %s, %s, false) threw %s.%n", data[0], toString(loc), data[2], data[3], e);
            }
        }
    }
    ...
    private static void testGetTimeToLive() {
        ...
        if (ttl != CONTROL.TTL_NO_EXPIRATION_CONTROL) {
            error("getTimeToLive: got %d, expected %d%n", ttl, CONTROL.TTL_NO_EXPIRATION_CONTROL);
        }
        final int NARGS = 2;
        for (int mask = 0; mask < (1 << NARGS) - 1; mask++) {
            Object[] data = getNpeArgs(NARGS, mask);
    ...
    private static void testNeedsReload() {
        ...
        for (int mask = 0; mask < (1 << NARGS) - 1; mask++) {
            ...
            Locale loc = (Locale) data[1];
            try {
                boolean b = CONTROL.needsReload((String) data[0], loc, (String) data[2], (ClassLoader) data[3], (ResourceBundle) data[4], loadTime);
                error("needsReload(%s, %s, %s, %s, %s, loadTime) doesn't throw NPE.%n", data[0], toString(loc), data[2], data[3], data[4]);
            } catch (NullPointerException e) {
            }
        }
    }
    ...
    private static void testToBundleName() {
        ...
        bundleNames.put(Locale.ROOT, name);
    ...
    private static void testToResourceName() {
        String[][] names = { { "com.sun.J2SE", "xml", "com/sun/J2SE.xml" }, { ".J2SE", "xml", "/J2SE.xml" }, { "J2SE", "xml", "J2SE.xml" }, { "com/sun/J2SE", "xml", "com/sun/J2SE.xml" }, { "com.sun.J2SE", "", "com/sun/J2SE." }, { ".", "", "/." }, { "", "", "." }, { null, "any", null }, { "any", null, null }, { null, null, null } };
