...
public class RightAltKeyTest {
    ...
    void testJOptionPane() {
        int selection = JOptionPane.showConfirmDialog(null, "Do you wish " + "to save file?", "Confirm", JOptionPane.YES_NO_CANCEL_OPTION);
        if (selection == 0) {
            action = true;
        }
    }
    ...
    void errLog(String componentName, String lookAndFeel) throws InvocationTargetException, InterruptedException {
        SwingUtilities.invokeAndWait(this::disposeUI);
        throw new RuntimeException("Actions are not performed for " + componentName + " with " + lookAndFeel + " look and feel.");
    }
