...
public class GetInstanceSecurity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            Configuration c = Configuration.getInstance(JAVA_CONFIG, null, Security.getProvider("SUN"));
            throw new RuntimeException("did not catch security exception");
        } catch (SecurityException se) {
        }
        File file = new File(System.getProperty("test.src", "."), "GetInstanceSecurity.grantedPolicy");
        URI uri = file.toURI();
        URIParameter param = new URIParameter(uri);
        Policy p = Policy.getInstance("JavaPolicy", param, "SUN");
        Policy.setPolicy(p);
        file = new File(System.getProperty("test.src", "."), "GetInstance.config");
        ...
        try {
            Configuration c = Configuration.getInstance(JAVA_CONFIG, uriParam, "SUN");
        } catch (SecurityException se) {
            throw new RuntimeException("unexpected SecurityException");
        }
        try {
            Configuration c = Configuration.getInstance(JAVA_CONFIG, uriParam, Security.getProvider("SUN"));
        } catch (SecurityException se) {
            throw new RuntimeException("unexpected SecurityException");
        }
