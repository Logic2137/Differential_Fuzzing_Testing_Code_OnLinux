...
public class AnnotationTest {
    ...
    private static Descriptor expectedDescriptor = new ImmutableDescriptor(new String[] { "x", "y" }, new Object[] { 3, "foo" });

    private static Descriptor expectedFullDescriptor = new ImmutableDescriptor(new String[] { "class", "enum", "boolean", "stringArray", "classArray", "intArray", "enumArray", "booleanArray" }, new Object[] { Full.class.getName(), RetentionPolicy.RUNTIME.name(), false, new String[] { "foo", "bar" }, new String[] { Full.class.getName() }, new int[] { 1, 2 }, new String[] { RetentionPolicy.RUNTIME.name() }, new boolean[] { false, true } });

    @Pair(x = 3, y = "foo")
    public static interface ThingMBean {

        @Pair(x = 3, y = "foo")
        @Full(classValue = Full.class, enumValue = RetentionPolicy.RUNTIME, booleanValue = false, stringArrayValue = { "foo", "bar" }, classArrayValue = { Full.class }, intArrayValue = { 1, 2 }, enumArrayValue = { RetentionPolicy.RUNTIME }, booleanArrayValue = { false, true })
        int getReadOnly();
        ...
        @Pair(x = 3, y = "foo")
        int operation(@Pair(x = 3, y = "foo") int p1, @Pair(x = 3, y = "foo") int p2);
    }
    ...
    public static class Thing implements ThingMBean {
        ...
        public int operation(int p1, int p2) {
            return 0;
        }
    }
    ...
    private static void check(DescriptorRead[] xx) {
        for (DescriptorRead x : xx) check(x);
    }
}
