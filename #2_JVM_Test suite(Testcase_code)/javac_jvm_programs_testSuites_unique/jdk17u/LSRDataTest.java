...
public class LSRDataTest {
    ...
    private static final String LSR_FILE_PATH = System.getProperty("test.src", ".") + "/../../../../../make/data/lsrdata/language-subtag-registry.txt";

    public static void main(String[] args) throws IOException {
        loadLSRData(Paths.get(LSR_FILE_PATH).toRealPath());
        String ranges = "Accept-Language: aam, adp, aue, bcg, cqu, ema," + " en-gb-oed, gti, koj, kwq, kxe, lii, lmm, mtm, ngv," + " oyb, phr, pub, suj, taj;q=0.9, yug;q=0.5, gfx;q=0.4";
    ...
    private static String generateLangRanges() {
        return Stream.concat(singleLangEquivMap.keySet().stream(), multiLangEquivsMap.keySet().stream()).collect(Collectors.joining(","));
    }

    private static String generateRegionRanges() {
        return regionVariantEquivMap.keySet().stream().map(r -> "en".concat(r)).collect(Collectors.joining(", "));
    }
    ...
    private static void processDataAndGenerateMaps(String type, String tag, String preferred, String prefix) {
        ...
        if (type.equals("region") || type.equals("variant")) {
            if (!regionVariantEquivMap.containsKey(preferred)) {
                ...
                throw new RuntimeException("New case, need implementation." + " A region/variant subtag \"" + preferred + "\" is registered for more than one subtags.");
            }
        } else {
            if (!singleLangEquivMap.containsKey(preferred) && !multiLangEquivsMap.containsKey(preferred)) {
                singleLangEquivMap.put(preferred, tag);
                singleLangEquivMap.put(tag, preferred);
            } else if (singleLangEquivMap.containsKey(preferred) && !multiLangEquivsMap.containsKey(preferred)) {
            ...
            } else if (multiLangEquivsMap.containsKey(preferred) && !singleLangEquivMap.containsKey(preferred)) {
    ...
    private static void addEntriesToMultiEquivsMap(List<String> subtags) {
        subtags.forEach(subtag -> multiLangEquivsMap.put(subtag, subtags.stream().filter(t -> !t.equals(subtag)).collect(Collectors.toList())));
    }
        ...
        for (String range : langRanges) {
            ...
            if (wIndex == -1) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException("weight= " + weight + " for" + " language range \"" + tag + "\", should be" + " represented as a double");
                }
                if (weight < MIN_WEIGHT || weight > MAX_WEIGHT) {
                    throw new IllegalArgumentException("weight=" + weight + " for language range \"" + tag + "\", must be between " + MIN_WEIGHT + " and " + MAX_WEIGHT + ".");
                }
            }
    ...
    private static void checkEquality(List<LanguageRange> expected, List<LanguageRange> actual) {
        ...
        if (expectedSize != actualSize) {
            throw new RuntimeException("[FAILED: Size of the priority list" + " does not match, Expected size=" + expectedSize + "]");
        } else {
            for (int i = 0; i < expectedSize; i++) {
                ...
                if (!lr1.getRange().equals(lr2.getRange()) || lr1.getWeight() != lr2.getWeight()) {
                    throw new RuntimeException("[FAILED: Ranges at index " + i + " do not match Expected: range=" + lr1.getRange() + ", weight=" + lr1.getWeight() + ", Actual: range=" + lr2.getRange() + ", weight=" + lr2.getWeight() + "]");
                }
            }
        }
    }
