...
public class TestSynchronization {
    ...
    private static final char[] CHAR_ARRAY_VAL = { 'c', 'h', 'a', 'r', 'a', 'r', 'r', 'a', 'y' };
    ...
    private static final StringBuilder STRING_BUILDER_VAL = new StringBuilder("StringBuilder value");

    private static final StringBuffer STRING_BUFFER_VAL = new StringBuffer("StringBuffer value");

    private static final CharSequence[] CHAR_SEQUENCE_VAL = { STRING_VAL, STRING_BUILDER_VAL, STRING_BUFFER_VAL };
    ...
    private static void testClass(Class<?> aClass, boolean isSelfTest) throws Exception {
        ...
        for (Method m : methods) {
            ...
            if (Modifier.isPublic(modifiers) && !Modifier.isSynchronized(modifiers)) {
                try {
                    testMethod(aClass, m);
                    ...
                    if (isSelfTest) {
                        ...
                        switch(methodName) {
                            case "should_pass":
                                throw new RuntimeException("Test failed: self-test failed.  The 'should_pass' method did not pass the synchronization test. Check the test code.");
                            case "should_fail":
                                break;
                            default:
                                throw new RuntimeException("Test failed: something is amiss with the test. A TestFailedException was generated on a call to " + methodName + " which we didn't expect to test in the first place.");
                        }
                    } else {
                        throw new RuntimeException("Test failed: the method " + e.getMethod().toString() + " should be synchronized, but isn't.");
                    }
                }
            }
        }
    ...
    private static void invokeMethod(Class<?> aClass, final Method m, final Object[] args) throws TestFailedException, Exception {
    ...
    private static void testMethod(Class<?> aClass, Method m) throws Exception {
        ...
        if (charSequenceArgs.isEmpty()) {
            ...
            if (charSequenceArgs.size() > 1) {
                throw new RuntimeException("Test Failed: the test cannot handle a method with multiple CharSequence arguments.  You must update the test to handle the method " + m.toString());
            }
    ...
    private static boolean isSynchronized(Method m, Object target, Object... args) {
