...
public class WrongPBES2 {
    ...
    private static void test() throws Exception {
        ...
        for (int i = 0; i < p12.length; i++) {
            p12[i] = Integer.valueOf(P12_FILE.substring(2 * i, 2 * i + 2), 16).byteValue();
        }
    ...
    private static void reEncodeWithPBES2() throws Exception {
        ...
        KeyStore.PrivateKeyEntry privateKeyEntry = new KeyStore.PrivateKeyEntry((PrivateKey) keyStore.getKey("a", PASS), new Certificate[] { keyStore.getCertificate("a") });
        keyStore.setEntry("a", privateKeyEntry, new KeyStore.PasswordProtection(PASS, "PBEWithHmacSHA512AndAES_256", new PBEParameterSpec(new byte[20], 100_000)));
