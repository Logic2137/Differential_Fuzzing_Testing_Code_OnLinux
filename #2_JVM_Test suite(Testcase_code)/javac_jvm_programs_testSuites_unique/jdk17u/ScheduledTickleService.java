...
public class ScheduledTickleService {
    ...
    public static void realMain(String... args) throws InterruptedException {
        ScheduledExecutorService tickleService = new ScheduledThreadPoolExecutor(concurrency) {
            ...
            protected <V> RunnableScheduledFuture<V> decorateTask(Runnable runnable, RunnableScheduledFuture<V> task) {
                ...
                return new CustomRunnableScheduledFuture<V>(task) {
                    ...
                    public boolean cancel(boolean b) {
                        return (task().cancel(b) && exec.remove(this));
                    }
                };
            }
        };
        for (int i = 0; i < concurrency; i++) new ScheduledTickle(i, tickleService).setUpdateInterval(25, MILLISECONDS);
        done.await();
    ...
    static class ScheduledTickle implements Runnable {
        ...
        public synchronized void setUpdateInterval(long interval, TimeUnit unit) {
            if (ticket != null) {
                ticket.cancel(false);
                ticket = null;
            }
            if (interval > 0 && !service.isShutdown()) {
                ticket = service.scheduleAtFixedRate(this, interval, interval, unit);
            }
        }
    ...
    static class CustomRunnableScheduledFuture<V> implements RunnableScheduledFuture<V> {
