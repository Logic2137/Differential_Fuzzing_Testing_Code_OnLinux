...
public class UpdateManifest {
    ...
    static void testManifestExistence() throws Throwable {
        ...
        int status = JAR_TOOL.run(out, err, "cfe", jarFileName, "Hello", existence.getPath());
        check(status == 0);
    ...
    static void testManifestContents() throws Throwable {
        ...
        new File(jarFileName).delete();
        int status = JAR_TOOL.run(out, err, "cfm", jarFileName, manifestOrig.getPath(), hello.getPath());
        check(status == 0);
        ...
        pw.close();
        status = JAR_TOOL.run(out, err, "ufm", jarFileName, manifestUpdate.getPath());
        check(status == 0);
    ...
    static void checkManifest(String jarFileName, String mainClass) throws Throwable {
        File f = new File(jarFileName);
        if (!debug)
            f.deleteOnExit();
        ZipFile zf = new ZipFile(f);
