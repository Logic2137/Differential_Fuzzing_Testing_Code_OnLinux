...
public class ProbeKeystores {
    ...
    private static final LoadStoreParameter LOAD_STORE_PARAM = new MyLoadStoreParameter(new PasswordProtection(PASSWORD));

    private static final LoadStoreParameter BAD_LOAD_STORE_PARAM = new MyLoadStoreParameter(new PasswordProtection(BAD_PASSWORD));
    ...
    private static void init(String file, String type, X509Certificate cert) throws Exception {
        KeyStore ks = KeyStore.getInstance(type);
        ks.load(null, null);
    ...
    private static void init(String file, String type, SecretKey key) throws Exception {
        KeyStore ks = KeyStore.getInstance(type);
        ks.load(null, null);
    ...
    private static void probe(String file, String type) throws Exception {
        ...
        if (!type.equalsIgnoreCase(ks.getType())) {
            throw new Exception("ERROR: expected a " + type + " keystore, " + "got a " + ks.getType() + " keystore instead");
        } else {
            System.out.println("Probed a " + type + " keystore named '" + file + "' with " + ks.size() + " entries");
        }
        try {
    ...
    private static void build(String file, String type, boolean usePassword) throws Exception {
        Builder builder;
        if (usePassword) {
            builder = Builder.newInstance(new File(file), new PasswordProtection(PASSWORD));
        } else {
            builder = Builder.newInstance(new File(file), new CallbackHandlerProtection(new DummyHandler()));
        }
    ...
    private static void load(String file, String type, boolean expectFailure) throws Exception {
