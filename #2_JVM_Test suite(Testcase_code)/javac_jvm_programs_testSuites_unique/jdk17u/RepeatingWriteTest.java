...
public class RepeatingWriteTest {
    ...
    private static final int[] TYPES = new int[] { BufferedImage.TYPE_INT_RGB, BufferedImage.TYPE_INT_ARGB, BufferedImage.TYPE_INT_ARGB_PRE, BufferedImage.TYPE_INT_BGR, BufferedImage.TYPE_3BYTE_BGR, BufferedImage.TYPE_4BYTE_ABGR, BufferedImage.TYPE_4BYTE_ABGR_PRE, BufferedImage.TYPE_BYTE_GRAY, BufferedImage.TYPE_USHORT_GRAY, BufferedImage.TYPE_BYTE_BINARY, BufferedImage.TYPE_BYTE_INDEXED, BufferedImage.TYPE_USHORT_565_RGB, BufferedImage.TYPE_USHORT_555_RGB };

    private static final String[] NAMES = new String[] { "TYPE_INT_RGB", "TYPE_INT_ARGB", "TYPE_INT_ARGB_PRE", "TYPE_INT_BGR", "TYPE_3BYTE_BGR", "TYPE_4BYTE_ABGR", "TYPE_4BYTE_ABGR_PRE", "TYPE_BYTE_GRAY", "TYPE_USHORT_GRAY", "TYPE_BYTE_BINARY", "TYPE_BYTE_INDEXED", "TYPE_USHORT_565_RGB", "TYPE_USHORT_555_RGB" };
    ...
    private static class ImageCheckException extends Exception {
        ...
        public ImageCheckException(String msg) {
            super(msg);
        }
    }
    ...
    private void checkImage(String fileName, boolean is1st) throws Exception {
        ...
        if (w != szRef || h != szRef) {
            throw new ImageCheckException(fileName + ": invalid image size " + w + " x " + h + " expected " + szRef + " x " + szRef);
        }
        if (!c.equals(cRef)) {
            throw new ImageCheckException(fileName + ": invalid image color " + c + " expected " + cRef);
        }
    }

    private void doTest(int i, int j) throws Exception {
        String pair = NAMES[i] + " " + NAMES[j];
        if ((format.equals("jpeg") || format.equals("bmp")) && (pair.contains("USHORT_GRAY") || pair.contains("ARGB") || pair.contains("ABGR"))) {
            return;
        }
        if (format.equals("tiff") && (NAMES[i].equals("TYPE_USHORT_555_RGB") || NAMES[i].equals("TYPE_USHORT_565_RGB")) && (NAMES[j].equals("TYPE_USHORT_555_RGB") || NAMES[j].equals("TYPE_USHORT_565_RGB"))) {
            return;
        }
        ...
        if (format.equals("jpeg")) {
            writer.write(null, new IIOImage(img, null, null), new JPEGImageWriteParam(Locale.getDefault()));
        }
        if (format.equals("bmp")) {
            writer.write(null, new IIOImage(img, null, null), new BMPImageWriteParam());
        } else {
            writer.write(img);
        }
        ios.flush();
    ...
    public static void main(String[] args) throws Exception {
            ...
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
                    try {
                        ...
                        System.err.println(f + ": AIOOB for pair " + NAMES[i] + ", " + NAMES[j] + ": " + e.getMessage());
                        nAIOOB++;
                    } catch (ImageCheckException e) {
                        System.err.println(f + ": image check failed for " + e.getMessage());
                        nChecksFailed++;
                    }
                }
            }
        ...
        if (nAIOOB > 0 || nChecksFailed > 0) {
            throw new RuntimeException("test failed: " + nAIOOB + " AIOOBs, " + nChecksFailed + " image check failures");
        }
    }
}
