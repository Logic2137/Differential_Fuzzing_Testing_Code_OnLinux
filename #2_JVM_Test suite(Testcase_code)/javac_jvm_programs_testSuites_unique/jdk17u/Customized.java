...
public class Customized {
    ...
    static class MyFutureTask<V> extends FutureTask<V> {
        ...
        MyFutureTask(Runnable r, V result) {
            super(r, result);
        }
    ...
    static <V> void checkReady(final FutureTask<V> task) {
        ...
        THROWS(TimeoutException.class, () -> task.get(0L, TimeUnit.SECONDS));
    }
    ...
    static <V> void checkCancelled(final FutureTask<V> task) {
        check(task.isDone());
        check(task.isCancelled());
        THROWS(CancellationException.class, () -> task.get(0L, TimeUnit.SECONDS), () -> task.get());
    }

    static <V> void checkThrew(final FutureTask<V> task) {
        check(task.isDone());
        check(!task.isCancelled());
        THROWS(ExecutionException.class, () -> task.get(0L, TimeUnit.SECONDS), () -> task.get());
    }
