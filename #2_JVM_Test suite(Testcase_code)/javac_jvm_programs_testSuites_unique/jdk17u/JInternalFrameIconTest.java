...
public class JInternalFrameIconTest {
    ...
    private static void createImageIconUI(final String lookAndFeelString) throws Exception {
    ...
    private static void createIconUI(final String lookAndFeelString) throws Exception {
        SwingUtilities.invokeAndWait(new Runnable() {
    ...
    private static void getImageIconBufferedImage() throws Exception {
        ...
        Rectangle captureRect = new Rectangle(point.x + internalFrame.getInsets().left, point.y + internalFrame.getInsets().top, titleImageIcon.getIconWidth(), titleImageIcon.getIconHeight());
    ...
    private static void getIconBufferedImage() throws Exception {
        Point point = internalFrame.getLocationOnScreen();
        Rectangle rect = internalFrame.getBounds();
        Rectangle captureRect = new Rectangle(point.x + internalFrame.getInsets().left, point.y + internalFrame.getInsets().top, titleIcon.getIconWidth(), titleIcon.getIconHeight());
        System.out.println("icon captureRect " + captureRect);
        iconImage = robot.createScreenCapture(captureRect);
    }

    private static void testIfSame(final String lookAndFeelString) throws Exception {
        if (!bufferedImagesEqual(imageIconImage, iconImage)) {
            ...
            String error = "[" + lookAndFeelString + "] : ERROR: icon and imageIcon not same.";
            errorString += error;
            System.err.println(error);
        } else {
            System.out.println("[" + lookAndFeelString + "] : SUCCESS: icon and imageIcon same.");
        }
    }

    private static boolean bufferedImagesEqual(BufferedImage bufferedImage1, BufferedImage bufferedImage2) {
        boolean flag = true;
        if (bufferedImage1.getWidth() == bufferedImage2.getWidth() && bufferedImage1.getHeight() == bufferedImage2.getHeight()) {
            ...
            final int mismatchTolerance = (int) (0.1 * bufferedImage1.getWidth() * bufferedImage1.getHeight());
            ...
            for (int x = 0; x < bufferedImage1.getWidth(); x++) {
                for (int y = 0; y < bufferedImage1.getHeight(); y++) {
                    ...
                    if (red1 != red2 || green1 != green2 || blue1 != blue2) {
                        ++mismatchCounter;
                        if ((Math.abs(red1 - red2) > colorTolerance) || (Math.abs(green1 - green2) > colorTolerance) || (Math.abs(blue1 - blue2) > colorTolerance)) {
                            flag = false;
                        }
                    }
                }
    ...
    private static boolean tryLookAndFeel(String lookAndFeelString) throws Exception {
        if (lookAndFeelString.contains("motif") || lookAndFeelString.contains("gtk")) {
            return false;
        }
        try {
