...
public final class TestKit {
    ...
    public static <V> V assertNotThrows(ThrowingFunction<V> code) {
        ...
        try {
            ...
            throw new RuntimeException("Expected to run normally, but threw " + t.getClass().getCanonicalName(), t);
        }
    }

    public static <T extends Throwable> T assertThrows(Class<T> clazz, ThrowingProcedure code) {
        ...
        try {
            ...
            throw new RuntimeException("Expected to catch an exception of type " + clazz.getCanonicalName() + ", but caught " + t.getClass().getCanonicalName(), t);
        }
        throw new RuntimeException("Expected to catch an exception of type " + clazz.getCanonicalName() + ", but caught nothing");
    }
    ...
    public static <T extends Throwable> T assertThrows(Class<T> clazz, String messageRegex, ThrowingProcedure code) {
        requireNonNull(messageRegex, "messagePattern");
        T t = assertThrows(clazz, code);
        String m = t.getMessage();
        if (m == null) {
            throw new RuntimeException(String.format("Expected exception message to match the regex '%s', " + "but the message was null", messageRegex), t);
        }
        if (!Pattern.matches(messageRegex, m)) {
            throw new RuntimeException(String.format("Expected exception message to match the regex '%s', " + "actual message: %s", messageRegex, m), t);
        }
        return t;
    }
    ...
    public static <E> void assertUnmodifiableCollection(Collection<E> collection, Supplier<? extends E> elementsFactory) {
    ...
    public static <E> void assertUnmodifiableSet(Set<E> set, Supplier<? extends E> elementsFactory) {
    ...
    public static <E> void assertUnmodifiableList(List<E> list, Supplier<? extends E> elementsFactory) {
        assertUnmodifiableList(list, elementsFactory, 3);
    }

    private static <E> void assertUnmodifiableList(List<E> list, Supplier<? extends E> elementsFactory, int depth) {
    ...
    public static <K, V> void assertUnmodifiableMap(Map<K, V> map, Supplier<? extends Map.Entry<? extends K, ? extends V>> entriesFactory) {
