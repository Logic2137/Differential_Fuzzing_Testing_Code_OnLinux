...
public class QueryExpStringTest {
    ...
    private static final ValueExp attr = Query.attr("attr"), qattr = Query.attr("className", "attr"), aa = Query.attr("A"), bb = Query.attr("B"), cc = Query.attr("C"), dd = Query.attr("D"), zero = Query.value(0), classattr = Query.classattr(), simpleString = Query.value("simpleString"), complexString = Query.value("a'b\\'\""), intValue = Query.value(12345678), integerValue = Query.value(new Integer(12345678)), longValue = Query.value(12345678L), floatValue = Query.value(2.5f), doubleValue = Query.value(2.5d), booleanValue = Query.value(true), plusValue = Query.plus(intValue, integerValue), timesValue = Query.times(doubleValue, floatValue), minusValue = Query.minus(floatValue, doubleValue), divValue = Query.div(doubleValue, floatValue);

    private static final QueryExp gt = Query.gt(intValue, floatValue), geq = Query.geq(intValue, floatValue), leq = Query.leq(intValue, floatValue), lt = Query.lt(intValue, floatValue), eq = Query.eq(intValue, floatValue), between = Query.between(intValue, floatValue, doubleValue), match = Query.match((AttributeValueExp) attr, (StringValueExp) simpleString), initial = Query.initialSubString((AttributeValueExp) attr, (StringValueExp) simpleString), initialStar = Query.initialSubString((AttributeValueExp) attr, Query.value("*")), initialPercent = Query.initialSubString((AttributeValueExp) attr, Query.value("%")), any = Query.anySubString((AttributeValueExp) attr, (StringValueExp) simpleString), anyStar = Query.anySubString((AttributeValueExp) attr, Query.value("*")), anyPercent = Query.anySubString((AttributeValueExp) attr, Query.value("%")), ffinal = Query.finalSubString((AttributeValueExp) attr, (StringValueExp) simpleString), finalMagic = Query.finalSubString((AttributeValueExp) attr, Query.value("?*[\\")), in = Query.in(intValue, new ValueExp[] { intValue, floatValue }), and = Query.and(gt, lt), or = Query.or(gt, lt), not = Query.not(gt), aPlusB_PlusC = Query.gt(Query.plus(Query.plus(aa, bb), cc), zero), aPlus_BPlusC = Query.gt(Query.plus(aa, Query.plus(bb, cc)), zero);

    private static final Object[] tests = { attr, "attr", classattr, "Class", simpleString, "'simpleString'", complexString, "'a''b\\\''\"'", intValue, "12345678", integerValue, "12345678", longValue, "12345678", floatValue, "2.5", doubleValue, "2.5", booleanValue, "true", plusValue, "12345678 + 12345678", timesValue, "2.5 * 2.5", minusValue, "2.5 - 2.5", divValue, "2.5 / 2.5", gt, "(12345678) > (2.5)", geq, "(12345678) >= (2.5)", leq, "(12345678) <= (2.5)", lt, "(12345678) < (2.5)", eq, "(12345678) = (2.5)", between, "(12345678) between (2.5) and (2.5)", match, "attr like 'simpleString'", initial, "attr like 'simpleString*'", initialStar, "attr like '\\**'", initialPercent, "attr like '%*'", any, "attr like '*simpleString*'", anyStar, "attr like '*\\**'", anyPercent, "attr like '*%*'", ffinal, "attr like '*simpleString'", finalMagic, "attr like '*\\?\\*\\[\\\\'", in, "12345678 in (12345678, 2.5)", and, "((12345678) > (2.5)) and ((12345678) < (2.5))", or, "((12345678) > (2.5)) or ((12345678) < (2.5))", not, "not ((12345678) > (2.5))", aPlusB_PlusC, "(A + B + C) > (0)" };
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < tests.length; i += 2) {
            ...
            if (expected.equals(testString))
                ...
                System.err.println("Expected: {" + expected + "}; got: {" + testString + "}");
                ok = false;
            }
            try {
                Object parsed;
                ...
                if (parsedString.equals(expected))
                    ...
                    System.err.println("Parse differs: expected: {" + expected + "}; got: {" + parsedString + "}");
                    ok = false;
                }
            } catch (Exception e) {
                System.err.println("Parse got exception: {" + expected + "}: " + e);
                ok = false;
            }
        }
        if (ok)
    ...
    private static QueryExp parseQuery(String[] ss) throws Exception {
        ...
        if (skip(ss, " like ")) {
            ...
            if (!(exp instanceof AttributeValueExp && pat instanceof StringValueExp)) {
                throw new Exception("Expected types `attr like string': " + exp + " like " + pat);
            }
    ...
    private static QueryExp parseQueryAfterParen(String[] ss) throws Exception {
        String start = ss[0];
        try {
        ...
        while (true) {
            ...
            switch(op.charAt(0)) {
                ...
                case '/':
                    ...
                    break;
                default:
                    throw new Exception("Can't happen: " + op.charAt(0));
            }
        }
    }
    ...
    private static ValueExp parsePrimary(String[] ss) throws Exception {
        ...
        if (lastDot < 0)
            ...
            return Query.attr(word.substring(0, lastDot), word.substring(lastDot + 1));
    }
    ...
    private static boolean matchWord(String[] ss, String word) throws Exception {
        String s = ss[0];
        if (s.startsWith(word)) {
            int len = word.length();
            if (s.length() == len || s.charAt(len) == ' ' || s.charAt(len) == ')') {
