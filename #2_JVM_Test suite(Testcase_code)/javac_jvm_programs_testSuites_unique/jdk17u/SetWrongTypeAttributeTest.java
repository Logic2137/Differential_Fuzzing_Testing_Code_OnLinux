...
public class SetWrongTypeAttributeTest {
    ...
    private static final Object[][] TEST_VALUES = { { "Foo", null, 5, "false", Collections.<String, String>emptyMap() }, { "Name", 5, false, Collections.<String, String>emptyMap() }, { "Properties", 5, false, Collections.singleton("foo") } };
    ...
    public static void main(String[] args) throws Exception {
        ...
        test("StandardMBean wrapping Blah", new StandardMBean(new Blah(), BlahMBean.class));
        test("MXBean Blah", new MXBlah());
        test("StandardMBean implementing MXBean Blah", new StdMXBlah());
        test("StandardMBean wrapping MXBean Blah", new StandardMBean(new MXBlah(), BlahMXBean.class, true));
        if (failure == null)
    ...
    private static void test(String what, Object obj) throws Exception {
        ...
        for (Object[] testValue : TEST_VALUES) {
            ...
            for (int i = 1; i < testValue.length; i++) {
                ...
                final String doing = "setAttribute(" + attrName + ", " + value + ")";
                try {
                    mbs.setAttribute(on, new Attribute("Foo", 5));
                    fail(what, doing + " succeeded but should fail!");
                } catch (InvalidAttributeValueException e) {
                    final String msg = doing + ": OK, got expected " + "InvalidAttributeValueException";
