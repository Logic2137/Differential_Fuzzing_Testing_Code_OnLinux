...
public class RepositoryWildcardTest {
    ...
    private static final String classname = "javax.management.monitor.StringMonitor";

    private static int mbeanCreation(MBeanServer mbs, String name) throws Exception {
        int error = 0;
        try {
            ...
            System.out.println("Got expected exception = " + e.getCause().toString());
            System.out.println("Test passed!");
        }
        return error;
    }

    private static int mbeanDeletion(MBeanServer mbs, String name) throws Exception {
        int error = 0;
        try {
    ...
    private static int mbeanQuery(MBeanServer mbs, String name, Set<ObjectName> expectedSet) throws Exception {
        int error = 0;
        System.out.println("Test: queryNames(" + name + ")");
        Set<ObjectName> returnedSet = mbs.queryNames(ObjectName.getInstance(name), null);
    ...
    public static void main(String[] args) throws Exception {
        ...
        ObjectName[] namesArray = { ObjectName.getInstance("d:k=abc"), ObjectName.getInstance("d:k=abcd"), ObjectName.getInstance("d:k=abcde"), ObjectName.getInstance("d:k=abc,k2=v2"), ObjectName.getInstance("d:k=abcd,k2=v2"), ObjectName.getInstance("d:k=abcde,k2=v2"), ObjectName.getInstance("d:k=\"abc\""), ObjectName.getInstance("d:k=\"abc\",k2=v2"), ObjectName.getInstance("d:p1=v1,p2=v2,p3=v3,p4=v4,p5=v5,p6=v6," + "p7=v7,p8=v8,p9=v9,p10=v10,p11=v11,p12=v12") };
        for (ObjectName name : namesArray) mbs.createMBean(classname, name);
        System.out.println("----------------------------------------------");
        ...
        for (ObjectName name : namesArray) expectedSet.add(name);
        expectedSet.add(MBeanServerDelegate.DELEGATE_NAME);
        Set<ObjectName> returnedSet = mbs.queryNames(ObjectName.getInstance("*:*"), null);
        ...
        error += mbeanQuery(mbs, "d:p1=v?,p2=v?,p3=v?,p4=v?,p5=v?,p6=v?," + "p7=v?,p8=v?,p9=v?,p10=v??,p11=v??,p12=v??", expectedSet);
        System.out.println("----------------------------------------------");
