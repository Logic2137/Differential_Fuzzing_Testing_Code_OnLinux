...
public class RacingCollections {
    ...
    private static void frob(Object theRock, Object elLoco) {
        ...
        try {
            ...
            for (Looper looper = new Looper(); looper.keepGoing(); ) checkEqualSanity(theRock, elLoco);
        } catch (Throwable t) {
            unexpected(t);
        } finally {
    ...
    private static void realMain(String[] args) {
        for (Map<Integer, Boolean> x : maps()) for (Map<Integer, Boolean> y : newConcurrentMaps()) {
        ...
        for (Set<Integer> x : newSets()) for (Set<Integer> y : newConcurrentSets()) {
        ...
        for (List<Integer> x : newLists()) for (List<Integer> y : newConcurrentLists()) {
        ...
        for (Queue<Integer> x : newQueues()) for (Queue<Integer> y : newConcurrentQueues()) {
            describe(Queue.class, x, y);
            frob(x, y);
        }
        for (Deque<Integer> x : newDeques()) for (Deque<Integer> y : newConcurrentDeques()) {
    ...
    static String toString(Object x) {
        return ((x instanceof Collection) || (x instanceof Map)) ? x.getClass().getName() : x.toString();
    }
    ...
    static void equal(Object x, Object y) {
        if (x == null ? y == null : x.equals(y))
            ...
            fail(toString(x) + " not equal to " + toString(y));
    }
