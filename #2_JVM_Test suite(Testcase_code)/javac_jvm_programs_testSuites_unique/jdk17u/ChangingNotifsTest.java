...
public class ChangingNotifsTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (boolean mx : new boolean[] { false, true }) test(mbs, name, mx);
        if (failure != null)
            throw new Exception(failure);
        System.out.println("TEST PASSED");
    }

    private static void test(MBeanServer mbs, ObjectName name, boolean mx) throws Exception {
        ...
        try {
            ...
            if (immutable != mx) {
                fail(what + " has immutableInfo=" + immut + ", should be " + mx);
                return;
            }
            ...
            if (unchanged != mx) {
                fail(what + " notif info " + (unchanged ? "did not change" : "changed"));
                return;
            }
