...
public class GradientTransformTest extends Frame {
    ...
    public GradientTransformTest() {
        ...
        dstImg = getGraphicsConfiguration().createCompatibleVolatileImage(20, 20);
    }
    ...
    public void test() {
        renderToVI(createSrcImage(), dstImg);
        BufferedImage validationImg = new BufferedImage(20, 20, BufferedImage.TYPE_INT_RGB);
        ...
        for (int x = 0; x < validationImg.getWidth() && !gradientTranslated; x++) {
            for (int y = 0; y < validationImg.getHeight() && !gradientTranslated; y++) {
