public class Supplementary {
    ...
    static void testConstants() {
        ...
        if (Character.MIN_SUPPLEMENTARY_CODE_POINT != MIN_SUPPLEMENTARY) {
            constantError("MIN_SUPPLEMENTARY_CODE_POINT", Character.MIN_SUPPLEMENTARY_CODE_POINT, MIN_SUPPLEMENTARY);
        }
    ...
    static int test01(char[] cu) {
        ...
        for (int i = 0; i <= Character.MAX_VALUE; i++) {
            ...
            if (u.length != 1 || u[0] != i) {
                throw new RuntimeException("wrong toChars(int) result for BMP: " + toHexString("u", u));
            }
            int n = Character.toChars(i, cu, index);
            if (n != 1 || cu[index] != i) {
                throw new RuntimeException("wrong toChars(int, char[], int) result for BMP:" + " len=" + n + ", cu[" + index + "]=" + toHexString(cu[index]));
            }
            boolean isHigh = i >= MIN_HIGH && i <= MAX_HIGH;
            if (Character.isHighSurrogate((char) i) != isHigh) {
                throw new RuntimeException("wrong high-surrogate test for " + toHexString(i));
            }
            boolean isLow = i >= MIN_LOW && i <= MAX_LOW;
            if (Character.isLowSurrogate((char) i) != isLow) {
                throw new RuntimeException("wrong low-surrogate test for " + toHexString(i));
            }
            if (!isHigh && !isLow) {
                index++;
            }
        ...
        for (int i = Character.MAX_VALUE / 2; i <= Character.MAX_VALUE; i++) {
            ...
            for (int j = Character.MAX_VALUE / 2; j <= Character.MAX_VALUE; j++) {
                ...
                if (Character.isSurrogatePair(hi, lo) != isSurrogatePair) {
                    throw new RuntimeException("wrong surrogate pair test for hi=" + toHexString(hi) + ", lo=" + toHexString(lo));
                }
                if (isSurrogatePair) {
                    int cp = Character.toCodePoint(hi, lo);
                    if (cp != supplementary) {
                        throw new RuntimeException("wrong code point: got " + toHexString(cp) + ", expected=" + toHexString(supplementary));
                    }
                    char[] u = Character.toChars(cp);
                    if (u.length != 2 || u[0] != hi || u[1] != lo) {
                        throw new RuntimeException("wrong toChars(int) result for supplementary: " + toHexString("u", u));
                    }
                    int n = Character.toChars(cp, cu, index);
                    if (n != 2 || cu[index] != hi || cu[index + 1] != lo) {
                        throw new RuntimeException("wrong toChars(int, char[], int) result " + "for supplementary: len=" + n + ", cu[" + index + "]=" + toHexString(cu[index]) + ", cu[" + (index + 1) + "]=" + toHexString(cu[index + 1]));
                    }
                    index += n;
                    supplementary++;
                }
        ...
        if (supplementary != MAX_SUPPLEMENTARY + 1) {
            throw new RuntimeException("wrong supplementary count (supplementary=" + toHexString(supplementary) + ")");
        }
        int nCodeUnits = Character.MAX_VALUE + 1 - (MAX_LOW - MIN_HIGH + 1) + ((MAX_SUPPLEMENTARY - MIN_SUPPLEMENTARY + 1) * 2);
        if (index != nCodeUnits) {
            throw new RuntimeException("wrong number of code units: " + index + ", expected " + nCodeUnits);
        }
        return index;
    }
    ...
    static void test06() {
        for (int cp = Character.MIN_CODE_POINT; cp <= Character.MAX_CODE_POINT; cp++) {
            ...
            if (!result.equals(expected)) {
                throw new RuntimeException("Wrong string is created. code point: " + cp + ", result: " + result + ", expected: " + expected);
            }
        }
    }
    ...
    static void testUnpaired() {
        ...
        testCodePoint("\uD800\uD800\uD800\uD800\uDC00\uDC00\uDC00\uDC00", new int[] { 0xD800, 0xD800, 0xD800, 0x10000, 0xDC00, 0xDC00, 0xDC00 });
    }
    ...
    static void testCodePoint(String str, int[] codepoints) {
        ...
        for (int i = 0; i < str.length(); i += Character.charCount(c)) {
            ...
            if (c != codepoints[j++]) {
                throw new RuntimeException("codePointAt(CharSequence, " + i + ") returned " + toHexString(c) + ", expected " + toHexString(codepoints[j - 1]));
            }
        }
        if (j != codepoints.length) {
            throw new RuntimeException("j != codepoints.length after codePointAt(CharSequence, int)" + " (j=" + j + ")" + ", expected: " + codepoints.length);
        }
        j = codepoints.length;
        for (int i = str.length(); i > 0; i -= Character.charCount(c)) {
            c = Character.codePointBefore(str, i);
            if (c != codepoints[--j]) {
                throw new RuntimeException("codePointBefore(CharSequence, " + i + ") returned " + toHexString(c) + ", expected " + toHexString(codepoints[j]));
            }
        }
        if (j != 0) {
            throw new RuntimeException("j != 0 after codePointBefore(CharSequence, int)" + " (j=" + j + ")");
        }
        char[] a = str.toCharArray();
        j = 0;
        for (int i = 0; i < a.length; i += Character.charCount(c)) {
            ...
            if (c != codepoints[j++]) {
                throw new RuntimeException("codePointAt(char[], " + i + ") returned " + toHexString(c) + ", expected " + toHexString(codepoints[j - 1]));
            }
        }
        if (j != codepoints.length) {
            throw new RuntimeException("j != codepoints.length after codePointAt(char[], int)" + " (j=" + j + ")" + ", expected: " + codepoints.length);
        }
        j = codepoints.length;
        for (int i = a.length; i > 0; i -= Character.charCount(c)) {
            c = Character.codePointBefore(a, i);
            if (c != codepoints[--j]) {
                throw new RuntimeException("codePointBefore(char[], " + i + ") returned " + toHexString(c) + ", expected " + toHexString(codepoints[j]));
            }
        }
        if (j != 0) {
            throw new RuntimeException("j != 0 after codePointBefore(char[], int)" + " (j=" + j + ")");
        }
        j = 0;
        for (int i = 0; i < codepoints.length; i++) {
            ...
            for (int k = 0; k < a.length; k++) {
                if (str.charAt(j++) != a[k]) {
                    throw new RuntimeException("toChars(int) returned " + toHexString("result", a) + " from codepoint=" + toHexString(codepoints[i]));
                }
            }
        }
        ...
        if (!str.equals(s)) {
            throw new RuntimeException("toChars(int, char[], int) returned " + toHexString("dst", s.toCharArray()) + ", expected " + toHexString("data", str.toCharArray()));
        }
    }
    ...
    static void testExceptions01(CharSequence cs) {
        ...
        callOffsetByCodePoints(cs, cs.length(), -cs.length() * 2, IndexOutOfBoundsException.class);
        callOffsetByCodePoints(cs, cs.length(), Integer.MIN_VALUE, IndexOutOfBoundsException.class);
    ...
    static void testExceptions02(char[] a) {
        ...
        callOffsetByCodePoints(a, 0, a.length, a.length, -a.length * 2, IndexOutOfBoundsException.class);
        callOffsetByCodePoints(a, 0, a.length, a.length, Integer.MIN_VALUE, IndexOutOfBoundsException.class);
        callOffsetByCodePoints(a, 0, a.length, 0, Integer.MAX_VALUE, IndexOutOfBoundsException.class);
    }
    ...
    private static void callToChars3(int codePoint, char[] dst, int index, Class expectedException) {
        try {
    ...
    private static void callCodePoint(boolean isAt, CharSequence cs, int index, Class expectedException) {
        try {
    ...
    private static void callCodePoint(boolean isAt, char[] a, int index, Class expectedException) {
        try {
    ...
    private static void callCodePoint(boolean isAt, char[] a, int index, int limit, Class expectedException) {
        try {
    ...
    private static void callCodePointCount(Object data, int beginIndex, int endIndex, Class expectedException) {
        String type = getType(data);
        try {
    ...
    private static void callOffsetByCodePoints(CharSequence seq, int index, int offset, Class expectedException) {
        try {
    ...
    private static void callOffsetByCodePoints(char[] a, int start, int count, int index, int offset, Class expectedException) {
        try {
    ...
    private static void callToString(int codePoint, Class expectedException) {
        ...
        throw new RuntimeException("toString(int) didn't throw " + expectedException.getName());
    }
