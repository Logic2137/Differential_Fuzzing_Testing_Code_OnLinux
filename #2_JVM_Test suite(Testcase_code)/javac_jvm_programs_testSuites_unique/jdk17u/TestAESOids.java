...
public class TestAESOids {
    ...
    private static void runTest(DataTuple dataTuple, boolean supportedKeyLength) throws NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, InvalidKeyException, ShortBufferException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException {
        ...
        if (!algorithmCipher.getAlgorithm().equals(dataTuple.algorithm)) {
            throw new RuntimeException(String.format("Test failed: algorithm string %s getInstance " + "doesn't generate expected algorithm.%n", dataTuple.algorithm));
        }
        ...
        int offset = algorithmCipher.update(INPUT, 0, INPUT.length, cipherText, 0);
        ...
        if (!Arrays.equals(INPUT, recoveredText)) {
            throw new RuntimeException("Decrypted data is not the same as the original text");
        }
    }
    ...
    private static class DataTuple {
        ...
        private DataTuple(String oid, String algorithm, int keyLength, String mode) {
            this.oid = oid;
