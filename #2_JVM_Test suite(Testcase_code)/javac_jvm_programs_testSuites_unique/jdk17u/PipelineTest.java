...
public class PipelineTest {
    ...
    static void t1_simplePipeline() {
        try {
            ...
            verify(s1, s1, asList(new ProcessBuilder("cat"), new ProcessBuilder("cat"), new ProcessBuilder("cat")));
        } catch (Throwable t) {
            unexpected(t);
        }
    }
    ...
    static void t2_translatePipeline() {
        try {
            ...
            verify(s2, r2, asList(new ProcessBuilder("tr", "e", "E"), new ProcessBuilder("tr", "o", "O")));
        } catch (Throwable t) {
            unexpected(t);
        }
    }
    ...
    static void t3_redirectErrorStream() {
        try {
            ...
            List<Process> processes = ProcessBuilder.startPipeline(asList(new ProcessBuilder("cat", "NON-EXISTENT-FILE").redirectErrorStream(true).redirectError(p1err), new ProcessBuilder("cat").redirectOutput(p2out)));
    ...
    static void t4_failStartPipeline() {
        ...
        THROWS(IllegalArgumentException.class, () -> {
            List<Process> processes = ProcessBuilder.startPipeline(asList(new ProcessBuilder("cat", "NON-EXISTENT-FILE1").redirectOutput(p1err), new ProcessBuilder("cat")));
        }, () -> {
            List<Process> processes = ProcessBuilder.startPipeline(asList(new ProcessBuilder("cat", "NON-EXISTENT-FILE2"), new ProcessBuilder("cat").redirectInput(p2out)));
        });
        THROWS(NullPointerException.class, () -> {
            List<Process> processes = ProcessBuilder.startPipeline(asList(new ProcessBuilder("cat", "a"), null));
        }, () -> {
            List<Process> processes = ProcessBuilder.startPipeline(asList(null, new ProcessBuilder("cat", "b")));
        });
        THROWS(IOException.class, () -> {
            List<Process> processes = ProcessBuilder.startPipeline(asList(new ProcessBuilder("cat", "c"), new ProcessBuilder("NON-EXISTENT-COMMAND")));
        });
        ProcessHandle.current().children().forEach(PipelineTest::print);
        ProcessHandle.current().children().filter(p -> p.info().command().orElse("").contains("cat")).forEach(p -> fail("process should have been destroyed: " + p));
    }
