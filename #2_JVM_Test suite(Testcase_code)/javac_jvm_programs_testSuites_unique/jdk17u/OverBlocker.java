...
public class OverBlocker {
    ...
    private static void init() {
        String[] instructions = { " the test will be run 4 times, to start next test just close all ", " windows of previous; the instructions are the same for all tests: ", " 1) there is a frame with 'show modal' button, ", " 2) press the button to show a dialog, ", " 3) activate any non-Java application, move the app over the dialog, ", " 4) click on the frame by mouse, ", " 5) make sure that the dialog comes up from the application and ", "    now the dialog overlaps the app as well as the frame, ", "    if it's true, then the test passed, otherwise, it failed. ", " Press 'pass' button only after all of the 4 tests are completed, ", " the number of the currently executed test is displayed on the ", " output window. " };
        Sysout.createDialog();
