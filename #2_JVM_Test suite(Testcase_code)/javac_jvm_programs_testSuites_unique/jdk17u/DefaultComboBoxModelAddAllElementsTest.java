...
public class DefaultComboBoxModelAddAllElementsTest {
    ...
    private static final Vector<Integer> vector = IntStream.range(START, END).collect(Vector::new, Vector::add, Vector::addAll);

    private static final TreeSet<Integer> set = IntStream.range(START, END).collect(TreeSet::new, TreeSet::add, TreeSet::addAll);

    private static final ArrayList<Integer> arrayList = IntStream.range(START, END).collect(ArrayList::new, ArrayList::add, ArrayList::addAll);
    ...
    private static class MyListDataListener implements ListDataListener {
        ...
        public void intervalAdded(ListDataEvent e) {
            if (e.getIndex1() - e.getIndex0() != END - START - 1) {
                throw new RuntimeException("Test case failed. Expected " + (END - START) + " elements to be added, but only got " + (e.getIndex1() - e.getIndex0()));
            }
        }
    ...
    private static void checkAddAllWithIndex() {
        ...
        try {
            ...
            System.out.println("Encountered exception as expected, when trying to add elements" + "before the start of the list.");
        }
        try {
            cm.addAll(15, vector);
        ...
        try {
            ...
            System.out.println("Encountered exception as expected, when trying to add elements" + "after the end of the list.");
        }
    }
}
