...
public class isvalid001 {
    ...
    public static int run(String[] argv, PrintStream out) {
                ...
                for (; ; ) {
                    try {
                        ...
                        try {
                            ...
                            try {
                                booleanArg = (Connector.BooleanArgument) defaultArguments.get(argName);
                                break;
        ...
        if (!argument.isValid("true")) {
            exitCode = exitCode2;
            out.println(sErr2 + "check: isValid('true')\n" + "error: returned value != true\n");
        }
        if (!argument.isValid("false")) {
            exitCode = exitCode2;
            out.println(sErr2 + "check: isValid('false')\n" + "error: returned value != true\n");
        }
        if (argument.isValid("fals")) {
            exitCode = exitCode2;
            out.println(sErr2 + "check: isValid('fals')\n" + "error: returned value == true\n");
        }
        if (argument.isValid("")) {
            exitCode = exitCode2;
            out.println(sErr2 + "check: isValid(<empty_string>)\n" + "error: returned value == true\n");
        }
