...
public class LdapMessage {
    ...
    public enum Operation {
        ...
        private static Operation fromId(int id) {
            Optional<Operation> optional = Stream.of(Operation.values()).filter(o -> o.id == id).findFirst();
    ...
    private void parse() {
        ...
        if (message[0] != 0x30) {
            throw new RuntimeException("Bad LDAP encoding in message, " + "expected ASN.1 SEQUENCE tag (0x30), encountered " + message[0]);
        }
        int index = 2;
        ...
        if (message[index] != 0x02) {
            throw new RuntimeException("Bad LDAP encoding in message, " + "expected ASN.1 INTEGER tag (0x02), encountered " + message[index]);
        }
    ...
    public static int getMessageLength(byte[] encoding) {
        ...
        if (encoding[0] != 0x30) {
            throw new RuntimeException("Error: bad LDAP encoding message: " + "expected ASN.1 SEQUENCE tag (0x30), encountered " + encoding[0]);
        }
        int len;
        int index = 1;
        int payloadLen = 0;
        ...
        if (payloadLen <= 0) {
            throw new RuntimeException("Error: invalid LDAP encoding message length or payload too large");
        }
