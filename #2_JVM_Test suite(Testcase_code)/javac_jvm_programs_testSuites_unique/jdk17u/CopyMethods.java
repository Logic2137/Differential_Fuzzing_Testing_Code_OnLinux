...
public class CopyMethods {
    ...
    static void simpleTests() {
        ...
        for (int j = 0; j <= ba.length + 2; j++) {
            ...
            if (muzzle == 0)
                System.out.println("copyOf(ba," + j + ") = " + Arrays.toString(bb));
        ...
        for (int i = 0; i <= a.length; i++) {
            for (int j = i; j <= a.length + 2; j++) {
                ...
                if (muzzle == 0)
                    System.out.println("copyOfRange(ba," + i + "," + j + ") = " + Arrays.toString(br));
    ...
    static void checkArray(Object a, Class<?> c, int requiredLen, int offset, int firstNull) {
        ...
        if (misses != 0) {
            ...
            if (toString == null)
                toString = toStrings.get(Object.class);
            throw new RuntimeException("checkArray failed at " + firstMiss + " " + c + "[]" + " : " + invoke(toString, a));
        }
    }
    ...
    static Object coerceToObject(int x) {
        return (x & 0xF) == 0 ? null : new Integer(x);
    }

    static String coerceToString(int x) {
        return (x == 0) ? null : Integer.toHexString(x);
    }

    static Integer coerceToInteger(int x) {
        return (x == 0) ? null : x;
    }

    static byte coerceToByte(int x) {
        return (byte) x;
    }

    static short coerceToShort(int x) {
        return (short) x;
    }

    static int coerceToInt(int x) {
        return x;
    }

    static long coerceToLong(int x) {
        return x;
    }

    static char coerceToChar(int x) {
        return (char) x;
    }

    static float coerceToFloat(int x) {
        return x;
    }

    static double coerceToDouble(int x) {
        return x;
    }

    static boolean coerceToBoolean(int x) {
        return (x & 1) != 0;
    }
    ...
    static final List<Class<?>> allTypes = Arrays.asList(new Class<?>[] { Object.class, String.class, Integer.class, byte.class, short.class, int.class, long.class, char.class, float.class, double.class, boolean.class });
    ...
    static {
        ...
        for (int i = 0; i < arrayMethods.length; i++) {
            ...
            if (!Modifier.isStatic(m.getModifiers()))
                continue;
            Class<?> rt = m.getReturnType();
            if (m.getName().equals("copyOf") && m.getParameterTypes().length == 2)
                cloners.put(rt.getComponentType(), m);
            if (m.getName().equals("copyOfRange") && m.getParameterTypes().length == 3)
