...
public class QuadRotInverseBug {
    ...
    public static boolean test(AffineTransform xform) {
        ...
        try {
            ...
            xform.inverseTransform(transformedPoint, 0, inverseFromOriginalXForm, 0, 1);
            xform.inverseTransform(transformedPoint2D, inverseFromOriginalPoint2D);
        } catch (NoninvertibleTransformException e) {
            throw new InternalError("transform wasn't invertible!");
        }
        System.out.println("Both points should be identical:");
        System.out.println("Original Point: " + originalPoint[0] + " " + originalPoint[1]);
        System.out.println("inverseTransform method used: " + inverseFromOriginalXForm[0] + " " + inverseFromOriginalXForm[1]);
        System.out.println("Original Point2D: " + originalPoint2D);
        System.out.println("inverseTransform method used: " + inverseFromOriginalPoint2D);
        return (originalPoint[0] != inverseFromOriginalXForm[0] || originalPoint[1] != inverseFromOriginalXForm[1] || !originalPoint2D.equals(inverseFromOriginalPoint2D));
    }
}
