...
public class ReadWithoutBaselineTagSet {
    ...
    private static final Map<Integer, String[]> typeToCompression = Map.of(BufferedImage.TYPE_3BYTE_BGR, new String[] { null, "LZW", "JPEG", "ZLib", "PackBits" }, BufferedImage.TYPE_BYTE_BINARY, new String[] { null, "CCITT RLE", "CCITT T.4", "CCITT T.6", "LZW", "PackBits" }, BufferedImage.TYPE_BYTE_GRAY, new String[] { null, "LZW", "JPEG", "ZLib", "PackBits" }, BufferedImage.TYPE_USHORT_GRAY, new String[] { null, "LZW", "ZLib", "PackBits" });
    ...
    private static void test() throws IOException {
        ...
        for (int imageType : typeToCompression.keySet()) {
            ...
            for (String compression : typeToCompression.get(imageType)) {
                System.out.println("Compression: " + (compression == null ? "None" : compression));
                ...
                if (resultFalse.getWidth() != WIDTH || resultFalse.getHeight() != HEIGHT) {
                    System.err.printf("Read image dimensions != %d x %d%n", WIDTH, HEIGHT);
                    failures++;
                } else {
                ...
                try {
                    ...
                    if (resultTrue.getWidth() != WIDTH || resultTrue.getHeight() != HEIGHT) {
                        System.err.printf("Read image dimensions != %d x %d%n", WIDTH, HEIGHT);
                        failures++;
                    } else {
