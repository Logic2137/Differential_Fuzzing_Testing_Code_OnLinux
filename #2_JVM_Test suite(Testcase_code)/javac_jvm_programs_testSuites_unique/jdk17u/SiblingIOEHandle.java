...
public class SiblingIOEHandle {
    ...
    private static String[] getCommandArray(String processName) {
        String[] cmdArray = { JAVA_EXE, "-cp", System.getProperty("java.class.path"), SiblingIOEHandle.class.getName(), processName };
        return cmdArray;
    }
        ...
        public void run() {
            try {
                if (waitBarrier(barrier)) {
                    waitAbit();
                    ProcessBuilder builderC = new ProcessBuilder(getCommandArray(APP.C.name()));
