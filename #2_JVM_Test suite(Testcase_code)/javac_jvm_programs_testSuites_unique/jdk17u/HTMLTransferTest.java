...
public class HTMLTransferTest {
    ...
    static {
        try {
            HTMLFlavors = new DataFlavor[] { new DataFlavor("text/html; document=selection; Class=" + InputStream.class.getName() + "; charset=UTF-8"), new DataFlavor("text/html; document=selection; Class=" + String.class.getName() + "; charset=UTF-8") };
            SyncFlavor = new DataFlavor("application/x-java-serialized-object; class=" + SyncMessage.class.getName() + "; charset=UTF-8");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void start() {
        try {
            ...
            String cmd = javaPath + File.separator + "bin" + File.separator + "java -cp " + System.getProperty("test.classes", ".") + " THTMLConsumer";
        ...
        switch(returnCode) {
            ...
            case CODE_CONSUMER_TEST_FAILED:
                throw new RuntimeException("test failed: HTMLs in some " + "native formats are not transferred properly: " + "see output of child VM");
            default:
                ...
                if (failed) {
                    throw new RuntimeException("test failed: HTMLs in following " + "native formats are not transferred properly: " + failedFormats);
                } else {
                    System.err.println("HTMLs in following native formats are " + "transferred properly: " + passedFormats);
                }
        }
    }
}
...
abstract class HTMLTransferer implements ClipboardOwner {
    ...
    static Object createTRInstance(int i) {
        try {
            String _htmlText = "The quick <font color='#78650d'>brown</font> <b>mouse</b> jumped over the lazy <b>cat</b>.";
            switch(i) {
                case 0:
    ...
    void setClipboardContents(Transferable contents, ClipboardOwner owner) {
...
class THTMLProducer extends HTMLTransferer {
    ...
    void begin() {
        setClipboardContents(new HTMLSelection(HTMLTransferTest.SyncFlavor, S_BEGIN), this);
    }
    ...
    public void lostOwnership(Clipboard cb, Transferable contents) {
        ...
        if (t.isDataFlavorSupported(HTMLTransferTest.SyncFlavor)) {
            ...
            if (msg.equals(S_PASSED)) {
                ...
                throw new RuntimeException("wrong message in " + "THTMLProducer.lostOwnership(): " + msg + "  (possibly due to bug 4683804)");
            }
        } else {
            throw new RuntimeException("DataFlavor.stringFlavor is not " + "suppurted by transferable in " + "THTMLProducer.lostOwnership()");
        }
        if (fi < HTMLTransferTest.HTMLFlavors.length) {
            System.err.println("testing native HTML format \"" + HTMLTransferTest.HTMLFlavors[fi].getMimeType() + "\"...");
            setClipboardContents(new HTMLSelection(HTMLTransferTest.HTMLFlavors[fi], HTMLTransferer.createTRInstance(fi)), this);
        } else {
            setClipboardContents(new HTMLSelection(HTMLTransferTest.SyncFlavor, S_END), null);
        }
        System.err.println("}PRODUCER: lost clipboard ownership");
    }
...
class THTMLConsumer extends HTMLTransferer {
        ...
        if (bContinue) {
            ...
            for (int i = 0; i < HTMLTransferTest.HTMLFlavors.length; ++i) {
                ...
                if (t.isDataFlavorSupported(HTMLTransferTest.HTMLFlavors[i])) {
                    ...
                    int ii;
                    for (ii = 0; ii < dfs.length; ++ii) System.err.println("Supported:" + dfs[ii] + "\n");
                    dfs = HTMLTransferTest.HTMLFlavors;
                    for (ii = 0; ii < dfs.length; ++ii) System.err.println("Accepted:" + dfs[ii] + "\n");
                }
            }
            System.err.println("----------------------------------------------------------");
    ...
    void notifyTransferSuccess(boolean status) {
        System.err.println("format " + (status ? "passed" : "failed") + "!!!");
        setClipboardContents(new HTMLSelection(HTMLTransferTest.SyncFlavor, status ? S_PASSED : S_FAILED), this);
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            ic.setClipboardContents(new HTMLSelection(HTMLTransferTest.SyncFlavor, S_BEGIN_ANSWER), ic);
            synchronized (LOCK) {
                LOCK.wait();
            }
            System.err.println("}CONSUMER: start");
...
class HTMLSelection implements Transferable {
    ...
    public HTMLSelection(DataFlavor flavor, Object data) {
        m_flavor = flavor;
        m_data = data;
    }
