    ...
    public static void main(String[] args) {
        ...
        try {
            for (String id : tzids) {
                ...
                dateloop: for (int year = BEGIN_YEAR; year < END_YEAR; year++) {
                    for (int month = MARCH; month <= NOVEMBER; month++) {
                        ...
                        for (int dom = 1; dom <= maxDom; dom++) {
                            ...
                            if (date.getYear() + 1900 != year || date.getMonth() != month || date.getDate() != dom) {
                                System.err.printf("%s: got %04d-%02d-%02d, expected %04d-%02d-%02d%n", id, date.getYear() + 1900, date.getMonth() + 1, date.getDate(), year, month + 1, dom);
                                errors++;
                                break dateloop;
                            }
                        }
