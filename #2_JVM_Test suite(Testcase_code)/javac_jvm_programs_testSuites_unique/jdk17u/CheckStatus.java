...
public class CheckStatus {
    ...
    private void test() throws Exception {
        ...
        String[] suite1 = new String[] { "SSL_RSA_WITH_RC4_128_MD5" };
        String[] suite2 = new String[] { "SSL_DHE_DSS_WITH_3DES_EDE_CBC_SHA" };
        ...
        checkResult(twoToOne, appIn1, result1, Status.OK, HandshakeStatus.NEED_WRAP, 0, 0);
        ...
        result2 = ssle2.unwrap(oneToTwo, appIn2);
        checkResult(oneToTwo, appIn2, result2, Status.OK, HandshakeStatus.NEED_WRAP, 0, 0);
        log("======================================");
        log("ServerHello");
        ...
        if (!suite.equals(suite1[0])) {
            throw new Exception("suites not equal: " + suite + "/" + suite1[0]);
        }
        ...
        log("DATA");
        result1 = ssle1.wrap(appOut1, oneToTwo);
        checkResult(appOut1, oneToTwo, result1, Status.OK, HandshakeStatus.NOT_HANDSHAKING, appOut1.capacity(), -1);
        oneToTwo.flip();
        result2 = ssle2.wrap(appOut2, twoToOne);
        checkResult(appOut2, twoToOne, result2, Status.OK, HandshakeStatus.NOT_HANDSHAKING, appOut2.capacity(), -1);
        twoToOne.flip();
        SSLEngineResult result3 = ssle1.unwrap(twoToOne, appIn1);
        checkResult(twoToOne, appIn1, result3, Status.OK, HandshakeStatus.NOT_HANDSHAKING, result2.bytesProduced(), result2.bytesConsumed());
        twoToOne.compact();
        SSLEngineResult result4 = ssle2.unwrap(oneToTwo, appIn2);
        checkResult(oneToTwo, appIn2, result4, Status.OK, HandshakeStatus.NOT_HANDSHAKING, result1.bytesProduced(), result1.bytesConsumed());
        oneToTwo.compact();
        appIn1.clear();
        appIn2.clear();
        appOut1.rewind();
        ...
        result1 = ssle1.wrap(appOut1, oneToTwo);
        checkResult(appOut1, oneToTwo, result1, Status.OK, HandshakeStatus.NEED_UNWRAP, appOut1.capacity(), -1);
        oneToTwo.flip();
        result4 = ssle2.unwrap(oneToTwo, appIn2);
        checkResult(oneToTwo, appIn2, result4, Status.OK, HandshakeStatus.NEED_WRAP, result1.bytesProduced(), result1.bytesConsumed());
        oneToTwo.compact();
        appIn2.clear();
        appOut1.rewind();
        log("======================================");
        ...
        result2 = ssle2.wrap(appOut2, twoToOne);
        checkResult(appOut2, twoToOne, result2, Status.OK, HandshakeStatus.NEED_UNWRAP, appOut2.capacity(), -1);
        twoToOne.flip();
        result3 = ssle1.unwrap(twoToOne, appIn1);
        checkResult(twoToOne, appIn1, result3, Status.OK, HandshakeStatus.NEED_WRAP, result2.bytesProduced(), result2.bytesConsumed());
        twoToOne.compact();
        appIn1.clear();
        appOut2.rewind();
        log("======================================");
        ...
        if (!suite.equals(suite2[0])) {
            throw new Exception("suites not equal: " + suite + "/" + suite2[0]);
        }
        suite = ssle2.getSession().getCipherSuite();
        if (!suite.equals(suite2[0])) {
            throw new Exception("suites not equal: " + suite + "/" + suite2[0]);
        ...
        appOut2.rewind();
        result2 = ssle2.wrap(appOut2, twoToOne);
        checkResult(appOut2, twoToOne, result2, Status.CLOSED, HandshakeStatus.NEED_UNWRAP, 0, -1);
        twoToOne.flip();
        if (ssle1.isInboundDone()) {
            throw new Exception("ssle1 inboundDone");
        }
        result1 = ssle1.unwrap(twoToOne, appIn1);
        checkResult(twoToOne, appIn1, result1, Status.CLOSED, HandshakeStatus.NEED_WRAP, result2.bytesProduced(), 0);
        twoToOne.compact();
        if (!ssle1.isInboundDone()) {
            throw new Exception("ssle1 inboundDone");
        }
        ...
        result2 = ssle2.wrap(appOut2, twoToOne);
        checkResult(appOut2, twoToOne, result2, Status.CLOSED, HandshakeStatus.NEED_UNWRAP, 0, 0);
        twoToOne.flip();
        log("======================================");
        log("CN response");
        ...
        result1 = ssle1.wrap(appOut1, oneToTwo);
        checkResult(appOut1, oneToTwo, result1, Status.CLOSED, HandshakeStatus.NOT_HANDSHAKING, 0, -1);
        if (!ssle1.isOutboundDone()) {
        ...
        result2 = ssle2.unwrap(oneToTwo, appIn2);
        checkResult(oneToTwo, appIn2, result2, Status.CLOSED, HandshakeStatus.NOT_HANDSHAKING, result1.bytesProduced(), 0);
        if (!ssle2.isOutboundDone()) {
            throw new Exception("ssle1 outboundDone is NOT done");
        }
        if (!ssle2.isInboundDone()) {
