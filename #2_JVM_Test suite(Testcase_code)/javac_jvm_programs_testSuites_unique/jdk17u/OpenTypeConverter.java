...
public class OpenTypeConverter {
    ...
    private static final WeakHashMap<Class, OpenType> convertedTypes = new WeakHashMap<Class, OpenType>();
    ...
    public static synchronized OpenType toOpenType(Class c) throws OpenDataException {
        OpenType t;
    ...
    private static OpenType makeArrayType(Class c) throws OpenDataException {
        int dim;
        for (dim = 0; c.isArray(); dim++) c = c.getComponentType();
        return new ArrayType(dim, toOpenType(c));
    }

    private static OpenType makeCompositeType(Class c) throws OpenDataException {
        ...
        return new CompositeType(c.getName(), c.getName(), nameArray, nameArray, types.toArray(new OpenType[0]));
    }
