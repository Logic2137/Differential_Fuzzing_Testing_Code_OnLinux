...
public class TesterClient {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        for (int i = 0; i < r.length; i++) {
            r[i] = new Rectangle(Integer.parseInt(args[2 + i * 4]), Integer.parseInt(args[2 + i * 4 + 1]), Integer.parseInt(args[2 + i * 4 + 2]), Integer.parseInt(args[2 + i * 4 + 3]));
        }
        startClient(r, window);
    }

    public static void startClient(Rectangle[] bounds, long window) throws Throwable {
        Method m_getTester = Class.forName("sun.awt.X11.XEmbedServerTester").getMethod("getTester", new Class[] { bounds.getClass(), Long.TYPE });
