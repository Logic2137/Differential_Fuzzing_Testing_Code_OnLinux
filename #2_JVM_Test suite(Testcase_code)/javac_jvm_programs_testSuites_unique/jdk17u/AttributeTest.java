            ...
            for (int y = 0; y < flavors.length; y++) {
                ...
                for (int z = 0; z < attr.length; z++) {
                    if (!service[x].isAttributeValueSupported(attr[z], flavors[y], null)) {
                        throw new RuntimeException("ERROR: There is a conflict between getSupportedAttrValues " + " and isAttributeValueSupported, for the attribute: " + attr[z] + ", where the flavor is: " + flavors[y] + " and the print service is: " + service[x] + "\n");
                    }
                }
            }
        }
