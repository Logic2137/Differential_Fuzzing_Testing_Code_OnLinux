...
public class BashStreams {
    ...
    static class CharacterGenerator {
        ...
        char next() {
            ...
            for (; ; ) {
                c = rand.nextInt(max);
                if ((Character.isBmpCodePoint(c) && (Character.isSurrogate((char) c) || (c == 0xfffe) || (c == 0xffff))))
                    continue;
                if (Character.isSupplementaryCodePoint(c) && (count == limit - 1))
                    continue;
                break;
            }
            count++;
    ...
    static void mismatch(String csn, int count, char c, char d) {
        throw new RuntimeException(csn + ": Mismatch at count " + count + ": " + Integer.toHexString(c) + " != " + Integer.toHexString(d));
    }

    static void mismatchedEOF(String csn, int count, int cgCount) {
        throw new RuntimeException(csn + ": Mismatched EOFs: " + count + " != " + cgCount);
    }

    static class Sink extends OutputStream implements WritableByteChannel {
        ...
        public int write(ByteBuffer bb) throws IOException {
            int n = check(bb.array(), bb.arrayOffset() + bb.position(), bb.remaining());
    ...
    static void testWrite(String csn, int limit, long seed, Writer w) throws IOException {
        Random rand = new Random(seed);
        CharacterGenerator cg = new CharacterGenerator(seed, csn, Integer.MAX_VALUE);
        int count = 0;
        char[] ca = new char[16384];
        int n = 0;
        while (count < limit) {
            n = rand.nextInt(ca.length);
            for (int i = 0; i < n; i++) ca[i] = cg.next();
            w.write(ca, 0, n);
            count += n;
        }
    ...
    static void testStreamWrite(String csn, int limit, long seed) throws IOException {
        log.println("  write stream");
        testWrite(csn, limit, seed, new OutputStreamWriter(new Sink(csn, seed), csn));
    }

    static void testChannelWrite(String csn, int limit, long seed) throws IOException {
        log.println("  write channel");
        testWrite(csn, limit, seed, Channels.newWriter(new Sink(csn, seed), Charset.forName(csn).newEncoder().onMalformedInput(CodingErrorAction.REPLACE).onUnmappableCharacter(CodingErrorAction.REPLACE), 8192));
    }

    static class Source extends InputStream implements ReadableByteChannel {
        ...
        Source(String csn, long seed, int limit) {
            ...
            this.enc = Charset.forName(this.csn).newEncoder().onMalformedInput(CodingErrorAction.REPLACE).onUnmappableCharacter(CodingErrorAction.REPLACE);
                ...
                if ((bb == null) || !bb.hasRemaining()) {
                    ...
                    while (cb.hasRemaining()) {
                        if (!cg.hasNext())
                            break;
                        char c = cg.next();
                        if (Character.isHighSurrogate(c) && cb.remaining() == 1) {
                            cg.push(c);
                            break;
                        }
                        cb.put(c);
        ...
        public int read(ByteBuffer bb) throws IOException {
            int n = read(bb.array(), bb.arrayOffset() + bb.position(), bb.remaining());
            if (n >= 0)
                bb.position(bb.position() + n);
            return n;
        }
    ...
    static void testRead(String csn, int limit, long seed, int max, Reader rd) throws IOException {
        Random rand = new Random(seed);
    ...
    static void testStreamRead(String csn, int limit, long seed, int max) throws IOException {
        log.println("  read stream");
        testRead(csn, limit, seed, max, new InputStreamReader(new Source(csn, seed, limit), csn));
    }

    static void testChannelRead(String csn, int limit, long seed, int max) throws IOException {
        log.println("  read channel");
        testRead(csn, limit, seed, max, Channels.newReader(new Source(csn, seed, limit), csn));
    }

    static final int LIMIT = 1 << 21;

    static void test(String csn, int limit, long seed, int max) throws Exception {
        log.println();
        log.println(csn);
