...
public class intvalue002 {
    ...
    public static int run(String[] argv, PrintStream out) {
        ...
        if (intArgument.intValue() != i) {
            exitCode = exitCode2;
            out.println(sErr2 + "check: setValue(stringValueOf(min()); " + "intValue() == min()\n" + "result: false\n");
        }
        i = intArgument.max();
        intArgument.setValue(intArgument.stringValueOf(i));
        if (intArgument.intValue() != i) {
            exitCode = exitCode2;
            out.println(sErr2 + "check: setValue(stringValueOf(max)); " + "intValue() == max()\n" + "result: false\n");
        }
        if (intArgument.min() < intArgument.max()) {
            ...
            if (intArgument.intValue() != i) {
                exitCode = exitCode2;
                out.println(sErr2 + "check: setValue(stringValueOf(min()+1); " + "intValue() == min()+1\n" + "result: false\n");
            }
        }
        ...
        if (intArgument.max() < intI.MAX_VALUE) {
            ...
            if (intArgument.intValue() != i) {
                exitCode = exitCode2;
                out.println(sErr2 + "check: setValue(stringValueOf(max()+1); " + "intValue() == max()+1\n" + "result: false\n");
            }
        }
