...
public class bug8057791 {
    ...
    private static boolean tryNimbusLookAndFeel() throws Exception {
        try {
    ...
    private static void createUI() throws Exception {
        SwingUtilities.invokeAndWait(new Runnable() {
            ...
            public void run() {
                ...
                list.setFont(new Font(font.getFontName(), Font.BOLD, (int) (font.getSize2D() * 2)));
                list.setModel(model);
    ...
    private static void checkColor(Color colorCheck, Rectangle bounds) {
        if (!findColor(colorCheck, bounds)) {
            String error = "[ERROR][" + colorCheck.toString() + "] Not found in selected cell";
            errorString += error;
        }
    }
    ...
    private static void checkNotColor(Color colorCheck, Rectangle bounds) {
        if (findColor(colorCheck, bounds)) {
            String error = "[ERROR][" + colorCheck.toString() + "] is found in selected cell. " + "Not supposed to be found in negative test case";
            errorString += error;
        }
    }
    ...
    private static boolean findColor(Color colorCheck, Rectangle bounds) {
        ...
        list.paint(g);
        g.dispose();
        checkLoops: for (int x = 0; x < bounds.width; x++) {
            for (int y = 0; y < bounds.height; y++) {
                Point relativePointCheck = new Point(bounds.x + x, bounds.y + y);
                robot.mouseMove(relativePointCheck.x, relativePointCheck.y);
                Color detectedColor = robot.getPixelColor(relativePointCheck.x, relativePointCheck.y);
