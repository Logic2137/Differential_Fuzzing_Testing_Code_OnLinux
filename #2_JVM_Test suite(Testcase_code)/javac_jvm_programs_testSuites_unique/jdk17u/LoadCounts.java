...
public class LoadCounts {
    ...
    private static ClassLoadingMXBean mbean = ManagementFactory.getClassLoadingMXBean();
    ...
    public static void main(String[] argv) throws Exception {
        ...
        if (classesNow > classesTotal)
            throw new RuntimeException("getLoadedClassCount() > " + "getTotalLoadedClassCount()");
        if (classesNowPrev + 4 > classesNow)
            throw new RuntimeException("Number of loaded classes is " + "expected to be at least " + (classesNowPrev + 4) + ", but " + "MBean.getLoadedClassCount() returned " + classesNow);
        if (classesTotalPrev + 4 > classesTotal)
            throw new RuntimeException("Total number of loaded classes is " + "expected to be at least " + (classesTotalPrev + 4) + ", but " + "MBean.getTotalLoadedClassCount() " + "returned " + classesTotal);
        ...
        System.out.println("Loading 2 class instances; each by " + "2 initiating class loaders.");
