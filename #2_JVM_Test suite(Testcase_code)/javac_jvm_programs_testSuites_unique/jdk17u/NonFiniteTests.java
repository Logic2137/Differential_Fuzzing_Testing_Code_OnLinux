...
public class NonFiniteTests {
    ...
    public static int[] types = { SEG_MOVETO, SEG_LINETO, SEG_QUADTO, SEG_CUBICTO, SEG_CLOSE };

    public static double[] coords = { 0.0, 0.0, 50.0, 10.0, 100.0, 20.0, 100.0, 100.0, 50.0, 150.0, 0.0, 100.0, -50.0, 50.0 };

    public static double[] testpoints = { -100, -100, 0, 0, 50, 50, DBL_NaN, DBL_NaN, DBL_POS_INF, DBL_POS_INF, DBL_NEG_INF, DBL_NEG_INF, DBL_POS_INF, DBL_NEG_INF, DBL_NEG_INF, DBL_POS_INF };

    public static double[] testrects = { -100, -100, 10, 10, 0, 0, 10, 10, 50, 50, 10, 10, DBL_NaN, DBL_NaN, 10, 10, 10, 10, DBL_NaN, DBL_NaN, DBL_NaN, DBL_NaN, DBL_NaN, DBL_NaN, 10, 10, DBL_POS_INF, DBL_POS_INF, 10, 10, DBL_NEG_INF, DBL_NEG_INF, 10, 10, DBL_POS_INF, DBL_NEG_INF, 10, 10, DBL_NEG_INF, DBL_POS_INF, DBL_NEG_INF, DBL_NEG_INF, DBL_POS_INF, DBL_POS_INF, DBL_POS_INF, DBL_POS_INF, 10, 10, DBL_NEG_INF, DBL_NEG_INF, 10, 10, DBL_POS_INF, DBL_NEG_INF, 10, 10, DBL_NEG_INF, DBL_POS_INF, 10, 10 };

    public static double[] replacecoords = { DBL_NEG_INF, DBL_MIN, FLT_MIN, DBL_NaN, FLT_MAX, DBL_MAX, DBL_POS_INF };
    ...
    public static void testNonFinites(int[] types, double[] coords, int numvalues) {
        if (numvalues == 0) {
            test(types, coords);
            return;
        }
        ...
        for (int i = 0; i < types.length; i++) {
            switch(types[i]) {
                ...
                case SEG_LINETO:
                    ...
                    break;
                case SEG_QUADTO:
                    gp.quadTo((float) coords[ci++], (float) coords[ci++], (float) coords[ci++], (float) coords[ci++]);
                    break;
                case SEG_CUBICTO:
                    gp.curveTo((float) coords[ci++], (float) coords[ci++], (float) coords[ci++], (float) coords[ci++], (float) coords[ci++], (float) coords[ci++]);
                    break;
                case SEG_CLOSE:
                    gp.closePath();
                    break;
    ...
    public static void testGP(GeneralPath gp) {
        ...
        for (int i = 0; i < testrects.length; i += 4) {
            gp.contains(testrects[i + 0], testrects[i + 1], testrects[i + 2], testrects[i + 3]);
            gp.intersects(testrects[i + 0], testrects[i + 1], testrects[i + 2], testrects[i + 3]);
        }
    }
    ...
    public static class P2DTest {
            ...
            for (int i = 0; i < types.length; i++) {
                switch(types[i]) {
                    ...
                    case SEG_LINETO:
                        ...
                        break;
                    case SEG_QUADTO:
                        p2d.quadTo(coords[ci++], coords[ci++], coords[ci++], coords[ci++]);
                        break;
                    case SEG_CUBICTO:
                        p2d.curveTo(coords[ci++], coords[ci++], coords[ci++], coords[ci++], coords[ci++], coords[ci++]);
                        break;
                    case SEG_CLOSE:
                        p2d.closePath();
                        break;
        ...
        public static void testPath(Path2D p2d) {
            ...
            for (int i = 0; i < testrects.length; i += 4) {
                p2d.contains(testrects[i + 0], testrects[i + 1], testrects[i + 2], testrects[i + 3]);
                contains(p2d, testrects[i + 0], testrects[i + 1], testrects[i + 2], testrects[i + 3]);
                p2d.intersects(testrects[i + 0], testrects[i + 1], testrects[i + 2], testrects[i + 3]);
                intersects(p2d, testrects[i + 0], testrects[i + 1], testrects[i + 2], testrects[i + 3]);
            }
        }
        ...
        public static boolean contains(Path2D p2d, double x, double y, double w, double h) {
            return Path2D.contains(p2d.getPathIterator(null), x, y, w, h);
        }

        public static boolean intersects(Path2D p2d, double x, double y, double w, double h) {
