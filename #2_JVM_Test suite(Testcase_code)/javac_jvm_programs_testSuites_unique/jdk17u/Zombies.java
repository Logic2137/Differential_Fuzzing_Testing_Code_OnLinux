...
public class Zombies {
    ...
    static final String TrueCommand = os.contains("OS X") ? "/usr/bin/true" : "/bin/true";

    public static void main(String[] args) throws Throwable {
        if (!new File("/usr/bin/perl").canExecute() || !new File("/bin/ps").canExecute())
            return;
        ...
        p.waitFor();
        final String[] zombieCounter = { "/usr/bin/perl", "-e", "$a=`/bin/ps -eo ppid,pid,s,command`;" + "print @b=$a=~/^ *@{[getppid]} +[0-9]+ +Z.*$/mog;" + "exit @b" };
