...
public class GcInfoCompositeType {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.gc();
        for (ObjectName n : names) tested += test(mbs, n);
        if (tested == 0)
    ...
    private static int test(MBeanServer mbs, ObjectName n) throws Exception {
        ...
        CompositeType declaredType = (CompositeType) lastGcAI.getDescriptor().getFieldValue("openType");
        checkType(declaredType);
        CompositeData cd = (CompositeData) mbs.getAttribute(n, "LastGcInfo");
        if (cd == null) {
