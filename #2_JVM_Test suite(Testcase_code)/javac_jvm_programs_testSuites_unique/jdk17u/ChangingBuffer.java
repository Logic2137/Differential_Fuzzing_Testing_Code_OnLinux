...
public class ChangingBuffer {
    ...
    private static boolean doMixerClip(Mixer mixer, AudioFormat format) {
        ...
        try {
            ...
            DataLine.Info info = new DataLine.Info(Clip.class, format, (int) samplerate);
    ...
    private static boolean doMixerSDL(Mixer mixer, AudioFormat format) {
        ...
        try {
            System.out.println("Trying mixer " + mixer + ":");
            DataLine.Info info = new DataLine.Info(SourceDataLine.class, format, (int) samplerate);
    ...
    private static void doAll(boolean bigEndian) throws Exception {
        AudioFormat pcm = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, samplerate, 16, 1, 2, samplerate, bigEndian);
        Mixer.Info[] mixers = AudioSystem.getMixerInfo();
        for (int i = 0; i < mixers.length; i++) {
            Mixer mixer = AudioSystem.getMixer(mixers[i]);
            makeBuffer();
            if (doMixerClip(mixer, pcm))
                checkBufferClip();
            makeBuffer();
            if (doMixerSDL(mixer, pcm))
                checkBufferSDL();
        }
        if (mixers.length == 0) {
