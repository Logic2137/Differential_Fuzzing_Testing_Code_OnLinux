...
public class ECCBasic {
    ...
    private static final EllipticCurve CURVE = new EllipticCurve(new ECFieldFp(TEN), ONE, ONE);
    ...
    private static final ECParameterSpec PARAMS = new ECParameterSpec(CURVE, POINT, ORDER, COFACTOR);

    private static final ECGenParameterSpec GENPARAMS = new ECGenParameterSpec("prime192v1");

    private static final ECPrivateKeySpec PRIV_KEY = new ECPrivateKeySpec(TEN, PARAMS);

    private static final ECPublicKeySpec PUB_KEY = new ECPublicKeySpec(POINT, PARAMS);
    ...
    private static void testECFieldF2m() throws Exception {
        ...
        if (!(Arrays.equals(F2M_KS, field.getMidTermsOfReductionPolynomial()))) {
