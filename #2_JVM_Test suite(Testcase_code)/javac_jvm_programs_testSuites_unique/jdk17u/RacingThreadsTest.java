...
public class RacingThreadsTest {
    ...
    private static boolean verbose = Boolean.getBoolean("RacingThreadsTest.verbose");
    ...
    public static class DriverThread extends Thread {
        ...
        private void run(WorkerThread[] workers) {
            ...
            for (; !test.getDone() && test.getLoopCnt() < test.N_LOOPS; test.incAndGetLoopCnt()) {
                if (test.getVerbose() && (test.N_LOOPS < 10 || (test.getLoopCnt() % (test.N_LOOPS / 10)) == 0)) {
                    System.out.println(getName() + ": race loop #" + test.getLoopCnt());
                }
                ...
                if (test.getLoopCnt() + 1 >= test.N_LOOPS || System.currentTimeMillis() >= endTime) {
                    test.setDone(true);
                }
                try {
            ...
            System.out.println(getName() + ": completed " + test.getLoopCnt() + " race loops.");
            if (test.getLoopCnt() < test.N_LOOPS) {
                System.out.println(getName() + ": race stopped @ " + test.N_SECS + " seconds.");
            }
            for (int i = 0; i < workers.length; i++) {
                try {
                    workers[i].join();
