...
public class ShadowingTest {
    ...
    private enum InsideDef {
        ...
        NONE(""), STATIC("static class T { public void inner() {} }\n"), INSTANCE("class T { public void inner() {} }\n");
    ...
    private enum MethodCall {
        ...
        METHOD_TYPEVAR("intValue"), INNER_CLASS("inner"), TYPEVAR("iterator"), OUTER_CLASS("outer");
    ...
    public boolean succeeds(final MethodCall call, final MethodTypeParameterDecl mtyvar, final MethodContext ctx, final InsideDef inside, final TypeParameterDecl tyvar) {
        switch(call) {
            case METHOD_TYPEVAR:
                switch(mtyvar) {
                    case YES:
    ...
    private void doTest(final MethodTypeParameterDecl mtyvar, final TypeParameterDecl tyvar, final InsideDef insidedef, final MethodContext ctx, final MethodCall call) throws IOException {
    ...
    private void run() throws Exception {
        classesdir.mkdir();
        for (MethodTypeParameterDecl mtyvar : MethodTypeParameterDecl.values()) for (TypeParameterDecl tyvar : TypeParameterDecl.values()) for (InsideDef insidedef : InsideDef.values()) for (MethodContext ctx : MethodContext.values()) for (MethodCall methodcall : MethodCall.values()) doTest(mtyvar, tyvar, insidedef, ctx, methodcall);
        if (errors != 0)
            throw new Exception("ShadowingTest test failed with " + errors + " errors.");
    }
    ...
    private boolean assert_compile_fail(final File file) {
        ...
        pw.close();
        if (rc == 0) {
            System.err.println("Compilation of " + file.getName() + " didn't fail as expected.");
            errors++;
            return false;
        } else
            return true;
    ...
    private boolean assert_compile_succeed(final File file) {
        ...
        pw.close();
        if (rc != 0) {
            System.err.println("Compilation of " + file.getName() + " didn't succeed as expected.  Output:");
    ...
    private File writeFile(final File dir, final String path, final String body) throws IOException {
