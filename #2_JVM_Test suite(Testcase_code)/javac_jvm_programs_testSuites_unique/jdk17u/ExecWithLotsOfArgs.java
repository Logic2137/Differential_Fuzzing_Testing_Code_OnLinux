...
public class ExecWithLotsOfArgs {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int n = 0;
        command[n++] = System.getProperty("java.home") + File.separator + "bin" + File.separator + "java";
        ...
        while ((s = in.readLine()) != null) {
            if (count >= command.length) {
                failed("Was expecting " + (command.length - 2) + " strings to be echo'ed back, but got " + (count - 1) + " instead");
            }
            if (!s.equals(command[count])) {
                failed("Exec'd process returned \"" + s + "\", was expecting \"" + command[count] + "\"");
            }
            count++;
        }
        if (count == n) {
