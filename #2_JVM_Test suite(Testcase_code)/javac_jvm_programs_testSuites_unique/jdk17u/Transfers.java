...
public class Transfers {
    ...
    private static void writeBytes(byte[] ba, FileChannel fc, int off, int len) throws IOException {
        fc.position(off);
    ...
    private static void writeRandomBytes(long seed, FileChannel fc, int off, int len) throws IOException {
    ...
    private static void writeZeroBytes(FileChannel fc, int off, int len) throws IOException {
        byte[] ba = new byte[len];
        writeBytes(ba, fc, off, len);
    }

    private static void checkBytes(FileChannel fc, int off, int len, byte[] bytes) throws IOException {
    ...
    private static void checkRandomBytes(FileChannel fc, int off, int len, long seed) throws IOException {
        byte[] ba = new byte[len];
        Random r = new Random(seed);
        r.nextBytes(ba);
    ...
    private static void checkZeroBytes(FileChannel fc, int off, int len) throws IOException {
        byte[] ba = new byte[len];
        checkBytes(fc, off, len, ba);
    }

    private static void dump(FileChannel fc) throws IOException {
        int sz = (int) fc.size();
    ...
    static class FileSource extends Source {
        ...
        void verify() throws IOException {
            if (fc.position() != size)
                throw new Failure("Wrong position: " + fc.position() + " (expected " + size + ")");
    ...
    static class UserSource extends Source {
        ...
        void verify() {
            if (src.remaining() != 1)
                throw new Failure("Source has " + src.remaining() + " bytes remaining (expected 1)");
        }
    }
    ...
    static class UserTarget extends Target {
        ...
        void verify() {
            if (dst.remaining() != 1)
                throw new Failure("Destination has " + dst.remaining() + " bytes remaining (expected 1)");
    ...
    static void show(String dir, String channelName, int off, int len) {
        if (!verbose)
            return;
        out.println(dir + " " + channelName + ": offset " + off + ", length " + len);
    }

    static void testTo(long seed, FileChannel fc, int off, int len, Target tgt) throws IOException {
        ...
        if (n != len)
            throw new Failure("Incorrect transfer length: " + n + " (expected " + len + ")");
        if (fc.position() != pos)
    ...
    static void testFrom(long seed, Source src, FileChannel fc, int off, int len) throws IOException {
        ...
        try (FileChannel fc = new RandomAccessFile(fn, "rw").getChannel()) {
            for (boolean to = false; ; to = true) {
                for (boolean user = false; ; user = true) {
                    if (!verbose)
                        out.print((to ? "To " : "From ") + (user ? "user channel" : "file channel") + ":");
                    ...
                    while (offGen.hasNext()) {
                        ...
                        while (lenGen.hasNext()) {
                            ...
                            try {
                                ...
                                out.println();
                                out.println("FAILURE: " + chName + ", offset " + off + ", length " + len);
