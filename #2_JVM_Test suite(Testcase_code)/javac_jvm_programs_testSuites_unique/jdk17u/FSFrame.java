...
public class FSFrame extends Frame implements Runnable {
    ...
    public void paint(Graphics g) {
        if (!visible && getWidth() != 0 && getHeight() != 0) {
            ...
            try {
                ...
                System.out.println("Problem creating robot: cannot verify FS " + "window display");
            }
        }
    ...
    void checkFSDisplay(boolean fsSupported) {
        ...
        if (!fsSupported) {
            ...
            r = new Rectangle(in.left, in.top, r.width - (in.left + in.right), r.height - (in.top + in.bottom));
        }
        ...
        if (!colorCorrect) {
            ...
            String name = "FSFrame_fs_" + (fsSupported ? "supported" : "not_supported") + ".png";
            try {
                ImageIO.write(bImg, "png", new File(name));
                System.out.println("Dumped screen shot to " + name);
            } catch (IOException ex) {
            }
            throw new Error("Some pixel colors not correct; FS window may not" + " have been displayed correctly");
        }
    }
    ...
    void checkFSFunctionality(boolean withSecurity) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new Error("Failure: should not get an exception when " + "calling isFSSupported or setFSWindow");
        }
    }
