...
public class MemoryUsageCompositeData {
    ...
    public static void badTypeCompositeData() throws Exception {
        final int K = 1024;
        final Object[] values = { new Integer(5 * K), new Long(1 * K), new Long(10 * K), new Long(2 * K), "Dummy", "Dummy" };
        CompositeType muct = new CompositeType("MyMemoryUsageCompositeType", "CompositeType for MemoryUsage", memoryUsageItemNames, memoryUsageItemNames, badMUItemTypes);
        CompositeData cd = new CompositeDataSupport(muct, memoryUsageItemNames, values);
        try {
            MemoryUsage u = MemoryUsage.from(cd);
    ...
    private static final String[] memoryUsageItemNames = { "committed", "init", "max", "used", "dummy1", "dummy2" };

    private static final OpenType[] memoryUsageItemTypes = { SimpleType.LONG, SimpleType.LONG, SimpleType.LONG, SimpleType.LONG, SimpleType.STRING, SimpleType.STRING };

    private static final String[] badMUItemNames = { "Committed", "Init", "max", "used", "dummy1", "dummy2" };

    private static final OpenType[] badMUItemTypes = { SimpleType.INTEGER, SimpleType.LONG, SimpleType.LONG, SimpleType.LONG, SimpleType.STRING, SimpleType.STRING };
}
