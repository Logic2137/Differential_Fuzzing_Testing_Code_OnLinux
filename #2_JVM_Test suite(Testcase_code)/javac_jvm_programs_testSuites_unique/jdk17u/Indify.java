...
public class Indify {
    ...
    public void parseOptions(List<String> av) throws IOException {
        ...
        if (expandProperties) {
            for (int i = 0; i < av.size(); i++) av.set(i, maybeExpandProperties(av.get(i)));
        }
    }
    ...
    private String maybeExpandProperties(String s) {
        ...
        while (s.contains("${")) {
            ...
            if (verbose)
                System.err.println("expanding ${" + prop + "} => " + value);
            if (value == null)
                break;
    ...
    public void indifyTree(File f, File dest) throws IOException {
        if (verbose)
            System.err.println("reading directory: " + f);
    ...
    private class Loader extends ClassLoader {
        ...
        private Class<?> transformAndLoadClass(File f) throws ClassNotFoundException, IOException {
            if (verbose)
                System.err.println("Loading class from " + f);
            ClassFile cf = new ClassFile(f);
            Logic logic = new Logic(cf);
            boolean changed = logic.transform();
            if (verbose && !changed)
    ...
    private class Logic {
        ...
        boolean transform() {
            ...
            for (Method m : cf.methods) {
                if (constants.containsKey(m))
                    continue;
                int blab = 0;
                for (Instruction i = m.instructions(); i != null; i = i.next()) {
                    if (i.bc != opc_invokestatic)
                        continue;
                    int methi = i.u2At(1);
                    if (poolMarks[methi] == 0)
                        continue;
                    ...
                    if (con.tag == CONSTANT_InvokeDynamic) {
                        ...
                        Short[] ref2 = null;
                        short ref2i = 0;
                        if (i2 != null && i2.bc == opc_invokevirtual && poolMarks[(char) (ref2i = (short) i2.u2At(1))] == 'D')
                        ...
                        if (!quiet)
                            System.err.println(i + " " + conm + ";...; " + i2 + " => invokedynamic " + con);
                        ...
                        if (!quiet)
                            System.err.println(i + " " + conm + " => ldc " + con);
                        assert (i.len == 3);
        ...
        Instruction findPop(Instruction i) {
            ...
            decode: for (i = i.clone().next(); i != null; i = i.next()) {
                ...
                if (pops.charAt(k++) != '$')
                    break decode;
                args.clear();
                while (k < pops.length()) args.add(pops.charAt(k++));
            }
        ...
        void reportPatternMethods(boolean quietly, boolean allowMatchFailure) {
            ...
            for (Method m : cf.methods) {
                if (nameMark(cf.pool.getString(m.name)) != 0 && constants.get(m) == null) {
        ...
        char nameMark(String s) {
            if (s.startsWith("MT_"))
                return 'T';
            else if (s.startsWith("MH_"))
                return 'H';
            else if (s.startsWith("INDY_"))
                return 'I';
            else if (s.startsWith("java/lang/invoke/"))
                return 'D';
        ...
        char nameAndTypeMark(short n1, short n2) {
            ...
            switch(poolMarks[(char) n1]) {
                case 'H':
                    requiredType = "()Ljava/lang/invoke/MethodHandle;";
                    break;
                case 'T':
                    requiredType = "()Ljava/lang/invoke/MethodType;";
                    break;
                case 'I':
                    requiredType = "()Ljava/lang/invoke/MethodHandle;";
                    break;
        ...
        private class JVMState {
            ...
            void push2(Object x) {
                stack.add(EMPTY_SLOT);
                stack.add(x);
            }

            void pushAt(int pos, Object x) {
        ...
        private void removeEmptyJVMSlots(List<Object> args) {
            for (; ; ) {
                ...
                if (i >= 0 && i + 1 < args.size() && (isConstant(args.get(i + 1), CONSTANT_Long) || isConstant(args.get(i + 1), CONSTANT_Double)))
                    args.remove(i);
                else
                    break;
            }
        ...
        private Constant scanPattern(Method m, char patternMark) {
            if (verbose)
                System.err.println("scan " + m + " for pattern=" + patternMark);
            int wantTag;
            switch(patternMark) {
                case 'T':
                    wantTag = CONSTANT_MethodType;
                    break;
                case 'H':
                    wantTag = CONSTANT_MethodHandle;
                    break;
                case 'I':
                    wantTag = CONSTANT_InvokeDynamic;
                    break;
                default:
                    throw new InternalError();
            }
            ...
            decode: for (; i != null; i = i.next()) {
                int bc = i.bc;
                switch(bc) {
                    case opc_ldc:
                        jvm.push(pool.get(i.u1At(1)));
                        break;
                    case opc_ldc_w:
                        jvm.push(pool.get(i.u2At(1)));
                        break;
                    case opc_ldc2_w:
                        jvm.push2(pool.get(i.u2At(1)));
                        break;
                    case opc_aconst_null:
                        jvm.push(null);
                        break;
                    case opc_bipush:
                        jvm.push((int) (byte) i.u1At(1));
                        break;
                    case opc_sipush:
                        jvm.push((int) (short) i.u2At(1));
                        break;
                    case opc_anewarray:
                        arg = jvm.pop();
                    ...
                    case opc_aastore:
                        args = jvm.args(3);
                        if (args.get(0) instanceof List && args.get(1) instanceof Integer) {
                            ...
                            switch(intrinsic == null ? "" : intrinsic) {
                                                ...
                                                if (argClass.isPrimitive()) {
                                                    ...
                                                    switch(argClass.getName()) {
                                                        ...
                                                        case "double":
                                                            ...
                                                            break;
                                                        default:
                                                            throw new InternalError(argClass.toString());
                                                    }
                                                    buf.append(tchar);
                                                } else {
                                ...
                                case "Double.valueOf":
                                    ...
                                    if (args.size() == 1) {
                                        ...
                                        if (isConstant(arg, CONSTANT_Integer + "IFLD".indexOf(intrinsic.charAt(0))) || arg instanceof Number) {
                                            args.add(arg);
                                            continue;
                                        }
                                    }
                    ...
                    default:
                        ...
                        if (bc >= opc_nconst_MIN && bc <= opc_nconst_MAX) {
                            jvm.push(INSTRUCTION_CONSTANTS[bc - opc_nconst_MIN]);
                            break;
                        }
                        if (patternMark == 'I') {
                            ...
                            switch(bc) {
                                ...
                                case opc_checkcast:
                                    arg = jvm.top();
                                    if ("invokeWithArguments".equals(arg) || "invokeGeneric".equals(arg))
                                        break;
                                    break decode;
                                default:
                                    break decode;
        ...
        private Constant parseMemberLookup(byte refKind, List<Object> args) {
            if (args.size() != 4)
                return null;
            int argi = 0;
            if (!"lookup".equals(args.get(argi++)))
                return null;
            short refindex, cindex, ntindex, nindex, tindex;
            Object con;
            if (!isConstant(con = args.get(argi++), CONSTANT_Class))
                return null;
            cindex = (short) ((Constant) con).index;
            if (!isConstant(con = args.get(argi++), CONSTANT_String))
                return null;
            nindex = ((Constant) con).itemIndex();
            if (isConstant(con = args.get(argi++), CONSTANT_MethodType) || isConstant(con, CONSTANT_Class)) {
                tindex = ((Constant) con).itemIndex();
            } else
                return null;
            ntindex = (short) cf.pool.addConstant(CONSTANT_NameAndType, new Short[] { nindex, tindex }).index;
        ...
        private Constant makeInvokeDynamicCon(List<Object> args) {
            ...
            Object con;
            if (!isConstant(con = args.get(argi++), CONSTANT_MethodHandle))
                return null;
            ...
            for (Object x : extraArgs) {
                if (x instanceof Number) {
                    ...
                    if (x instanceof Integer) {
                        num = x;
                        numTag = CONSTANT_Integer;
                    }
                    if (x instanceof Float) {
                        num = Float.floatToRawIntBits((Float) x);
                        numTag = CONSTANT_Float;
                    }
                    if (x instanceof Long) {
                        num = x;
                        numTag = CONSTANT_Long;
                    }
                    if (x instanceof Double) {
                        num = Double.doubleToRawLongBits((Double) x);
                        numTag = CONSTANT_Double;
                    }
                    if (num != null)
                        x = cf.pool.addConstant(numTag, x);
                }
            ...
            for (Object[] spec1 : specs) {
                if (Arrays.equals(spec1, spec)) {
                    ...
                    if (verbose)
                        System.err.println("reusing BSM specifier: " + spec1[0] + spec1[1]);
                    break;
                }
            }
            if (specindex == -1) {
                ...
                specs.add(spec);
                if (verbose)
                    System.err.println("adding BSM specifier: " + spec[0] + spec[1]);
            }
            return cf.pool.addConstant(CONSTANT_InvokeDynamic, new Short[] { (short) specindex, ntindex });
        }
            ...
            if (bsms.item instanceof byte[]) {
                ...
                try {
                    ...
                    for (int i = 0; i < len; i++) {
                        ...
                        for (int j = 0; j < argc; j++) argv.add(in.readShort());
    ...
    private static class CountedList<T> extends ArrayList<T> implements Chunk {
        ...
        CountedList(Class<? extends T> itemClass) {
            this(itemClass, -1);
        }
        ...
        public void readFrom(DataInputStream in) throws IOException {
            ...
            while (size() < count) {
                if (rowlen < 0) {
                    ...
                    for (int i = 0; i < rowlen; i++) row[i] = readInput(in, elemClass);
    ...
    private static <T> T readInput(DataInputStream in, Class<T> dataClass) throws IOException {
        ...
        if (dataClass == Integer.class) {
            ...
            T obj;
            try {
                obj = dataClass.getDeclaredConstructor().newInstance();
            } catch (Exception ex) {
                throw new RuntimeException(ex);
            }
    ...
    private static void writeOutput(DataOutputStream out, Object data) throws IOException {
        ...
        if (data instanceof Integer) {
            ...
            for (Object x : (Object[]) data) writeOutput(out, x);
        } else if (data instanceof Chunk) {
            Chunk x = (Chunk) data;
            x.writeTo(out);
        } else if (data instanceof List) {
            for (Object x : (List<?>) data) writeOutput(out, x);
        } else {
    ...
    public static class Constant<T> implements Chunk {
        ...
        public boolean equals(Object x) {
            return (x instanceof Constant && equals((Constant) x));
        }
        ...
        public int hashCode() {
            return (tag * 31) + this.itemAsComparable().hashCode();
        }

        public Object itemAsComparable() {
            switch(tag) {
                case CONSTANT_Double:
                    return Double.longBitsToDouble((Long) item);
                case CONSTANT_Float:
                    return Float.intBitsToFloat((Integer) item);
            }
        ...
        public static String tagName(byte tag) {
            if (TAG_NAMES == null)
                TAG_NAMES = ("None Utf8 Unicode Integer Float Long Double Class String" + " Fieldref Methodref InterfaceMethodref NameAndType #13 #14" + " MethodHandle MethodType InvokeDynamic#17 InvokeDynamic").split(" ");
    ...
    public static class Pool extends CountedList<Constant> implements Chunk {
        ...
        public <T> Constant addConstant(byte tag, T item) {
            ...
            if (tag == CONSTANT_Utf8)
                strings.put((String) item, (short) con.index);
            return con;
        }
        ...
        String[] getStrings(Short[] indexes) {
            ...
            for (int i = 0; i < indexes.length; i++) res[i] = getString(indexes[i]);
            return res;
        }
    ...
    public class ClassFile extends Outer implements Chunk {
        ...
        public final void readFrom(DataInputStream in) throws IOException {
            ...
            if (in.read() >= 0)
                throw new IOException("junk after end of file");
            linkInners();
        }
        ...
        public void writeTo(DataOutputStream out) throws IOException {
            writeOutputs(out, magic, version, pool, access, thisc, superc, interfaces, fields, methods, attrs);
        }
    ...
    public static class Member extends InnerOuter implements Chunk {
        ...
        public ClassFile outer() {
            return (ClassFile) outer;
        }

        public String nameString() {
            return outer().pool.getString(CONSTANT_Utf8, name);
        }

        public String typeString() {
            return outer().pool.getString(CONSTANT_Utf8, type);
        }

        public String toString() {
            if (outer == null)
                return super.toString();
            return nameString() + (this instanceof Method ? "" : ":") + simplifyType(typeString());
        }
    }
    ...
    public static class Attr extends InnerOuter implements Chunk {
        ...
        public void linkOuter(Outer o) {
            super.linkOuter(o);
            if (item instanceof byte[] && outer instanceof Method && ((Method) outer).outer().Code_index == name) {
    ...
    public static class Instruction implements Cloneable {
        ...
        public String toString() {
            ...
            switch(len) {
                case 3:
                    buf.append(" ").append(u2At(1));
                    break;
                case 5:
                    buf.append(" ").append(u2At(1)).append(" ").append(u2At(3));
                    break;
                default:
                    for (int i = 1; i < len; i++) buf.append(" ").append(u1At(1));
            }
            return buf.toString();
        }
        ...
        private void computeLength() {
            ...
            switch(bc) {
                case opc_wide:
                    ...
                    len = ((info >> 4) & 0x0F);
                    if (len == 0)
                        throw new RuntimeException("misplaced wide bytecode: " + bc);
                    return;
