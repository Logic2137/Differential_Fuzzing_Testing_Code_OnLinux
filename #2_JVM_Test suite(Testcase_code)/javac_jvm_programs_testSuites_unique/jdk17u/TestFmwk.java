...
public class TestFmwk {
    ...
    protected TestFmwk() {
        ...
        for (int i = 0; i < methods.length; i++) {
            if (methods[i].getName().startsWith("Test") || methods[i].getName().startsWith("test")) {
    ...
    protected void run(String[] args) throws Exception {
        ...
        for (int i = 0; i < testsToRun.size(); i++) {
            ...
            try {
                ...
                errln("Uncaught exception thrown in test method " + testMethod.getName());
    ...
    void usage() {
        System.out.println(getClass().getName() + ": [-verbose] [-nothrow] [-prompt] [test names]");
