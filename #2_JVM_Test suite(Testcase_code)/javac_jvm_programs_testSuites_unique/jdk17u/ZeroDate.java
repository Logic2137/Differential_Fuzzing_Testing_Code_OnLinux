...
public class ZeroDate {
    ...
    public static void main(String[] args) throws Exception {
        ...
        testDate(data.clone(), 0 << 25 | 2 << 21 | 30 << 16 | 24 << 11 | 60 << 5 | 60 >> 1, LocalDateTime.of(1980, 3, 2, 1, 1, 0));
    }
    ...
    private static void testDate(byte[] data, int date, LocalDateTime expected) throws IOException {
        ...
        try (FileSystem fs = FileSystems.newFileSystem(uri, Collections.emptyMap())) {
            ...
            Instant actualInstant = Files.readAttributes(entry, BasicFileAttributes.class).lastModifiedTime().toInstant();
            Instant expectedInstant = expected.atZone(ZoneId.systemDefault()).toInstant();
            if (!actualInstant.equals(expectedInstant)) {
                throw new AssertionError(String.format("actual: %s, expected: %s", actualInstant, expectedInstant));
            }
        } finally {
            Files.delete(path);
        }
