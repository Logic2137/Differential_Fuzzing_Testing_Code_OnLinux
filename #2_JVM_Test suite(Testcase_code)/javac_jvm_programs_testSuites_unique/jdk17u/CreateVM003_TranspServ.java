...
public class CreateVM003_TranspServ extends TransportService {
    ...
    void handshake(Socket s, long timeout) throws IOException {
        ...
        while (received < hello.length) {
            ...
            if (n < 0) {
                s.close();
                throw new IOException("##> CreateVM003_TranspServ: Handshake FAILED - connection prematurally closed!");
            }
            received += n;
        }
        for (int i = 0; i < hello.length; i++) {
            if (b[i] != hello[i]) {
                throw new IOException("##> CreateVM003_TranspServ: Handshake FAILED - unrecognized message from target VM");
            }
        }
        s.setSoTimeout(0);
    }
    ...
    public Connection attach(String address, long attachTimeout, long handshakeTimeout) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException("##> CreateVM003_TranspServ: attach() - unable to parse port number in address");
        }
        ...
        try {
            ...
            throw new TransportTimeoutException("##> CreateVM003_TranspServ: attach() - timed out trying to establish connection");
        }
        try {
    ...
    public ListenKey startListening(String address) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException("##> CreateVM003_TranspServ: startListening() - unable to parse port number in address");
        }
    ...
    public void stopListening(ListenKey listener) throws IOException {
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException("##> CreateVM003_TranspServ: stopListening() - Invalid listener");
        }
    ...
    public Connection accept(ListenKey listener, long acceptTimeout, long handshakeTimeout) throws IOException {
        if (acceptTimeout < 0 || handshakeTimeout < 0) {
            throw new IllegalArgumentException("##> CreateVM003_TranspServ: accept() - timeout is negative");
        }
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException("##> CreateVM003_TranspServ: accept() - Invalid listener");
        }
        ServerSocket ss;
        synchronized (listener) {
        ...
        try {
            ...
            throw new TransportTimeoutException("##> CreateVM003_TranspServ: accept() - timeout waiting for connection");
        }
        handshake(s, handshakeTimeout);
        return new CreateVM003_Connection(s);
    }
...
class CreateVM003_Connection extends Connection {
    ...
    public byte[] readPacket() throws IOException {
        if (!isOpen()) {
            throw new ClosedConnectionException("##> CreateVM003_Connection: readPacket() - connection is closed");
        }
        synchronized (receiveLock) {
            ...
            if (len < 0) {
                throw new IOException("##> CreateVM003_Connection: readPacket() - protocol error: invalid Packet's length");
            }
            byte[] b = new byte[len];
            b[0] = (byte) b1;
            b[1] = (byte) b2;
            ...
            while (len > 0) {
                ...
                if (count < 0) {
                    throw new EOFException("##> CreateVM003_Connection: readPacket() - read() method returns negative value");
                }
                len -= count;
                off += count;
            }
    ...
    public void writePacket(byte[] b) throws IOException {
        if (!isOpen()) {
            throw new ClosedConnectionException("##> CreateVM003_Connection: writePacket() - connection is closed");
        }
        if (b.length < 11) {
            throw new IllegalArgumentException("##> CreateVM003_Connection: writePacket() - packet is insufficient size");
        }
        int b0 = b[0] & 0xff;
        int b1 = b[1] & 0xff;
        int b2 = b[2] & 0xff;
        ...
        if (len > b.length) {
            throw new IllegalArgumentException("##> CreateVM003_Connection: writePacket() - length mis-match");
        }
