...
public class Bug4944439 {
    ...
    private static void check_Long(String s) throws Exception {
        ...
        if (!(number instanceof Long)) {
            err = true;
            System.err.println("Failed: DecimalFormat.parse(\"" + s + "\") should return a Long, but returned a " + number.getClass().getName());
        }
        int index = s.indexOf('.');
        Long l = Long.valueOf(s.substring(0, index));
        if (!l.equals(number)) {
            err = true;
            System.err.println("Failed: DecimalFormat.parse(" + s + ") should return a Long(" + l + "), but returned " + number);
        }
    }
    ...
    private static void check_Double(String s) throws Exception {
        ...
        if (!(number instanceof Double)) {
            err = true;
            System.err.println("Failed: DecimalFormat.parse(\"" + s + "\") should return a Double, but returned a " + number.getClass().getName());
        }
        Double d = Double.valueOf(s);
        if (!d.equals(number)) {
            err = true;
            System.err.println("Failed: DecimalFormat.parse(" + s + ") should return a Double(" + d + "), but returned " + number);
        }
    }
}
