...
public class TestInferCaller {
    ...
    static final class TestHandler extends Handler {
        ...
        public void publish(LogRecord record) {
            LogEvent event = new LogEvent(record.getSourceClassName(), record.getSourceMethodName(), record);
            PUBLISHED.add(event);
        }
    ...
    void test2(Logger logger) {
        ...
        logger.setLevel(Level.ALL);
        assertEquals(0, TestHandler.PUBLISHED.size(), "Queue should be empty: ");
        for (Level l : Arrays.asList(Level.SEVERE, Level.WARNING, Level.INFO, Level.CONFIG, Level.FINE, Level.FINER, Level.FINEST)) {
    ...
    public static void main(String[] args) {
        ...
        for (Logger logger : Arrays.asList(root, Logger.getGlobal(), Logger.getAnonymousLogger(), Logger.getLogger("foo.bar"))) {
