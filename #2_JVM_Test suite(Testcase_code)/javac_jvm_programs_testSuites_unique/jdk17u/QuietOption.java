...
public class QuietOption {
    ...
    void run1() throws Exception {
        List<String> output = doTest(javadoc.getPath(), "-classpath", ".", "-quiet", new File(testSrc, thisClassName + ".java").getPath());
        if (!output.isEmpty()) {
    ...
    void run2() throws Exception {
        List<String> output = doTest(javadoc.getPath(), "-classpath", ".", new File(testSrc, thisClassName + ".java").getPath());
        if (output.isEmpty()) {
            System.out.println(output);
