...
public class Bug8166994 {
    ...
    public static void main(String[] args) {
        ...
        testParseConsistency(list, ranges);
        list = Arrays.asList("gfx-xz", "oun-xz", "mwj-xz", "vaj-xz", "taj-xy", "tsf-xy");
    ...
    private static void testParseConsistency(List<String> list, String ranges) {
        ...
        if (!list.equals(priorityList)) {
            throw new RuntimeException("Failed to parse the language range [" + ranges + "], Expected: " + list + " Found: " + priorityList);
        }
    }

    private static List<String> parseRanges(String s) {
        return Locale.LanguageRange.parse(s).stream().map(Locale.LanguageRange::getRange).collect(Collectors.toList());
    }
}
