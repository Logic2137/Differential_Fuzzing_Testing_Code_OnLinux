...
public class ImageTypeSpecifierTest {
    ...
    public static void createIndexedTest() {
        ...
        int dataType = DataBuffer.TYPE_BYTE;
        try {
            ImageTypeSpecifier.createIndexed(redLUT, greenLUT, blueLUT, alphaLUT, 0, dataType);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
        try {
            ImageTypeSpecifier.createIndexed(redLUT, greenLUT, blueLUT, alphaLUT, 17, dataType);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
        try {
            ImageTypeSpecifier.createIndexed(redLUT, greenLUT, blueLUT, alphaLUT, 10, dataType);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
        byte[] greenLUT_4 = {};
        try {
            ImageTypeSpecifier.createIndexed(redLUT, greenLUT_4, blueLUT, alphaLUT, bits, dataType);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
        byte[] redLUT_5 = {};
        try {
            ImageTypeSpecifier.createIndexed(redLUT_5, greenLUT, blueLUT, alphaLUT, bits, dataType);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
        byte[] alphaLUT_6 = {};
        try {
            ImageTypeSpecifier.createIndexed(redLUT, greenLUT, blueLUT, alphaLUT_6, bits, dataType);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
        try {
            ImageTypeSpecifier.createIndexed(redLUT, greenLUT, blueLUT, alphaLUT, bits, 100);
            fail("Failed to get IAE!");
        } catch (IllegalArgumentException e) {
        }
    }
