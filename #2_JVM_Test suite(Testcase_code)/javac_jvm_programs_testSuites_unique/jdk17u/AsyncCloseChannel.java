...
public class AsyncCloseChannel {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Thread ss = new SensorServer();
        ss.start();
        Thread ts = new TargetServer();
        ts.start();
        sensorPort = ((ServerThread) ss).server.getLocalPort();
        targetPort = ((ServerThread) ts).server.getLocalPort();
        Thread sc = new SensorClient();
        sc.start();
        Thread tc = new TargetClient();
        tc.start();
        ...
        if (failed)
            throw new RuntimeException("AsyncCloseChannel2 failed after <" + acceptCount + "> times of accept!");
    }
                    ...
                    new Thread() {
                        ...
                        public void run() {
                            try {
                                ...
                                if (c != -1) {
                                    System.err.println("Oops: read a character: " + (char) c);
                                    failed = true;
                                }
    ...
    static class TargetClient extends Thread {
        ...
        public void run() {
            while (keepGoing) {
                try {
                    final SocketChannel s = SocketChannel.open(new InetSocketAddress(InetAddress.getLoopbackAddress(), targetPort));
                    s.finishConnect();
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                if (!(ex instanceof ClosedChannelException))
                                    System.err.println("Exception in target client child " + ex.toString());
                            }
                        }
                    };
                    t.start();
                    ...
                    System.err.println("Exception in target client parent " + ex.getMessage());
                } catch (InterruptedException ex) {
                }
            }
        }
