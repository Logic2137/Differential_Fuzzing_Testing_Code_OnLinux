...
public class CloseRace {
    ...
    private static final int testDurationSeconds = Integer.getInteger("test.duration", 600);

    private static final CountDownLatch threadsStarted = new CountDownLatch(2);
    ...
    static boolean[] procFDsInUse() {
        ...
        for (int i = 0; i < procFDs.length; i++) inUse[i] = fdInUse(procFDs[i]);
        return inUse;
    }
    ...
    static void dumpAllStacks() {
        ...
        for (Thread thread : allStackTraces.keySet()) {
            ...
            for (StackTraceElement element : allStackTraces.get(thread)) System.err.println("\t" + element);
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0, j = 0; j < procFDs.length; i++) if (!fdInUse(i))
            procFDs[j++] = i;
        Thread[] threads = { new Thread(new OpenLoop()), new Thread(new ExecLoop()) };
        ...
        for (Thread thread : threads) {
            ...
            if (thread.isAlive()) {
                dumpAllStacks();
                throw new Error("At least one child thread (" + thread.getName() + ") failed to finish gracefully");
            }
        }
    }
        ...
        public void run() {
            ...
            while (!Thread.interrupted()) {
                try {
                    ...
                    for (int i = 0; i < 3; i++) iss.add(new FileInputStream(BIG_FILE));
                    do {
                        if (Thread.interrupted())
                            return;
                    ...
                    Thread.sleep(1);
                    for (InputStream is : iss) is.close();
