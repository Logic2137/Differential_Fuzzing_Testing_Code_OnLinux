...
public class bug8081411 {
    ...
    private static void testSynthIcon() {
        ...
        Icon centeredIcon = new CenteredSynthIcon((SynthIcon) subMenuIcon, newWidth, newHeight);
        paintAndCheckIcon(centeredIcon, synthContext, newWidth, newHeight);
    }

    private static void paintAndCheckIcon(Icon icon, SynthContext synthContext, int width, int height) {
    ...
    private static class CenteredSynthIcon implements SynthIcon {
        ...
        @Override
        public void paintIcon(SynthContext syntContext, Graphics g, int x, int y, int w, int h) {
            int dw = icon.getIconWidth(syntContext);
            int dh = icon.getIconHeight(syntContext);
            int dx = width - dw;
            int dy = height - dh;
            icon.paintIcon(syntContext, g, x + dx / 2, y + dy / 2, dw + 2, dh + 2);
        }
