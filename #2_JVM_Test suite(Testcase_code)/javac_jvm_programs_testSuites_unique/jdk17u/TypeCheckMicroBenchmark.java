...
public class TypeCheckMicroBenchmark {
    ...
    private static java.util.regex.Pattern patternArg(String[] args, String keyword) {
        String val = keywordValue(args, keyword);
        return val == null ? null : java.util.regex.Pattern.compile(val);
    }

    private static Job[] filter(java.util.regex.Pattern filter, Job[] jobs) {
        if (filter == null)
            return jobs;
        Job[] newJobs = new Job[jobs.length];
        int n = 0;
        for (Job job : jobs) if (filter.matcher(job.name()).find())
            newJobs[n++] = job;
        Job[] ret = new Job[n];
    ...
    public static void main(String[] args) throws Throwable {
        ...
        final java.util.regex.Pattern filter = patternArg(args, "filter");
        final List<Integer> list = new ArrayList<Integer>();
        final Random rnd = new Random();
        for (int i = 0; i < size; i++) list.add(rnd.nextInt());
        final Class klazz = Integer.class;
        final Job[] jobs = { new Job("toArray(T[])") {
            ...
            void work() {
                for (int i = 0; i < iterations; i++) {
                    for (Object x : list.toArray()) if (!(x != null && klazz.isInstance(x)))
                        throw new ClassCastException();
                }
            }
        }, new Job("Class.cast") {
            ...
            void work() {
                for (int i = 0; i < iterations; i++) {
                    for (Object x : list.toArray()) klazz.cast(x);
                }
            }
        }, new Job("write into array") {
        ...
        }, new Job("write into dynamic array") {

            void work() {
                for (int i = 0; i < iterations; i++) {
                    for (Object x : list.toArray()) {
                        Object[] a = (Object[]) java.lang.reflect.Array.newInstance(klazz, 1);
                        try {
                            a[0] = x;
                        } catch (ArrayStoreException unused) {
                            throw new ClassCastException();
