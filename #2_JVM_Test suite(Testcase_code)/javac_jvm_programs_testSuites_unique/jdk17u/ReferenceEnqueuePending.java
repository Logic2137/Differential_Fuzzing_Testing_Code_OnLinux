...
public class ReferenceEnqueuePending {
    ...
    private static void checkResult(ReferenceQueue<Integer> queue, int expected) {
        if (debug) {
        ...
        if (fail) {
            ...
            throw new RuntimeException("TEST FAILED: only " + length + " reference objects have been queued out of " + expected);
        }
    }
