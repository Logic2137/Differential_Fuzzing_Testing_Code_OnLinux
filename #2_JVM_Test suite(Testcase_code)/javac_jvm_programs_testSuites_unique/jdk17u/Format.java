...
public class Format implements java.io.Serializable, java.security.Principal {
    ...
    public static void main(String[] args) {
        try {
            PrivateCredentialPermission pcp = new PrivateCredentialPermission("test", "write");
            throw new SecurityException("test 1 failed");
        } catch (IllegalArgumentException iae) {
            System.out.println("Test 1: " + iae.getMessage());
        }
        try {
            PrivateCredentialPermission pcp = new PrivateCredentialPermission("Format", "read");
            throw new SecurityException("test 2 failed");
        } catch (IllegalArgumentException iae) {
            System.out.println("Test 2: " + iae.getMessage());
        }
        try {
            PrivateCredentialPermission pcp = new PrivateCredentialPermission("Format Format", "read");
            throw new SecurityException("test 3 failed");
        } catch (IllegalArgumentException iae) {
            System.out.println("Test 3: " + iae.getMessage());
        }
        try {
            PrivateCredentialPermission pcp = new PrivateCredentialPermission("Format Format Format", "read");
        ...
        try {
            javax.security.auth.x500.X500Principal p = new javax.security.auth.x500.X500Principal((String) null);
        ...
        try {
            Subject s = new Subject();
            s.getPrincipals().add(new javax.security.auth.x500.X500Principal("cn=test1"));
        ...
        try {
            NTSidPrimaryGroupPrincipal p = new NTSidPrimaryGroupPrincipal("test");
        ...
        try {
            UnixNumericGroupPrincipal p = new UnixNumericGroupPrincipal("0", true);
            System.out.println("NTPrimaryGroupPrincipal = " + p.toString());
        } catch (Exception e) {
            throw new SecurityException("test 32 failed");
        }
        try {
            UnixNumericGroupPrincipal p = new UnixNumericGroupPrincipal("1", false);
