...
public class HtmlSection {
    ...
    public void link(HtmlSection section, String child, String name) {
        ...
        pw.printf("<a href=\"#%1$s\" onclick=\"show(document.getElementById('%1$s')); return true;\">%2$s</a>%n", path, name);
    }
    ...
    public HtmlSection createChildren(String[] sections) {
        ...
        for (; i < n && current.child != null; ++i, current = current.child) {
    ...
    private static class SubSection extends HtmlSection {
        ...
        public SubSection(HtmlSection parent, String name, HtmlSection rootSection) {
            super(parent.pw, parent.id.isEmpty() ? name : String.format("%s.%s", parent.id, name), name, rootSection);
            this.parent = parent;
            pw.printf("<li><a name='%1$s'/><a href='#%1$s' onclick=\"toggle('%1$s'); return false;\">%2$s</a><div id='%1$s'><code><pre>", id, name);
        }
