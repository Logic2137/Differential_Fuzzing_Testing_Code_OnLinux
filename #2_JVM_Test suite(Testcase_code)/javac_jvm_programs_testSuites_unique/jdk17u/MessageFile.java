...
class MessageFile {
    ...
    static final Pattern infoPattern = Pattern.compile(String.format("# ([0-9]+: %s, )*[0-9]+: %s", typePattern.pattern(), typePattern.pattern()));
    ...
    static final class Message {
        ...
        Set<Integer> getPlaceholders() {
            ...
            while (true) {
                ...
                while (m.find()) results.add(Integer.parseInt(m.group(1)));
                if (!l.hasContinuation())
                    return results;
                l = l.next;
            }
        ...
        List<Line> getLines(boolean includeAllPrecedingComments) {
            ...
            if (includeAllPrecedingComments) {
                while (l.prev != null && l.prev.isEmptyOrComment()) l = l.prev;
                while (l.text.isEmpty()) l = l.next;
            } else {
                if (l.prev != null && l.prev.isInfo())
                    l = l.prev;
            }
            for (; l != firstLine; l = l.next) lines.add(l);
            for (l = firstLine; l != null && l.hasContinuation(); l = l.next) lines.add(l);
            lines.add(l);
            l = l.next;
    ...
    static final class Info {
        ...
        Info(Set<String> infos) throws IllegalArgumentException {
            for (String s : infos) merge(new Info(s));
        }
