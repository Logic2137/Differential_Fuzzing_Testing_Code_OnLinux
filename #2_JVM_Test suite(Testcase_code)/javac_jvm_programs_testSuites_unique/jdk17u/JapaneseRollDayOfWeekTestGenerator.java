...
public class JapaneseRollDayOfWeekTestGenerator {
    ...
    private static final String[] ERAS = { "BeforeMeiji", "Meiji", "Taisho", "Showa", "Heisei" };

    private static final String[] DAYOFWEEK = { null, "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };

    private static final int[] DAYOFMONTH = { 25, 26, 27, 28, 29, 30, 31, 1, 2, 3, 4, 5, 6, 7 };

    private static final int DOM_BASE = 7;

    public static void generateTestScript(String[] args) throws Exception {
        Locale.setDefault(Locale.ROOT);
        int nYears = (args.length == 1) ? Math.max(3, Math.min(Integer.parseInt(args[0]), 40)) : 28;
        int era = 3;
        int year = 63;
                ...
                for (int fdw = SUNDAY; fdw <= SATURDAY; fdw++) {
                    ...
                    for (int mdifw = 1; mdifw <= 7; mdifw++) {
                        ...
                        for (int i = 0; i < 10; i++) {
                            ...
                            out.printf("\tcheck date %d %s %d%n", (dom >= 25) ? year - 1 : year, (dom >= 25) ? "Dec" : "Jan", dom);
