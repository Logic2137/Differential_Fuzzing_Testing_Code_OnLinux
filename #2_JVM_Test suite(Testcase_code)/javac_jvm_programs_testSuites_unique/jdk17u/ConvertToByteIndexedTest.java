...
public class ConvertToByteIndexedTest {
    ...
    static final int[] SRC_TYPES = new int[] { BufferedImage.TYPE_INT_RGB, BufferedImage.TYPE_INT_ARGB, BufferedImage.TYPE_INT_ARGB_PRE, BufferedImage.TYPE_INT_BGR, BufferedImage.TYPE_3BYTE_BGR, BufferedImage.TYPE_4BYTE_ABGR, BufferedImage.TYPE_4BYTE_ABGR_PRE, BufferedImage.TYPE_USHORT_565_RGB, BufferedImage.TYPE_USHORT_555_RGB, BufferedImage.TYPE_BYTE_INDEXED };

    static final String[] TYPE_NAME = new String[] { "INT_RGB", "INT_ARGB", "INT_ARGB_PRE", "INT_BGR", "3BYTE_BGR", "4BYTE_ABGR", "4BYTE_ABGR_PRE", "USHORT_565_RGB", "USHORT_555_RGB", "BYTE_INDEXED" };

    static final Color[] COLORS = new Color[] { Color.BLACK, Color.RED, Color.YELLOW, Color.GREEN, Color.MAGENTA, Color.CYAN, Color.BLUE };

    static final HashMap<Integer, String> TYPE_TABLE = new HashMap<Integer, String>();
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                if (color.getRGB() != argbImage.getRGB(i, j)) {
                    throw new RuntimeException("Conversion from " + TYPE_TABLE.get(srcType) + " to BYTE_INDEXED is not" + " done properly for " + color);
                }
            }
        }
    }
