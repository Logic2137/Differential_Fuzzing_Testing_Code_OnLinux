...
public class B8025710 {
    ...
    private static final String keystorefile = System.getProperty("test.src", "./") + "/../../../../../javax/net/ssl/etc/keystore";
    ...
    private void runTest() throws Exception {
        ...
        httpServer.start();
        proxyServer.start();
        URL url = new URL("https", InetAddress.getLocalHost().getHostName(), httpServer.getPort(), "/");
    ...
    class ProxyServer extends Thread implements Closeable {
        ...
        private final Pattern connectLinePattern = Pattern.compile("^CONNECT ([^: ]+):([0-9]+) HTTP/[0-9.]+$");

        private final String PROXY_RESPONSE = "HTTP/1.0 200 Connection Established\r\n" + "Proxy-Agent: TestProxy/1.0\r\n" + "\r\n";
        ...
        public void run() {
            ...
            try {
                while (connectionCount++ < 2) {
                    ...
                    System.out.println("Proxy: NEW CONNECTION " + proxyConnectionCount);
                    Thread t = new Thread("ProxySocket" + proxyConnectionCount) {

                        @Override
                        public void run() {
                            try {
                                String firstLine = readHeader(clientSocket.getInputStream());
                                Matcher connectLineMatcher = connectLinePattern.matcher(firstLine);
                                ...
                                Socket serverSocket = SocketFactory.getDefault().createSocket(host, port);
                                clientSocket.getOutputStream().write(PROXY_RESPONSE.getBytes("UTF-8"));
    ...
    class HttpServer extends Thread implements Closeable {
        ...
        private final String serverResponse = "HTTP/1.1 200 OK\r\n" + "Content-Type: text/plain\r\n" + "Content-Length: 3\r\n" + "\r\n" + "Hi!";
        ...
        public void run() {
            try {
                while (connectionCount++ < 2) {
                    ...
                    System.out.println("Server: NEW CONNECTION " + connectionCount);
                    SSLSocket sslSocket = (SSLSocket) sslSocketFactory.createSocket(socket, null, getPort(), false);
