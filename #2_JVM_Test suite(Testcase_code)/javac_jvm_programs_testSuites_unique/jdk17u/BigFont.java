...
public class BigFont {
    ...
    public static void main(final String[] args) {
        id = args[0];
        fileName = args[1];
        System.out.println("Applet " + id + " " + Thread.currentThread().getThreadGroup());
        ...
        try {
            ...
            t.printStackTrace();
            if (t instanceof FontFormatException || fontSize <= sis.getCurrentSize()) {
    ...
    int getFileSize(String fileName) {
        try {
            String path = Paths.get(System.getProperty("test.src", "."), fileName).toAbsolutePath().normalize().toString();
