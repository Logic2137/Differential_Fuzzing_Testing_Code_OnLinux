            ...
            allThreads[i] = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        fail.set(new Exception("Unexpected InterruptedException"));
                    }
                }
            }, THREAD_PREFIX + i);
        ...
        try {
            ...
            for (int i = 0; i < allThreadInfos.length; i++) {
                ...
                if (threadName.startsWith(THREAD_PREFIX)) {
                    ...
                    if (allThreads[threadNum].isDaemon() != allThreadInfos[i].isDaemon()) {
                        throw new RuntimeException(allThreads[threadNum] + " is not like " + allThreadInfos[i] + ". TEST FAILED.");
                    }
                }
            }
