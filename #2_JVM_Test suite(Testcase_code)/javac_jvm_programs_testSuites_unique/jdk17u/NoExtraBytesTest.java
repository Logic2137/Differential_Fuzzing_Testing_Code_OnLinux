...
public class NoExtraBytesTest {
    ...
    private void checkImageContent() {
        BufferedImage dst = (BufferedImage) iio_dst.getRenderedImage();
        int y = h / 2;
        int x = dx / 2;
        for (int i = 0; i < usedColors.length; i++, x += dx) {
            ...
            if (dstRgb != srcRgb || dstRgb != rgb) {
                throw new RuntimeException("Wrong color at [" + x + ", " + y + "] " + Integer.toHexString(dstRgb) + " (srcRgb=" + Integer.toHexString(srcRgb) + ", original color is " + Integer.toHexString(rgb) + ")");
            }
        }
