...
public class isvalid005 {
    ...
    public static int run(String[] argv, PrintStream out) {
        ...
        if (!argument.isValid("")) {
            ...
            exitCode = exitCode2;
            out.println(sErr2 + "check: super.isValid('')\n" + "result: true\n");
        }
        if (!argument.isValid("a")) {
        } else {
            exitCode = exitCode2;
            out.println(sErr2 + "check: super.isValid('a')\n" + "result: true\n");
        }
        if (!argument.isValid((String) null)) {
        } else {
            exitCode = exitCode2;
            out.println(sErr2 + "check: super.isValid((String) null))\n" + "result: true\n");
        }
        String s = null;
        if (!argument.isValid(s)) {
        } else {
            exitCode = exitCode2;
            out.println(sErr2 + "check: super.isValid(String s = null)\n" + "result: true\n");
        }
