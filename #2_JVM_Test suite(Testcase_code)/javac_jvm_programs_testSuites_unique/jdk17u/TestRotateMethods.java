...
public class TestRotateMethods {
    ...
    public static Point2D[] testtxpts = { new Point2D.Double(5, 5), new Point2D.Double(20, -10), new Point2D.Double(-Math.PI, Math.E) };
    ...
    public static void verify(AffineTransform at1, AffineTransform at2, Mode mode, double vectorscale, Point2D txpt, String message, double num, String units) {
        if (!compare(at1, at2)) {
    ...
    public static void test(Mode mode, double vectorscale, Point2D txpt) {
        ...
        for (int deg = -720; deg <= 720; deg++) {
            ...
            verify(at1, at2, mode, vectorscale, txpt, "vector and radians do not match for ", deg, " degrees");
            if (txpt == null) {
                if (vectorscale == 1.0) {
                    at3 = makeAT(mode, zeropt, radians);
                    verify(at1, at3, mode, vectorscale, zeropt, "radians not invariant with 0,0 translate at ", deg, " degrees");
                }
                at3 = makeAT(mode, zeropt, vecx, vecy);
                verify(at2, at3, mode, vectorscale, zeropt, "vector not invariant with 0,0 translate at ", deg, " degrees");
            }
        }
        for (int quad = -8; quad <= 8; quad++) {
            ...
            at1 = makeAT(mode, txpt, radians);
            at2 = makeAT(mode, txpt, vecx, vecy);
            verify(at1, at2, mode, vectorscale, txpt, "quadrant vector and radians do not match for ", degrees, " degrees");
            at2 = makeQuadAT(mode, txpt, quad);
            verify(at1, at2, mode, vectorscale, txpt, "quadrant and radians do not match for ", quad, " quadrants");
            if (txpt == null) {
                at3 = makeQuadAT(mode, zeropt, quad);
                verify(at2, at3, mode, vectorscale, zeropt, "quadrant not invariant with 0,0 translate at ", quad, " quadrants");
            }
        }
    }
    ...
    public static AffineTransform makeAT(Mode mode, Point2D txpt, double radians) {
        AffineTransform at;
    ...
    public static AffineTransform makeAT(Mode mode, Point2D txpt, double vx, double vy) {
        AffineTransform at;
        double tx = (txpt == null) ? 0.0 : txpt.getX();
        double ty = (txpt == null) ? 0.0 : txpt.getY();
        switch(mode) {
    ...
    public static AffineTransform makeQuadAT(Mode mode, Point2D txpt, int quads) {
        AffineTransform at;
        double tx = (txpt == null) ? 0.0 : txpt.getX();
        double ty = (txpt == null) ? 0.0 : txpt.getY();
        switch(mode) {
