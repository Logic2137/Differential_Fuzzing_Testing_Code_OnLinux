...
public final class IsRunningHang {
                ...
                for (final int sampleRate : sampleRates) {
                    for (final int channel : channels) {
                        for (final AudioFormat.Encoding enc : encodings) {
                            ...
                            formats.add(new AudioFormat(enc, sampleRate, sampleSize, channel, s, sampleRate, end));
                        }
                    }
                }
            }
    ...
    public static void main(final String[] args) throws Exception {
        for (final AudioFormat format : formats) {
            ...
            byte[] soundData = new byte[(int) (format.getFrameRate() * format.getFrameSize() / 2)];
            try {
    ...
    private static void test(final AudioFormat format, final byte[] data) throws Exception {
