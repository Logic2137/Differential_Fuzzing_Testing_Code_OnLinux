...
public class ExecutorTest {
    ...
    private static enum TestType {
        ...
        NO_EXECUTOR, NULL_EXECUTOR, COUNT_EXECUTOR
    }

    private static String test(String proto) throws Exception {
        System.out.println();
        System.out.println("TEST: " + proto);
        ClassLoader myClassLoader = CountInvocationHandler.class.getClassLoader();
        ExecutorService wrappedExecutor = Executors.newCachedThreadPool();
        CountInvocationHandler executorHandler = new CountInvocationHandler(wrappedExecutor);
        Executor countExecutor = (Executor) Proxy.newProxyInstance(myClassLoader, new Class<?>[] { Executor.class }, executorHandler);
        JMXServiceURL url = new JMXServiceURL(proto, null, 0);
    ...
    private static class CountInvocationHandler implements InvocationHandler {
        ...
        final Map<Method, Integer> methodCount = new HashMap<Method, Integer>();
