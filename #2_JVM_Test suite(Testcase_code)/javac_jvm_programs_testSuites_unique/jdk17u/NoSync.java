...
public class NoSync {
    ...
    public static void main(String[] args) throws Exception {
        for (Provider p : Security.getProviders()) {
            for (Provider.Service s : p.getServices()) {
                if (s.getType().equals("SecureRandom") && !s.getAlgorithm().contains("Block")) {
    ...
    static void test(SecureRandom sr) throws Exception {
        test(sr, 20, 3000);
        String attr = sr.getProvider().getProperty("SecureRandom." + sr.getAlgorithm() + " ThreadSafe");
        if (!"true".equals(attr)) {
            throw new Exception("Not ThreadSafe: " + attr);
        }
    }

    public static void test(SecureRandom sr, int tnum, int rnum) throws Exception {
