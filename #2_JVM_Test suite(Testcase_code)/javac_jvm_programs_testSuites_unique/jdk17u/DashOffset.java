...
public class DashOffset {
    ...
    private static final BasicStroke dash = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, new float[] { 2.0f, 2.0f }, 0.0f);
    ...
    public static void main(String[] argv) throws Exception {
        ...
        final BufferedImage img = new BufferedImage(WIDTH, HEIGHT, TYPE_INT_RGB);
        try {
            draw(img);
            validate(img);
        } finally {
    ...
    private static void checkHorizontalLine(final BufferedImage img, final int y) {
    ...
    private static void checkVerticalLine(final BufferedImage img, final int x) {
    ...
    private static void checkCorners(final BufferedImage img) {
        int[][] corners = { { OFFSET - 1, OFFSET - 1 }, { OFFSET, OFFSET - 1 }, { OFFSET - 1, OFFSET + 1 }, { OFFSET - 1, HEIGHT - OFFSET }, { OFFSET - 1, HEIGHT - OFFSET + 1 }, { OFFSET, HEIGHT - OFFSET + 1 }, { WIDTH - OFFSET, OFFSET - 1 }, { WIDTH - OFFSET + 1, OFFSET - 1 }, { WIDTH - OFFSET + 1, OFFSET }, { WIDTH - OFFSET + 1, HEIGHT - OFFSET }, { WIDTH - OFFSET + 1, HEIGHT - OFFSET + 1 }, { WIDTH - OFFSET, HEIGHT - OFFSET + 1 } };
    ...
    private static void checkColor(final int x, final int y, final int color, final Color... validColors) {
        checkColor(x, y, color, Arrays.stream(validColors).mapToInt(Color::getRGB).toArray());
    }

    private static void checkColor(final int x, final int y, final int color, final int... validColors) {
        for (int valid : validColors) {
            if (color == valid) {
                return;
            }
        }
        throw new RuntimeException("Unexpected color at " + x + ", " + y + ": " + Integer.toHexString(color) + "; expected: " + Arrays.stream(validColors).mapToObj(Integer::toHexString).collect(Collectors.joining(", ")));
    }
    ...
    private static void save(final BufferedImage img, final String fileName) throws IOException {
