...
public class LoggerInterfaceTest {
    ...
    public static class LoggerImpl implements System.Logger {
        ...
        public void log(Level level, ResourceBundle bundle, String msg, Throwable thrown) {
            builder.clear().level(level).bundle(bundle).msg(msg).thrown(thrown).stack(new Exception().getStackTrace());
            consumer.accept(builder.build());
        }

        @Override
        public void log(Level level, ResourceBundle bundle, String format, Object... params) {
            builder.clear().level(level).bundle(bundle).msg(format).params(params).stack(new Exception().getStackTrace());
            consumer.accept(builder.build());
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (Level l : Level.values()) {
            ...
            Object[][] cases = new Object[][] { { null }, { "baz" } };
            for (Object[] p : cases) {
                String msg = (String) p[0];
                final Object obj = msg == null ? null : logged ? new NotTrowing(msg) : new Throwing();
                String par1 = msg == null ? "(Object)null" : logged ? "new NotTrowing(\"" + msg + "\")" : "new Throwing()";
                System.out.println("  logger.log(" + l + ", " + par1 + ")");
                try {
                    logger.log(l, obj);
                    if (obj == null) {
                        throw new RuntimeException("Expected NullPointerException not thrown for" + " logger.log(" + l + ", " + par1 + ")");
                    }
                ...
                if (logged) {
                    ...
                    assertEquals(e.bundle, null, "e.bundle", "    ");
                    assertEquals(e.callStack[0].getMethodName(), "log", "e.callStack[0].getMethodName()", "    ");
                    assertEquals(e.callStack[0].getClassName(), logger.getClass().getName(), "e.callStack[0].getClassName() ", "    ");
                    assertEquals(e.callStack[1].getMethodName(), "log", "e.callStack[1].getMethodName()", "    ");
                    assertEquals(e.callStack[1].getClassName(), System.Logger.class.getName(), "e.callStack[1].getClassName() ", "    ");
                    assertEquals(e.callStack[2].getMethodName(), "main", "e.callStack[2].getMethodName()", "    ");
                } else {
                    assertEquals(e, null, "e", "    ");
                }
            }
        }
        System.out.println("  logger.log(" + null + ", " + "new NotThrowing(\"foobar\")" + ")");
        try {
            logger.log(null, new NotTrowing("foobar"));
            throw new RuntimeException("Expected NullPointerException not thrown for" + " logger.log(" + null + ", " + "new NotThrowing(\"foobar\")" + ")");
        } catch (NullPointerException x) {
            System.out.println("    Got expected exception: " + x);
        }
        ...
        for (Level l : Level.values()) {
            ...
            for (Object[] p : cases) {
                ...
                String par1 = msg == null ? "(Supplier<String>)null" : logged ? "() -> new NotTrowing(\"" + msg + "\").toString()" : "new Throwing()";
                System.out.println("  logger.log(" + l + ", " + par1 + ")");
                try {
                    logger.log(l, s);
                    if (s == null) {
        ...
        try {
            logger.log(null, () -> "biz");
            throw new RuntimeException("Expected NullPointerException not thrown for" + " logger.log(" + null + ", " + "() -> \"biz\"" + ")");
        } catch (NullPointerException x) {
            System.out.println("    Got expected exception: " + x);
        }
        ...
        for (Level l : Level.values()) {
            ...
            par = "bam {0}";
            System.out.println("  logger.log(" + l + ", \"" + par + "\", new NotTrowing(\"one\"));");
            ...
            par = "bam {0} {1}";
            System.out.println("  logger.log(" + l + ", \"" + par + "\", new NotTrowing(\"fisrt\")," + " new NotTrowing(\"second\"));");
            ...
            par = "bam {2}";
            System.out.println("  logger.log(" + l + ", \"" + par + "\", new Object[] {new NotTrowing(\"third\")," + " new NotTrowing(\"fourth\")});");
            logger.log(l, par, params);
            e = events.poll();
            assertNonNull(e, "e", "    ");
            assertEquals(l, e.level, "e.level", "    ");
        ...
        for (Level l : Level.values()) {
            boolean logged = l.compareTo(Level.WARNING) >= 0;
            Object[][] cases = new Object[][] { { null, null }, { null, new Throwable() }, { "biz", null }, { "boz", new Throwable() } };
            for (Object[] p : cases) {
                String msg = (String) p[0];
        ...
        for (Level l : Level.values()) {
            ...
            for (Object[] p : cases) {
                ...
                try {
                    ...
                    if (s == null) {
                        throw new RuntimeException("Expected NullPointerException not thrown for" + " logger.log(" + l + ", " + par1 + ", " + par2 + ")");
                    }
                } catch (NullPointerException x) {
                    if (s == null) {
                        System.out.println("    Got expected exception: " + x);
        ...
        System.out.println("  logger.log(" + null + ", " + "() -> \"biz\"" + ", " + "new Throwable()" + ")");
        try {
            logger.log(null, () -> "biz", new Throwable());
            throw new RuntimeException("Expected NullPointerException not thrown for" + " logger.log(" + null + ", " + "() -> \"biz\"" + ", " + "new Throwable()" + ")");
        } catch (NullPointerException x) {
            System.out.println("    Got expected exception: " + x);
        }
