    ...
    static void validateArg(String name, String ext, Map<String, String> args) {
        ...
        if (arg != null) {
            if (!Objects.equals(arg, prop)) {
                throw new RuntimeException(extName + ": -D value should match property: " + arg + " != " + prop);
            }
        } else if (prop != null) {
            if (ext != null && !ext.isEmpty()) {
                String value = System.getProperty(name);
                if (Objects.equals(value, prop)) {
                    throw new RuntimeException(extName + " property should not be equals to " + name + " property: " + prop);
                }
            }
        }
    }
