...
public class RemovePollRace {
    ...
    void prettyPrintResults() {
        ...
        String format = String.format("%%%ds %%%ds nanos/item%%n", maxClassNameLength, maxNanosLength);
        for (String name : classNames) System.out.printf(format, name, results.get(name));
    }

    void test(String[] args) throws Throwable {
        if (args.length > 0)
            count = Integer.valueOf(args[0]);
        for (Queue<Boolean> queue : concurrentQueues()) test(queue);
        results.clear();
        for (Queue<Boolean> queue : concurrentQueues()) test(queue);
        prettyPrintResults();
    }
    ...
    void test(final Queue<Boolean> q) throws Throwable {
        ...
        for (int i = 0; i < adderCount; i++) adders.add(checkedThread(adder));
        for (int i = 0; i < removerCount; i++) removers.add(checkedThread(remover));
        for (int i = 0; i < pollerCount; i++) pollers.add(checkedThread(poller));
        ...
        if (removes.get() + polls.get() != adderCount * count) {
            String msg = String.format("class=%s removes=%s polls=%d count=%d", className, removes.get(), polls.get(), count);
            fail(msg);
        }
    }
