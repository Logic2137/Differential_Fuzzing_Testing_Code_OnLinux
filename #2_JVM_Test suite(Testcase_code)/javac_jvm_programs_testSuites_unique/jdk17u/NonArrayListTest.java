...
public class NonArrayListTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        RelationServiceMBean rsProxy = (RelationServiceMBean) MBeanServerInvocationHandler.newProxyInstance(mbs, rsName, RelationServiceMBean.class, false);
        ...
        ArrayList leftRoleValues = new ArrayList(Arrays.asList(new ObjectName[] { mlet1Name }));
        ArrayList rightRoleValues = new ArrayList(Arrays.asList(new ObjectName[] { mlet2Name }));
        Role leftRole = new Role("left", leftRoleValues);
        Role rightRole = new Role("right", rightRoleValues);
        RelationType leftRightType = new RelationTypeSupport("leftRight", new RoleInfo[] { leftRoleInfo, rightRoleInfo });
        RoleList roleList = new RoleList(new ArrayList(Arrays.asList(new Role[] { leftRole, rightRole })));
        ...
        List onameList = new Vector(Arrays.asList(new ObjectName[] { oname }));
        String testName;
        testName = "RelationNotification constructor with only 9 arguments";
        try {
            RelationNotification notif = new RelationNotification(RelationNotification.RELATION_BASIC_CREATION, rs, 0L, 0L, "theMsg", "theRelId", "theRelTypeName", oname, onameList);
        ...
        try {
            RelationNotification notif = new RelationNotification(RelationNotification.RELATION_BASIC_UPDATE, rs, 0L, 0L, "theMsg", "theRelId", "theRelTypeName", oname, "theRoleName", onameList, onameList);
            System.out.println("OK: " + testName);
        } catch (Exception e) {
            System.err.println("Exception for " + testName);
            e.printStackTrace();
