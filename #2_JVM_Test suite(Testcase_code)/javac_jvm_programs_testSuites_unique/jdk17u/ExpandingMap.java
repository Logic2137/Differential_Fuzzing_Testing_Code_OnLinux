...
public class ExpandingMap {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (currentPosition + currentSize < maximumFileSize) {
            ...
            while (currentSize + inc > maximumMapSize) {
                if (currentSize < maximumMapSize) {
                    System.out.format("map %d -> %d\n", currentPosition, (currentPosition + maximumMapSize));
                    buffers[currentBuffer] = fc.map(FileChannel.MapMode.READ_WRITE, currentPosition, maximumMapSize);
            ...
            if (currentSize > 0) {
                ...
                buffers[currentBuffer] = fc.map(FileChannel.MapMode.READ_WRITE, currentPosition, currentSize);
        ...
        fc.close();
        for (int i = 0; i < buffers.length; i++) buffers[i] = null;
        System.gc();
        Thread.sleep(1000);
