...
public class B6299712 {
    ...
    public static void makeHttpCall() throws IOException {
        try {
            ...
            if (uc.getResponseCode() != 200)
                throw new RuntimeException("Expected Response Code was 200," + "received: " + uc.getResponseCode());
            uc.disconnect();
        } finally {
            server.stop(0);
        }
    ...
    static class DeployCacheHandler extends java.net.ResponseCache {
        ...
        public synchronized CacheResponse get(final URI uri, String rqstMethod, Map<String, List<String>> requestHeaders) throws IOException {
            System.out.println("get!!!: " + uri);
            if (!uri.toString().endsWith("redirect")) {
                return null;
            }
            System.out.println("Serving request from cache");
            return new DeployCacheResponse(new EmptyInputStream(), new HashMap<String, List<String>>());
        }
