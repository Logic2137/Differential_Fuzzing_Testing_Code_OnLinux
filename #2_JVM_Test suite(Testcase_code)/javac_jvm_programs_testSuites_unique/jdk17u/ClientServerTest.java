...
public class ClientServerTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        success &= runTest("", CRAM_MD5, new String[] { QOP_AUTH }, new String[] { QOP_AUTH }, false);
        success &= runTest("", DIGEST_MD5, new String[] { QOP_AUTH }, new String[] { QOP_AUTH }, false);
        success &= runTest(AUTHID_SASL_TESTER, DIGEST_MD5, new String[] { QOP_AUTH }, new String[] { QOP_AUTH }, false);
    ...
    private static boolean runTest(String authId, String mech, String[] clientQops, String[] serverQops, boolean expectException) throws Exception {
        System.out.println("AuthId:" + authId + " mechanism:" + mech + " clientQops: " + Arrays.toString(clientQops) + " serverQops: " + Arrays.toString(serverQops) + " expect exception:" + expectException);
        try (Server server = Server.start(LOCALHOST, authId, serverQops)) {
            new Client(LOCALHOST, server.getPort(), mech, authId, clientQops).run();
            if (expectException) {
    ...
    static class Server extends SaslPeer implements Runnable, Closeable {
        ...
        static Server start(String host, String authId, String[] serverQops) throws UnknownHostException {
        ...
        private void processConnection(SaslEndpoint endpoint) throws SaslException, IOException, ClassNotFoundException {
            ...
            while (!saslServer.isComplete()) {
                byte[] data = processData(msg.getData(), endpoint, saslServer);
        ...
        private byte[] processData(byte[] data, SaslEndpoint endpoint, SaslServer server) throws SaslException, IOException {
            try {
        ...
        private SaslServer createSaslServer(String mechanism) throws SaslException {
            Map<String, String> props = new HashMap<>();
            props.put(Sasl.QOP, qop);
            return Sasl.createSaslServer(mechanism, PROTOCOL, host, props, callback);
        }
    ...
    static class Client extends SaslPeer {
        ...
        Client(String host, int port, String mech, String authId, String... qops) {
        ...
        public void run() throws Exception {
            ...
            try (SaslEndpoint endpoint = SaslEndpoint.create(host, port)) {
                ...
                endpoint.send(new Message(SaslStatus.CONTINUE, data));
                Message msg = getMessage(endpoint.receive());
                while (!client.isComplete() && msg.getStatus() != SaslStatus.FAILURE) {
                    switch(msg.getStatus()) {
                        ...
                        case SUCCESS:
                            ...
                            break;
                        default:
                            throw new RuntimeException("Wrong status:" + msg.getStatus());
                    }
                }
        ...
        private SaslClient createSaslClient() throws SaslException {
            Map<String, String> props = new HashMap<>();
            props.put(Sasl.QOP, qop);
            return Sasl.createSaslClient(new String[] { mechanism }, USER_ID, PROTOCOL, host, props, callback);
        }

        private void negotiateMechanism(SaslEndpoint endpoint) throws ClassNotFoundException, IOException {
            Object o = endpoint.receive();
            if (o instanceof ArrayList) {
                ArrayList list = (ArrayList) o;
                if (!list.contains(mechanism)) {
                    throw new RuntimeException("Server does not support specified mechanism:" + mechanism);
                }
            } else {
                throw new RuntimeException("Expected an instance of ArrayList, but received " + o);
            }
    ...
    static class TestCallbackHandler implements CallbackHandler {
        ...
        TestCallbackHandler(String userId, String passwd, String realm, String authId) {
            this.userId = userId;
