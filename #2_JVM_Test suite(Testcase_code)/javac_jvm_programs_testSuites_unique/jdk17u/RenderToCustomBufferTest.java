...
public class RenderToCustomBufferTest {
    ...
    public static void main(String[] args) {
        ...
        final BufferedImage dst_dcm = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        renderTo(dst_custom);
        renderTo(dst_dcm);
    ...
    private static void check(BufferedImage a, BufferedImage b) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                int pb = b.getRGB(x, y);
                if (pa != pb) {
                    String msg = String.format("Point [%d, %d] has different colors: %08X and %08X", x, y, pa, pb);
