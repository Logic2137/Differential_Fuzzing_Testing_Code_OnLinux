...
public class CustomLevel extends Level {
    ...
    private static class CustomLevelReference extends WeakReference<Level> {
        ...
        public String toString() {
            return "CustomLevelReference(\"" + name + "\", " + value + ", \"" + resourceBundleName + "\")";
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (Level level : levels) {
            ...
            } else if (l != Level.WARNING && l != Level.INFO && l != Level.SEVERE || !name.equals(l.getName())) {
            ...
            if (l != level) {
                throw new RuntimeException("Unexpected level " + l + " " + l.getClass() + " for " + localizedName + " in " + rb.getBaseBundleName());
            }
            ...
            if (l != level) {
                if (l == null || l.intValue() != level.intValue()) {
                    throw new RuntimeException("Unexpected level " + l + (l == null ? "" : (" " + l.getClass())) + " for " + level.intValue());
                }
            }
            ...
            if (l2 != level) {
                if (l2 == null || l2.intValue() != level.intValue()) {
                    throw new RuntimeException("Unexpected level " + l2 + (l2 == null ? "" : (" " + l2.getClass())) + " for " + level.intValue());
                }
            }
        }
        final long otherLevelCount = levels.stream().filter(CustomLevel::isCustomLoader).count();
        ...
        while (!levels.isEmpty()) {
            ...
            if (otherLevels == otherLevelCount) {
                if (customRefs.size() != otherLevelCount) {
                    throw new RuntimeException("Test bug: customRefs.size() != " + otherLevelCount);
                }
                waitForGC(customRefs, queue);
            }
        }
        if (otherLevelCount != otherLevels || otherLevelCount == 0) {
            throw new RuntimeException("Test bug: " + "no or wrong count of levels loaded from custom loader");
        }
        if (!customRefs.isEmpty()) {
            throw new RuntimeException("Test bug: customRefs.size() should be empty!");
        }
        while (!refs.isEmpty()) {
            final Reference<?> ref = refs.remove(0);
            if (ref.get() == null) {
                throw new RuntimeException("Unexpected garbage collection for " + ref);
            }
        }
    }

    private static void waitForGC(List<CustomLevelReference> customRefs, ReferenceQueue<Level> queue) throws InterruptedException {
    ...
    private static boolean isCustomLoader(Level level) {
        ...
        return cl != null && cl != ClassLoader.getPlatformClassLoader() && cl != ClassLoader.getSystemClassLoader();
    }

    static ResourceBundle getResourceBundle(Level level) {
        return isCustomLoader(level) ? ResourceBundle.getBundle(OTHERRB_NAME, Locale.getDefault(), level.getClass().getClassLoader()) : ResourceBundle.getBundle(RB_NAME);
    }
    ...
    static void setUpCustomLevelsOtherLoader() throws MalformedURLException, ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        ...
        URLClassLoader loader = new URLClassLoader(new URL[] { curl, surl }, ClassLoader.getPlatformClassLoader());
