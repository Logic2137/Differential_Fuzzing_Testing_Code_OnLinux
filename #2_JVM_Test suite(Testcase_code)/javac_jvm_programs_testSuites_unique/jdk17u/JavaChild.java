...
public class JavaChild extends Process {
    ...
    private static final Set<JavaChild> completedChildren = Collections.synchronizedSet(new HashSet<>());
    ...
    static final String javaHome = (System.getProperty("test.jdk") != null) ? System.getProperty("test.jdk") : System.getProperty("java.home");

    static final String javaExe = javaHome + File.separator + "bin" + File.separator + "java";

    static final String classpath = System.getProperty("java.class.path");

    static final List<String> javaChildArgs = Arrays.asList(javaExe, "-XX:+DisplayVMOutputToStderr", "-Dtest.jdk=" + javaHome, "-classpath", absolutifyPath(classpath), "JavaChild");
            ...
            while (nextArg < args.length) {
                ...
                switch(action) {
                    ...
                    case "pid":
                        ...
                        break;
                    case "exit":
                        int exitValue = (nextArg < args.length) ? Integer.valueOf(args[nextArg]) : 0;
                    ...
                    case "spawn":
                        {
                            if (args.length - nextArg < 2) {
                                throw new RuntimeException("not enough args for respawn: " + (args.length - 2));
                            }
                    ...
                    case "child":
                        {
                            int sentCount = 0;
                            Object[] result = Arrays.copyOfRange(args, nextArg - 1, args.length);
                            Object[] subargs = Arrays.copyOfRange(args, nextArg + 1, args.length);
                            for (JavaChild p : children) {
                                if (p.isAlive()) {
                                    sentCount++;
