...
public class LotsOfCancels {
    ...
    static void closeAll(List<SocketChannel> channels) throws Exception {
        for (SocketChannel channel : channels) {
            channel.close();
        }
    }
            ...
            while (true) {
                ...
                synchronized (this) {
                    if (connectionsNeeded > 0) {
                        ...
                        log("Started total of " + totalCreated + " client connections");
                        Thread.sleep(200);
                    }
                }
                if (createdCount > 0) {
                    ...
                    selector.select();
                    long duration = durationMillis(startTime);
                    log("Exited clientSelector.select(), loop #" + selectCount + ", duration = " + duration + "ms");
                }
                int keyCount = -1;
                ...
                while (keys.hasNext()) {
                    ...
                    synchronized (key) {
                        ...
                        if (readyOps == SelectionKey.OP_CONNECT) {
                            ...
                            log("readyOps() on client key #" + keyCount + " returned " + readyOps);
                        }
                    }
                }
            }
