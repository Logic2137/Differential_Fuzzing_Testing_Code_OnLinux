...
public class ReadUnknownTagsTest {
    ...
    private final static int DESCRIPTION_TAG = BaselineTIFFTagSet.TAG_IMAGE_DESCRIPTION;
    ...
    public ReadUnknownTagsTest(boolean ignoreMetadata, boolean readUnknownTags) {
    ...
    private void writeImage() throws Exception {
        ...
        try (ImageOutputStream ios = ImageIO.createImageOutputStream(s)) {
            ...
            IIOMetadata md = writer.getDefaultImageMetadata(new ImageTypeSpecifier(img), param);
            TIFFDirectory dir = TIFFDirectory.createFromMetadata(md);
            TIFFTag descTag = BaselineTIFFTagSet.getInstance().getTag(DESCRIPTION_TAG);
            dir.addTIFFField(new TIFFField(descTag, TIFFTag.TIFF_ASCII, 1, new String[] { DESCRIPTION }));
    ...
    private void readAndCheckImage() throws Exception {
        ...
        if (param.getReadUnknownTags() != readUnknownTags) {
            throw new RuntimeException("Incorrect readUnknownTags setting " + "\"" + readUnknownTags + "\"");
        }
        ...
        if (!dir.containsTIFFField(BaselineTIFFTagSet.TAG_IMAGE_WIDTH)) {
            ...
            TIFFField widthField = dir.getTIFFField(BaselineTIFFTagSet.TAG_IMAGE_WIDTH);
            System.out.printf("ImageWidth: %d%n", widthField.getAsLong(0));
        }
        boolean hasDescription = dir.containsTIFFField(DESCRIPTION_TAG);
        System.out.println("ImageDescription (" + !ignoreMetadata + "): " + hasDescription);
        if (ignoreMetadata && hasDescription) {
            System.err.println("Description metadata present despite ignoreMetadata");
            failures++;
        } else if (!ignoreMetadata && !hasDescription) {
            System.err.println("Description metadata absent despite !ignoreMetadata");
            failures++;
        }
        boolean shouldHaveFaxField = !ignoreMetadata && readUnknownTags;
        boolean hasFaxField = dir.containsTIFFField(FAX_TAG);
        System.out.println("CleanFaxData (" + shouldHaveFaxField + "): " + hasFaxField);
        if (ignoreMetadata) {
            if (hasFaxField) {
                System.err.println("Fax metadata present despite ignoreMetadata");
                failures++;
            }
        } else {
            if (!readUnknownTags && hasFaxField) {
                System.err.println("Fax metadata present despite !readUnknownTags");
                failures++;
            } else if (readUnknownTags && !hasFaxField) {
                System.err.println("Fax metadata absent despite readUnknownTags");
                failures++;
            }
        }
        if (failures > 0) {
            throw new RuntimeException("Test failed for ignoreMetadata " + ignoreMetadata + " and readUnknownTags " + readUnknownTags);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (boolean ignoreMetadata : new boolean[] { false, true }) {
            for (boolean readUnknownTags : new boolean[] { false, true }) {
                try {
                    System.out.printf("ignoreMetadata: %s, readUnknownTags: %s%n", ignoreMetadata, readUnknownTags);
                    (new ReadUnknownTagsTest(ignoreMetadata, readUnknownTags)).run();
                } catch (Exception e) {
                    e.printStackTrace();
                    failures++;
                } finally {
