...
public class BasicCancelTest {
        ...
        try {
            if (new Random().nextBoolean()) {
                ...
                if (es instanceof ThreadPoolExecutor)
                    check(((ThreadPoolExecutor) es).isTerminating() || es.isTerminated());
                THROWS(RejectedExecutionException.class, new F() {
    ...
    void test(String[] args) throws Throwable {
        ...
        for (long i = 0; i < moreThanYouCanChew; i++) pool.schedule(noopTask, 10, TimeUnit.MINUTES).cancel(true);
        pool.shutdown();
