...
public class ChorusLine {
    ...
    private static final Tweaker[] tweakers = { new Tweaker() {

        public void run(Deque<Integer> deq) {
            for (int i = 0; i < 7; i++) deq.addLast(i);
            deq.removeFirst();
            deq.removeFirst();
            deq.addLast(7);
            deq.addLast(8);
    ...
    private static void realMain(String[] args) throws Throwable {
        ...
        for (Tweaker tweaker : tweakers) {
            for (Deque<Integer> deq : deqs) tweaker.run(deq);
            equal(deqs);
        }
    }
    ...
    private static void equal(Deque<Iterator<Integer>> its) {
        ...
        while (it0.hasNext()) {
            Integer i = it0.next();
            for (Iterator<Integer> it : its) equal(it.next(), i);
        }
