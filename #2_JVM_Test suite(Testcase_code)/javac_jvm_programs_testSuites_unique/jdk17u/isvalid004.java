...
public class isvalid004 {
    ...
    public static int run(String[] argv, PrintStream out) {
            ...
            try {
                ...
                for (; ; ) {
                    try {
                        ...
                        try {
                            argument = (Connector.IntegerArgument) defaultArguments.get(argName);
                            try {
        ...
        if (argument.isValid(intArgument.stringValueOf(intArgument.min()))) {
            ...
            exitCode = exitCode2;
            out.println(sErr2 + "check: super.isValid(stringValueOf(min()))\n" + "result: false\n");
        }
        if (argument.isValid(intArgument.stringValueOf(intArgument.max()))) {
        } else {
            exitCode = exitCode2;
            out.println(sErr2 + "check: super.isValid(stringValueOf(max()))\n" + "result: false\n");
        }
        if (intArgument.min() < intArgument.max()) {
            if (argument.isValid(intArgument.stringValueOf(intArgument.min() + 1))) {
            } else {
                exitCode = exitCode2;
                out.println(sErr2 + "check: super.isValid(stringValueOf(min()+1))\n" + "result: false\n");
            }
        }
        ...
        if (intArgument.max() < intI.MAX_VALUE) {
            if (!argument.isValid(intArgument.stringValueOf(intArgument.max() + 1))) {
            } else {
                exitCode = exitCode2;
                out.println(sErr2 + "check: super.isValid(stringValueOf(max()+1))\n" + "result: true\n");
            }
        }
