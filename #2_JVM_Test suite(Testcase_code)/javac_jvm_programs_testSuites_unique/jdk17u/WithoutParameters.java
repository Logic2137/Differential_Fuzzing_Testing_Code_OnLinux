...
public class WithoutParameters {
    ...
    void runTest(Executable e) throws Exception {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (epi != null) {
                ...
                errorIfTrue(!p.getType().equals(expectedParameterType), "Wrong parameter type for " + p + ": expected " + expectedParameterType + ", but got " + p.getType());
                ...
                if (expectedParameterizedTypes.length > 0) {
                    ...
                    errorIfTrue(!expectedParameterziedTypeType.isAssignableFrom(parameterizedType.getClass()), "Wrong class of parameteried type of " + p + ": expected " + expectedParameterziedTypeType + ", but got " + parameterizedType.getClass());
                    if (expectedParameterziedTypeType.equals(Class.class)) {
                        errorIfTrue(!parameterizedType.equals(expectedParameterType), "Wrong parameteried type for " + p + ": expected " + expectedParameterType + ", but got " + parameterizedType);
                    } else {
                        if (expectedParameterziedTypeType.equals(ParameterizedType.class)) {
                            ...
                            errorIfTrue(!ptype.getRawType().equals(expectedParameterType), "Wrong raw type for " + p + ": expected " + expectedParameterType + ", but got " + ptype.getRawType());
                        }
                        String expectedStringOfType = epi.parameterizedTypes()[i].string();
                        errorIfTrue(!expectedStringOfType.equals(parameterizedType.toString()), "Bad type string" + p + ": expected " + expectedStringOfType + ", but got " + parameterizedType.toString());
                    }
                }
            }
        }
    ...
    public class Foo {
        ...
        public class Inner {
            ...
            @ExpectedParameterInfo(parameterCount = 2, parameterTypes = { Foo.class, int.class })
