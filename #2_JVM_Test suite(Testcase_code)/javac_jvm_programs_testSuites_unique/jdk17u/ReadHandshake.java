...
public class ReadHandshake {
    ...
    private final static String[] CLIENT_SUITES = new String[] { "SSL_DH_anon_WITH_3DES_EDE_CBC_SHA" };

    private final static String[] SERVER_SUITES = new String[] { "SSL_DH_anon_WITH_RC4_128_MD5" };
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                try {
                    int ch;
                    if ((ch = sslIS.read()) != -1) {
                        throw new Exception("Read succeeded server side: " + ch);
                    }
                } catch (IOException e) {
