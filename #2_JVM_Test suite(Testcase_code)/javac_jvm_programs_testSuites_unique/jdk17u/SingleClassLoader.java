...
public class SingleClassLoader extends ClassLoader {
    ...
    SingleClassLoader(String singleClassName, Class superclass, ClassLoader parent) {
        super(parent);
        Constructor superConstr;
        try {
            superConstr = superclass.getDeclaredConstructor(new Class[0]);
        } catch (NoSuchMethodException e) {
            throw new IllegalArgumentException("Superclass must have no-arg " + "constructor");
        }
        int superConstrMods = superConstr.getModifiers();
        if ((superConstrMods & (Modifier.PUBLIC | Modifier.PROTECTED)) == 0) {
            final String msg = "Superclass no-arg constructor must be public or protected";
    ...
    private Class makeClass(Class superclass) throws IOException {
        ...
        int superConstructorNameAndTypeConst = writeNameAndTypeConst(initNameConst, voidNoArgSigConst);
        int superConstructorMethodRefConst = writeMethodRefConst(superClassConst, superConstructorNameAndTypeConst);
        ...
        dout = null;
        return defineClass(singleClassName, classBytes, 0, classBytes.length);
    }
    ...
    private int writeNameAndTypeConst(int nameIndex, int typeIndex) throws IOException {
        dout.writeByte(12);
        dout.writeShort((short) nameIndex);
    ...
    private int writeMethodRefConst(int classIndex, int nameAndTypeIndex) throws IOException {
        dout.writeByte(10);
