...
public class AISReadFraction {
    ...
    static AudioFormat[] formats = { new AudioFormat(44100.0f, 8, 1, false, false), new AudioFormat(44100.0f, 8, 2, false, false), new AudioFormat(44100.0f, 16, 1, true, false), new AudioFormat(44100.0f, 24, 1, true, false), new AudioFormat(44100.0f, 16, 2, true, false), new AudioFormat(44100.0f, 8, 5, false, false), new AudioFormat(44100.0f, 16, 3, true, false), new AudioFormat(44100.0f, 8, 7, false, false), new AudioFormat(44100.0f, 32, 2, true, false) };
    ...
    static void doTest(AudioFormat format, boolean doMark) {
        out("Test with" + (doMark ? "" : "out") + " marking. Audio format: " + "sampleSize=" + format.getSampleSizeInBits() + "bits " + "channels=" + format.getChannels() + " " + "frameSize=" + format.getFrameSize() + "byte(s)");
        ...
        while (true) {
            try {
                ...
                if (DEBUG)
                    out("  -> ais.read(data, 0, " + toBeRead + "): " + read + "  (frameSize=" + frameSize + ")");
                if ((totalReadBytes == maxReadBytes) && (read != -1) && ((read > 0) || hasRead0)) {
                    if (read == 0) {
    ...
    static class FractionalIS extends InputStream {
        ...
        public int read(byte[] b, int off, int len) throws IOException {
            ...
            if (reducedLen <= 0)
                reducedLen = 1;
            if (DEBUG)
                out("  FIS.read(data, 0, " + len + "): reducing len to " + reducedLen + " bytes.");
