...
public class TIFFTagSetTest {
    ...
    public static class TestSet extends TIFFTagSet {
        ...
        public static final TIFFTagSet SOME_SET = new TIFFTagSet(new ArrayList<TIFFTag>());
        ...
        static class Tag1 extends TIFFTag {
            ...
            public Tag1() {
                super(TAG_NAME_1, TAG_NUM_1, 1 << TIFF_SHORT | 1 << TIFF_LONG, VALUE_COUNT);
            }
        }
        ...
        static class Tag2 extends TIFFTag {
            ...
            public Tag2() {
                super(TAG_NAME_2, TAG_NUM_2, 1 << TIFF_DOUBLE);
        ...
        static class Tag3 extends TIFFTag {
            ...
            public Tag3() {
                super(TAG_NAME_3, TAG_NUM_3, SOME_SET);
            }
        }
    ...
    private static void checkEq(String what, Object v, Object ref) {
        if (v == null) {
            ...
            throw new RuntimeException("invalid " + what + ", expected: " + ref + ", got: " + v);
        }
    }

    private final String className;

    public TIFFTagSetTest(String cName) {
        className = cName;
    }
    ...
    public void testNamesNumbers() throws ReflectiveOperationException {
        ...
        if (nTagNames != tagNumbers.size()) {
            throw new RuntimeException("Error: unequal sizes for tag names set " + "and tag numbers set");
        }
        ...
        for (final Iterator itName = tagNames.iterator(); itName.hasNext(); ) {
            ...
            System.out.println("name/number: \t" + tagName + "/" + tagByName.getNumber());
            ...
            if (tagByName.isIFDPointer() && !tagByName.isDataTypeOK(TIFFTag.TIFF_IFD_POINTER)) {
                throw new RuntimeException("Error: " + tagName + "must be an IFD pointer");
            }
        }
        System.out.println("");
        for (final Iterator itNum = tagNumbers.iterator(); itNum.hasNext(); ) {
            ...
            System.out.println("number/name: \t" + tagNum + "/" + tagByNum.getName());
    ...
    private static void testUserDefTagSet() {
        ...
        if (!(tagNames.contains(TestSet.TAG_NAME_1) && tagNames.contains(TestSet.TAG_NAME_2) && tagNames.contains(TestSet.TAG_NAME_3))) {
        ...
        if (!(tagNumbers.contains(TestSet.TAG_NUM_1) && tagNumbers.contains(TestSet.TAG_NUM_2) && tagNumbers.contains(TestSet.TAG_NUM_3))) {
            throw new RuntimeException("invalid tag numbers");
        }
        TIFFTag t1 = set.getTag(TestSet.TAG_NUM_1), t2 = set.getTag(TestSet.TAG_NUM_2), t3 = set.getTag(TestSet.TAG_NUM_3);
        ...
        checkEq(TestSet.TAG_NAME_1 + " count", t1.getCount(), TestSet.VALUE_COUNT);
        checkEq(TestSet.TAG_NAME_2 + " count", t2.getCount(), -1);
        checkEq(TestSet.TAG_NAME_3 + " count", t3.getCount(), 1);
        checkEq(TestSet.TAG_NAME_1 + " datatypes", t1.getDataTypes(), 1 << TIFFTag.TIFF_SHORT | 1 << TIFFTag.TIFF_LONG);
        boolean ok = t1.isDataTypeOK(TIFFTag.TIFF_SHORT) && t1.isDataTypeOK(TIFFTag.TIFF_LONG) && !t1.isDataTypeOK(TIFFTag.TIFF_DOUBLE) && !t1.isDataTypeOK(TIFFTag.TIFF_IFD_POINTER);
        if (!ok) {
            throw new RuntimeException(TestSet.TAG_NAME_1 + ": " + "isDataTypeOK check failed");
        }
        checkEq(TestSet.TAG_NAME_1 + ".isIFDPointer()", t1.isIFDPointer(), false);
        checkEq(TestSet.TAG_NAME_2 + " datatypes", t2.getDataTypes(), 1 << TIFFTag.TIFF_DOUBLE);
        ok = !t2.isDataTypeOK(TIFFTag.TIFF_SHORT) && !t2.isDataTypeOK(TIFFTag.TIFF_LONG) && t2.isDataTypeOK(TIFFTag.TIFF_DOUBLE) && !t2.isDataTypeOK(TIFFTag.TIFF_IFD_POINTER);
        if (!ok) {
            throw new RuntimeException(TestSet.TAG_NAME_2 + ": " + "isDataTypeOK check failed");
        }
        checkEq(TestSet.TAG_NAME_2 + ".isIFDPointer()", t2.isIFDPointer(), false);
        checkEq(TestSet.TAG_NAME_3 + " datatypes", t3.getDataTypes(), 1 << TIFFTag.TIFF_LONG | 1 << TIFFTag.TIFF_IFD_POINTER);
        ok = !t3.isDataTypeOK(TIFFTag.TIFF_SHORT) && t3.isDataTypeOK(TIFFTag.TIFF_LONG) && !t3.isDataTypeOK(TIFFTag.TIFF_DOUBLE) && t3.isDataTypeOK(TIFFTag.TIFF_IFD_POINTER);
        if (!ok) {
            throw new RuntimeException(TestSet.TAG_NAME_3 + ": " + "isDataTypeOK check failed");
        }
        checkEq(TestSet.TAG_NAME_3 + ".isIFDPointer()", t3.isIFDPointer(), true);
        checkEq(TestSet.TAG_NAME_1 + ".hasValueNames()", t1.hasValueNames(), false);
        checkEq(TestSet.TAG_NAME_2 + ".hasValueNames()", t2.hasValueNames(), true);
        checkEq(TestSet.TAG_NAME_3 + ".hasValueNames()", t3.hasValueNames(), false);
        if (t1.getNamedValues() != null && t3.getNamedValues() != null) {
            throw new RuntimeException(TestSet.TAG_NAME_1 + " and " + TestSet.TAG_NAME_3 + " must have null value names arrays");
        }
        checkEq("number of " + TestSet.TAG_NAME_2 + " values", t2.getNamedValues().length, 2);
        checkEq("name of value " + TestSet.VALUE_1, t2.getValueName(TestSet.VALUE_1), TestSet.VALUE_NAME_1);
        checkEq("name of value " + TestSet.VALUE_2, t2.getValueName(TestSet.VALUE_2), TestSet.VALUE_NAME_2);
        if (!(t1.getTagSet() == null && t2.getTagSet() == null) && t3.getTagSet().equals(TestSet.SOME_SET)) {
    ...
    private static void checkArgs() {
        ...
        if (!ok) {
            throw new RuntimeException("missing data types check for getSizeOfType()");
        }
        ok = false;
        try {
    ...
    public static void main(String[] args) throws ReflectiveOperationException {
        String[] classNames = { "BaselineTIFFTagSet", "ExifGPSTagSet", "ExifInteroperabilityTagSet", "ExifParentTIFFTagSet", "ExifTIFFTagSet", "FaxTIFFTagSet", "GeoTIFFTagSet" };
        for (String cn : classNames) {
            System.out.println("Testing " + cn + ":");
            (new TIFFTagSetTest("javax.imageio.plugins.tiff." + cn)).testNamesNumbers();
        }
