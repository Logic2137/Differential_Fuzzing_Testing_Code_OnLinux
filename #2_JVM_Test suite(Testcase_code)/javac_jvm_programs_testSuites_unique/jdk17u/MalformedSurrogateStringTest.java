...
public class MalformedSurrogateStringTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (fe.equalsIgnoreCase("UTF8") || fe.equalsIgnoreCase("UTF-8") || fe.equalsIgnoreCase("UTF_8"))
            return;
        ...
        for (int i = 0; i < b.length; i++) System.err.println("[" + i + "]" + "=" + (char) b[i] + "=" + (int) b[i]);
        if (b.length != 7) {
        ...
        for (int i = 0; i < b2.length; i++) System.err.println("[" + i + "]" + "=" + (char) b2[i]);
        if (b2.length != 8) {
