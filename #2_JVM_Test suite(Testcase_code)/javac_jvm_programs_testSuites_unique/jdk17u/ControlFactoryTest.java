...
public class ControlFactoryTest {
    ...
    private static void testControlFactory(Factory factory, Locale loc) {
        ...
        String[][] data = { { "java.class", "java.properties", "java.xml" }, { "java.class", "java.props" }, { "java.properties", "java.class" }, { "java.foo", "java.properties" }, { "java.foo" }, { null } };
    ...
    private static void testGetControl(Factory factory, Locale loc, final List<String> FORMATS, String... fmtStrings) {
        final ResourceBundle.Control CONTROL = factory.getControl(FORMATS);
        List<String> fmt = CONTROL.getFormats("any");
        if (fmt != FORMATS) {
            error("%s: returns %s, expected %s.%n", factory.name(), fmt, FORMATS);
        }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (control != CONTROL) {
                error("%s: doesn't return the singleton: got %s, expected %s%n", factory.name(), control, CONTROL);
            }
        }
        ...
        if (loc != null) {
            ...
            if (!defaultLocale.equals(l)) {
                error("%s: getFallbackLocale doesn't return default locale. got %s, expected %s%n", factory.name(), toString(l), toString(defaultLocale));
            }
            l = CONTROL.getFallbackLocale("any", defaultLocale);
            if (l != null) {
                error("%s: getFallbackLocale doesn't return null. got %s%n", factory.name(), toString(l));
            }
        } else {
            Locale l = CONTROL.getFallbackLocale("any", nonDefaultLocale);
            if (l != null) {
