...
public class BrokenConnectionTest {
    ...
    private static interface Breakable {
        ...
        public JMXConnectorServer createConnectorServer(MBeanServer mbs) throws IOException;
    ...
    private static interface TestAction {
        ...
        public boolean test(MBeanServerConnection mbsc, Breakable breakable) throws Exception;
    }
    ...
    private static abstract class Operation implements TestAction {
        ...
        public boolean test(MBeanServerConnection mbsc, Breakable breakable) throws Exception {
            ...
            try {
                operation(mbsc);
                System.out.println("TEST FAILED: " + opName() + " should fail!");
                return false;
            } catch (IOException e) {
    ...
    private static TestAction[] tests = { new Operation() {
        ...
        void operation(MBeanServerConnection mbsc) throws Exception {
            mbsc.addNotificationListener(DELEGATE_NAME, new CountListener(), null, null);
        }
    }, new Operation() {
        ...
        void init(MBeanServerConnection mbsc) throws Exception {
            mbsc.createMBean(CountListener.class.getName(), LISTENER_NAME);
        }

        void operation(MBeanServerConnection mbsc) throws Exception {
            mbsc.addNotificationListener(DELEGATE_NAME, LISTENER_NAME, null, null);
        }
    }, new Operation() {
        ...
        void operation(MBeanServerConnection mbsc) throws Exception {
            NotificationListener l = (NotificationListener) listeners.remove(0);
        ...
        void operation(MBeanServerConnection mbsc) throws Exception {
            try {
                mbsc.removeNotificationListener(DELEGATE_NAME, LISTENER_NAME, null, null);
                throw new IllegalArgumentException("removeNL should not " + "have worked!");
        ...
        void operation(MBeanServerConnection mbsc) throws Exception {
            ObjectName name = new ObjectName("test:instance=" + nextInstance());
        ...
        void operation(MBeanServerConnection mbsc) throws Exception {
            mbsc.invoke(BREAK_NAME, "doNothing", new Object[0], new String[0]);
        }
    }, new Operation() {
        ...
        void operation(MBeanServerConnection mbsc) throws Exception {
            mbsc.setAttribute(BREAK_NAME, new Attribute("Nothing", null));
        }
    }, new Operation() {
        ...
        public boolean test(MBeanServerConnection mbsc, Breakable breakable) throws Exception {
            Attribute attr = new Attribute("Break", new BreakWhenSerialized(breakable));
            try {
                mbsc.setAttribute(BREAK_NAME, attr);
                System.out.println("TEST FAILED: setAttribute with " + "BreakWhenSerializable did not fail!");
                return false;
            } catch (IOException e) {
                System.out.println("Got IOException as expected: " + e);
                return true;
        ...
        public boolean test(MBeanServerConnection mbsc, Breakable breakable) throws Exception {
            try {
                ...
                System.out.println("TEST FAILED: getAttribute of " + "BreakWhenSerializable did not fail!");
                return false;
            } catch (IOException e) {
                System.out.println("Got IOException as expected: " + e);
                return true;
    ...
    public static class Break extends NotificationBroadcasterSupport implements BreakMBean {
        ...
        public void setBreak(BreakWhenSerialized x) {
            throw new IllegalArgumentException("setBreak worked but " + "should not!");
        }
    ...
    private static class FailureNotificationFilter implements NotificationFilter {
    ...
    public static class CountListener implements CountListenerMBean, NotificationListener {
    ...
    private static boolean test(Breakable breakable) throws Exception {
        ...
        for (int i = 1; i <= tests.length; i++) {
            ...
            cc.addConnectionNotificationListener(failureListener, failureFilter, null);
            ...
            if (thisok) {
                ...
                if (failureListener.count > 1) {
                    System.out.println("Got too many failure notifs: " + failureListener.count);
                    thisok = false;
                }
            }
            if (!thisok)
    ...
    private static class BreakableRMI implements Breakable {
        ...
        public JMXConnectorServer createConnectorServer(MBeanServer mbs) throws IOException {
            ...
            Map env = new HashMap();
            env.put(RMIConnectorServer.RMI_SERVER_SOCKET_FACTORY_ATTRIBUTE, brssf);
        ...
        private final BreakableRMIServerSocketFactory brssf = new BreakableRMIServerSocketFactory();
    }
    ...
    private static class BreakableRMIServerSocketFactory implements RMIServerSocketFactory {
        ...
        private final List bssList = new ArrayList();
    ...
    private static class BreakableJMXMP implements Breakable {
        ...
        public JMXConnectorServer createConnectorServer(MBeanServer mbs) throws IOException {
            try {
                InvocationHandler scsih = new SocketConnectionServerInvocationHandler(bss);
                final String mcs = "javax.management.remote.generic.MessageConnectionServer";
                final Class messageConnectionServerClass = Class.forName(mcs);
                final Class[] proxyInterfaces = { messageConnectionServerClass };
                Object socketConnectionServer = Proxy.newProxyInstance(this.getClass().getClassLoader(), proxyInterfaces, scsih);
                Map env = new HashMap();
                env.put("jmx.remote.message.connection.server", socketConnectionServer);
                final String gcs = "javax.management.remote.generic.GenericConnectorServer";
                final Class genericConnectorServerClass = Class.forName(gcs);
                final Class[] constrTypes = { Map.class, MBeanServer.class };
                final Constructor constr = genericConnectorServerClass.getConstructor(constrTypes);
                JMXConnectorServer cs = (JMXConnectorServer) constr.newInstance(new Object[] { env, mbs });
                cs.start();
                return cs;
            } catch (Exception e) {
                e.printStackTrace(System.out);
    ...
    private static class BreakableServerSocket extends ServerSocket {
        ...
        private final List sList = new ArrayList();

        private boolean broken = false;
    }

    private static class SocketConnectionServerInvocationHandler implements InvocationHandler {
        ...
        private Object accept() throws Exception {
            System.out.println("SCSIH.accept()");
            Socket s = ss.accept();
            Class socketConnectionClass = Class.forName("com.sun.jmx.remote.socket.SocketConnection");
            Constructor constr = socketConnectionClass.getConstructor(new Class[] { Socket.class });
