...
public class ISO8859x {
    ...
    private static void testCharset(Charset cs) throws Throwable {
        ...
        decoder.onUnmappableCharacter(CodingErrorAction.REPORT).onMalformedInput(CodingErrorAction.REPORT);
        encoder.onUnmappableCharacter(CodingErrorAction.REPORT).onMalformedInput(CodingErrorAction.REPORT);
        ...
        for (int c = 0xa0; c < 0x100; c++) {
            try {
                ...
                char[] chars;
                try {
                    chars = decoder.decode(ByteBuffer.wrap(bytes)).array();
