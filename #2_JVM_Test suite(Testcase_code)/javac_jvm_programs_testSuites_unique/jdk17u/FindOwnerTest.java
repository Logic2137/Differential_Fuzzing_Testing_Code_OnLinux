...
public class FindOwnerTest extends Applet {
    ...
    public void start() {
        ...
        try {
            ...
            JOptionPane optionPane = new JOptionPane("You are in the browser so test is manual. Try to " + "click \"JTextField\" in the opened window then press OK " + "below", JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION);
            JDialog dialog = optionPane.createDialog(null, "FindOwnerTest instruction");
            dialog.setModalityType(Dialog.ModalityType.DOCUMENT_MODAL);
            dialog.setVisible(true);
            if (!gained) {
                throw new RuntimeException("Focus is not gained upon mouse click");
            }
            System.out.println("ok");
        } catch (Exception e) {
