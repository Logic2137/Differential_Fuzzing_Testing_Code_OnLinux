...
public class CreateVM002_TranspServ extends TransportService {
    ...
    void handshake(Socket s, long timeout) throws IOException {
        ...
        while (received < hello.length) {
            ...
            if (n < 0) {
                s.close();
                throw new IOException("##> CreateVM002_TranspServ: Handshake FAILED - connection prematurally closed!");
            }
            received += n;
        }
        for (int i = 0; i < hello.length; i++) {
            if (b[i] != hello[i]) {
                throw new IOException("##> CreateVM002_TranspServ: Handshake FAILED - unrecognized message from target VM");
            }
        }
        s.setSoTimeout(0);
    }
    ...
    public Connection attach(String address, long attachTimeout, long handshakeTimeout) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException("##> CreateVM002_TranspServ: attach() - unable to parse port number in address");
        }
        ...
        try {
            ...
            throw new TransportTimeoutException("##> CreateVM002_TranspServ: attach() - timed out trying to establish connection");
        }
        try {
    ...
    public ListenKey startListening(String address) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException("##> CreateVM002_TranspServ: startListening() - unable to parse port number in address");
        }
    ...
    public void stopListening(ListenKey listener) throws IOException {
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException("##> CreateVM002_TranspServ: stopListening() - Invalid listener");
        }
    ...
    public Connection accept(ListenKey listener, long acceptTimeout, long handshakeTimeout) throws IOException {
        if (acceptTimeout < 0 || handshakeTimeout < 0) {
            throw new IllegalArgumentException("##> CreateVM002_TranspServ: accept() - timeout is negative");
        }
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException("##> CreateVM002_TranspServ: accept() - Invalid listener");
        }
        ServerSocket ss;
        synchronized (listener) {
        ...
        try {
            ...
            throw new TransportTimeoutException("##> CreateVM002_TranspServ: accept() - timeout waiting for connection");
        }
        handshake(s, handshakeTimeout);
        return new CreateVM002_Connection(s);
    }
...
class CreateVM002_Connection extends Connection {
    ...
    public byte[] readPacket() throws IOException {
        if (!isOpen()) {
            throw new ClosedConnectionException("##> CreateVM002_Connection: readPacket() - connection is closed: N1");
        }
        synchronized (receiveLock) {
            ...
            try {
                ...
                if (!isOpen()) {
                    throw new ClosedConnectionException("##> CreateVM002_Connection: readPacket() - connection is closed: N2");
                } else {
                    throw ioe;
                }
            }
            ...
            if (len < 0) {
                throw new IOException("##> CreateVM002_Connection: readPacket() - protocol error: invalid Packet's length");
            }
            byte[] b = new byte[len];
            b[0] = (byte) b1;
            b[1] = (byte) b2;
            ...
            while (len > 0) {
                ...
                try {
                    ...
                    if (!isOpen()) {
                        throw new ClosedConnectionException("##> CreateVM002_Connection: readPacket() - connection is closed: N3");
                    } else {
                        throw ioe;
                    }
                }
                if (count < 0) {
                    throw new EOFException("##> CreateVM002_Connection: readPacket() - read() method returns negative value");
                }
                len -= count;
                off += count;
            }
            ...
            if (readPacketRequestNumber == -1) {
                throw new IOException("Dummy IOException in CreateVM002_Connection.readPacket(); readPacketRequestNumber = " + readPacketRequestNumber);
            }
    ...
    public void writePacket(byte[] b) throws IOException {
        ...
        if (writePacketRequestNumber == 2) {
            wasIOException = true;
            throw new IOException("Dummy IOException in CreateVM002_Connection.writePacket(); writePacketRequestNumber = " + writePacketRequestNumber);
        }
        if (!isOpen()) {
            throw new ClosedConnectionException("##> CreateVM002_Connection: writePacket() - connection is closed: N1");
        }
        printPacket("writePacket:", b);
        if (b.length < 11) {
            throw new IllegalArgumentException("##> CreateVM002_Connection: writePacket() - packet is insufficient size: N1");
        }
        int b0 = b[0] & 0xff;
        int b1 = b[1] & 0xff;
        int b2 = b[2] & 0xff;
        ...
        if (len < 11) {
            throw new IllegalArgumentException("##> CreateVM002_Connection: writePacket() - packet is insufficient size: N2");
        }
        if (len > b.length) {
            throw new IllegalArgumentException("##> CreateVM002_Connection: writePacket() - length mis-match");
        }
        synchronized (sendLock) {
            try {
                ...
                if (!isOpen()) {
                    throw new ClosedConnectionException("##> CreateVM002_Connection: writePacket() - connection is closed: N2");
                } else {
                    throw ioe;
                }
            }
    ...
    public void toPrintPacket(boolean toPrint) {
        if (toPrintPacket) {
            if (!toPrint) {
                toPrintPacket = false;
                System.out.println("\n>>>> CreateVM002_Connection: toPrintPacket - Off! " + currentDateWithMlsecs());
            }
        } else {
            if (toPrint) {
                toPrintPacket = true;
                System.out.println("\n>>>> CreateVM002_Connection: toPrintPacket - On! " + currentDateWithMlsecs());
            }
        }
    }
    ...
    String currentDateWithMlsecs() {
        ...
        return "" + year + "." + strMonth + "." + strDay + "; " + strHours + ":" + strMinutes + ":" + strSeconds + "::" + strMlsecs;
    }
