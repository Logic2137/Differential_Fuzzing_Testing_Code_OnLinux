...
public class PBKDF2Translate {
    ...
    private static final String[] ALGO_TO_TEST = { "PBKDF2WithHmacSHA1", "PBKDF2WithHmacSHA224", "PBKDF2WithHmacSHA256", "PBKDF2WithHmacSHA384", "PBKDF2WithHmacSHA512" };
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String algo : ALGO_TO_TEST) {
            ...
            try {
                if (!theTest.testMyOwnSecretKey() || !theTest.generateAndTranslateKey() || !theTest.translateSpoiledKey()) {
                    failed = true;
                }
    ...
    public boolean testMyOwnSecretKey() throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException {
        SecretKey key1 = getSecretKeyForPBKDF2(algoToTest);
        SecretKey key2 = getMyOwnSecretKey();
        if (!Arrays.equals(key1.getEncoded(), key2.getEncoded())) {
            System.err.println("We shouldn't be here. The key1 and key2 values " + "in its primary encoding format have to be the same!");
            return false;
        }
        SecretKeyFactory skf = SecretKeyFactory.getInstance(algoToTest);
        SecretKey key3 = skf.translateKey(key2);
        if (!Arrays.equals(key1.getEncoded(), key3.getEncoded())) {
            System.err.println("testMyOwnSecretKey test case failed: the key1 " + "and key3 values in its primary encoding format are not " + "the same for " + algoToTest + "algorithm.");
            return false;
        }
        return true;
    }

    public boolean translateSpoiledKey() throws NoSuchAlgorithmException, InvalidKeySpecException {
    ...
    private SecretKey getMyOwnSecretKey() throws InvalidKeySpecException, NoSuchAlgorithmException {
        return new MyPBKDF2SecretKey(PASS_PHRASE, this.algoToTest, this.salt, ITERATION_COUNT, KEY_SIZE);
    }
}
...
class MyPBKDF2SecretKey implements PBEKey {
    ...
    public MyPBKDF2SecretKey(String passPhrase, String algo, byte[] salt, int iterationCount, int keySize) throws InvalidKeySpecException, NoSuchAlgorithmException {
        this.algorithm = algo;
        this.salt = salt;
        this.itereationCount = iterationCount;
        this.keySize = keySize;
        this.pass = passPhrase;
        PBEKeySpec spec = new PBEKeySpec(passPhrase.toCharArray(), this.salt, iterationCount, this.keySize);
        ...
        System.arraycopy(realKey.getEncoded(), 0, this.key, 0, this.keyLength);
    }
