...
public class InterruptedExceptionTest {
    ...
    enum SuppressLevel {
        ...
        String getSuppressAnno() {
            return this == SUPPRESS ? "@SuppressWarnings(\"try\")" : "";
        }
    }

    enum ClassKind {

        ABSTRACT_CLASS("abstract class", "implements", false), CLASS("class", "implements", true), INTERFACE("interface", "extends", false);
    ...
    enum ExceptionKind {
        ...
        NONE("", false), EXCEPTION("Exception", true), INTERRUPTED_EXCEPTION("InterruptedException", true), ILLEGAL_ARGUMENT_EXCEPTION("IllegalArgumentException", false), X("X", false);
    ...
    InterruptedExceptionTest(XlintOption xlint, SuppressLevel suppress_decl, SuppressLevel suppress_use, ClassKind ck, ExceptionKind ek_decl, ExceptionKind ek_use) {
        this.xlint = xlint;
    ...
    class JavaSource extends SimpleJavaFileObject {
        ...
        public JavaSource() {
            ...
            source = template.replace("#S1", suppress_decl.getSuppressAnno()).replace("#S2", suppress_use.getSuppressAnno()).replace("#CK", ck.kindName).replace("#EC", ck.extendsClause).replace("#G", ek_decl.getTypeParameter()).replace("#TK", ek_decl.getThrowsClause()).replace("#BK", ck.getBody()).replace("#PK", ek_use.getTypeArguments(ek_decl));
        }
    ...
    void check() {
        boolean shouldWarnDecl = ek_decl.shouldWarn && xlint == XlintOption.TRY && suppress_decl != SuppressLevel.SUPPRESS;
        boolean shouldWarnUse = (ek_decl.shouldWarn || ((ek_use.shouldWarn || ek_use == ExceptionKind.NONE) && ek_decl == ExceptionKind.X)) && xlint == XlintOption.TRY && suppress_use != SuppressLevel.SUPPRESS;
        int foundWarnings = 0;
        if (shouldWarnDecl)
            foundWarnings++;
        if (shouldWarnUse)
    ...
    static class DiagnosticChecker implements javax.tools.DiagnosticListener<JavaFileObject> {
        ...
        public void report(Diagnostic<? extends JavaFileObject> diagnostic) {
            if (diagnostic.getKind() == Diagnostic.Kind.WARNING && diagnostic.getCode().contains("try.resource.throws.interrupted.exc")) {
                tryWarnFound++;
            }
        }
    }
