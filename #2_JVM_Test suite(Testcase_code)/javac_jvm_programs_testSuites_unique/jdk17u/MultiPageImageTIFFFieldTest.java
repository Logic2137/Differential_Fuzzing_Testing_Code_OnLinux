...
public class MultiPageImageTIFFFieldTest {
    ...
    private final static int N_DESCRIPTION = BaselineTIFFTagSet.TAG_IMAGE_DESCRIPTION;
    ...
    private final static String[] GPS_DATA = { ExifGPSTagSet.STATUS_MEASUREMENT_IN_PROGRESS };

    private final static int N_GPS = ExifGPSTagSet.TAG_GPS_STATUS;

    private final static short FAX_DATA = FaxTIFFTagSet.CLEAN_FAX_DATA_ERRORS_UNCORRECTED;

    private final static int N_FAX = FaxTIFFTagSet.TAG_CLEAN_FAX_DATA;

    private static final byte[] ICC_PROFILE_2 = ICC_ProfileRGB.getInstance(ColorSpace.CS_sRGB).getData();
    ...
    private static final int COMPRESSION_1 = BaselineTIFFTagSet.COMPRESSION_DEFLATE, COMPRESSION_2 = BaselineTIFFTagSet.COMPRESSION_LZW;

    private static final int N_COMPRESSION = BaselineTIFFTagSet.TAG_COMPRESSION;

    private static final int GRAY_1 = BaselineTIFFTagSet.PHOTOMETRIC_INTERPRETATION_WHITE_IS_ZERO, GRAY_2 = BaselineTIFFTagSet.PHOTOMETRIC_INTERPRETATION_BLACK_IS_ZERO, RGB = BaselineTIFFTagSet.PHOTOMETRIC_INTERPRETATION_RGB;

    private static final int N_PHOTO = BaselineTIFFTagSet.TAG_PHOTOMETRIC_INTERPRETATION;

    private ImageWriter getTIFFWriter() {
        java.util.Iterator<ImageWriter> writers = ImageIO.getImageWritersByFormatName("TIFF");
        if (!writers.hasNext()) {
    ...
    private ImageReader getTIFFReader() {
        java.util.Iterator<ImageReader> readers = ImageIO.getImageReadersByFormatName("TIFF");
        if (!readers.hasNext()) {
    ...
    private void addASCIIField(TIFFDirectory d, String name, String[] data, int num) {
        d.addTIFFField(new TIFFField(new TIFFTag(name, num, 1 << TIFFTag.TIFF_ASCII), TIFFTag.TIFF_ASCII, data.length, data));
    }

    private void checkASCIIField(TIFFDirectory d, String what, String[] data, int num) {
        ...
        for (int i = 0; i < data.length; i++) {
            check(f.getValueAsString(i).equals(data[i]), "invalid " + what + " data");
        }
    }
    ...
    private void writeImage() throws Exception {
        ...
        try (ImageOutputStream ios = ImageIO.createImageOutputStream(s)) {
            ...
            BufferedImage img1 = new BufferedImage(W1, H1, BufferedImage.TYPE_BYTE_GRAY);
            ...
            IIOMetadata md1 = writer.getDefaultImageMetadata(new ImageTypeSpecifier(img1), param1), md2 = writer.getDefaultImageMetadata(new ImageTypeSpecifier(img2), param2);
            TIFFDirectory dir1 = TIFFDirectory.createFromMetadata(md1), dir2 = TIFFDirectory.createFromMetadata(md2);
            ...
            dir2.addTIFFField(new TIFFField(new TIFFTag("ICC Profile", N_ICC, 1 << TIFFTag.TIFF_UNDEFINED), TIFFTag.TIFF_UNDEFINED, ICC_PROFILE_2.length, ICC_PROFILE_2));
            writer.prepareWriteSequence(null);
            writer.writeToSequence(new IIOImage(img1, null, dir1.getAsMetadata()), param1);
            writer.writeToSequence(new IIOImage(img2, null, dir2.getAsMetadata()), param2);
    ...
    private void checkBufferedImages(BufferedImage im1, BufferedImage im2) {
        ...
        Color c1 = new Color(im1.getRGB(W1 / 2, H1 / 2)), c2 = new Color(im2.getRGB(W2 / 2, H2 / 2));
    ...
    private void readAndCheckImage() throws Exception {
        ...
        TIFFImageReadParam param1 = new TIFFImageReadParam(), param2 = new TIFFImageReadParam();
        ...
        BufferedImage bi1 = (BufferedImage) i1.getRenderedImage(), bi2 = (BufferedImage) i2.getRenderedImage();
        checkBufferedImages(bi1, bi2);
        TIFFDirectory dir1 = TIFFDirectory.createFromMetadata(i1.getMetadata()), dir2 = TIFFDirectory.createFromMetadata(i2.getMetadata());
        checkASCIIField(dir1, "image 1 description", DESCRIPTION_1, N_DESCRIPTION);
        checkASCIIField(dir2, "image 2 description", DESCRIPTION_2, N_DESCRIPTION);
        ...
        check((f.getCount() == 1) && (f.getAsInt(0) == W1), "invalid width field for image 1");
        f = dir2.getTIFFField(N_WIDTH);
        check((f.getCount() == 1) && (f.getAsInt(0) == W2), "invalid width field for image 2");
        f = dir1.getTIFFField(N_HEIGHT);
        check((f.getCount() == 1) && (f.getAsInt(0) == H1), "invalid height field for image 1");
        f = dir2.getTIFFField(N_HEIGHT);
        check((f.getCount() == 1) && (f.getAsInt(0) == H2), "invalid height field for image 2");
        check(dir1.containsTIFFField(N_FAX), "image 2 TIFF directory " + "must contain clean fax data");
        f = dir1.getTIFFField(N_FAX);
        check((f.getCount() == 1) && f.isIntegral() && (f.getAsInt(0) == FAX_DATA), "invalid clean fax data");
        check(!dir2.containsTIFFField(N_FAX), "image 2 TIFF directory " + "must not contain fax fields");
        checkASCIIField(dir1, "GPS status", GPS_DATA, N_GPS);
        check(!dir2.containsTIFFField(N_GPS), "image 2 TIFF directory " + "must not contain GPS fields");
        check(!dir1.containsTIFFField(N_ICC), "image 1 TIFF directory " + "must not contain ICC Profile field");
        check(dir2.containsTIFFField(N_ICC), "image 2 TIFF directory " + "must contain ICC Profile field");
        f = dir2.getTIFFField(N_ICC);
        check(f.getType() == TIFFTag.TIFF_UNDEFINED, "invalid ICC profile field type");
        int cnt = f.getCount();
        byte[] icc = f.getAsBytes();
        check((cnt == ICC_PROFILE_2.length) && (cnt == icc.length), "invalid ICC profile");
        for (int i = 0; i < cnt; i++) {
            check(icc[i] == ICC_PROFILE_2[i], "invalid ICC profile data");
        }
        check(dir1.getTIFFField(N_BPS).isIntegral() && dir2.getTIFFField(N_BPS).isIntegral(), "invalid bits per sample type");
        int[] sz1 = bi1.getColorModel().getComponentSize(), sz2 = bi2.getColorModel().getComponentSize(), bps1 = dir1.getTIFFField(N_BPS).getAsInts(), bps2 = dir2.getTIFFField(N_BPS).getAsInts();
        check((bps1.length == sz1.length) && (bps2.length == sz2.length), "invalid component size count");
        ...
        check(dir1.containsTIFFField(N_COMPRESSION) && dir2.containsTIFFField(N_COMPRESSION), "compression info lost");
        f = dir1.getTIFFField(N_COMPRESSION);
        check(f.isIntegral() && (f.getCount() == 1) && (f.getAsInt(0) == COMPRESSION_1), "invalid image 1 compression data");
        f = dir2.getTIFFField(N_COMPRESSION);
        check(f.isIntegral() && (f.getCount() == 1) && (f.getAsInt(0) == COMPRESSION_2), "invalid image 2 compression data");
        f = dir1.getTIFFField(N_PHOTO);
        check(f.isIntegral() && (f.getCount() == 1) && ((f.getAsInt(0) == GRAY_1) || (f.getAsInt(0) == GRAY_2)), "invalid photometric interpretation for image 1");
        f = dir2.getTIFFField(N_PHOTO);
        check(f.isIntegral() && (f.getCount() == 1) && (f.getAsInt(0) == RGB), "invalid photometric interpretation for image 2");
    }
