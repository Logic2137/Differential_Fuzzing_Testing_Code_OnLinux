...
public class TestJProgressBarHighlightColor {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            robot.waitForIdle();
            robot.delay(500);
            Color backgroundColor = robot.getPixelColor(point.x + rect.width * 3 / 4, point.y + rect.height / 2);
            robot.waitForIdle();
            robot.delay(500);
            Color highlightColor = robot.getPixelColor(point.x + rect.width / 4, point.y + rect.height / 2);
            robot.waitForIdle();
            robot.delay(500);
            int actualColorDifference = getMaxColorDiff(backgroundColor, highlightColor);
            if (actualColorDifference < minColorDifference) {
                throw new RuntimeException("The expected highlight color for " + "JProgressBar was not found");
            }
        } finally {
            if (frame != null) {
