...
public class JavatimeTest {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (ta.getTime() != inst0.toEpochMilli() || ta.getNanos() != inst0.getNano() || !ta.equals(Timestamp.from(inst0))) {
                System.out.printf("ms: %16d  ns: %10d  ldt:[%s]%n", millis, nanos, ldt);
            ...
            if (ta0.getTime() != inst.toEpochMilli() || ta0.getNanos() != inst.getNano() || !inst.equals(ta0.toInstant())) {
                System.out.printf("ms: %16d  ns: %10d  ldt:[%s]%n", millis, nanos, ldt);
    ...
    private static boolean isEqual(LocalDateTime ldt, Timestamp ts) {
        ...
        return zdt.getYear() == ts.getYear() + 1900 && zdt.getMonthValue() == ts.getMonth() + 1 && zdt.getDayOfMonth() == ts.getDate() && zdt.getHour() == ts.getHours() && zdt.getMinute() == ts.getMinutes() && zdt.getSecond() == ts.getSeconds() && zdt.getNano() == ts.getNanos();
    }

    private static void print(LocalDateTime ldt, Timestamp ts) {
        ZonedDateTime zdt = ZonedDateTime.of(ldt, ZoneId.systemDefault());
        System.out.printf("ldt:ts  %d/%d, %d/%d, %d/%d, %d/%d, %d/%d, %d/%d, nano:[%d/%d]%n", zdt.getYear(), ts.getYear() + 1900, zdt.getMonthValue(), ts.getMonth() + 1, zdt.getDayOfMonth(), ts.getDate(), zdt.getHour(), ts.getHours(), zdt.getMinute(), ts.getMinutes(), zdt.getSecond(), ts.getSeconds(), zdt.getNano(), ts.getNanos());
    }

    private static boolean isEqual(LocalDate ld, java.sql.Date d) {
        return ld.getYear() == d.getYear() + 1900 && ld.getMonthValue() == d.getMonth() + 1 && ld.getDayOfMonth() == d.getDate();
    }

    private static void print(LocalDate ld, java.sql.Date d) {
        System.out.printf("%d/%d, %d/%d, %d/%d%n", ld.getYear(), d.getYear() + 1900, ld.getMonthValue(), d.getMonth() + 1, ld.getDayOfMonth(), d.getDate());
    }

    private static boolean isEqual(LocalTime lt, java.sql.Time t) {
        return lt.getHour() == t.getHours() && lt.getMinute() == t.getMinutes() && lt.getSecond() == t.getSeconds();
    }

    private static void print(LocalTime lt, java.sql.Time t) {
        System.out.printf("%d/%d, %d/%d, %d/%d%n", lt.getHour(), t.getHours(), lt.getMinute(), t.getMinutes(), lt.getSecond(), t.getSeconds());
    }
}
