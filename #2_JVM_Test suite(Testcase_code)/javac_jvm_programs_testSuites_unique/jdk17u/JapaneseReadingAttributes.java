...
public class JapaneseReadingAttributes {
        ...
        textFieldMain.addInputMethodListener(new InputMethodListener() {
            ...
            public void inputMethodTextChanged(InputMethodEvent event) {
                ...
                if (itr != null) {
                    ...
                    if (toCopy > 0) {
                        itr.first();
                        StringBuilder yomigana = new StringBuilder(textFieldReading.getText());
                        while (toCopy-- > 0) {
                            if (itr.getIndex() == itr.getRunStart(AttributedCharacterIterator.Attribute.READING)) {
                                java.text.Annotation annotatedText = (java.text.Annotation) itr.getAttribute(AttributedCharacterIterator.Attribute.READING);
        ...
        if (setTaskStatus(readingPass1, 1)) {
            ...
            if (setTaskStatus(readingPass2, 2)) {
                if (readingPass1.equals(readingPass2)) {
                    testPassed = true;
                    testResult = "Test Passed : Same reading attribute " + "obtained from both passes ";
                    lblTestStatus.setText(testResult);
                } else {
                    testResult = "Test Failed : Reading attribute from Pass 1 <" + readingPass1 + "> != Reading attribute " + "from Pass 2 <" + readingPass2 + ">";
                }
            }
        }
    }
    ...
    public static boolean setTaskStatus(String readingValue, int passCount) {
        ...
        if (!readingValue.isEmpty()) {
            testResult = "Attribute : " + readingValue + "read from pass " + Integer.toString(passCount);
            status = true;
        } else {
            testResult = "Failed to read Reading attribute from pass  " + Integer.toString(passCount);
        }
        lblTestStatus.setText(testResult);
        return status;
    }
