...
public class UnixMultiResolutionSplashTest {
    ...
    private static final ImageInfo[] tests = { new ImageInfo("splash1.png", "splash1@200pct.png", Color.BLUE, Color.GREEN), new ImageInfo("splash2", "splash2@2x", Color.WHITE, Color.BLACK), new ImageInfo("splash3.", "splash3@200pct.", Color.YELLOW, Color.RED) };
    ...
    static void createChildProcess(ImageInfo test) {
        ...
        env.put("GDK_SCALE", "2");
        int exitValue = doExec(env, javaPath + File.separator + "bin" + File.separator + "java", "-splash:" + file.getAbsolutePath(), "-cp", classPathDir, "UnixMultiResolutionSplashTest", String.valueOf(inx++));
        if (exitValue != 0) {
    ...
    static void testFocus() throws Exception {
        ...
        if (!textField.getText().equals("ab")) {
            throw new RuntimeException("Focus is lost! " + "Expected 'ab' got " + "'" + textField.getText() + "'.");
        }
    }
