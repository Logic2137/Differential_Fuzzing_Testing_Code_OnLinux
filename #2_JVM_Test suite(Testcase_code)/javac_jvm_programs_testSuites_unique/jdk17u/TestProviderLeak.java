...
public class TestProviderLeak {
    ...
    static {
        int timeout = 5;
        try {
            double timeoutFactor = Double.parseDouble(System.getProperty("test.timeout.factor", "1.0"));
            timeout = (int) (timeout * timeoutFactor);
        } catch (Exception e) {
            System.out.println("Warning: " + e);
        }
        TIME_OUT = timeout;
        System.out.println("Timeout for each iteration is " + TIME_OUT + " seconds");
    }
    ...
    private static Deque<byte[]> eatupMemory() throws Exception {
        ...
        while (!hasException) {
            ...
            try {
                ...
                System.gc();
                hasException = true;
                System.out.println("OOME is thrown when allocating " + size + "MB memory.");
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        final SecretKeyFactory skf = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1", "SunJCE");
        final PBEKeySpec pbeKS = new PBEKeySpec("passPhrase".toCharArray(), new byte[] { 0 }, 5, 512);
