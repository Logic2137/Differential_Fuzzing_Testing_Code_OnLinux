...
public class FindOneCharEncoderBugs {
    ...
    final static String[] brokenCharsets = { "x-IBM970", "x-COMPOUND_TEXT" };
    ...
    private static CharBuffer directCharBuffer(CharBuffer ocb) {
        final CharBuffer dcb = ByteBuffer.allocateDirect(ocb.capacity() * Character.SIZE / Byte.SIZE).asCharBuffer();
        check(!ocb.isDirect());
        check(dcb.isDirect());
    ...
    private static void testChar(byte[] expected, CharBuffer cb, Charset cs, char c) {
        try {
            ...
            if (!equals(expected, bb))
                fail("bytes differ charset=%s direct=%s char=\\u%04x%n%s%n%s", cs, cb.isDirect(), (int) c, toString(expected), toString(bb));
        } catch (Throwable t) {
            System.out.printf("Unexpected exception charset=%s direct=%s char=\\u%04x%n", cs, cb.isDirect(), (int) c);
            unexpected(t);
            failed++;
        }
    }
    ...
    private static void testCharset(Charset cs) throws Throwable {
        ...
        for (String n : brokenCharsets) if (csn.equals(n)) {
        ...
        for (char c = '\u0000'; (c + 1 != 0x10000) && (failed - failed0 < maxFailuresPerCharset); c++) {
            theChar[0] = c;
