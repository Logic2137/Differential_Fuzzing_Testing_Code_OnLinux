...
public class SuspendWithRawMonitorEnter {
    ...
    public int doWork(int timeMax, PrintStream out) {
        ...
        if (retCode != 0) {
            throw new RuntimeException("error in JVMTI DestroyRawMonitor: " + "retCode=" + retCode);
        }
    ...
    public void run() {
        ...
        if (getName().equals("blocker")) {
            ...
            synchronized (SuspendWithRawMonitorEnter.barrierBlocker) {
                ...
                while (SuspendWithRawMonitorEnter.testState != SuspendWithRawMonitorEnter.TS_DONE_BLOCKING && SuspendWithRawMonitorEnter.testState != SuspendWithRawMonitorEnter.TS_READY_TO_RESUME) {
                    try {
