...
public class RecognizedActionTest implements AWTEventListener {
    ...
    final int[] modifiers = { 0, InputEvent.CTRL_DOWN_MASK, InputEvent.SHIFT_DOWN_MASK, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK };
    ...
    final DragGestureListener dragGestureListener = new DragGestureListener() {
        ...
        public void dragGestureRecognized(DragGestureEvent dge) {
            ...
            if (dge.getDragAction() != currentDragAction) {
                throw new RuntimeException("Expected: " + Integer.toHexString(currentDragAction) + " recognized: " + Integer.toHexString(dge.getDragAction()));
            }
        }
    };
    ...
    public void init() {
        try {
            ...
            dragSource.createDefaultDragGestureRecognizer(frame, DnDConstants.ACTION_COPY | DnDConstants.ACTION_MOVE | DnDConstants.ACTION_LINK, dragGestureListener);
            ...
            for (int i = 0; i < modifiers.length; i++) {
                ...
                Thread.sleep(100);
                for (; !curPoint.equals(dstPoint) && !dragGestureRecognized; curPoint.translate(sign(dstPoint.x - curPoint.x), sign(dstPoint.y - curPoint.y))) {
                    robot.mouseMove(curPoint.x, curPoint.y);
                    Thread.sleep(50);
                }
                Thread.sleep(100);
    ...
    public boolean pointInComponent(Robot robot, Point p, Component comp) throws InterruptedException {
        reset();
