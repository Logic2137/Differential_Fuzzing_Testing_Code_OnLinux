...
public class CoderTest {
    ...
    private static final String testRootDir = System.getProperty("test.src", ".");
    ...
    private static final String[] extension = new String[] { ".b2c", ".c2b-irreversible", ".b2c-irreversible" };
    ...
    private class Test {
        ...
        private boolean decode(ByteBuffer refByte, CharBuffer refChars) throws Exception {
        ...
        private boolean encode(ByteBuffer refByte, CharBuffer refChars) throws Exception {
            ...
            while (refChars.hasRemaining()) {
                ...
                out.get(obs);
                boolean eq = true;
                for (int i = 0; i < bytesPC; i++) eq &= rbs[i] == obs[i];
                if (!eq) {
    ...
    private void loadTests(File f) throws Exception {
        clearTests();
    ...
    private boolean run() throws Exception {
        ...
        decoder.onUnmappableCharacter(CodingErrorAction.REPLACE).onMalformedInput(CodingErrorAction.REPLACE);
        boolean rv = true;
        log.println();
        ...
        for (int mode = ROUNDTRIP; mode <= DECODE; mode++) {
            ...
            if (f == null)
                continue;
            loadTests(f);
            for (int i = 0; i < maxBytesPerChar; i++) rv &= tests[i].run(mode);
        }
        return rv;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        log.println();
        log.println(tested + " charset" + plural(tested) + " tested, " + skipped + " not supported");
        log.println();
        if (errors > 0)
