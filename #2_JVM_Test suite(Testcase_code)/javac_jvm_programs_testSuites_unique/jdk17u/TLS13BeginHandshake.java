...
public class TLS13BeginHandshake {
    ...
    static String pathToStores = System.getProperty("test.src") + "/../../../../javax/net/ssl/etc/";
    ...
    private void runDemo() throws Exception {
        ...
        while (!isEngineClosed(clientEngine) || !isEngineClosed(serverEngine)) {
            ...
            cTOs.compact();
            sTOc.compact();
            if (done < 2 && (clientOut.limit() == serverIn.position()) && (serverOut.limit() == clientIn.position())) {
                if (done == 0) {
    ...
    private SSLContext initContext() throws Exception {
        ...
        KeyStore ks = KeyStore.getInstance(new File(pathToStores + keyStoreFile), passwd.toCharArray());
