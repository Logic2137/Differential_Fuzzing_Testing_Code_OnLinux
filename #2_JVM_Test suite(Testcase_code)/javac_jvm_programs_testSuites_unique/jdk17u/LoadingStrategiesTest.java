...
public class LoadingStrategiesTest {
    ...
    public static void main(String[] args) {
        ...
        rb = ResourceBundle.getBundle("test.package.Messages", Locale.US, new PerLocalePackageControl());
        s = rb.getString("data");
        check("Per-locale package with Locale.US", s, "");
        rb = ResourceBundle.getBundle("test.package.Messages", Locale.GERMAN, new PerLocalePackageControl());
        s = rb.getString("data");
        check("Per-locale package with Locale.GERMAN", s, "de");
        rb = ResourceBundle.getBundle("test.package.Messages", Locale.JAPAN, new PerLocalePackageControl());
        s = rb.getString("data");
    ...
    private static class ChineseControl extends ResourceBundle.Control {
        ...
        public List<Locale> getCandidateLocales(String baseName, Locale locale) {
            if (locale.equals(Locale.TAIWAN)) {
                return Arrays.asList(locale, new Locale(""));
            }
    ...
    private static class PerLocalePackageControl extends ResourceBundle.Control {
        ...
        public String toBundleName(String baseName, Locale locale) {
            ...
            if (loc.length() > 0) {
                return baseName.replaceFirst("^([\\w\\.]+)\\.(\\w+)$", "$1." + loc.substring(1) + ".$2");
            }
            return baseName;
        }
