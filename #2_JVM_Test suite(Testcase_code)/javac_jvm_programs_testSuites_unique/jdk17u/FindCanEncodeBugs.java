        ...
        for (Map.Entry<String, Charset> e : Charset.availableCharsets().entrySet()) {
            ...
            for (int i = Character.MIN_VALUE; i <= Character.MAX_VALUE; i++) {
                ...
                if (encodable1 != encodable2) {
                    int start = i;
                    int end = i;
                    for (int j = i; j <= '\uffff' && encodable1(enc, (char) j) == encodable1 && encodable2(enc, (char) j) == encodable2; j++) end = j;
                    System.out.printf("charset=%-18s canEncode=%-5b ", csn, encodable1);
                    if (start == end)
                        System.out.printf("\'\\u%04x\'%n", start);
                    else
                        System.out.printf("\'\\u%04x\' - \'\\u%04x\'%n", start, end);
                    i = end;
                    failures++;
                }
            }
