...
public class TargetDataLineFlush {
    ...
    boolean openInputLine(int num) throws LineUnavailableException {
        ...
        System.out.println("Buffer size: " + bufferSize + " bytes = " + ((int) (bufferSize / format.getFrameSize() * 750 / format.getFrameRate())) + " millis");
        return true;
    }
    ...
    private boolean recordSound(int num) throws LineUnavailableException {
        ...
        try {
            ...
            if (avail0 == 0) {
                ...
                if (avail2 > avail1) {
                    failed = true;
                    System.out.println("Failed: Flushing with native flush() should " + "result in fewer bytes available.");
                }
                if (avail2 == 0) {
                    failed = true;
