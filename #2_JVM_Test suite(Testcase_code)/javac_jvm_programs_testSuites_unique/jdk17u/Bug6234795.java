...
public class Bug6234795 {
    ...
    static void testRoll(int field) {
        ...
        for (h = 0; h <= 72; h++) {
            ...
            int expected = h % 12;
            if (hour != expected) {
                throw new RuntimeException((field == HOUR ? "HOUR" : "HOUR_OF_DAY") + "+: h=" + h + ", got " + hour + ", expected " + expected);
            }
            if (field == HOUR_OF_DAY) {
                int ampm = cal.get(AM_PM);
                expected = (h % unit) / 12;
                if (ampm != expected) {
                    throw new RuntimeException((field == HOUR ? "HOUR" : "HOUR_OF_DAY") + "+: h=" + h + ", got " + toString(ampm) + ", expected " + toString(expected));
                }
            }
            cal.roll(field, +1);
        }
