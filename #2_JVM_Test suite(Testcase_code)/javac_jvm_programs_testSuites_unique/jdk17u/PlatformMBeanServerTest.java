...
public class PlatformMBeanServerTest {
    ...
    private static void checkMBean(MBeanServer mbs, String mbeanName) throws Exception {
        try {
    ...
    private static void checkStandardMBeans(MBeanServer mbs) throws Exception {
        ...
        if (set.size() != pools.size()) {
            throw new RuntimeException("Unexpected number of memory pools:" + "MBeanServer has " + set.size() + ". Expected = " + pools.size());
        }
        ...
        for (ListIterator iter = mgrs.listIterator(); iter.hasNext(); ) {
            ...
            if (m instanceof GarbageCollectorMXBean) {
                checkMBean(mbs, GARBAGE_COLLECTOR_MXBEAN_DOMAIN_TYPE + ",name=" + m.getName());
            } else {
                checkMBean(mbs, MEMORY_MANAGER_MXBEAN_DOMAIN_TYPE + ",name=" + m.getName());
                num_mgrs++;
            }
        }
        ...
        for (ListIterator iter = gcs.listIterator(); iter.hasNext(); ) {
            ...
            checkMBean(mbs, GARBAGE_COLLECTOR_MXBEAN_DOMAIN_TYPE + ",name=" + gc.getName());
        }
        set = mbs.queryNames(new ObjectName(MEMORY_MANAGER_MXBEAN_DOMAIN_TYPE + ",*"), null);
        if (set.size() != num_mgrs) {
            throw new RuntimeException("Unexpected number of memory managers:" + "MBeanServer has " + set.size() + ". Expected = " + num_mgrs);
        }
        set = mbs.queryNames(new ObjectName(GARBAGE_COLLECTOR_MXBEAN_DOMAIN_TYPE + ",*"), null);
        if (set.size() != gcs.size()) {
            throw new RuntimeException("Unexpected number of garbage collectors:" + "MBeanServer has " + set.size() + ". Expected = " + gcs.size());
        }
    }
