...
public class CheckExecutable {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(bin)) {
            EnumSet<PosixFilePermission> execPerms = EnumSet.of(GROUP_EXECUTE, OTHERS_EXECUTE, OWNER_EXECUTE);
            for (Path entry : stream) {
                ...
                try {
                    ...
                    if (!perm.containsAll(execPerms)) {
                        throw new RuntimeException(entry + " has not all executable permissions!\n" + "Should have: " + execPerms + "\nbut has: " + perm);
                    }
