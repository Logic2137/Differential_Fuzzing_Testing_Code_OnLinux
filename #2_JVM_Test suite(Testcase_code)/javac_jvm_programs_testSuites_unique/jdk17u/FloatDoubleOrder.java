...
public class FloatDoubleOrder {
    ...
    void test(String[] args) throws Throwable {
        double[] unsortedDbl = new double[] { 1.0d, 3.7d, Double.NaN, -2.0d, Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY, 0.0d, -0.0d };
        double[] sortedDbl = new double[] { Double.NEGATIVE_INFINITY, -2.0d, -0.0d, 0.0d, 1.0d, 3.7d, Double.POSITIVE_INFINITY, Double.NaN };
        List list = new ArrayList();
        for (int i = 0; i < unsortedDbl.length; i++) list.add(new Double(unsortedDbl[i]));
        Collections.sort(list);
        List sortedList = new ArrayList();
        for (int i = 0; i < sortedDbl.length; i++) sortedList.add(new Double(sortedDbl[i]));
        ...
        float[] unsortedFlt = new float[] { 1.0f, 3.7f, Float.NaN, -2.0f, Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY, 0.0f, -0.0f };
        float[] sortedFlt = new float[] { Float.NEGATIVE_INFINITY, -2.0f, -0.0f, 0.0f, 1.0f, 3.7f, Float.POSITIVE_INFINITY, Float.NaN };
        list.clear();
        for (int i = 0; i < unsortedFlt.length; i++) list.add(new Float(unsortedFlt[i]));
        Collections.sort(list);
        sortedList.clear();
        for (int i = 0; i < sortedFlt.length; i++) sortedList.add(new Float(sortedFlt[i]));
        check(list.equals(sortedList));
