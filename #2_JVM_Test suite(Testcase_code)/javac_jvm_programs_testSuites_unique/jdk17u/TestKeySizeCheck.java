...
public class TestKeySizeCheck {
    ...
    public static void test(String algo, int[] invalidKeySizes) throws Exception {
        ...
        for (int ks : invalidKeySizes) {
            ...
            for (int m : modes) {
                try {
                    c.init(m, key);
                    throw new RuntimeException("Expected IKE not thrown for " + getModeStr(m));
                } catch (InvalidKeyException ike) {
                    System.out.println(" => expected IKE thrown for " + getModeStr(m));
                }
            }
        }
    }
