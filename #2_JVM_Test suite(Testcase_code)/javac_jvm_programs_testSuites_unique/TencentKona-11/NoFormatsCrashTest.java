...
public class NoFormatsCrashTest extends Applet {
    ...
    public void run(String[] args) {
        try {
            ...
            for (; !sourcePoint.equals(targetPoint); sourcePoint.translate(sign(targetPoint.x - sourcePoint.x), sign(targetPoint.y - sourcePoint.y))) {
                robot.mouseMove(sourcePoint.x, sourcePoint.y);
                Thread.sleep(50);
            }
    ...
    public void start() {
        ...
        try {
            ...
            String command = javaPath + File.separator + "bin" + File.separator + "java -cp " + System.getProperty("test.classes", ".") + " NoFormatsCrashTest " + p.x + " " + p.y + " " + d.width + " " + d.height;
...
class TestTransferable implements Transferable {
    ...
    static {
        DataFlavor df = null;
        try {
            df = new DataFlavor(DataFlavor.javaJVMLocalObjectMimeType + "; class=java.lang.Object");
...
class DragSourcePanel extends Panel {
    ...
    public DragSourcePanel() {
        ...
        final DragGestureRecognizer dgr = ds.createDefaultDragGestureRecognizer(this, DnDConstants.ACTION_COPY, dgl);
    }
