    ...
    private static class Scanner extends TreeScanner<Void, Void> {
        ...
        public Void visitCompilationUnit(CompilationUnitTree node, Void ignore) {
            ...
            if (foundAnnotations != expectedAnnotations) {
                throw new AssertionError("Expected " + expectedAnnotations + " annotations but found: " + foundAnnotations);
            }
            return null;
        }
        ...
        private void testAnnotations(List<? extends AnnotationTree> annos, int found) {
            ...
            if (found != expected) {
                throw new AssertionError("Unexpected found length" + ", found " + found + " but expected " + expected);
            }
        }
        ...
        public Void visitNewArray(NewArrayTree node, Void ignore) {
            ...
            int totalLength = node.getDimensions().size() + arrayLength(node.getType()) + ((newArray.getInitializers() != null) ? 1 : 0);
