...
public class CertificateBuilder {
    ...
    public void addSubjectAltNameDNSExt(List<String> dnsNames) throws IOException {
        if (!dnsNames.isEmpty()) {
            ...
            addExtension(new SubjectAlternativeNameExtension(false, gNames));
        }
    }

    public void addAIAExt(List<String> locations) throws IOException {
        if (!locations.isEmpty()) {
            List<AccessDescription> acDescList = new ArrayList<>();
            for (String ocspUri : locations) {
                acDescList.add(new AccessDescription(AccessDescription.Ad_OCSP_Id, new GeneralName(new URIName(ocspUri))));
            }
    ...
    public void addBasicConstraintsExt(boolean crit, boolean isCA, int maxPathLen) throws IOException {
        addExtension(new BasicConstraintsExtension(crit, isCA, maxPathLen));
    }

    public void addAuthorityKeyIdExt(X509Certificate authorityCert) throws IOException {
    ...
    public void addExtendedKeyUsageExt(List<String> ekuOids) throws IOException {
        if (!ekuOids.isEmpty()) {
    ...
    public X509Certificate build(X509Certificate issuerCert, PrivateKey issuerKey, String algName) throws IOException, CertificateException, NoSuchAlgorithmException {
    ...
    private byte[] encodeTopLevel(X509Certificate issuerCert, PrivateKey issuerKey, AlgorithmId signAlg) throws CertificateException, IOException {
    ...
    private byte[] encodeTbsCert(X509Certificate issuerCert, AlgorithmId signAlg) throws IOException {
        ...
        tbsCertItems.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0), v3int);
    ...
    private void encodeExtensions(DerOutputStream tbsStream) throws IOException {
        ...
        tbsStream.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 3), extSequence);
    }

    private byte[] signCert(PrivateKey issuerKey, AlgorithmId signAlg) throws GeneralSecurityException {
