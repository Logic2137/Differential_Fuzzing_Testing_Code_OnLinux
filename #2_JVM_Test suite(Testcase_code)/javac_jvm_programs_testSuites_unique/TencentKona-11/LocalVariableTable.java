...
public class LocalVariableTable {
    ...
    void check(Class<?> c) throws Exception {
        ...
        LocalVariableTable_attribute lvt = (LocalVariableTable_attribute) code.attributes.get(Attribute.LocalVariableTable);
        if (lvt == null) {
        ...
        if (!foundNames.equals(expectNames)) {
            ...
            for (String s : foundOnly) error("Unexpected name found: " + s);
            Set<String> expectOnly = new LinkedHashSet<>(expectNames);
            expectOnly.removeAll(foundNames);
            for (String s : expectOnly) error("Expected name not found: " + s);
        }
    }
