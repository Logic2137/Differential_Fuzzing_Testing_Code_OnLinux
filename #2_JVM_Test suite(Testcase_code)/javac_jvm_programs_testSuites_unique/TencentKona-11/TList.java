...
public class TList {
    ...
    String[][] data = { {}, { "1" }, { "1", "2" }, { "1", "2" }, { "1", "2", "3", "4", "X", "X", "X", "8", "9", "10" } };
    ...
    static <T> com.sun.tools.javac.util.List<T> createList(List<T> d) {
        ...
        for (ListIterator<T> iter = d.listIterator(d.size()); iter.hasPrevious(); ) l = l.prepend(iter.previous());
        return l;
    }

    static <T> com.sun.tools.javac.util.List<T> createList(T... d) {
        com.sun.tools.javac.util.List<T> l = com.sun.tools.javac.util.List.nil();
        for (int i = d.length - 1; i >= 0; i--) l = l.prepend(d[i]);
        return l;
    }
    ...
    static <T> java.util.List<T> join(java.util.List<T>... lists) {
        ...
        for (java.util.List<T> l : lists) r.addAll(l);
        return r;
    }
