...
public class EmptyClipRenderingTest {
    ...
    public EmptyClipRenderingTest() {
        ...
        BufferedImage destBI = new BufferedImage(IMG_W, IMG_H, BufferedImage.TYPE_INT_RGB);
        ...
        if (errors.isEmpty()) {
            ...
            if (showErrors) {
                System.err.println("Test FAILED: " + errors.size() + " subtest failures.");
            } else {
                throw new RuntimeException("Test FAILED: " + errors.size() + " subtest failures.");
            }
        }
    }
    ...
    private BufferedImage testOnscreen() throws HeadlessException {
        ...
        do {
            ...
            bi = r.createScreenCapture(new Rectangle(destComponent.getLocationOnScreen().x, destComponent.getLocationOnScreen().y, destComponent.getWidth(), destComponent.getHeight()));
        } while (isPainted);
        f.setVisible(false);
        f.dispose();
        return bi;
    ...
    void runTest(Graphics2D destGraphics) {
        ...
        for (Rectangle clip : rects) {
            ...
            if (g2d instanceof ConstrainableGraphics) {
                ((ConstrainableGraphics) g2d).constrain(clip.x, clip.y, clip.width, clip.height);
            }
            g2d.setClip(clip);
            for (Rectangle renderRegion : rects) {
                if (renderRegion != clip) {
                    Graphics2D rG = (Graphics2D) g2d.create(renderRegion.x, renderRegion.y, renderRegion.width, renderRegion.height);
        ...
        for (int y = 0; y < bi.getHeight(); y++) {
            for (int x = 0; x < bi.getWidth(); x++) {
                if (bi.getRGB(x, y) != Color.black.getRGB()) {
                    ...
                    try {
                        String fileName = "EmptyClipRenderingTest_" + desc + "_res.png";
                        System.out.println("Writing resulting image: " + fileName);
                        ImageIO.write(bi, "png", new File(fileName));
                    } catch (IOException ex) {
                        ex.printStackTrace();
                    }
                    throw new RuntimeException("Dest: " + desc + " was rendered to at x=" + x + " y=" + y + " pixel=" + Integer.toHexString(bi.getRGB(x, y)));
                }
            }
        }
    }
