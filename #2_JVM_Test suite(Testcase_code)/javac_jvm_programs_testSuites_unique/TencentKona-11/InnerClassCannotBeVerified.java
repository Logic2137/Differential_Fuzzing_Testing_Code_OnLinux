...
public class InnerClassCannotBeVerified {
    ...
    enum CompilationKind {
        ...
        PRE_NESTMATES("-source", "10", "-target", "10"), POST_NESTMATES;
    ...
    private static final String errorMessage = "Compile error while compiling the following source:\n";
    ...
    void run() throws Exception {
        for (CompilationKind ck : CompilationKind.values()) {
            ...
            JavacTask ct = (JavacTask) comp.getTask(null, null, null, Arrays.asList(ck.opts), null, Arrays.asList(source));
            try {
                if (!ct.call()) {
    ...
    private void check(CompilationKind ck) throws IOException, ConstantPoolException {
        try {
            ...
            boolean inheritsFromObject = classFile.getSuperclassName().equals("java/lang/Object");
            boolean implementsNoInterface = classFile.interfaces.length == 0;
            boolean noMethods = classFile.methods.length == 0;
            if (!(inheritsFromObject && implementsNoInterface && noMethods)) {
                throw new AssertionError("The inner classes reused as " + "access constructor tag for this code must be empty");
            }
    ...
    class JavaSource extends SimpleJavaFileObject {
        ...
        String internalSource = "public class Test {\n" + "    private static class Foo {}\n" + "    public static void main(String[] args){ \n" + "        new Foo();\n" + "        if(false) {\n" + "            new Runnable() {\n" + "                @Override\n" + "                public void run() {\n" + "                    System.out.println();\n" + "                }\n" + "            }.run();\n" + "        }\n" + "   }\n" + "}";
