...
public class TestIndyStringConcat {
    ...
    public static boolean hasStringConcatFactoryCall(String methodName) throws Exception {
        ClassFile classFile = ClassFile.read(new File(System.getProperty("test.classes", "."), TestIndyStringConcat.class.getName() + ".class"));
        ConstantPool constantPool = classFile.constant_pool;
        BootstrapMethods_attribute bsm_attr = (BootstrapMethods_attribute) classFile.getAttribute(Attribute.BootstrapMethods);
            ...
            if (method.getName(constantPool).equals(methodName)) {
                ...
                for (Instruction i : code.getInstructions()) {
                    if (i.getOpcode() == Opcode.INVOKEDYNAMIC) {
                        ...
                        BootstrapMethodSpecifier bsmSpec = bsm_attr.bootstrap_method_specifiers[indyInfo.bootstrap_method_attr_index];
                        CONSTANT_MethodHandle_info bsmInfo = (CONSTANT_MethodHandle_info) constantPool.get(bsmSpec.bootstrap_method_ref);
