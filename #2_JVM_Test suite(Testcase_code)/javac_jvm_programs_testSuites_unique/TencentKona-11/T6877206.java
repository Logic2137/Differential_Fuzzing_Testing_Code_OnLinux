...
public class T6877206 {
    ...
    void run() throws Exception {
        ...
        checkCoverage("classes", foundClasses, "DirectoryFileObject", "JarFileObject");
    ...
    void test(StandardJavaFileManager fm, File f, String pkg, int expect) throws Exception {
        ...
        try {
            ...
            int count = 0;
            for (JavaFileObject fo : fm.list(StandardLocation.CLASS_PATH, pkg, EnumSet.allOf(JavaFileObject.Kind.class), true)) {
            ...
            if (expect > 0 && count != expect)
                throw new Exception("wrong number of entries found: " + count + ", expected " + expect);
        } finally {
            fm.close();
        }
    }
        ...
        try {
            ...
            if (!Arrays.equals(uriData, foData)) {
                if (uriData.length != foData.length)
                    throw new Exception("data size differs: uri data " + uriData.length + " bytes, fo data " + foData.length + " bytes");
                for (int i = 0; i < uriData.length; i++) {
                    if (uriData[i] != foData[i])
                        throw new Exception("unexpected data returned at offset " + i + ", uri data " + uriData[i] + ", fo data " + foData[i]);
                }
