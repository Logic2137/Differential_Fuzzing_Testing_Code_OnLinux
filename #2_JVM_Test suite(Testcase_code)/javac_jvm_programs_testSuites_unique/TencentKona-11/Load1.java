...
public class Load1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int ox = 0;
        for (int i = 0; i < head.length; i++) msg[ox++] = head[i];
        for (int i = 0; i < name.length; i++) msg[ox++] = name[i];
        for (int i = 0; i < 128; i++) {
            msg[ox++] = i;
            msg[ox++] = 64;
            msg[ox++] = 0;
        ...
        x = x ^ (msg[4] & 0xFF);
        x = x ^ (msg[5] & 0xFF);
        for (int i = 22; i < msg.length - 2; i++) x = x ^ (msg[i] & 0xFF);
        msg[ox++] = (x & 127);
        msg[ox++] = 0xf7;
        ...
        for (int i = 0; i < tunings.length; i++) assertTrue(Math.abs(tunings[i] - (i * 100 + 50)) < 0.00001);
    }
}
