...
public class CheckErrorsForSource7 {
    ...
    private void run(String... args) throws IOException, URISyntaxException {
        if (args.length != 1)
            throw new IllegalStateException("Must provide source file!");
        File testSrc = new File(System.getProperty("test.src"));
        File testFile = new File(testSrc, args[0]);
        if (!testFile.canRead())
            throw new IllegalStateException("Cannot read the test source");
        try (JavacFileManager fm = JavacTool.create().getStandardFileManager(null, null, null)) {
            JavacTask task = JavacTool.create().getTask(null, fm, null, Collections.<String>emptyList(), null, fm.getJavaFileObjects(testFile));
            ...
            new TreePathScanner<Void, Void>() {
                ...
                public Void visitAnnotation(AnnotationTree node, Void p) {
                    if (node.getAnnotationType().getKind() == Kind.IDENTIFIER && ((IdentifierTree) node.getAnnotationType()).getName().contentEquals("TA")) {
            ...
            for (int[] toKeep : typeAnnotationSpans) {
                ...
                JavacTask task2 = JavacTool.create().getTask(null, fm, errors, Arrays.asList("-source", "7"), null, Arrays.asList(updatedFile));
                task2.parse();
                boolean found = false;
...
class CheckErrorsForSource7Data {
    ...
    @TA
    <T> int @TA [] @TA [] ret(Object obj) throws @TA Exception {
    ...
    abstract class C<@TA T extends @TA Number & @TA Runnable> extends @TA ArrayList<@TA String> implements java.util.@TA Comparator<@TA T> {
    }
