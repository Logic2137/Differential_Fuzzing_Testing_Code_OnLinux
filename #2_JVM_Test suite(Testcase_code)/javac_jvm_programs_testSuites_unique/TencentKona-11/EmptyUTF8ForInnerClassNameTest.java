...
public class EmptyUTF8ForInnerClassNameTest {
    ...
    void run() throws Exception {
        checkClassFile(Paths.get(System.getProperty("test.classes"), this.getClass().getName() + "$1.class"));
        checkClassFile(Paths.get(System.getProperty("test.classes"), this.getClass().getName() + "$EnumPlusSwitch.class"));
    }
    ...
    void checkClassFile(final Path path) throws Exception {
        ...
        for (CPInfo cpInfo : classFile.constant_pool.entries()) {
            if (cpInfo.getTag() == CONSTANT_Utf8) {
                ...
                Assert.check(utf8Info.value.length() > 0, "UTF8 with length 0 found at class " + classFile.getName());
            }
        }
    }
