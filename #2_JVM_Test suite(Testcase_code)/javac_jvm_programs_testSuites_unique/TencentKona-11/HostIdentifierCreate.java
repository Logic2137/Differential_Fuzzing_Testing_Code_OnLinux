...
class HostIdentifierTestHandler extends DefaultHandler {
    ...
    public void characters(char[] ch, int start, int length) {
        ...
        if (debug) {
            System.out.println("characters: start = " + start + " length = " + length + " chars = " + s);
        }
        if (value == null) {
            value = s.trim();
        } else {
            value = value + s.trim();
            if (debug) {
                System.out.println("characters: appended characters to " + "previous value: new value = " + value);
            }
        }
    }
    ...
    public void endElement(String namespaceURI, String localName, String qName) throws SAXException {
        if (debug) {
            System.out.println("endElement(): namespaceURI = " + namespaceURI + " localName = " + localName + " qName = " + qName + " state = " + state);
        }
        switch(state) {
            case START:
    ...
    public void ignorableWhitespace(char[] ch, int start, int length) {
        if (debug) {
            System.out.println("ignoreableWhitespace():" + " ch = " + new String(ch, start, length) + " start = " + start + " length = " + length);
        }
    }
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes attributes) {
        if (debug) {
            System.out.println("startElement():" + " namespaceURI = " + namespaceURI + " localName = " + localName + " qName = " + qName + " state = " + state);
            System.out.println("   Attributes(" + attributes.getLength() + ")");
            for (int i = 0; i < attributes.getLength(); i++) {
                System.out.println("     name = " + attributes.getQName(i) + " value = " + attributes.getValue(i));
            }
        }
        ...
        switch(state) {
            case START:
                if (qName.compareTo("HostIdentifierTests") == 0) {
                    ...
                    System.err.println("unexpected input: state = " + state + " input = " + qName);
                }
                break;
            case HOSTIDENTIFIER_TESTS:
...
class HostIdentifierException extends Exception {
    ...
    public String getMessage() {
        return "Test " + test.id + " " + "Failed: " + "Expected = " + test.expectedHostIdentifier + " " + "Actual = " + result;
    }
}
...
class TestCase {
    ...
    public void run() throws Exception {
        if (expectedHostIdentifier == null) {
            throw new IllegalArgumentException("HostIdentifier not initialized");
        }
