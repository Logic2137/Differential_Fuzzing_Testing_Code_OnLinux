...
class TestThread extends Thread {
    ...
    public TestThread(ThreadGroup tg, boolean useCache, File cacheDirectory, String threadName) {
        super(tg, threadName);
    ...
    public void run() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (newUseCache != useCache || newCacheDirectory != cacheDirectory) {
                gotCrosstalk = true;
            }
        }
    }
...
public class AppContextTest {
    ...
    public AppContextTest() {
        ...
        TestThread t0 = new TestThread(tg0, false, null, "TestThread 0");
        TestThread t1 = new TestThread(tg1, true, new File("."), "TestThread 1");
        t0.start();
        t1.start();
        try {
            t0.join();
