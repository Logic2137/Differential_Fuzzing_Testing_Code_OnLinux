...
for (int newIndex = 0; newIndex < 20; ++newIndex)
    ...
    public boolean test(String[] args) throws Exception {
        ...
        key = SecretKeyFactory.getInstance(algorithm, p).generateSecret(new PBEKeySpec(PBEPASS.toCharArray()));
        byte[] inputText = new byte[INPUT_LENGTH + NUM_PAD_BYTES + PBKDF2_ADD_PAD_BYTES];
        new Random().nextBytes(inputText);
        try {
            execute(Cipher.ENCRYPT_MODE, inputText, 0, INPUT_LENGTH);
            int padLength = NUM_PAD_BYTES + PBKDF2_ADD_PAD_BYTES;
            execute(Cipher.DECRYPT_MODE, inputText, 0, INPUT_LENGTH + padLength);
        } catch (Exception ex) {
    ...
    private void execute(int edMode, byte[] inputText, int offset, int len) {
        try {
            ...
            boolean e = equalsBlock(inputText, myoff, outputText, 0, outputText.length);
        } catch (Exception ex) {
    ...
    private boolean equalsBlock(byte[] b1, int off1, byte[] b2, int off2, int len) {
//方法已经for语句变异
for (int newIndex = 0; newIndex < 20; ++newIndex)
