public class PolynomialRoot {
    ...
    public static int findPolynomialRoots(final int n, final double[] p, final double[] re_root, final double[] im_root) {
        if (n == 4) {
    ...
    public static int root4(final double[] p, final double[] re_root, final double[] im_root) {
        if (PRINT_DEBUG)
            System.err.println("=====================root4:p=" + java.util.Arrays.toString(p));
        final double vs = p[4];
        if (PRINT_DEBUG)
            System.err.println("p[4]=" + p[4]);
        if (!(Math.abs(vs) > EPS)) {
            re_root[0] = re_root[1] = re_root[2] = re_root[3] = im_root[0] = im_root[1] = im_root[2] = im_root[3] = Double.NaN;
            return -1;
        }
        final double a = p[3] / vs, b = p[2] / vs, c = p[1] / vs, d = p[0] / vs;
        if (PRINT_DEBUG)
            System.err.println("input a=" + a + " b=" + b + " c=" + c + " d=" + d);
        ...
        if (0 == b && 0 == c) {
            if (0 == d) {
                ...
                if (d > 0) {
                    ...
                    im_root[3] = -re_root[0];
                    if (PRINT_DEBUG)
                        System.err.println("Path a=0 d>0");
                } else {
                    final double sq4 = Math.sqrt(Math.sqrt(-d));
                    re_root[0] = sq4;
                    im_root[0] = 0;
        ...
        if (PRINT_DEBUG)
            System.err.println("G Path c=" + c + " d=" + d);
        final double[] u = new double[3];
        if (PRINT_DEBUG)
            System.err.println("Generic Path");
        final double aa = a * a;
        ...
        final boolean flag_realroots;
        {
            ...
            if (PRINT_DEBUG)
                System.err.println("CR2=" + CR2 + " CQ3=" + CQ3 + " R=" + R + " Q=" + Q);
            if (0 == R && 0 == Q) {
                flag_realroots = true;
                u[0] = -rc3;
                u[1] = -rc3;
...
for (int newIndex = 0; newIndex < 20; ++newIndex)
            ...
            if (PRINT_DEBUG)
                System.err.println("u1=" + u1 + " u2=" + u2 + " jmin=" + jmin);
        } else {
            mt = 3;
...
for (int newIndex = 0; newIndex < 20; ++newIndex)
    ...
    static void checkValues(final double[] p, final int n, final double rex, final double imx, final double eps, final String txt) {
...
for (int newIndex = 0; newIndex < 20; ++newIndex)
        ...
        sabs = Math.abs(sabs);
        if (false && sabs > 1 / eps ? (!(Math.abs(res / sabs) <= eps) || !(Math.abs(ims / sabs) <= eps)) : (!(Math.abs(res) <= eps) || !(Math.abs(ims) <= eps))) {
            throw new RuntimeException(getPolinomTXT(p) + "\n" + "\t x.r=" + rex + " x.i=" + imx + "\n" + "res/sabs=" + (res / sabs) + " ims/sabs=" + (ims / sabs) + " sabs=" + sabs + "\nres=" + res + " ims=" + ims + " n=" + n + " eps=" + eps + " " + " sabs>1/eps=" + (sabs > 1 / eps) + " f1=" + (!(Math.abs(res / sabs) <= eps) || !(Math.abs(ims / sabs) <= eps)) + " f2=" + (!(Math.abs(res) <= eps) || !(Math.abs(ims) <= eps)) + " " + txt);
        }
    }
...
for (int newIndex = 0; newIndex < 20; ++newIndex)
    ...
    static void testRoots(final int n, final int n_tests, final java.util.Random rn, final double eps) {
...
for (int newIndex = 0; newIndex < 20; ++newIndex)
            for (int i = 0; i <= n; i++) {
                if (i < n) {
                    System.err.println(String.valueOf(i) + "\t" + p[i] + "\t" + re_root[i] + "\t" + im_root[i]);
                } else {
    ...
    public static void main(final String[] args) {
        if (System.getProperty("os.arch").equals("x86") || System.getProperty("os.arch").equals("amd64") || System.getProperty("os.arch").equals("x86_64")) {
